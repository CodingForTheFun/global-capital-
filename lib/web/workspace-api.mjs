...
async function ncaafArchiveHistory(params) {
  const {fetchNcaaMfbResearch} = await import('../data-sources/sportsdataverse/ncaaf-game-research.mjs');
  return fetchNcaaMfbResearch(params);
}
...
export function createWorkspaceHandler({authenticate:auth=authenticate,read:provider=read,predict:prediction=predict,historyFallback=publicHistory,wnbaHistoryFallback=wnbaArchiveHistory,nflHistoryFallback=nflverseHistory,ncaafHistoryFallback=ncaafArchiveHistory,now=Date.now}={}) {
    ...
    try {
      ...
      if(action==='history') {
        ...
        if(historyParams.sport==='NCAAF') {
          attemptedSources.push('NCAA Stats archive');
          let ncaaArchive=null;
          try { ncaaArchive=await ncaafHistoryFallback(historyParams); } catch { ncaaArchive=null; }
          if(ncaaArchive?.available===true&&Array.isArray(ncaaArchive.gameLog)&&ncaaArchive.gameLog.length) {
            send(res,200,{ok:true,...ncaaArchive,sourceProvider:ncaaArchive.source||'NCAA Stats archive',fallback:true,primaryCode:primary.code,attemptedSources});return true;
          }
        }
