...
const server = http.createServer(async (req, res) => {
  ...
  // Account endpoints (sign-up, email code, sign-in, recovery) run before the gate.
  ...
  // Every remaining /api/* route requires a verified account.
  let session = null;
  ...
  if (url.pathname === '/api/status' && req.method === 'GET') {
    ...
    return json(res, 200, { running, lastError, connection, rules, demoMode: false, autoScanMinutes: intervalMinutes, payoutMultiplier: process.env.PAYOUT_MULTIPLIER || null, paymentConfigured: payment.enabled, scannerVersion: 'masterpiece-2', progress: scanProgress, latest, user: session?.user || null }, authHeaders);
  }
