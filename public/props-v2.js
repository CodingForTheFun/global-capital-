...
function emptyFilters() {
  return {
    search: '', sports: [], markets: [], side: 'ALL', timeWindow: 'ALL',
    minScore: null, minHitRate: null, hitRateWindow: 'l10', minProjectionEdge: null,
    minExpectedMinutes: null, startersOnly: false, excludeInjured: false,
...
function readFiltersFromUrl() {
  ...
  // Ordinary visits always start wide open. Filter query params are honored
  // only by an explicitly shared URL so stale searches never trap mobile users
  // on an empty board after refresh.
  if (p.get('share') !== '1') return emptyFilters();
  ...
  return {
    ...
    timeWindow: p.get('timeWindow') || 'ALL', minScore: number('minScore'), minHitRate: number('minHitRate'),
    ...
    excludeInjured: p.get('excludeInjured') === '1', applyScoutRules: p.get('rules') === '1',
  };
}
...
function savePreferences() {
  try { localStorage.setItem(LIMIT_PREF, String(state.limit)); } catch {}
}
...
function syncUrl() {
  // Keep normal navigation clean and unfiltered. Filters remain live in the
  // current session but do not become sticky simply because the user refreshed.
...
function renderSummary(data, rows) {
  ...
  const available = finiteOrNull(counts.total) ?? 0;
  const matching = finiteOrNull(counts.matching) ?? available;
  ...
  $('dataSummary').textContent = state.filters.applyScoutRules ? `${qualified.toLocaleString()} pass Scout Rules` : `${available.toLocaleString()} current/upcoming main-line props`;
  if (available === 0) return showNotice('No current or upcoming prop lines are available from the active data source yet. Refresh shortly.', false);
  if (state.filters.applyScoutRules && matching > 0 && filtered === 0) return showNotice(`${matching.toLocaleString()} props match your filters. 0 currently pass Scout Rules. Turn Rules OFF to browse them.`, false, true);
  if (data?.emptyReason && filtered === 0) return showNotice(`${available.toLocaleString()} props are available, but none match the active filters.`, false);
...
function showNotice(message, isError = false, offerRulesOff = false) {
  ...
  $('viewAllPropsAction')?.addEventListener('click', () => { state.filters.applyScoutRules=false; syncControlsFromState(); loadProps({reset:true}); });
}
...
function renderProps(rows) {
  const grid = $('propGrid');
  if (!rows.length) { grid.innerHTML='<div class="empty-state"><b>No props to show</b>Refresh the board or choose a different filter.</div>'; return; }
...
function renderFilterChips() {
  ...
  const container=$('activeChips'); container.hidden=chips.length===0; container.innerHTML=chips.map(([id,label,value])=>`<span class="active-chip">${esc(label)}<button type="button" data-clear-filter="${esc(id)}" ${value?`data-clear-value="${esc(value)}"`:''}>×</button></span>`).join(''); $('filterCount').textContent=String(chips.length);
}
function clearFilter(id,value=null){if(id==='sports')state.filters.sports=value?state.filters.sports.filter((sport)=>String(sport).toUpperCase()!==String(value).toUpperCase()):[];else if(id==='search'){state.filters.search='';$('searchInput').value='';}else if(id==='side')state.filters.side='ALL';else if(id==='timeWindow')state.filters.timeWindow='ALL';else if(id==='minScore')state.filters.minScore=null;else if(id==='minHitRate')state.filters.minHitRate=null;else if(id==='minProjectionEdge')state.filters.minProjectionEdge=null;else if(id==='minExpectedMinutes')state.filters.minExpectedMinutes=null;else if(id==='startersOnly')state.filters.startersOnly=false;else if(id==='excludeInjured')state.filters.excludeInjured=false;else if(id==='applyScoutRules')state.filters.applyScoutRules=false;syncControlsFromState();loadProps({reset:true});}
...
function resetFilters(){state.filters=emptyFilters();syncControlsFromState();closeSheet();loadProps({reset:true});}
