...
export function proToolsHtml(result){
  ...
  return '<div class="asProTools"><p class="asNotice">Research from verified pre-game quotes observed within 5 minutes. Paired quotes must be within 60 seconds of each other. Adjusting the research line does not change these offers.</p>'
    +(result.evPlus.length?section('ev','EV+','<div class="asProCards">'+result.evPlus.map(evCard).join('')+'</div>'):'')
    +(result.ev.some(r=>r.available&&!r.qualifiesEVPlus)?section('model-ev','Model estimates','<div class="asProCards">'+result.ev.filter(r=>r.available&&!r.qualifiesEVPlus).map(evCard).join('')+'</div>'):'')
