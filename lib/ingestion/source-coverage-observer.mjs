...
/** Passive DB read only. Invoked by the existing scheduler; never starts a polling timer
...
export function createSourceCoverageObserver({
  ...
  let pending = null, nextAt = 0, lastTelemetry = null, lastTelemetryAt = 0;
  ...
  function unknown(code) {
    ...
    if (lastTelemetry !== code || now() - lastTelemetryAt >= 600_000) {
      lastTelemetryAt = now(); safeEmit({ scope: 'active-public-store', level: 'unknown', code });
    }
    lastTelemetry = code;
  ...
  async function read(feeds) {
    ...
    try {
      ...
      if (controller.signal.aborted) return unknown('SOURCE_COVERAGE_READ_FAILED');
