...
export function tacoOffer(outcome, { book, sport, event, market, now = Date.now() } = {}) {
  if (book?.key !== 'prizepicks') return null;
  ...
  return { id: ['taco',event.id,market,player,side,line].join('|'), sport, playerName: player,
...
export function activeTacos(rows = [], now = Date.now()) {
  return rows.filter(row => row.verified === true && row.promotionType === 'taco' && Date.parse(row.expiresAt) > now && Date.parse(row.gameStartTime) > now);
}
