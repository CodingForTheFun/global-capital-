...
function logs(n=22,{integer=true}={}){return Array.from({length:n},(_,i)=>({gameId:'g'+i,date:new Date(Date.parse('2026-01-01T00:00:00Z')+i*7*86400000).toISOString(),opponent:i%4===0?'BOS':'NYK',value:integer?18+(i%7)+Math.floor(i/8):18.2+(i%7)*.7+i*.08}));}
...
test('adaptive model is configured for every current Auto Scout research sport',()=>{
 ...
 for(const sport of ADAPTIVE_SPORTS){const out=adaptivePrediction({research:research(),target:target(sport),now:NOW});assert.equal(out.available,true,sport);assert.equal(out.engine,'Auto Scout Adaptive');assert.equal(out.validation.method,'rolling-player-history');assert.ok(out.validation.observations>0);assert.ok(Number.isFinite(out.projection));assert.ok(Math.abs(out.probabilityOver+out.probabilityUnder+out.probabilityPush-1)<1e-9);}
});

test('model fails closed on short history and rejects future rows',()=>{
 const short=adaptivePrediction({research:research(logs(7)),target:target(),now:NOW});assert.equal(short.available,false);assert.equal(short.code,'MODEL_NOT_READY');
...
test('snapshot store uses adaptive model when no Sportstradamus file exists',async()=>{
