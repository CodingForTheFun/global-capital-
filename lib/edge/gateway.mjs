...
function requestHost(req) {
  // Railway/proxies can replace Host with the internal service hostname while
  // preserving the browser-facing host in X-Forwarded-Host. Canonicalization
  // must key off the customer-visible host or retired domains can accidentally
  // serve a second copy of the app (and therefore a separate cookie/PWA origin).
  const forwarded = String(req?.headers?.['x-forwarded-host'] || '').split(',')[0].trim();
  return String(forwarded || req?.headers?.host || '').toLowerCase().split(':')[0].trim();
}
