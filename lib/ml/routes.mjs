import {clientIp} from '../session.mjs';
import {createMLStore} from './snapshot-store.mjs';
import {researchPlayerProp} from '../autoscout/research-service-v2.mjs';
import {createGlobalPredictor} from './global/predictor.mjs';
// Production explicitly opts into the adaptive verified-history fallback and the
// global model. Direct createMLStore callers retain snapshot-only semantics.
export const globalPredictor = createGlobalPredictor();
const defaultStore = createMLStore({research:researchPlayerProp,global:globalPredictor});
...
export function createMLHandler({store=defaultStore,clock=Date.now,status=()=>globalPredictor.status()}={}) {
  const rates = new Map();
  return async function handleML(req,res) {
    const pathname = new URL(req.url || '/','http://localhost').pathname;
    if (pathname === '/api/props/ml/status') {
      if (req.method !== 'GET') {send(res,405,{ok:false,code:'METHOD_NOT_ALLOWED'},{allow:'GET'});return true;}
      try {send(res,200,{ok:true,sports:await status()});} catch {send(res,503,{ok:false,code:'MODEL_STATUS_UNAVAILABLE'});}
      return true;
    }
    if (pathname !== '/api/props/ml') return false;
    if (req.method !== 'POST') {send(res,405,{ok:false,code:'METHOD_NOT_ALLOWED'},{allow:'POST'});return true;}
    const ip=clientIp(req)||'unknown',now=clock();
