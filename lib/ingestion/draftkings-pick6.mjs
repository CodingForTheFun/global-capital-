...
/**
 * The active Pick6 leagues that belong to one of our sports.
 *
 * DraftKings moved the league identity under a nested `league` object and
 * dropped the flat name/displayName fields. sportLeagueKey is only a numeric
 * pair ("1-1"), so reading the old fields matched nothing and every sport
 * returned zero rows. Both shapes are accepted so a rollback cannot re-break it.
 */
export function pick6Leagues(root, sport) {
  const requested = text(sport).toUpperCase();
  if (!SUPPORTED.has(requested)) return [];
  const leagueName = row => row?.league?.leagueAbbreviation || row?.league?.leagueName || row?.name || row?.displayName;
  return list(root?.sportLeagues)
    .filter(row => row?.hasPicksAvailable === true && text(row?.sportLeagueKey))
    .filter(row => sportKey(row.sportLeagueKey) === requested || sportKey(leagueName(row)) === requested);
}
...
export async function fetchDraftKingsPick6Records(sport) {
  ...
  const matches = pick6Leagues(root, requested);
