...
// Sportsbook regions. The Odds API bills [markets] x [regions] per event-odds
// request, so this list is the single biggest lever on credit spend — five
// regions cost five times one. Set THE_ODDS_API_REGIONS=off to fall back to
// the narrower bookmaker filter below, which bills as one region per ten books.
const DEFAULT_REGIONS = Object.freeze(['us', 'us2', 'us_dfs', 'eu', 'uk']);
...
function maxMarkets() { return intEnv('THE_ODDS_API_MAX_MARKETS_PER_EVENT', 25, 1, 25); }
 ...
 * per event and one event-list call. Surfaced in owner diagnostics so the
...
export function projectedCreditsPerRefresh() {
  ...
  return {
    ...
    creditsPerRefresh: events * (perEvent + 1) + 1,
...
async function discoverMarkets(sportKey, eventId, scope, { signal, sport, includeAlternates = false } = {}) {
  const scopeKey = scope.regions.length ? `r:${scope.regions.join(',')}` : `b:${scope.bookmakers.join(',')}`;
  const key = `${sportKey}|${eventId}|${includeAlternates ? 'alt' : 'main'}|${scopeKey}`;
...
export async function fetchBoard(sport, { signal, force = false, includeAlternates = false } = {}) {
  ...
  try {
    ...
    const results = await mapLimit(sourceEvents, 2, async (rawEvent) => {
      ...
      try {
        const markets = await discoverMarkets(sportKey, eventId, scope, { signal, sport: selected, includeAlternates });
