...
const sessionSecret = process.env.APP_SESSION_SECRET || process.env.DASHBOARD_SESSION_SECRET || crypto.createHash('sha256').update(`autoprop-v4:${process.env.AUTOPROP_MASTER_KEY || 'local'}`).digest('hex');
...
function sanitizeScanResult(result) {
  ...
  const validKey = (pick) => pick?.id || pick?.sourceUrl;
  const validIds = new Set(visible.map(validKey));
  const keep = (pick) => validPickIdentity(pick) && (!validKey(pick) || validIds.has(validKey(pick)));
  return {
    ...result,
    picks: visible,
    diversifiedCard: (result?.diversifiedCard || []).filter(keep),
    bestAvailable: (result?.bestAvailable || []).filter(validPickIdentity),
    greenGoblins: (result?.greenGoblins || []).filter(validPickIdentity),
    redGoblins: (result?.redGoblins || []).filter(validPickIdentity),
...
async function saveResult(userId, rawResult) {
  ...
  history.unshift({
    ...
    mode: result.mode,
    scannerVersion: result.scannerVersion || 'friends-v4',
...
function friendlyError(error) {
  ...
  if (/AUTH_UI_CHANGED|sign-in UI could not be opened|auth form could not be opened/i.test(message)) return 'PickFinder changed its sign-in screen. AutoProp captured a diagnostic and stopped safely.';
  if (/interactive verification|captcha|two.factor|2fa|security code/i.test(message)) return 'PickFinder requires interactive verification for this account. Complete it on PickFinder, then reconnect.';
  ...
  if (/reconnect|not unlocked|still locked|locked/i.test(message)) return 'This PickFinder account is not exposing unlocked analytics. Reconnect your own account before scanning.';
  if (/unexpected token|could not be decrypted|unreadable/i.test(message)) return 'Saved PickFinder connection data could not be read. Reconnect this account.';
  return message;
}
...
function runUserWorker(userId, action, payload = {}, onProgress = null, timeoutMs = 12 * 60 * 1000) {
  return new Promise(async (resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      child.kill('SIGKILL');
      const error = new Error('PickFinder worker timed out safely.');
...
async function scanNow(userId) {
  ...
  activeScans++;
  state.progress = { stage: 'starting', message: 'Starting your private PickFinder scan', reviewed: 0, total: 0, currentPlayer: null, qualifiedSoFar: 0, startedAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
  try {
...
function signPayload(payload) { return crypto.createHmac('sha256', sessionSecret).update(payload).digest('base64url'); }
...
function clientKey(req, bucket, userId = '') {
  const ip = String(req.headers['x-forwarded-for'] || req.socket.remoteAddress || 'unknown').split(',')[0].trim();
  return `${bucket}:${userId || ip}`;
}
function allowRate(req, bucket, max, windowMs, userId = '') {
  const key = clientKey(req, bucket, userId);
  const now = Date.now();
...
function searchCacheKey(userId, id) { return `${userId}:${id}`; }
function cacheSearchResults(userId, rows) {
  const now = Date.now();
  for (const [key, entry] of focusedSearchCache) if (now - entry.at > 20 * 60 * 1000) focusedSearchCache.delete(key);
  for (const row of rows || []) if (row?.id) focusedSearchCache.set(searchCacheKey(userId, row.id), { at: now, selection: row });
}
async function recordPayment(user, capture) {
  ...
  rows.unshift({ userId: user.id, autoPropEmail: user.email, orderId: capture?.id || null, status: capture?.status || null, captureId: purchase?.id || null, amount: purchase?.amount || null, payerEmail: capture?.payer?.email_address || null, capturedAt: new Date().toISOString() });
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/api/auth/register' && req.method === 'POST') {
    ...
    if (!allowRate(req, 'register', 8, 30 * 60 * 1000)) return json(res, 429, { ok: false, message: 'Too many account attempts. Try again later.' });
    try {
  ...
  if (url.pathname === '/api/auth/login' && req.method === 'POST') {
    if (!sameOrigin(req)) return json(res, 403, { ok: false, message: 'Cross-origin request rejected.' });
    if (!allowRate(req, 'login', 14, 15 * 60 * 1000)) return json(res, 429, { ok: false, message: 'Too many login attempts. Try again later.' });
    try {
      const body = await readJsonBody(req, 16_000);
  ...
  if (url.pathname.startsWith('/api/')) {
    ...
    if (url.pathname === '/api/status' && req.method === 'GET') {
      ...
      return json(res, 200, { user, running: state.running, lastError: state.lastError, connection, rules, progress: state.progress, demoMode: false, autoScanMinutes: intervalMinutes, liveSports: Object.keys(LIVE_SPORTS), paymentConfigured: payment.enabled, scannerVersion: 'friends-v4-masterpiece', latest });
    }
    if (url.pathname === '/api/history' && req.method === 'GET') return json(res, 200, await readJson(stateFile(userId, 'history.json'), []));
    if (url.pathname === '/api/rules' && req.method === 'GET') return json(res, 200, { rules: await loadRules(userId), presets: RULE_PRESETS, locked: { verifiedSourcesOnly: true, todayOnly: true, lineTypes: ['REGULAR', 'GREEN_GOBLIN', 'RED_GOBLIN'], allPickFinderApps: true } });
    if (url.pathname === '/api/rules' && req.method === 'PUT') {
      ...
      try {
        const body = await readJsonBody(req, 16_000);
        return json(res, 200, { ok: true, rules: await saveRules(userId, body.rules || body), locked: { verifiedSourcesOnly: true, todayOnly: true, lineTypes: ['REGULAR', 'GREEN_GOBLIN', 'RED_GOBLIN'], allPickFinderApps: true } });
      } catch (error) { return json(res, 400, { ok: false, message: error?.message || String(error) }); }
    }
    if (url.pathname === '/api/connect' && req.method === 'POST') {
      ...
      try {
        ...
        const connection = await runUserWorker(userId, 'connect', { email: body.email, password: body.password }, (next) => { state.progress = { ...state.progress, ...next, updatedAt: new Date().toISOString() }; }, 150_000);
        await Promise.all([
    ...
    if (url.pathname === '/api/scan' && req.method === 'POST') {
      if (!sameOrigin(req)) return json(res, 403, { ok: false, message: 'Cross-origin request rejected.' });
      if (!allowRate(req, 'full-scan', 8, 15 * 60 * 1000, userId)) return json(res, 429, { ok: false, message: 'Full scan rate limit reached. Try again shortly.' });
      ...
      return json(res, 202, { ok: true, message: 'Your private production scan started.' });
    }
    if (url.pathname === '/api/prop-search' && req.method === 'GET') {
      if (!allowRate(req, 'prop-search', 40, 10 * 60 * 1000, userId)) return json(res, 429, { ok: false, message: 'Search rate limit reached. Try again shortly.' });
      const query = String(url.searchParams.get('q') || '').trim().slice(0, 120);
      try {
        const result = await runUserWorker(userId, 'search', { query }, null, 90_000);
        cacheSearchResults(userId, result?.results || []);
        return json(res, 200, result);
      } catch (error) { return json(res, 400, { ok: false, message: friendlyError(error), results: [] }); }
    }
    if (url.pathname === '/api/scan-prop' && req.method === 'POST') {
      if (!sameOrigin(req)) return json(res, 403, { ok: false, message: 'Cross-origin request rejected.' });
      if (!allowRate(req, 'focused-scan', 24, 15 * 60 * 1000, userId)) return json(res, 429, { ok: false, message: 'Focused scan rate limit reached. Try again shortly.' });
      try {
        const body = await readJsonBody(req, 8_000);
        const cached = focusedSearchCache.get(searchCacheKey(userId, String(body.id || '')));
        ...
        const result = await runUserWorker(userId, 'scan-prop', { selection: cached.selection }, null, 120_000);
        return json(res, 200, { ok: true, ...result });
      } catch (error) { return json(res, 400, { ok: false, message: friendlyError(error) }); }
    }
    ...
    if (url.pathname === '/api/payments/create-order' && req.method === 'POST') {
      if (!sameOrigin(req)) return json(res, 403, { ok: false, message: 'Cross-origin request rejected.' });
      if (!allowRate(req, 'paypal-create', 20, 10 * 60 * 1000, userId)) return json(res, 429, { ok: false, message: 'Too many checkout attempts. Try again later.' });
    ...
    if (url.pathname === '/api/payments/capture-order' && req.method === 'POST') {
      if (!sameOrigin(req)) return json(res, 403, { ok: false, message: 'Cross-origin request rejected.' });
      if (!allowRate(req, 'paypal-capture', 30, 10 * 60 * 1000, userId)) return json(res, 429, { ok: false, message: 'Too many payment attempts. Try again later.' });
      try {
        const body = await readJsonBody(req, 8_000);
        const capture = await capturePayPalOrder(body.orderId);
        if (capture?.status === 'COMPLETED') await recordPayment(user, capture);
...
server.listen(port, () => {
  console.log(`AutoProp Scout Pro Friends v4 running on http://localhost:${port}`);
  console.log('Auth: isolated multi-user AutoProp accounts');
  console.log(`PickFinder: one encrypted session per user • max concurrent scans ${maxConcurrentScans}`);
  console.log('Scanner: other-chat production masterpiece + unlock-aware auth-v3');
  console.log(`Live sports: ESPN-backed • ${Object.keys(LIVE_SPORTS).join(', ')}`);
