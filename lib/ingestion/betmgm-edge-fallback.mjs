...
async function edgeFetcher(url) {
  ...
  const response = await globalThis.fetch(PROXY_URL, {
    ...
    signal: ingestionSignal(AbortSignal.timeout(8_000)),
  });
...
export async function fetchBetMgmPublic(sport, options = {}) {
  ...
  // Railway egress can be challenged by BetMGM. Prefer the authenticated edge
  // relay first when configured so a blocked direct path cannot consume most
  // of the sportsbook refresh window.
  if (PROXY_URL && ANON_KEY) {
    try {
      proxied = await fetchDirectBetMgm(sport, { ...options, force: true, fetcher: edgeFetcher });
  ...
  throw proxiedError || directError || Object.assign(new Error('BETMGM_PUBLIC_FAILED'), { code: 'BETMGM_PUBLIC_FAILED' });
}
