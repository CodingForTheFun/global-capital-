  ...
  return <section className="min-h-screen bg-[#070b12] px-2 py-3 text-slate-100 sm:px-4 lg:px-6" data-design="premium-player-research-v1" data-deep-dive="player-prop-deep-dive-v1" data-research-route={routeKind}>
    ...
    <div className="mx-auto max-w-[1180px] overflow-hidden rounded-2xl border border-slate-800/90 bg-[#0b101a] shadow-[0_30px_90px_rgba(0,0,0,0.45)]">
      ...
      <div className="space-y-3 p-3 sm:p-4 lg:p-5">
        <div data-research-hero className="rounded-2xl border border-slate-800 bg-[radial-gradient(circle_at_15%_10%,rgba(59,130,246,0.13),transparent_30%),linear-gradient(180deg,rgba(19,27,46,0.9),rgba(10,15,24,0.96))] p-2.5 sm:p-4">
          <div className="flex items-start justify-between gap-3">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0"><div className="flex flex-wrap items-center gap-1.5"><h1 className="truncate text-lg font-black tracking-tight text-white sm:text-2xl">{player.name}</h1><span className="rounded-md border border-slate-700 bg-slate-900/90 px-1.5 py-0.5 text-[10px] font-black text-slate-400">{position}</span></div><p className="mt-0.5 text-[11px] font-semibold text-slate-400 sm:mt-1 sm:text-xs">{game} · {kickoffLabel(player.startsAt)}</p><div className="mt-1 flex flex-wrap items-center gap-2 text-[9px] text-slate-500 sm:mt-2 sm:text-[10px]"><span>{team || history?.player?.team || 'Team unavailable'}</span>{analysis.loading ? <><span>•</span><span>Loading history…</span></> : analysis.unavailable ? <><span>•</span><span className="text-amber-300">History unavailable</span></> : <><span>•</span><span className="text-emerald-400">Verified history</span></>}</div></div>
            </div>
          ...
          <div className="mt-2.5 flex flex-col gap-1.5 rounded-xl border border-sky-500/20 bg-sky-500/[0.07] p-2 sm:mt-4 sm:flex-row sm:gap-2 sm:p-2.5 sm:items-center sm:justify-between"><div className="flex min-w-0 items-center gap-2"><span className="rounded-md bg-sky-500 px-2 py-1 text-[10px] font-black tracking-wide text-white">{initials(selected.bookName)}</span><div className="min-w-0"><div className="flex min-w-0 items-center gap-2"><h2 className="truncate text-xs font-bold text-white sm:text-sm">{title}</h2>{variantLabel && <span data-variant={selected.dfsOddsType} className="shrink-0 rounded-md border border-emerald-400/20 bg-emerald-400/10 px-1.5 py-0.5 text-[8px] font-black uppercase tracking-wide text-emerald-300">{variantLabel}</span>}</div><div className="text-[10px] text-slate-500">Main prop · {period}</div></div></div><div className="flex items-center gap-2"><label className="relative block"><span className="sr-only">Selected book</span><select aria-label="Selected book" value={currentBook || ''} onChange={event => onBook(event.target.value)} className="h-8 appearance-none rounded-lg border border-sky-400/20 bg-slate-950/60 pl-2 pr-6 text-[10px] font-bold text-slate-300 outline-none">{allowBestPrices && <option value="">Best prices · all books</option>}{applicableBooks.map(book => <option key={book.key} value={book.key}>{book.name}</option>)}</select><ChevronDown className="pointer-events-none absolute right-1.5 top-2 h-3 w-3 text-slate-500"/></label><div className="flex items-center gap-2 text-sm font-black"><span className={selected.side === 'UNDER' ? 'text-rose-400' : 'text-emerald-400'}>{selected.side === 'OVER' ? 'O' : selected.side === 'UNDER' ? 'U' : selected.choice} {selected.line ?? ''}</span><span className="text-slate-400">{offerPrice(selected)}</span></div></div></div>
        </div>

        <div className="space-y-3">
          <div className="flex items-center gap-2"><div role="group" aria-label="Stat categories" className="flex min-w-0 flex-1 gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{statFamilies.map(item => { const active = marketFamily(item) === family; return <button key={marketFamily(item)} type="button" onClick={() => chooseStat(item)} aria-pressed={active} title={marketName(item)} className={`h-9 shrink-0 rounded-lg border px-3 text-[11px] font-black tracking-wide transition ${active ? 'border-sky-400/50 bg-sky-500/15 text-sky-300 shadow-[0_0_20px_rgba(56,189,248,0.16)]' : 'border-slate-800 bg-slate-900/60 text-slate-500 hover:border-slate-700 hover:text-slate-300'}`}>{compactStatLabel(marketName(item))}</button>; })}</div><select aria-label="Player stat category" value={market.key} onChange={event => onCategory(event.target.value)} className="h-9 max-w-[130px] shrink-0 rounded-lg border border-slate-700 bg-slate-900 px-2 text-[10px] font-bold text-slate-300 outline-none">{player.markets.map(item => <option key={item.key} value={item.key}>{marketOptionName(item)}</option>)}</select></div>
          <div role="group" aria-label="Available game periods" className="flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{periodMarkets.map(item => { const active = item.key === market.key; return <button key={item.key} type="button" onClick={() => onCategory(item.key)} aria-pressed={active} className={`h-8 shrink-0 rounded-full border px-3 text-[10px] font-bold transition ${active ? 'border-blue-400/40 bg-blue-500 text-white shadow-[0_0_18px_rgba(59,130,246,0.24)]' : 'border-slate-800 bg-[#0e1522] text-slate-500 hover:text-slate-300'}`}>{periodName(item.period)}</button>; })}</div>
        ...
        <div aria-label="Trained model prediction" className="rounded-2xl border border-slate-800 bg-[#0d1420] p-3 sm:p-4"><div className="mb-2 text-[10px] font-black uppercase tracking-[0.16em] text-slate-500">Model choice</div><div className="pointer-events-none">{model}</div></div>
