...
function prizePicksSpecialStrip(g){
 ...
 var chips=rows.map(function(row){
  ...
  var fallback=direction||(row.specialType==='demon'?'Demon':'Goblin');
  var type=row.specialType==='demon'?'Demon':'Goblin';
  var gap=num(row.lineGap),gapText=gap==null?'':(gap>0?'+':'')+dec(gap,1)+' vs standard';
  return '<span class="asPpSpecialChip '+esc(row.specialType)+'">'+face+'<b>'+esc(type+' · '+direction)+'</b><em>'+esc(dec(row.line,1))+(gapText?' · '+esc(gapText):'')+'</em></span>';
 }).filter(Boolean);
...
function underdogSpecialStrip(g){
 if(g.archived||!Array.isArray(g.underdogSpecialRows)||!g.underdogSpecialRows.length)return '';
 var rows=g.underdogSpecialRows.slice().sort(function(a,b){return num(b.payoutMultiplier)-num(a.payoutMultiplier)||num(a.line)-num(b.line);});
 var chips=rows.map(function(row){
  var mult=num(row.payoutMultiplier);if(mult==null||mult===1)return '';
  var direction=row.side==='UNDER'?'Lower':'Higher',tone=mult>1?'boost':'discount';
  return '<span class="asUdSpecialChip '+tone+'"><b>'+esc(mult.toFixed(2)+'×')+'</b><em>'+esc(direction+' '+dec(row.line,1))+'</em></span>';
 }).filter(Boolean);
 if(!chips.length)return '';
 return '<div class="asUdSpecialStrip" aria-label="Underdog payout modifiers"><span class="asPpSpecialLabel">Underdog modifiers</span>'+chips.join('')+'</div>';
}
`;

const GROUP_FUNCTION = String.raw`function groups(allBooks=false){var m=new Map(),specials=new Map(),underdogSpecials=new Map(),pm=playerMap();(payload.props||[]).forEach(function(r){if(!allBooks&&!bookEnabled(r,selectedBooks))return;var k=groupKey(r);if(r.isAlternate){if(r.specialVerified===true&&r.sportsbookKey==='prizepicks'&&['goblin','demon'].includes(r.specialType)){if(!specials.has(k))specials.set(k,[]);specials.get(k).push(r);}else if(r.specialVerified===true&&r.sportsbookKey==='underdog'&&num(r.payoutMultiplier)!=null&&num(r.payoutMultiplier)!==1){if(!underdogSpecials.has(k))underdogSpecials.set(k,[]);underdogSpecials.get(k).push(r);}return;}if(!m.has(k)){var p=pm.get(r.playerId)||{};m.set(k,{key:k,sport:r.sport,eventId:r.eventId,providerEventId:r.providerEventId||'',espnEventId:r.espnEventId||'',playerId:r.playerId,playerName:r.playerName,entityType:r.entityType||p.entityType||'player',providerPlayerId:r.providerPlayerId||p.providerPlayerId||'',team:r.team||p.team||'',position:p.position||'',marketId:r.marketId,market:r.market,homeTeam:r.homeTeam,awayTeam:r.awayTeam,homeTeamLogoUrl:r.homeTeamLogoUrl||'',awayTeamLogoUrl:r.awayTeamLogoUrl||'',gameStartTime:r.gameStartTime,live:!!r.live,rows:[]});}var g=m.get(k);if(!g.providerEventId&&r.providerEventId)g.providerEventId=r.providerEventId;if(!g.espnEventId&&r.espnEventId)g.espnEventId=r.espnEventId;if(!g.homeTeamLogoUrl&&r.homeTeamLogoUrl)g.homeTeamLogoUrl=r.homeTeamLogoUrl;if(!g.awayTeamLogoUrl&&r.awayTeamLogoUrl)g.awayTeamLogoUrl=r.awayTeamLogoUrl;g.rows.push(r);});return Array.from(m.values()).map(function(g){return {...g,market:propType(g),specialRows:specials.get(g.key)||[],underdogSpecialRows:underdogSpecials.get(g.key)||[],comparisonOffers:g.rows,rows:dedupeOffers(g.rows)};});}`;
...
#as5 .asUdSpecialStrip{display:flex;align-items:center;gap:6px;overflow:auto;margin:6px 9px 0;padding:6px 8px;border:1px solid rgba(53,133,255,.36);border-radius:9px;background:rgba(8,18,36,.82)}
#as5 .asUdSpecialChip{flex:0 0 auto;display:inline-flex;align-items:center;gap:5px;padding:4px 7px;border:1px solid #334155;border-radius:999px;background:#0d1522;font-size:8px}
#as5 .asUdSpecialChip.boost{border-color:rgba(16,185,129,.42);background:rgba(6,78,59,.23)}#as5 .asUdSpecialChip.discount{border-color:rgba(245,158,11,.4);background:rgba(120,53,15,.2)}
#as5 .asUdSpecialChip b{color:#eef8ff}#as5 .asUdSpecialChip em{font-style:normal;color:#aeb9c9;font-weight:900}
...
@media(max-width:540px){#as5 .asRecentFive{margin-left:8px;margin-right:8px;padding:9px 8px 8px}#as5 .asRecentFiveChart{gap:4px;min-height:99px}#as5 .asRecentFiveGame{grid-template-rows:17px 57px 15px 12px}#as5 .asRecentFiveBar{height:57px}#as5 .asRecentFiveHead em{max-width:132px}#as5 .asRecentFiveFoot{font-size:7px}#as5 .asPpSpecialStrip,#as5 .asUdSpecialStrip{margin-left:8px;margin-right:8px}}
`;
