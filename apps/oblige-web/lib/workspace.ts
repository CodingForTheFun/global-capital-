...
export type WorkspacePlayer={key:string;playerId:string|null;name:string;position?:string|null;aliases:string[];sport:string;eventId:string;startsAt:string|null;homeTeam:string|null;awayTeam:string|null;markets:WorkspaceMarket[]};
...
export function toResearchGroup(player:WorkspacePlayer,market:WorkspaceMarket,offer:WorkspaceOffer):PropGroup{
 ...
 return {key:JSON.stringify([player.key,market.key,offer.line]),propId:offer.outcomeId,player:player.name,providerPlayerId:player.playerId,market:market.label,marketId:market.marketKey,line:offer.line,sport:player.sport,position:player.position??null,team:null,opponent:null,homeTeam:player.homeTeam,awayTeam:player.awayTeam,matchup:[player.awayTeam,player.homeTeam].filter(Boolean).join(' @ ')||'Matchup unavailable',startsAt:player.startsAt,live:!!player.startsAt&&Date.parse(player.startsAt)<=Date.now(),quotes,bestOver:best('OVER'),bestUnder:best('UNDER')};
}
