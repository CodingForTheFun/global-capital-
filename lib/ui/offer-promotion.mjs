...
/**
 * Source-verified PrizePicks alternate-line marker. These variants are display
 * metadata only: they never become Taco offers, standard sportsbook lines, or
 * consensus inputs.
 */
export function verifiedPrizePicksSpecial(offer, now=Date.now()) {
  if (!offer || offer.sportsbookKey !== 'prizepicks' || offer.isAlternate !== true || offer.specialVerified !== true ||
      ...
      !id(offer.specialSourceId) || offer.archived === true || offer.stale === true || offer.live === true || offer.completed === true || !finite(now)) return null;
...
export function prizePicksSpecialFaceHtml(offer,now=Date.now()) {
  ...
  return '<span class="asPpFace '+(demon?'demon':'goblin')+'" data-pp-special-until="'+special.validUntil+'" data-pp-special-id="'+esc(special.sourceId)+'" role="img" aria-label="'+label+'" title="'+label+' · source-verified '+(special.side==='OVER'?'More':'Less')+' variant">'+(demon?demonFaceSvg():goblinFaceSvg())+'</span>';
}
