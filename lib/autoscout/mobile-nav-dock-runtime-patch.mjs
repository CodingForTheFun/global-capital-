const MARKER = 'mobile-nav-scroll-hide-runtime-v5';

// The dock is centred by `left:50%` plus `transform:translateX(-50%)`, so every
// transform written here has to carry that -50% as its X component. A
// `.asNav.asNavAutoHideRail` rule outranks the one-class `.asNav` rule that
// centres it, so dropping the X translation does not merely skip an animation:
// it leaves the dock sitting at the 50% mark with nothing pulling it back, and
// on a 390px viewport that puts 181px of it — Best Lines and Compare — off the
// right edge, unreachable, with no scroll container to recover them.
...
@media(max-width:720px){
 #as5 .asNav.asNavAutoHideRail.asNavScrollHidden{
  transform:translate3d(-50%,calc(100% + 24px + env(safe-area-inset-bottom)),0)!important;
  opacity:0!important;
...
;(function scrollAwareMobileNavRuntime(){
 ...
 function applyHidden(){
  if(!nav)return;
  var shouldHide=desiredHidden&&isMobile();
  hidden=shouldHide;
  nav.classList.toggle('asNavScrollHidden',shouldHide);
 }

 function setHidden(next){
  desiredHidden=!!next&&isMobile();
  if(!nav)return;
  var shouldHide=desiredHidden;
 ...
 function updateFromScroll(){
  ...
  if(direction>0&&y-directionAnchor>=7){
   setHidden(true);
  }else if(direction<0&&directionAnchor-y>=3){
   setHidden(false);
  }
 ...
 function mount(){
  ...
  nav=nextNav;
  if(nav.dataset.scrollHideReady==='5'){
   applyHidden();
   return true;
  }
  nav.dataset.scrollHideReady='5';
