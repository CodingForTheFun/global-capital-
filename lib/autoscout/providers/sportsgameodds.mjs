...
export async function fetchSportsGameOddsBoard(sport, options = {}) {
  ...
  const task = (async () => {
    ...
    try {
      ...
      // Keep common league tabs on the existing two-call path (usage + events).
      // The entitlement catalog is needed only for aggregate sportID tabs and is
      // separately warmed once at process start.
      const catalog = sportID
      ...
      const payload = await sportsGameOddsGet('/events', {
        leagueID: sportID ? undefined : leagues.join(','),
      ...
      board.meta = {
        ...
        leaguesRequested: sportID ? [] : leagues,
