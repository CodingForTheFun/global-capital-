...
const DEFAULT_SITE = String(process.env.AUTOSCOUT_DRAFTKINGS_SITE || 'US-VA-SB').trim().toUpperCase() || 'US-VA-SB';
const SITE_CANDIDATES = [...new Set([DEFAULT_SITE, 'US-VA-SB', 'US-SB'])];
...
function controlBase(site) {
  return `https://sportsbook-nash.draftkings.com/sites/${site}/api/sportscontent/controldata/league/leagueSubcategory/v1/markets`;
}

function buildUrl(site, leagueId, subcategoryId) {
  const url = new URL(controlBase(site));
...
function futureGame(event) {
  ...
  const status = text(event?.status || event?.eventStatus).toLowerCase().replace(/[\s_-]+/g, '');
  return !['live', 'started', 'inprogress', 'final', 'finished', 'closed', 'settled', 'resulted'].includes(status);
}
...
function playerFromMarket(marketName, statLabel) {
  ...
  const statPattern = escapeRegex(statLabel)
    .replace(/\\\+/g, '\\s*\\+\\s*')
    .replace(/\\ /g, '\\s+');
  raw = raw
    .replace(new RegExp(statPattern, 'ig'), ' ')
...
async function fetchControlData(sport, { fetcher = globalThis.fetch } = {}) {
  ...
  for (const [subcategoryId, statLabel] of selected) {
    ...
    for (const site of SITE_CANDIDATES) {
      try {
        const url = buildUrl(site, leagueId, subcategoryId);
        const fetched = await fetchJson(url, fetcher);
    ...
    if (!categoryFetched && lastError?.status && lastError.status !== 403) break;
  }
  ...
  return {
    ...
    endpoint: endpoint || `sportsbook-nash.draftkings.com/sites/${DEFAULT_SITE}`,
