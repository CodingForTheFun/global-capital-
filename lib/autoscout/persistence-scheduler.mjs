...
async function persistBoardsCycle(reason, deps, budgets) {
  ...
  try {
    ...
    if (providerMode !== 'sportsgameodds' && publicWorkerConfigured()) {
      try {
    ...
    for (const sport of nextSports(activePersistenceSports, perCycle)) {
      try {
        ...
        console.log(`[AutoScout persistence] ${reason} ${sport} ${apiOnly ? providerMode + '-api-only' : 'public-only'} persisted=${Boolean(result?.persisted)} events=${counts.events || 0} props=${counts.props || 0} lines=${counts.lines || 0} snapshots=${counts.snapshots || 0}`);
