...
const containsEvidence = (stored, expected) => stored && historyKey(stored) === historyKey(expected)
  && stored.sport === expected.sport
  && stored.player_name === expected.player_name
  && Date.parse(stored.game_date) === Date.parse(expected.game_date)
  && String(stored.season) === String(expected.season)
  && stored.season_type === expected.season_type
  && stored.source === 'ESPN'
  // Existing rows may contain richer stats. Every field proved by the fresh
  // public evidence must agree exactly; extra stored fields are preserved.
