...
export const PUBLIC_FEEDS=Object.freeze([
 ...
 {id:'underdog',urls:['https://api.underdogfantasy.com/beta/v6/over_under_lines','https://api.underdogfantasy.com/beta/v5/over_under_lines'],ttl:THREE_MINUTES,origin:'https://underdogfantasy.com',referer:'https://underdogfantasy.com/',headers:{'client-type':'web','client-version':UNDERDOG_CLIENT_VERSION}},
...
export function createPublicFeeds({fetcher=globalThis.fetch,now=Date.now,storeDir=null,mappings=[],feeds=PUBLIC_FEEDS}={}){
 ...
 async function fetchPages(feed){
  ...
  for(const candidate of candidates){
   try{return await fetchFrom(feed,candidate);}catch(error){lastError=error;}
  }
 ...
 async function refreshOne(feed){
  ...
  const task=(async()=>{
   try{
    ...
    state.set(feed.id,{...parsed,fetchedAt:new Date(now()).toISOString(),nextAt:now()+feed.ttl,failures:0,status:parsed.records.length?'available':feed.id==='sleeper'?'roster_only':parsed.contracts.length?'unmapped_contracts':'no_props',partial,httpStatus:200,endpoint,configVersion:FEED_CONFIG_VERSION});
   }catch(error){const failures=(previous?.failures||0)+1;const delay=error.retryMs||Math.min(3600000,60000*2**Math.min(6,failures-1))+Math.floor(Math.random()*10000);state.set(feed.id,{...(previous||empty()),failures,nextAt:now()+delay,status:error.status===429?'cooldown':'unavailable',httpStatus:error.status||null,endpoint:error.endpoint||null,configVersion:FEED_CONFIG_VERSION});}
 ...
 function health(){return feeds.map(f=>{const s=state.get(f.id);return {id:f.id,status:s?.status||'not_checked',httpStatus:s?.httpStatus??null,endpoint:s?.endpoint||null,failures:s?.failures||0,lineCount:s?.records?.length||0,rosterCount:s?.players?.length||0,contractCount:s?.contracts?.length||0,fetchedAt:s?.fetchedAt||null,nextAt:s?.nextAt||null,partial:!!s?.partial};});}
