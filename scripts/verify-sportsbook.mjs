...
try{
 ...
 await page.route('**/api/props/ask',async r=>{const body=r.request().postDataJSON();check(body.prop.line===55.5&&body.prop.gameLog.length===5,'Ask sends active measured context');return r.fulfill({json:{available:true,answer:'Three of these five results exceeded 55.5.'}})});
