...
function proplineModeEnabled() {
  if (['1','true','yes','on'].includes(text(process.env.PROPLINE_REALTIME_DISABLED).toLowerCase())) return false;
...
export async function handleProplineRealtimeEvent({ type, payload, sequence, deliveryId = null } = {}) {
  if (!proplineModeEnabled()) return { accepted: 0, disabled: true };
...
export async function ensureProplineRealtimeSubscription() {
  if (!proplineModeEnabled()) return { configured: false, reason: 'PROPLINE_DISABLED_BY_PROVIDER_MODE' };
...
export function startProplineRealtime() {
  if (!proplineModeEnabled()) {
    console.log('[PropLine realtime] disabled by provider mode');
    return null;
  }
