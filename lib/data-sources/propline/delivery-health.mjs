...
// A delivery ledger read a second after the event was queued is mostly rows
// that have not been attempted yet. Treating "not yet succeeded" as "failed"
// reported a webhook pipeline that was accepting every event as unhealthy, so
// the three states are kept apart: only a terminal non-success is a failure,
// and a pending row counts towards neither.
const TERMINAL_SUCCESS = new Set(['delivered', 'success', 'succeeded', 'ok', 'complete', 'completed']);
const TERMINAL_FAILURE = new Set(['failed', 'failure', 'error', 'dead', 'exhausted', 'abandoned', 'dropped', 'expired']);

function outcome(row) {
  const code = httpStatus(row);
  if (code !== null) return code >= 200 && code < 300 ? 'succeeded' : 'failed';
  const status = text(row?.status).toLowerCase();
  if (TERMINAL_SUCCESS.has(status)) return 'succeeded';
...
export async function refreshProplineDeliveryHealth({ subscriptionId = null, fetcher = globalThis.fetch } = {}) {
  ...
  try {
    ...
    const successful = outcomes.filter((value) => value === 'succeeded').length;
    ...
    state.failed = failed;
    state.pending = rows.length - successful - failed;
...
export function __resetProplineDeliveryHealth() {
  Object.assign(state, {
    ...
    subscriptionId: null, deliveries: 0, successful: 0, failed: 0, pending: 0, maxAttempts: 0,
