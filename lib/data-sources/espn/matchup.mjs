...
export function resolveNearbyMatchupEvent(scoreboard,target,{maxDriftMs=48*60*60_000,minSeparationMs=3*60*60_000}={}) {
 const league=PUBLIC_LEAGUES[target.sport]?.[1],start=timestamp(target.gameStartTime);
 if(!league||!Number.isFinite(start)||!list(scoreboard?.leagues).some(l=>l.slug===league))return null;
 const candidates=[];
 for(const event of list(scoreboard.events)){
  if(!id(event?.id)||!Array.isArray(event.competitions)||event.competitions.length!==1)continue;
  const competition=event.competitions[0],teams=competitors(competition),when=timestamp(competition?.date||event?.date);
  if(String(competition?.id)!==String(event.id)||!teams||!Number.isFinite(when))continue;
  if(!matchesTeamRecord(target.homeTeam,teams.home.team,target.sport)||!matchesTeamRecord(target.awayTeam,teams.away.team,target.sport))continue;
  const drift=Math.abs(when-start);if(drift>maxDriftMs)continue;
  candidates.push({id:String(event.id),competition,drift,when});
 }
 candidates.sort((a,b)=>a.drift-b.drift||a.when-b.when||a.id.localeCompare(b.id));
 if(!candidates.length)return null;
 // Daily series can put the same teams on the board more than once. If the
 // nearest two candidates are too close to distinguish safely, fail closed
 // instead of guessing which ESPN event corresponds to the PropLine event.
 if(candidates[1]&&candidates[1].drift-candidates[0].drift<minSeparationMs)return null;
 return {id:candidates[0].id,competition:candidates[0].competition,driftMs:candidates[0].drift};
}
