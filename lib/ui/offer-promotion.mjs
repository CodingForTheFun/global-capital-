...
export function verifiedPrizePicksSpecial(offer, now=Date.now()) {
  ...
  const directional = ['OVER','UNDER'].includes(offer.side);
  const lineOnly = offer.side == null && offer.specialTypeSource === 'projection_odds_type' && offer.specialSideVerified !== true;
  if (!directional && !lineOnly) return null;
  if (directional && offer.specialTypeSource === 'outcome_metadata' && offer.specialSideVerified !== true) return null;
  ...
  return {type:offer.specialType,side:directional?offer.side:null,line:offer.line,sourceId:offer.specialSourceId,validUntil:Math.min(game,seen+SPECIAL_FRESHNESS_MS)};
}
...
export function prizePicksSpecialFaceHtml(offer,now=Date.now()) {
  ...
  const detail=special.side==='OVER'?'source-verified More variant':special.side==='UNDER'?'source-verified Less variant':'source-verified alternate line; direction not claimed';
