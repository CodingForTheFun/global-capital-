...
function isFantasyMarket(market, providerMarketKey) {
  return /fantasy/i.test(`${text(providerMarketKey)} ${text(market)}`);
}

async function fantasySportsGameOddsFallback({
  sport,
  playerName,
  team = null,
  homeTeam = null,
  ...
  // Keep paid historical fallback narrow: only fantasy markets with an exact
  // SportsGameOdds identity already attached to the live normalized prop.
  if (!isFantasyMarket(market, providerMarketKey) || !text(sportsGameOddsPlayerId)) return null;

  const fallback = await fetchSportsGameOddsResearch({
    sport,
    playerName,
    team,
    opponent,
  ...
  if (!fallback?.available || !Array.isArray(fallback.gameLog) || !fallback.gameLog.length) return null;

  return finalizeResearch({
    gameLog: fallback.gameLog,
    line,
    side,
    market,
    sport,
    source: fallback.source || 'SportsGameOdds results',
    player: fallback.player || {
      playerName: text(playerName),
      providerPlayerId: text(sportsGameOddsPlayerId),
      team: text(team) || null,
    },
    homeTeam,
    awayTeam,
...
export function finalizeResearch({
  ...
  homeTeam = null, awayTeam = null, opponent = null, opponentId = null, team = null, isHome = null,
  ...
  const result = analyzeResearch({
    ...
    player,
    matchup: { opponent: inferredOpponent, opponentId, isHome: typeof isHome === 'boolean' ? isHome : null, homeTeam: text(homeTeam) || null, awayTeam: text(awayTeam) || null },
    market,
    line: lineValue,
    side: selectedSide,
    season,
...
export async function researchPlayerProp({
  ...
  const tryFantasyFallback = () => fantasySportsGameOddsFallback({
    sport: leagueKey,
  ...
  if (!fieldsFor(leagueKey, selectedMarket, providerMarketKey)) {
    const fallback = await tryFantasyFallback();
