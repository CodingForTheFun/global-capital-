...
function adaptLine(row) {
  ...
  const options = (Array.isArray(row?.options) ? row.options : []).map(option => {
    ...
    return {
      ...
      // The public v2 API may attach payout modifiers to the main threshold.
      // We never turn those modifiers into American odds; the normalized line
      // remains the standard target value and price stays null.
...
export async function fetchUnderdogV2Payload({ maxSports = 16 } = {}) {
  ...
  for (const data of results.filter(Boolean)) {
    ...
    merged.games = mergeById(merged.games, data.games);
    merged.solo_games = mergeById(merged.solo_games, data.solo_games);
