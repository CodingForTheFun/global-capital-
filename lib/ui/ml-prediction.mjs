...
export function predictionHtml(value,side='OVER') {
  ...
  return '<section class="asML asMLReady" data-ml-state="READY" aria-label="Machine learning prediction"><div class="asMLHeading">ML prediction <small>'+esc(p.engine)+' · Model estimate</small></div>'
    ...
    +'<details><summary>Model details</summary><p>Version: '+esc(p.modelVersion)+' · Generated: '+esc(new Date(p.generatedAt).toLocaleString())+'</p><p>Held-out evaluation: '+esc(p.validation.observations)+' records across '+esc(p.validation.events)+' games. Not a guarantee of future performance.</p></details>'
  ...
  async function flush(){
    ...
    while(queue.length){
      ...
      batch.forEach((job,i)=>{
        ...
        // Never attach another requested target's result, even after a UI race or bad response.
