...
export function createVerifiedArtwork({fetchImpl=(...args)=>fetch(...args),now=()=>Date.now(),dataDir=process.env.DATA_DIR||'/app/data'}={}){
 ...
 async function resolveMlbOfficial(sport,name){
  if(sport!=='MLB')return null;
  const data=await json('https://statsapi.mlb.com/api/v1/people/search?'+new URLSearchParams({names:name,sportIds:'1'}),DAY);
