...
function headers() {
  return { accept: 'application/json', 'accept-language': 'en-US,en;q=0.9', 'user-agent': UA, origin: 'https://www.pinnacle.com', referer: 'https://www.pinnacle.com/', 'x-api-key': PUBLIC_WEB_KEY };
}
...
async function fetchJson(fetcher, path) {
  ...
  try { return await directJson(fetcher, url); }
    ...
    return browserJsonFetch(url, {
      ...
      headers: { 'x-api-key': PUBLIC_WEB_KEY },
    });
  }
}
