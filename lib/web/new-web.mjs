...
const PAGES = new Set(['/', '/board', '/scores', '/research', '/account', '/app.webmanifest', '/app-worker.js', '/app-icons/180.png', '/app-icons/192.png', '/app-icons/512.png']);
...
export async function serveNewWeb(req, res, { origin = newWebOrigin(), fetchImpl = fetch } = {}) {
  ...
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/scores' && upstream.status === 404) {
      const shell = await fetchImpl(origin + '/', { method: 'GET', headers, redirect: 'manual', signal: AbortSignal.timeout(8000) });
      if (shell.ok) { upstream = shell; scoresFallback = true; }
    }
  } catch { return false; }
