...
const defaultAuditCache=new WeakMap();
export function decorateBoardWithScoutAudit(board, rules = DEFAULT_AUTO_SCOUT_RULES) {
  ...
  const cached=rules===DEFAULT_AUTO_SCOUT_RULES?defaultAuditCache.get(rows):null;
  if(cached)return {...board,props:cached.props,meta:{...board.meta,...cached.meta}};
  ...
  if(rules===DEFAULT_AUTO_SCOUT_RULES)defaultAuditCache.set(rows,{props,meta:{autoScoutRulesVersion:AUTO_SCOUT_RULES_VERSION,autoScoutCounts:counts,autoScoutResearchDataComplete:counts.UNAVAILABLE===0}});
  return result;
}
