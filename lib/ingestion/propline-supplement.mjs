...
function exactQuoteIdentity(row) {
  ...
  return [base, line, text(row?.specialType || row?.dfsOddsType).toLowerCase(), Number(row?.payoutMultiplier) || 1].join('|');
}
    ...
    if (row?.isAlternate === true) {
      const supportedSpecial = row?.specialVerified === true
        && ((row?.sportsbookKey === 'prizepicks' && ['goblin','demon'].includes(row?.specialType))
          || (row?.sportsbookKey === 'underdog' && Number.isFinite(Number(row?.payoutMultiplier)) && Number(row.payoutMultiplier) !== 1));
