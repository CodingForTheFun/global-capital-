...
test('tennis ignores same-named leagues assigned to another sport in the live catalog', async () => {
  const previousKey = process.env.SPORTS_ODDS_API_KEY_HEADER;
  const previousMode = process.env.OBLIGE_PROP_PROVIDER_MODE;
  const previousFetch = globalThis.fetch;
  const urls = [];
  ...
  try {
    ...
    globalThis.fetch = async (input) => {
      ...
      if (url.pathname.endsWith('/leagues')) {
        return new Response(JSON.stringify({
          success: true,
          data: [{ leagueID: 'ATP', sportID: 'BASEBALL', name: 'ATP' }],
        }), { status: 200, headers: { 'content-type': 'application/json' } });
      }

      if (url.pathname.endsWith('/events')) {
        throw new Error('Tennis /events must not be called for a non-Tennis catalog league');
      }
