...
// #288 observed retry-owned claims moving the database next_at roughly a minute
// beyond the fixed process timer. Wait past that observed drift rather than
// polling toward it. The database claim remains the final authority, and the
// retry is still exactly one non-recursive second chance.
const LEASE_RETRY_DELAY_MS = 75_000;
...
export function __leaseRetryPolicy(reason, timerPending = false) {
  return {
    delayMs: LEASE_RETRY_DELAY_MS,
    shouldSchedule: !timerPending && !String(reason).includes('lease-retry'),
  };
}

function scheduleLeaseRetry(reason) {
  const policy = __leaseRetryPolicy(reason, Boolean(leaseRetryTimer));
