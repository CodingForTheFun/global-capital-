...
.authStep{border-top:1px solid #29425f80;padding-top:4px}.authStep h3{margin:0 0 8px;font-size:21px}.stepCopy{margin:0 0 18px;color:var(--muted);font-size:13px;line-height:1.6}.stepActions{display:flex;justify-content:center;flex-wrap:wrap;gap:14px;margin-top:12px}.textBtn{border:0;background:transparent;color:#8fb9ff;padding:4px;font-weight:650}.textBtn:disabled{opacity:.55;cursor:not-allowed}.stepMsg{min-height:20px;margin:12px 0 0;color:#ff9aa6;font-size:13px;text-align:center}
...
<div class="wrap">
 ...
 <footer>
  Oblige Props is a research tool. It reports what the connected sportsbook feeds and game logs actually returned — it does not take bets, hold funds, or guarantee outcomes. 18+. Past results do not predict future ones.
...
(function(){
 ...
 var RESEND_COOLDOWN_SECONDS=60,resendTimers={};
 function resetResendButton(id){
  var button=document.getElementById(id);
  if(resendTimers[id]){clearInterval(resendTimers[id]);delete resendTimers[id];}
  if(!button)return;
  button.disabled=false;button.removeAttribute('aria-disabled');button.textContent='Send a new code';
 }
 function startResendTimer(id){
  var button=document.getElementById(id);if(!button)return;
  if(resendTimers[id])clearInterval(resendTimers[id]);
  var until=Date.now()+RESEND_COOLDOWN_SECONDS*1000;
  function render(){
   var left=Math.max(0,Math.ceil((until-Date.now())/1000));
   if(left<=0){resetResendButton(id);return;}
   button.disabled=true;button.setAttribute('aria-disabled','true');button.textContent='Resend code in '+left+'s';
  }
  render();resendTimers[id]=setInterval(render,250);
 }
 ...
 form.onsubmit=async function(event){
  ...
  try{
   ...
   if(data&&data.requiresVerification){
    ...
    showStep('verify');if(mode==='signup')startResendTimer('verifyResend');document.getElementById('verifyCode').focus();return;
   }
 ...
 document.getElementById('verifyResend').onclick=async function(){
  var button=this,msg=document.getElementById('verifyMsg'),sent=false;if(button.disabled)return;
  busy(button,true);button.textContent='Sending…';msg.classList.remove('ok');msg.textContent='Sending…';
  try{
   var result=await post('/api/account/resend',{email:pendingEmail});
   sent=result.response.ok;msg.textContent=(result.data&&result.data.message)||'If the account can receive a code, a new one is on the way.';msg.classList.toggle('ok',sent);
   if(sent)startResendTimer('verifyResend');
  }catch(_){msg.textContent='Could not reach the server. Try again.';}
  finally{if(!sent)resetResendButton('verifyResend');}
 };
 ...
 document.getElementById('forgotSubmit').onclick=async function(){
  ...
  try{
   ...
   if(result.response.ok){document.getElementById('resetCode').value='';document.getElementById('newPassword').value='';document.getElementById('confirmPassword').value='';document.getElementById('resetMsg').textContent='';showStep('reset');startResendTimer('resetResend');document.getElementById('resetCode').focus();}
  }catch(_){msg.textContent='Could not reach the server. Try again.';}
  finally{busy(button,false);}
 };
 ...
 document.getElementById('resetResend').onclick=async function(){
  var button=this,msg=document.getElementById('resetMsg'),sent=false;if(button.disabled)return;
  busy(button,true);button.textContent='Sending…';msg.classList.remove('ok');msg.textContent='Sending…';
  try{
   var result=await post('/api/account/password/forgot',{email:resetEmail});
   sent=result.response.ok;msg.textContent=(result.data&&result.data.message)||'If that email has an account, a new code is on the way.';msg.classList.toggle('ok',sent);
   if(sent)startResendTimer('resetResend');
  }catch(_){msg.textContent='Could not reach the server. Try again.';}
  finally{if(!sent)resetResendButton('resetResend');}
 };
