...
export function researchOpponentMatches(row, matchup = {}) {
  // A verified provider team id is the strongest match and a conflicting pair
  // must never be rescued by a label. Older persisted/provider rows can lack an
  // opponent id entirely; in that case an exact normalized team label remains
  // valid evidence instead of incorrectly turning a real meeting into 0g.
  if (matchup.opponentId != null && row?.opponentId != null) {
    return String(row.opponentId) === String(matchup.opponentId);
  }
