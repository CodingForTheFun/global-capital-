    ...
    <div className="mx-auto max-w-[1180px] overflow-hidden rounded-2xl border border-slate-800/90 bg-[#0b101a] shadow-[0_30px_90px_rgba(0,0,0,0.45)]">
      ...
      <div className="space-y-3 p-3 sm:p-4 lg:p-5">
        <div data-research-hero className="rounded-2xl border border-slate-800 bg-[radial-gradient(circle_at_15%_10%,rgba(59,130,246,0.13),transparent_30%),linear-gradient(180deg,rgba(19,27,46,0.9),rgba(10,15,24,0.96))] p-2.5 sm:p-4">
          ...
          <div className="mt-2 flex items-center justify-between gap-2 rounded-xl border border-sky-500/20 bg-sky-500/[0.07] p-1.5 sm:mt-4 sm:p-2.5"><div className="flex min-w-0 items-center gap-2"><span className="rounded-md bg-sky-500 px-2 py-1 text-[10px] font-black tracking-wide text-white">{initials(selected.bookName)}</span><div className="min-w-0"><div className="flex min-w-0 items-center gap-2"><h2 className="truncate text-xs font-bold text-white sm:text-sm">{title}</h2>{variantLabel && <span data-variant={selected.dfsOddsType} className="shrink-0 rounded-md border border-emerald-400/20 bg-emerald-400/10 px-1.5 py-0.5 text-[8px] font-black uppercase tracking-wide text-emerald-300">{variantLabel}</span>}</div><div className="hidden text-[10px] text-slate-500 sm:block">Main prop · {period}</div></div></div><div className="flex items-center gap-2"><label className="relative block"><span className="sr-only">Selected book</span><select aria-label="Selected book" value={currentBook || ''} onChange={event => onBook(event.target.value)} className="h-7 max-w-[96px] appearance-none rounded-lg border border-sky-400/20 bg-slate-950/60 pl-2 pr-5 text-[9px] font-bold text-slate-300 outline-none sm:h-8 sm:max-w-none sm:pr-6 sm:text-[10px]">{allowBestPrices && <option value="">Best prices · all books</option>}{applicableBooks.map(book => <option key={book.key} value={book.key}>{book.name}</option>)}</select><ChevronDown className="pointer-events-none absolute right-1.5 top-2 h-3 w-3 text-slate-500"/></label><div className="flex items-center gap-2 text-sm font-black"><span className={selected.side === 'UNDER' ? 'text-rose-400' : 'text-emerald-400'}>{selected.side === 'OVER' ? 'O' : selected.side === 'UNDER' ? 'U' : selected.choice} {selected.line ?? ''}</span><span className="text-slate-400">{offerPrice(selected)}</span></div></div></div>
        </div>
