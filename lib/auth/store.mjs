...
// A successful password check should not become an account lockout just because
// the shared data volume is temporarily full. These are the only fields the
// successful-login path is allowed to lose in that emergency. Security-critical
// changes such as sessionVersion, emailVerified, disabled, pendingCodes, role,
// or a password reset are deliberately excluded and still fail closed.
const RECOVERABLE_LOGIN_METADATA_KEYS = new Set([
  'failedAttempts',
  'lockedUntil',
  'lastLoginAt',
  'lastLoginIp',
  // Rehashing a password after it already verified successfully is maintenance,
  // not authorization. The existing valid hash remains in users.json if ENOSPC
  // prevents this opportunistic upgrade.
  'passwordHash',
]);

export function recoverableLoginMetadataPatch(patch) {
  if (!patch || typeof patch !== 'object' || !Object.prototype.hasOwnProperty.call(patch, 'lastLoginAt')) return false;
  return Object.keys(patch).every((key) => RECOVERABLE_LOGIN_METADATA_KEYS.has(key));
}
...
export async function updateUser(id, mutate) {
  let fallback = null;
  try {
    return await transaction(async (users) => {
      const index = users.findIndex((user) => user.id === String(id));
      if (index < 0) return { write: false, value: null };
      const patch = mutate(users[index]) || {};
      const updated = { ...users[index], ...patch, updatedAt: new Date().toISOString() };
      users[index] = updated;
      fallback = { patch, updated };
      return { value: updated };
    });
  } catch (error) {
    if (error?.code === 'ENOSPC' && recoverableLoginMetadataPatch(fallback?.patch)) {
      // users.json is written via temp+rename, so the canonical account record
      // remains untouched when the temp write runs out of space. Return the
      // already-verified in-memory account so routes can issue the normal signed
      // session. Never print the email, password hash, IP, token, or user id.
      console.warn('[Scout Pro auth] data volume full; successful-login metadata was not persisted');
