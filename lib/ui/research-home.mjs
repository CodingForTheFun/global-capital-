...
export function researchClient(source) {
  ...
  // Promote the existing account menu into the sticky header. The underlying
  // account flows remain the original server-backed forms and CSRF-protected APIs.
  ...
  if (!client.includes('headerBar.appendChild(profileMenu)') && client.includes(headerHook)) {
    client = client.replace(headerHook, " var profileMenu=document.getElementById('asProfileMenu'),headerBar=document.querySelector('.asTop .asBar');if(profileMenu&&headerBar){profileMenu.classList.add('asHeaderAccount');headerBar.appendChild(profileMenu);}\n" + headerHook);
  }
  ...
  // Reflect authentication state in the header menu itself. Owner/support links
  ...
  if (!client.includes("document.getElementById('asMenuReset').onclick") && client.includes(settingsHook)) {
