...
import { marketKey, marketFromProviderKey } from '../sportsdataio/markets.mjs';
...
const FOOTBALL_FIELDS = Object.freeze({
  ...
  'reception yards': ['ReceivingYards'], 'reception tds': ['ReceivingTouchdowns'],
  'pass tds': ['PassingTouchdowns'], 'rush tds': ['RushingTouchdowns'],
...
const BASKETBALL_FIELDS = Object.freeze({
  ...
  'rebounds assists': ['Rebounds', 'Assists'], 'blocks steals': ['BlockedShots', 'Steals'],
...
export const clearSportsStatAliases = Object.freeze({
...
/**
 * Resolve a market to ClearSports stat fields.
 *
 * The odds provider's stable key is tried first, for the same reason it is in
 * the SportsDataIO table: display labels drift. "Receiving Yards" now arrives
 * as "Reception Yards", which unmapped every receiving prop here too.
 */
export function fieldsForClearSports(sport, market, providerMarketKey = null) {
  const table = MARKET_FIELDS[String(sport || '').toUpperCase()];
  if (!table) return null;

  const canonical = marketFromProviderKey(providerMarketKey);
  if (canonical) {
    const viaProvider = table[canonical] || table[canonical.replace(/\s*\+\s*/g, '+')];
...
function marketTotal(sport, market, row, providerMarketKey = null) {
  const fields = fieldsForClearSports(sport, market, providerMarketKey);
  if (!fields) return null;
  let total = 0;
  for (const field of fields) {
    const value = pickNumber(row, clearSportsStatAliases[field] || [field, field.charAt(0).toLowerCase() + field.slice(1)]);
...
export async function fetchClearSportsSeasonResearch({ sport, playerName: wantedName, market, providerMarketKey = null } = {}) {
  ...
  if (!fieldsForClearSports(league, market, providerMarketKey)) return { ok: true, available: false, code: 'UNMAPPED_MARKET', message: 'Historical research is not mapped for this market yet.' };
  ...
  // A one-game response must never be relabeled as a season aggregate.
  const matches = rows.filter((item) => normalizePlayerName(playerName(item)) === normalized
    && ![item.game_id, item.gameId, item.GameID, item.event_id, item.eventId].some(value => value != null && value !== ''));
  ...
  const seasonStat = marketTotal(league, market, row, providerMarketKey);
