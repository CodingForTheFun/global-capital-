...
// The owner-security helper has to run before the ordinary sign-in handler so
// it can upgrade a correct owner password into a passkey ceremony. Keep it
// inline here so the account gate never depends on a second request to render a
// usable sign-in path.
const OWNER_SECURITY_CLIENT = readFileSync(new URL('../../public/owner-passkey.js', import.meta.url), 'utf8')
...
footer .legal-links{margin:12px 0 0}
footer .legal-links a{color:#93a9c6;text-decoration:none}
footer .legal-links a:hover{color:var(--text);text-decoration:underline}
...
.wrap{max-width:1200px;padding-top:30px}
header{padding-bottom:24px;border-bottom:1px solid #29425f80;margin-bottom:56px}
.hero{gap:64px;grid-template-columns:minmax(0,1.2fr) minmax(340px,.85fr)}
.eyebrow{font-size:11px;font-weight:700;letter-spacing:.16em;color:#8ddcc3;margin:0 0 20px}
h1{font-size:clamp(38px,4.5vw,62px);font-weight:750;line-height:1.06;letter-spacing:-.045em;margin-bottom:24px}
.sub{font-size:17px;line-height:1.75;max-width:31em;color:#b2c3d7}
.card{border-radius:20px;padding:30px;box-shadow:0 24px 64px #0003;background:linear-gradient(145deg,#15263b,#0e1d30)}
.card h2{font-size:24px;letter-spacing:-.025em}.tabs button{height:44px}
input[type=email],input[type=password]{height:50px}.primary{height:50px}
.features{gap:20px;margin-top:64px}.feat{background:#0d1d30;border-color:#253c53;padding:24px;border-radius:14px}
.feat span{color:#b0c2d7;line-height:1.7}.mark{background:#12352f;color:#8ce8c4}
.teaser{opacity:.14}footer{color:#a0b3ca}
...
<script>${OWNER_SECURITY_CLIENT}</script>
<script>
(function(){
