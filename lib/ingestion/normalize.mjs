...
export function normalizedDataFromBoardRows(rows=[],at=new Date().toISOString()){
 ...
 for(const row of Array.isArray(rows)?rows:[]){
  try{
   ...
   const player=normalizedPlayer({id:row.playerId,provider:row.provider,sport:row.sport,name:row.playerName,team:row.team,position:row.position,ingestedAt:at});
