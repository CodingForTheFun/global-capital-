...
const GEO = String(process.env.AUTOSCOUT_DRAFTKINGS_GEO || 'dkusva').trim().toLowerCase() || 'dkusva';
const BASE = `https://sportsbook-nash.draftkings.com/api/sportscontent/${GEO}/v1`;
...
async function fetchJson(fetcher, url) {
  try { return { data: await directJson(fetcher, url), transport: 'draftkings-sportscontent-public' }; }
    ...
    try {
      const data = await http2JsonFetch(url, { timeoutMs: 18_000, headers: { origin: 'https://sportsbook.draftkings.com', referer: 'https://sportsbook.draftkings.com/', 'user-agent': UA } });
...
export async function fetchDraftKingsSportsbookPublic(sport, { fetcher = globalThis.fetch, force = false } = {}) {
  ...
  const task = (async () => {
    ...
    const value = { records: [...all.values()], fetchedAt: new Date().toISOString(), endpoint: `sportsbook-nash.draftkings.com/${GEO}`, transport, eventsChecked, categoriesChecked: categories.length };
