...
export function patchEdgeFrontdoor(source) {
  ...
  output = output.replace(
    ...
    '  if (await edgeGuest(req, res)) return;\n'
      ...
      + " if (await handleAdminAccessRoutes(req, res, adminUrl, { sessions: accountSessions, json: directJson, secret: sessionSecret, log: console })) return;"
      + " if (await handleAdminRoutes(req, res, adminUrl, { sessions: accountSessions, json: directJson, secret: sessionSecret, log: console })) return; }",
  );
