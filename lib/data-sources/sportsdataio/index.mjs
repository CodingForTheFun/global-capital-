...
import { summarizeSportsbookCoverage } from './odds.mjs';
...
export function createSportsDataIoAdapter({
  ...
  async function loadFeed(name, sport, league, params) {
    ...
    // Skip a feed already known to be outside this subscription.
  ...
  async function operatorCoverage({ sportsbook = 'PrizePicks', sports = OPERATOR_COVERAGE_SPORTS } = {}) {
    ...
    for (const requested of sports) {
      ...
      const summary = summarizeSportsbookCoverage(raw, sportsbook);
    ...
    return {
      ...
      sportsbook,
      rows,
      // This is deliberately conservative: one observed PrizePicks offer proves
      // the operator is present in that league today, not that every market is.
  ...
  return {
    ...
    /** Runtime capability matrix for this specific key. */
    ...
    /**
     * Runtime-only check for picks/sportsbook operators in the player-props feed.
     * This is how Scout Pro proves whether PrizePicks is actually present before
     * reducing browser-scan dependence. Only counts and operator names leave the
     * provider layer; raw odds payloads do not.
     */
    operatorCoverage,

    /**
     * Game log for ONE player, for the prop detail view only.
     * Never called while rendering a list: that would be a request per prop.
     */
    ...
    async fetchEnrichment({ props = [] } = {}) {
      ...
      for (const [sport, sportProps] of bySport) {
        ...
        // Identity indexes: provider ids where present, names plus context
        // otherwise, with same-name collisions preserved rather than resolved
        // by insertion order.
        const feeds = {
