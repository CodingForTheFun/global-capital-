...
export function resolveHistoryEventIdentity(gameLog, params = {}, {maxDriftMs=48*60*60_000,minSeparationMs=3*60*60_000} = {}) {
  ...
  if(!candidates.length)return incompleteNear
    ?{ok:true,available:false,code:'MATCHUP_IDENTITY_UNVERIFIED'}
    :{ok:true,available:true,source:'ESPN',gameId:null,sourceEventId:null,matched:false};
  if(candidates[1]&&candidates[1].driftMs-candidates[0].driftMs<minSeparationMs)
    return {ok:true,available:false,code:'MATCHUP_IDENTITY_UNVERIFIED'};
