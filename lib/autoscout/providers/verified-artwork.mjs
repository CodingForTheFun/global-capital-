...
/** MLB's own headshot for an MLBAM person id; a missing photo 404s rather than returning a silhouette. */
export function mlbPhotoUrl(personId){return /^\d{3,9}$/.test(String(personId||''))?`https://img.mlbstatic.com/mlb-photos/image/upload/w_213,q_auto:best/v1/people/${personId}/headshot/67/current`:null;}
/** NHL's own mug shot, only from assets.nhle.com and only for the player id asked for. */
export function nhlPhotoUrl(value,playerId){
 try{const u=new URL(String(value||''));return u.protocol==='https:'&&u.hostname==='assets.nhle.com'&&!u.username&&!u.password&&new RegExp('/mugs/nhl/\\d{8}/[A-Z]{2,3}/'+playerId+'\\.png$').test(u.pathname)?u.href:null;}catch{return null;}
}
...
export function createVerifiedArtwork({fetchImpl=(...args)=>fetch(...args),now=()=>Date.now(),dataDir=process.env.DATA_DIR||'/app/data'}={}){
 ...
 /**
  * The prop's own game: PropLine gives each event its ESPN event id, so the
  * player is looked up only on the two rosters playing that game. This is the
  * most exact match available (a traded player, a common name) and still
  * fails closed when neither roster has exactly one match.
  */
 async function resolveEspnEvent(sport,name,context){
  const [family,league]=PUBLIC_LEAGUES[sport]||[];const eventId=String(context.espnEventId||'');
  if(!family||!league||!/^\d{5,12}$/.test(eventId))return null;
  const summary=await json(`https://site.api.espn.com/apis/site/v2/sports/${family}/${league}/summary?event=${eventId}`,6*3600000);
  const teams=(summary?.header?.competitions?.[0]?.competitors||[]).map(c=>c?.team).filter(t=>t&&/^\d+$/.test(String(t.id)));
  if(teams.length!==2)return null;
  const found=[];
  for(const team of teams){
   const roster=await json(`https://site.api.espn.com/apis/site/v2/sports/${family}/${league}/teams/${team.id}/roster`,DAY);
   const athlete=resolveRosterAthlete(roster,{sport,playerName:name,team});
   if(athlete){const row=(roster.athletes||[]).flatMap(g=>g.items||[g]).find(r=>String(r.id)===athlete.id);found.push({athlete,row,team});}
  }
  if(found.length!==1)return null;
  const {athlete,row,team}=found[0];
  const photo=verifiedPhotoUrl(row?.headshot?.href,athlete.id);
  if(!photo)return null;
  const {bytes,type}=await imageBytes(photo);
  return {body:bytes,contentType:type,source:'ESPN event roster',playerName:athlete.playerName,team:team.displayName||team.abbreviation||null,position:row?.position?.abbreviation||null,providerPlayerId:`history:${sport}:${athlete.id}`,verified:true};
 }
 /** MLB's official people search and headshots: one active exact-name match only. */
 async function resolveMlbOfficial(sport,name,context={}){
  if(sport!=='MLB')return null;
  const data=await json('https://statsapi.mlb.com/api/v1/people/search?'+new URLSearchParams({names:name,sportIds:'1',hydrate:'currentTeam'}),DAY);
  const wanted=normalizePlayerName(name);
  const rows=(Array.isArray(data?.people)?data.people:[]).filter(p=>p?.active!==false&&normalizePlayerName(p?.fullName)===wanted);
  if(rows.length!==1)return null;
  // When the prop names a team, the player's current MLB team must be it.
  const wantedTeam=String(context.team||'').trim().toLowerCase();
  if(wantedTeam){
   const teams=await json('https://statsapi.mlb.com/api/v1/teams?sportId=1',DAY);
   const current=(teams?.teams||[]).find(t=>t?.id===rows[0]?.currentTeam?.id);
   const labels=[current?.abbreviation,current?.teamName,current?.name,current?.shortName,current?.clubName].filter(Boolean).map(v=>String(v).toLowerCase());
   if(!current||!labels.includes(wantedTeam))return null;
  }
  const photo=mlbPhotoUrl(rows[0].id);if(!photo)return null;
  const {bytes,type}=await imageBytes(photo);
  return {body:bytes,contentType:type,source:'MLB',playerName:rows[0].fullName,team:null,position:rows[0].primaryPosition?.abbreviation||null,providerPlayerId:null,verified:true};
 }
 /** The NHL's official player search and mug shots: exact name, team when known, one match only. */
 async function resolveNhlOfficial(sport,name,context){
  if(sport!=='NHL')return null;
  const data=await json('https://search.d3.nhle.com/api/v1/search/player?'+new URLSearchParams({culture:'en-us',limit:'20',q:name,active:'true'}),DAY);
  const wanted=normalizePlayerName(name),team=String(context.team||'').toUpperCase().replace(/[^A-Z]/g,'');
  let rows=(Array.isArray(data)?data:[]).filter(p=>/^\d+$/.test(String(p?.playerId))&&normalizePlayerName(p?.name)===wanted);
  if(rows.length>1&&team)rows=rows.filter(p=>String(p.teamAbbrev||'').toUpperCase()===team);
  if(rows.length!==1)return null;
  const id=String(rows[0].playerId),landing=await json(`https://api-web.nhle.com/v1/player/${id}/landing`,DAY);
  if(String(landing?.playerId||id)!==id)return null;
  const photo=nhlPhotoUrl(landing?.headshot,id);if(!photo)return null;
  const {bytes,type}=await imageBytes(photo);
  return {body:bytes,contentType:type,source:'NHL',playerName:rows[0].name,team:rows[0].teamAbbrev||null,position:rows[0].positionCode||null,providerPlayerId:null,verified:true};
 }
 ...
 async function get(sport,name,context={}){
  ...
  // The event id is part of the key only through the answer it gives: the same
  // player in any game resolves to the same athlete, so it is left out here.
  ...
  const task=limit(async()=>{
   ...
   // Most exact source first: the prop's own game rosters, then ESPN search
   // and team roster, then the league's official photos, then TheSportsDB.
   for(const resolve of [resolveEspnEvent,PUBLIC_LEAGUES[sport]?.[1]?resolveEspn:null,resolveMlbOfficial,resolveNhlOfficial,resolveSportsDb]){
    if(result||!resolve)continue;
    try{result=await resolve(sport,name,context);}catch{failed=true;}
   }
