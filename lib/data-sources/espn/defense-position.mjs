...
const CONFIG = {
  ...
  // three, so NFL reads six weeks of completed games instead.
  NFL: { positions: ['QB','RB','WR','TE'], metrics: {passingYards:'passing:YDS',passingTouchdowns:'passing:TD',rushingYards:'rushing:YDS',receivingYards:'receiving:YDS',receptions:'receiving:REC'}, windowDays: 42, maxEvents: 96 },
};
  ...
  for (const box of list(summary.boxscore?.players)) {
    ...
    for (const group of list(box.statistics)) {
      ...
      for (const [metric,column] of Object.entries(config.metrics)) {
        ...
        if ((category&&group.name!==category)||!labels.includes(label)) continue;
    ...
    for(const row of totals.values()) if(!invalid.has(`${row.position}|${row.metric}`)) rows.push({...row,teamId:defense,offenseTeamId:offense,gameId:String(header.id),date:game.date});
  }
  return rows;
}
...
export function rankOffenseRows(input, teams) {
  return rankPositionRows(input.filter(row=>row.offenseTeamId).map(row=>({...row,teamId:row.offenseTeamId})),teams);
}
...
export function createDefensePosition({request,teamDirectory,now=Date.now}) {
  ...
  return async input => {
    ...
    const work=(async()=>{
      ...
      const queue=[...new Set(events.map(e=>String(e.id)).filter(id=>/^\d+$/.test(id)))].slice(0,maxEvents), summaries=[],allowed=new Set(teams.map(t=>t.id));
      ...
      const offenseRows=rankOffenseRows(rows,teams);
      const value={ok:true,available:ranked.length>0,sport,positions:config.positions,teams,rows:ranked,offenseRows,source:'ESPN completed box scores',sourceUrl:`https://www.espn.com/${sport.toLowerCase()}/scoreboard`,retrievedAt:new Date(clock).toISOString(),from:new Date(since).toISOString(),through:new Date(clock).toISOString(),windowDays,message:ranked.length?null:`No verified position samples were returned for the last ${windowDays} days.`,basis:`Position totals allowed per completed regular-season game. Latest ${maxEvents} games within ${windowDays} days; rank requires every league team to have at least 3 games at the position. Positions use box scores or the current team roster. Games with unclassified players are excluded. Offense ranks read the same team-games from the producing side.`};
