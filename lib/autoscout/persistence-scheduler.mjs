...
export function effectivePersistenceSportsPerCycle({
  configured = PUBLIC_PERSISTENCE_SPORTS.length,
  totalSports = PUBLIC_PERSISTENCE_SPORTS.length,
  remainingCycleMs = B.cycle,
  minimumSportBudgetMs = 35_000,
} = {}) {
  const total = Math.max(1, Math.floor(Number(totalSports) || 1));
  const requested = Math.max(1, Math.min(total, Math.floor(Number(configured) || total)));
  const minimum = Math.max(1_000, Math.floor(Number(minimumSportBudgetMs) || 35_000));
  const capacity = Math.max(1, Math.floor(Math.max(0, Number(remainingCycleMs) || 0) / minimum));
  return Math.min(requested, total, capacity);
}
...
async function persistBoardsCycle(reason, deps, budgets) {
  ...
  try {
    ...
    // gated on it, unchanged. Keep a separate "known" bit because false means
    // two different things: an explicit unclaimed lease is safe for PropLine,
    // while a timed-out/ambiguous lease attempt must not authorize new fan-out.
    let publicClaimed = false;
    let publicLeaseKnown = !publicWorkerActive;
    ...
    // PropLine release recovery: keep paid refresh independent of public lease ownership.
    // PropLine is a paid API provider with its own quota guard and process-local
    // in-flight/nextAt fencing. It must not depend on public lease OWNERSHIP:
    // another process may legitimately own that lease while this process is the
    // only one configured with the PropLine key. We only require that the lease
    // attempt resolved, so a timed-out/ambiguous claim cannot authorize late
    // provider fan-out.
    if (publicLeaseKnown) startPropLineRefresh();
    ...
    const configuredPerCycle = ['sportsgameodds','mesh'].includes(propProviderMode())
    ...
    const remainingCycleMs = Math.max(1_000, budgets.cycle - cycleElapsedMs - 10_000);
    const perCycle = effectivePersistenceSportsPerCycle({
      configured: configuredPerCycle,
      totalSports: activePersistenceSports.length,
      remainingCycleMs,
    });
    if (perCycle < configuredPerCycle) {
      console.log(`[AutoScout persistence] ${reason} adaptive-slice=${perCycle}/${configuredPerCycle} remainingCycleMs=${remainingCycleMs} minimumSportBudgetMs=35000`);
    }
