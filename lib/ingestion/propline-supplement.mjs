...
function exactQuoteIdentity(row) {
  ...
  const suppliedPeriod = text(row?.period || row?.periodKey).toLowerCase();
  const period = ['', 'game', 'full', 'full_game', 'match', 'single_stat'].includes(suppliedPeriod) ? 'game' : suppliedPeriod;
  const flavor = text(row?.specialType || row?.dfsOddsType).toLowerCase() || 'standard';
  return [base, period, line, flavor, Number(row?.payoutMultiplier) || 1].join('|');
}
...
function enrichFromPropline(row, source) {
  ...
  // Keep the quote itself from the existing board source; only PropLine's
  // identity/freshness metadata is overlaid on an exact player+market+book+
  // side+line match.
  return merged;
}
...
export function mergeCachedPropline(board, sport, { now = Date.now } = {}) {
  ...
  const baseProps = rawBaseProps.map((row) => {
    ...
    return quoteSource ? enrichFromPropline(merged, quoteSource) : merged;
  });
  ...
  if (!acceptedAll.length && baseProps.every((row, index) => row === rawBaseProps[index])) {
    return { ...board, meta: { ...(board?.meta || {}), proplineSupplement: { cached: true, added: 0, enriched: 0, fetchedAt: supplement.meta?.fetchedAt || null } } };
  }

  const acceptedLineIds = new Set(acceptedAll.map((row) => row.id));
