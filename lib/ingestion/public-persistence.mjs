import { assertIngestionActive, ingestionFetch } from './operation-deadline.mjs';
...
async function directStore(functionName, action, payload = {}, { timeoutMs = storeTimeoutMs() } = {}) {
  const response = await ingestionFetch(`${URL()}/functions/v1/autoscout-db-direct`, {
    method: 'POST',
    headers: {
...
async function rpcStore(functionName, action, payload = {}, { allowDirect = true, timeoutMs = storeTimeoutMs(), canonicalFirst = false } = {}) {
  assertIngestionActive();
  ...
  // Scheduler claim/release deliberately stay canonical-first even if an old
  // environment accidentally enables direct-primary. The direct path remains a
  // recovery route only after a real transient Data API failure.
  if (allowDirect && directPrimary() && !canonicalFirst) {
  ...
  let response;
  try {
    response = await ingestionFetch(`${URL()}/rest/v1/rpc/${functionName}`, {
      method: 'POST',
      headers: {
        accept: 'application/json',
        'content-type': 'application/json',
...
export async function claimPublicCycle(intervalSeconds = publicIntervalSeconds()) {
  // Lease claims are tiny coordination writes. Keep the canonical Data API as
  // the first choice, but a transient API Gateway outage must not block the
  // existing token-protected Supavisor Edge fallback. The database-side claim
  // also has a short lock timeout so an abandoned REST request cannot later
  // commit a ghost lease after the caller has given up.
  return store('claim', { interval_seconds: publicIntervalSeconds(intervalSeconds) }, { allowDirect: true, timeoutMs: 10_000, canonicalFirst: true });
}
export async function releasePublicCycle(owner) {
  if (!owner) return {};
  // Release is idempotent. It is also canonical-first even if direct-primary is
  // accidentally enabled; fallback is allowed only after a transient REST/API
  // Gateway failure so lease ownership has one authoritative first path.
  return store('release', { owner }, { allowDirect: true, timeoutMs: 2_500, canonicalFirst: true });
}
...
export async function readPublicProps(sport) {
  assertIngestionActive();
  if (!publicPersistenceConfigured()) return [];
  const requestedSport = text(sport).toUpperCase();
  let response;
  try {
    response = await ingestionFetch(`${URL()}/rest/v1/rpc/autoscout_read_props_fast`, {
      method: 'POST',
      headers: {
        accept: 'application/json',
        'content-type': 'application/json',
      ...
      body: JSON.stringify({ p_token: TOKEN(), p_sport: requestedSport }),
      // This RPC starts from the requested sport's future events and props,
      // never from a window function over the entire canonical line table.
      signal: AbortSignal.timeout(1_500),
    });
  } catch (error) {
    assertIngestionActive();
    noteStoreFailure('autoscout_read_props_fast', 'read_props', 'PUBLIC_STORE_FAST_READ_FAILED', 504);
  ...
  assertIngestionActive();
  if (!response.ok) {
    noteStoreFailure('autoscout_read_props_fast', 'read_props', safeStoreCode(result, response.status), response.status);
    throw Object.assign(new Error('PUBLIC_STORE_FAST_READ_FAILED'), { code: safeStoreCode(result, response.status), status: response.status });
  }
  noteStoreSuccess();
