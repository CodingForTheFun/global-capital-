...
const special=o=>o?.is_promotional===true||o?.is_alternate===true||o?.flash_sale_line_score!=null||o?.promotion!=null||o?.boost!=null||/goblin|demon|boost|discount|promo|alternate|taco/i.test([o?.odds_type,o?.type,o?.label].join(' '));
const v2Special=o=>o?.is_promotional===true||o?.is_alternate===true||o?.promotion!=null||o?.boost!=null||/goblin|demon|boost|discount|promo|alternate|taco/i.test([o?.odds_type,o?.label].join(' '));
...
function promotion(...rows){const raw=rows.find(special);if(!raw)return null;const label=text(raw.odds_type||raw.label||raw.type||raw.promotion?.name||raw.promotion?.label);const lower=label.toLowerCase();return {type:/goblin/.test(lower)?'goblin':/demon/.test(lower)?'demon':/taco/.test(lower)?'taco':/discount|flash/.test(lower)?'discount':'promotion',label:label||'Promotion',originalLine:numeric(raw.original_line_score??raw.originalLine??raw.promotion?.original_line),promotionalLine:numeric(raw.flash_sale_line_score??raw.line_score??raw.stat_value??raw.line),payoutMultiplier:numeric(raw.payout_multiplier??raw.multiplier)};}
export function record(input){
 ...
 return {...input,sport,line,gameStartTime:start,contract,updatedAt:iso(input.updatedAt),overOdds:numeric(input.overOdds),underOdds:numeric(input.underOdds),isAlternate:input.isAlternate===true,promotion:input.promotion||null};
}
export function normalizePrizePicks(payload){
 ...
 for(const row of payload.data){
  ...
  if(off(a)||off(pa)||off(ga)||!periodOK(a,a.stat_type||''))continue;
  const league=rel(row,'league').attributes||rel(p,'league').attributes||{},promo=promotion(a,row);
  const r=record({sourceId:text(row.id),book:'prizepicks',nativePlayerId:text(p.id),playerName:pa.name||pa.display_name,sport:league.name||league.abbreviation||a.sport||pa.sport,
   ...
   sides:['OVER','UNDER'],isAlternate:!!promo,promotion:promo});
  if(r)out.push(r);
 }
 return out;
}
export function normalizeUnderdog(payload){
 ...
 for(const row of payload.over_under_lines){
  ...
  if(!p||!appearance||!g||off(row)||off(p)||!periodOK(isV2?{period:null}:row,market)||(isV2?false:off(ou)))continue;
  const options=list(row.options).filter(o=>{if(off(o))return false;const choice=text(o.choice).toLowerCase();return ['higher','lower','better','worse'].includes(choice);});
  const sides=[...new Set(options.map(o=>{const choice=text(o.choice).toLowerCase();return choice==='higher'||choice==='better'?'OVER':choice==='lower'||choice==='worse'?'UNDER':null;}).filter(Boolean))];
  const modifier=options.find(o=>numeric(o.payout_multiplier)!==null&&numeric(o.payout_multiplier)!==1),promo=promotion(row,ou,modifier||{},...options.filter(o=>isV2?v2Special(o):special(o)));
  const r=record({sourceId:text(row.id),book:'underdog',nativePlayerId:text(p.id),playerName:p.full_name||[p.first_name,p.last_name].filter(Boolean).join(' '),sport:p.sport_id||g?.sport_id,
   ...
   sides,isAlternate:!!promo,promotion:promo});
...
function referenceIndex(board){
 ...
 for(const p of board.props||[]){if(p.isAlternate||p.entityType==='team')continue;const ck=[p.sport,p.marketId,p.market].join('|');if(!contracts.has(ck))contracts.set(ck,contractKey(marketContract({sport:p.sport,market:p.market,providerMarketKey:p.marketId})));const key=JSON.stringify([p.sport,normalizePlayerName(p.playerName),Date.parse(p.gameStartTime),contracts.get(ck)]);if(!index.has(key))index.set(key,new Map());index.get(key).set([p.eventId,p.playerId,p.marketId].join('|'),p);}return index;
}
function matchReference(r,index){const key=JSON.stringify([r.sport,normalizePlayerName(r.playerName),Date.parse(r.gameStartTime),contractKey(r.contract)]);const candidates=[...(index.get(key)?.values()||[])].filter(p=>r.team&&p.team&&samePublicTeam(r.team,p.team,r.sport)||r.homeTeam&&r.awayTeam&&samePublicTeam(r.homeTeam,p.homeTeam,r.sport)&&samePublicTeam(r.awayTeam,p.awayTeam,r.sport));return candidates.length===1?candidates[0]:null;}
export function normalizedFeedBoard(records,reference={props:[]},at=new Date().toISOString()){
 ...
 for(const r of records){
  ...
  for(const side of r.sides){const line=normalizedBookmakerLine({propId:prop.id,provider,bookmakerKey:provider,bookmakerName:bookInfo(provider).name,side,line:r.line,price:side==='OVER'?r.overOdds:r.underOdds,providerUpdatedAt:r.updatedAt,ingestedAt:at});lines.set(line.id,line);flat.push({id:line.id,source:bookInfo(provider).name,provider,sport:r.sport,eventId:event.id,playerId:player.id,playerName:player.name,entityType:'player',team:player.team,marketId:prop.marketKey,market:prop.marketName,period:'game',payoutType:['kalshi','polymarket'].includes(provider)?'exchange':'pickem',side,line:line.line,price:line.price,sportsbook:line.bookmakerName,sportsbookKey:provider,gameStartTime:event.commenceTime,homeTeam:event.homeTeam,awayTeam:event.awayTeam,isAlternate:!!r.isAlternate,promotion:r.promotion||null,providerUpdatedAt:r.updatedAt,updatedAt:r.updatedAt,ingestedAt:at});}
  active.push({id:stableId([prop.id,provider,r.line,r.isAlternate?'promo':'main']),player_id:player.id,book_id:provider,stat_type:prop.marketKey,target_line:r.line,over_odds:r.overOdds,under_odds:r.underOdds,updated_at:r.updatedAt,event_id:event.id,sport:r.sport,source_id:r.sourceId,is_alternate:!!r.isAlternate,promotion:r.promotion||null});
 }
...
export function activePropsFromBoard(board){
 ...
 for(const r of board.props||[]){if(r.isAlternate||numeric(r.line)===null||!['OVER','UNDER'].includes(r.side))continue;const book=bookId(r.sportsbookKey),id=stableId([r.sport,r.eventId,r.playerId,r.marketId,book,r.line]);const updated=iso(r.providerUpdatedAt);const p=result.get(id)||{id,player_id:r.playerId,book_id:book,stat_type:r.marketId,target_line:r.line,over_odds:null,under_odds:null,updated_at:updated,event_id:r.eventId,sport:r.sport};const odds=numeric(r.price);p[r.side==='OVER'?'over_odds':'under_odds']=odds!==null&&Math.abs(odds)>=100?odds:null;p.updated_at=p.updated_at&&updated?(p.updated_at<updated?p.updated_at:updated):null;result.set(id,p);}
