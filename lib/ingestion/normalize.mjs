...
export function normalizeUnderdog(payload){
 ...
 for(const row of payload.over_under_lines){
  ...
  if(!p||!appearance||off(row)||off(ou)||off(p)||special(row)||special(ou)||!periodOK(row,stat.stat||''))continue;
  const options=list(row.options).filter(o=>!off(o)&&!special(o)&&['higher','lower'].includes(o.choice));
  // A modified pick'em payout is not an American sportsbook price.
  if(options.some(o=>numeric(o.payout_multiplier)!==null&&numeric(o.payout_multiplier)!==1))continue;
  const r=record({sourceId:text(row.id),book:'underdog',nativePlayerId:text(p.id),playerName:p.full_name||[p.first_name,p.last_name].filter(Boolean).join(' '),sport:p.sport_id||g?.sport_id,
   market:stat.stat,line:row.stat_value,team:teams.get(text(appearance.team_id))?.abbr||p.team||'',opponent:'',nativeEventId:text(g?.id||appearance.match_id),
   homeTeam:teams.get(text(g?.home_team_id))?.abbr||'',awayTeam:teams.get(text(g?.away_team_id))?.abbr||'',gameStartTime:g?.scheduled_at||g?.starts_at||ou.starts_at,updatedAt:row.updated_at,headshot:p.image_url,position:appearance.position_id,
   sides:[...new Set(options.map(o=>o.choice==='higher'?'OVER':'UNDER'))]});
