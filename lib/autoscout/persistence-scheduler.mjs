...
export function startFrugalPersistence() {
  ...
  console.log(`[AutoScout persistence] scheduler interval=${ingestSeconds}s, sports-per-cycle=${perCycle}/${startupPersistenceSports.length}, snapshot-interval=${snapshotSeconds}s, provider-mode=${propProviderMode()}, public-feeds=${propProviderMode()==='sportsgameodds'?'off':'on'}, selected-provider-cache=enabled`);
  return timer;
}
