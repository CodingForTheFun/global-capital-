...
const DFS_HTTP2 = Object.freeze({
  prizepicks: { urls: ['https://api.prizepicks.com/projections?per_page=250','https://partner-api.prizepicks.com/projections?per_page=250'] },
...
const DFS_BROWSER = Object.freeze({
  prizepicks: {
    ...
    fallbackUrls: ['https://api.prizepicks.com/projections?per_page=250','https://partner-api.prizepicks.com/projections?per_page=250'],
  ...
  underdog: {
    ...
    fallbackUrls: ['https://api.underdogfantasy.com/beta/v5/over_under_lines','https://api.underdogfantasy.com/beta/v6/over_under_lines'],
...
export function createPublicIngestionRunner({ now = Date.now, fetcher = globalThis.fetch, feeds = publicFeeds, claim = claimPublicCycle, release = releasePublicCycle, persistSnapshot = persistPublicSnapshot, recordStatus = recordPublicStatus } = {}) {
  ...
  async function http2DfsSnapshot(source) {
    ...
    for (const url of config.urls) {
      try {
        const payload = await http2JsonFetch(url);
    ...
    try {
      ...
      if (blocked()) {
        try { snapshot = await browserDfsSnapshot(source) || snapshot; }
          ...
          return { source, persisted: false, retained: true, code: text(error?.code || 'PUBLIC_BROWSER_FAILED'), httpStatus: Number(error?.status) || null };
        }
      }
  ...
  async function ingestDraftKings(sport) {
    ...
    const url = draftKingsUrl(sport); if (!url) return { source, skipped: true };
    try {
      ...
      const observedAt = new Date(now()).toISOString(), records = normalizeDraftKings(payload, sport);
