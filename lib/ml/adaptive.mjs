...
function cleanLogs(gameLog,now){const seen=new Set();return (Array.isArray(gameLog)?gameLog:[]).filter(r=>r&&finite(Number(r.value))&&Number(r.value)>=0&&Number.isFinite(Date.parse(r.date||''))&&Date.parse(r.date)<=now).map(r=>({...r,value:Number(r.value),ts:Date.parse(r.date)})).sort((a,b)=>a.ts-b.ts).filter(r=>{const k=String(r.gameId||r.date||r.ts);if(seen.has(k))return false;seen.add(k);return true;});}
...
function betaRate(values,line){if(!values.length||!finite(line))return null;const hits=values.filter(v=>v>line).length,pushes=values.filter(v=>v===line).length;return {games:values.length,hits,pushes,rate:(hits+1)/(values.length+2)};}
function blendProbability(modelP,evidence,sampleSize){
 const usable=evidence.filter(e=>e&&finite(e.rate)&&finite(e.weight)&&e.weight>0);
 let total=1,p=modelP;
 for(const e of usable){p+=e.rate*e.weight;total+=e.weight;}
 p/=total;
 // Extreme probabilities need substantially more independent history than a
 // single player's short rolling window can provide. This cap is uncertainty,
 // not a fabricated estimate, and relaxes only as verified evidence grows.
 const cap=sampleSize>=50?.92:sampleSize>=30?.88:.85;
 return clamp(p,1-cap,cap);
}
...
export function adaptivePrediction({research,target,now=Date.now()}={}){
 ...
 const logs=cleanLogs(research.gameLog,now),values=logs.map(r=>r.value);
 if(values.length<9)return {available:false,modelled:true,engine:ENGINE,code:'MODEL_NOT_READY',message:'At least 9 verified completed games are required for the adaptive model.'};
 ...
 const sampleFactor=clamp((values.length-8)/12,.25,1);
 // The trained adjustment earns influence only when its time-ordered checks beat
 // the simple recent-mean baseline. Otherwise the safer baseline remains active.
 const qualityGain=modelRmse&&baseRmse?clamp((baseRmse-modelRmse)/Math.max(baseRmse,1e-9),0,1):0;
 const selectedStrategy=checks.length>=3&&qualityGain>0?'trained-adjustment':'recent-mean-fallback';
 let projection=selectedStrategy==='trained-adjustment'?base+(raw-base)*sampleFactor*qualityGain:base;
 const opponent=research?.matchup?.opponent||research?.opponent||null;
 const h2hValues=opponent?logs.filter(r=>String(r.opponent||'').toUpperCase()===String(opponent).toUpperCase()).map(r=>r.value):[];
 // H2H affects the point estimate with strong shrinkage. One meeting is useful
 // evidence but never enough to move the projection by itself.
 if(h2hValues.length){const adj=(mean(h2hValues)-base)*(h2hValues.length/(h2hValues.length+8));projection+=clamp(adj,-Math.max(.5,recentSd*.5),Math.max(.5,recentSd*.5));}
 ...
 const line=Number(target?.line),rawP=probs(projection,line,sigma,integerLike(values.slice(-20)));if(!rawP)return {available:false,modelled:true,engine:ENGINE,code:'TARGET_UNVERIFIED',message:'A valid current line is required for an adaptive probability estimate.'};
 const allLine=betaRate(values,line),recentLine=betaRate(values.slice(-10),line),h2hLine=betaRate(h2hValues,line);
 // Probability is not inferred from RMSE alone. Blend the fitted distribution
 // with smoothed observed outcomes at this exact current line. H2H earns weight
 // gradually, so a one-game matchup cannot manufacture a 90% forecast.
 const over=blendProbability(rawP.over,[
   {rate:allLine?.rate,weight:Math.min(1.5,(allLine?.games||0)/20)},
   {rate:recentLine?.rate,weight:Math.min(1,(recentLine?.games||0)/10)},
   {rate:h2hLine?.rate,weight:Math.min(1.25,(h2hLine?.games||0)/6)}
 ],values.length);
 const nonPush=rawP.over+rawP.under,push=rawP.push;
 const p={over:over*(1-push),under:(1-over)*(1-push),push};
 ...
 return {...target,available:true,modelled:true,engine:ENGINE,code:'READY',modelVersion:'adaptive-ridge-v3',projection,
  probabilityOver:p.over,probabilityUnder:p.under,probabilityPush:p.push,generatedAt:generated,expiresAt,featureCutoff,
  validation:{method:'rolling-player-history',observations:checks.length,events:checks.length,rmse:modelRmse,baselineRmse:baseRmse,selectedStrategy,
   probabilityEvidence:{all:allLine,recent10:recentLine,h2h:h2hLine,opponent:h2hValues.length?opponent:null}},
