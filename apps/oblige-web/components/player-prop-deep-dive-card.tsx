...
export function PlayerPropDeepDiveCard({
  ...
  const position = text(history?.player?.position || group?.position || player.position);
        ...
        <div data-research-hero className="rounded-2xl border border-slate-800 bg-[radial-gradient(circle_at_15%_10%,rgba(59,130,246,0.13),transparent_30%),linear-gradient(180deg,rgba(19,27,46,0.9),rgba(10,15,24,0.96))] p-2.5 sm:p-4">
          <div className="flex items-start justify-between gap-3">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0"><div className="flex min-w-0 flex-wrap items-center gap-1.5"><h1 className="min-w-0 truncate text-lg font-black tracking-tight text-white sm:text-2xl">{player.name}</h1>{position && <span className="shrink-0 rounded-md border border-slate-700 bg-slate-900/90 px-1.5 py-0.5 text-[10px] font-black text-slate-400">{position}</span>}</div><p className="mt-0.5 grid gap-0.5 text-[10px] font-semibold leading-4 text-slate-400 sm:mt-1 sm:flex sm:flex-wrap sm:items-center sm:gap-x-1.5 sm:text-xs"><span>{game}</span><span className="text-slate-500">· {kickoffLabel(player.startsAt)}</span></p><div className="hidden sm:mt-2 sm:flex sm:flex-wrap sm:items-center sm:gap-2 sm:text-[10px] sm:text-slate-500"><span>{team || history?.player?.team || 'Team unavailable'}</span>{analysis.loading ? <><span>•</span><span>Loading history…</span></> : analysis.unavailable ? <><span>•</span><span className="text-amber-300">History unavailable</span></> : <><span>•</span><span className="text-emerald-400">Verified history</span></>}</div></div>
            </div>
          ...
          <div className="mt-2 grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 rounded-xl border border-sky-500/20 bg-sky-500/[0.07] p-2 sm:mt-4 sm:p-2.5"><div className="flex min-w-0 items-center gap-2"><span className="shrink-0 rounded-md bg-sky-500 px-2 py-1 text-[9px] font-black tracking-wide text-white sm:text-[10px]">{initials(selected.bookName)}</span><div className="min-w-0"><div className="flex min-w-0 items-center gap-1.5"><h2 className="min-w-0 truncate text-[11px] font-bold text-white sm:text-sm">{title}</h2>{variantLabel && <span data-variant={selected.dfsOddsType} className="shrink-0 rounded-md border border-emerald-400/20 bg-emerald-400/10 px-1.5 py-0.5 text-[8px] font-black uppercase tracking-wide text-emerald-300">{variantLabel}</span>}</div><div className="hidden text-[10px] text-slate-500 sm:block">Main prop · {period}</div></div></div><div className="grid shrink-0 justify-items-end gap-1"><label className="relative block"><span className="sr-only">Selected book</span><select aria-label="Selected book" value={currentBook || ''} onChange={event => onBook(event.target.value)} className="h-7 w-[104px] appearance-none truncate rounded-lg border border-sky-400/20 bg-slate-950/60 pl-2 pr-5 text-[9px] font-bold text-slate-300 outline-none sm:h-8 sm:w-auto sm:min-w-[118px] sm:pr-6 sm:text-[10px]">{allowBestPrices && <option value="">Best prices · all books</option>}{applicableBooks.map(book => <option key={book.key} value={book.key}>{book.name}</option>)}</select><ChevronDown className="pointer-events-none absolute right-1.5 top-2 h-3 w-3 text-slate-500"/></label><div className="flex items-center gap-1.5 whitespace-nowrap text-[12px] font-black sm:text-sm"><span className={selected.side === 'UNDER' ? 'text-rose-400' : 'text-emerald-400'}>{selected.side === 'OVER' ? 'O' : selected.side === 'UNDER' ? 'U' : selected.choice} {selected.line ?? ''}</span><span className="text-slate-400">{offerPrice(selected)}</span></div></div></div>
        </div>

        <div className="space-y-3">
          <div className="flex items-center gap-2"><div role="group" aria-label="Stat categories" className="flex min-w-0 flex-1 gap-2 overflow-x-auto overscroll-x-contain pb-1 pr-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{statFamilies.map(item => { const active = marketFamily(item) === family; return <button key={marketFamily(item)} type="button" onClick={() => chooseStat(item)} aria-pressed={active} title={marketName(item)} className={`h-9 shrink-0 rounded-lg border px-3 text-[11px] font-black tracking-wide transition ${active ? 'border-sky-400/50 bg-sky-500/15 text-sky-300 shadow-[0_0_20px_rgba(56,189,248,0.16)]' : 'border-slate-800 bg-slate-900/60 text-slate-500 hover:border-slate-700 hover:text-slate-300'}`}>{compactStatLabel(marketName(item))}</button>; })}</div><select aria-label="Player stat category" value={market.key} onChange={event => onCategory(event.target.value)} className="h-9 max-w-[130px] shrink-0 rounded-lg border border-slate-700 bg-slate-900 px-2 text-[10px] font-bold text-slate-300 outline-none">{player.markets.map(item => <option key={item.key} value={item.key}>{marketOptionName(item)}</option>)}</select></div>
          <div role="group" aria-label="Available game periods" className="flex gap-2 overflow-x-auto overscroll-x-contain pb-1 pr-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{periodMarkets.map(item => { const active = item.key === market.key; return <button key={item.key} type="button" onClick={() => onCategory(item.key)} aria-pressed={active} className={`h-8 shrink-0 rounded-full border px-3 text-[10px] font-bold transition ${active ? 'border-blue-400/40 bg-blue-500 text-white shadow-[0_0_18px_rgba(59,130,246,0.24)]' : 'border-slate-800 bg-[#0e1522] text-slate-500 hover:text-slate-300'}`}>{periodName(item.period)}</button>; })}</div>
          <div className="-mx-1 flex gap-2 overflow-x-auto overscroll-x-contain px-1 pb-1 pr-6 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
