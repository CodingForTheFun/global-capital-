...
test('each prop gets a real-book selector that reprices that card only', () => {
  ...
  assert.match(output, /quote\(over,'OVER'\)\+quote\(under,'UNDER'\)/, 'the selector must expose both selected over and under lines');
  assert.match(output, /asPropBookQuote/, 'selected sportsbook quotes must be visibly rendered');
