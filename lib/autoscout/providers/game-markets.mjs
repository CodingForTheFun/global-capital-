...
const paidPaused = () => truthy(process.env.THE_ODDS_API_PAUSED);
...
export function createGameBoard({ request, scope, sportKey, remaining = () => null, directory = process.env.DATA_DIR || './data' }) {
  ...
  async function reserve(cost) {
    if (paidPaused()) throw Object.assign(new Error('Paid odds feed is paused.'), { code: 'PAID_PROVIDER_PAUSED' });
    const pending = budgetQueue.then(async () => {
      ...
      const configured = Number(process.env.SPORTSBOOK_DAILY_CREDITS || 12);
      const cap = Number.isFinite(configured) ? Math.max(1,Math.min(configured,1000)) : 12;
      const reserve = Number(process.env.THE_ODDS_API_MIN_REMAINING_CREDITS || 7000);
      const left = remaining();
      if (!Number.isFinite(ledger.credits) || ledger.credits + cost > cap || (left !== null && left < Math.max(cost, reserve))) throw Object.assign(new Error('Game market budget reached'), { code: 'GAME_BUDGET_LIMIT' });
      ledger.credits += cost;
  ...
  return async function gameBoard(sport) {
    ...
    const pending = (async()=>{
      try {
        ...
        const paused=e.code==='PAID_PROVIDER_PAUSED';
        const value={ok:true,available:false,sport,games:[],code:e.code||'GAME_FEED_UNAVAILABLE',message:paused?'Paid sportsbook refresh is paused to protect the monthly request budget. Cached player props remain available.':e.code==='GAME_BUDGET_LIMIT'?'Game odds refresh is at capacity. Existing props and Auto Scout remain available.':'Game odds are temporarily unavailable. Try again later.'};
