 ...
 * Player name plus game, both normalised, because provider player ids differ.
 */
...
export function enrichmentKey({ playerName, gameId, sport } = {}) {
  return [
...
export function sanitizeEnrichment(raw, capabilities = []) {
  ...
  for (const [key, value] of Object.entries(raw)) {
    ...
    clean[key] = ['injuryStatus', 'liveStatus'].includes(key) ? String(value).toUpperCase() : value;
  }
  return clean;
}
