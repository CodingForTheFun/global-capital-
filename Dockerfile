...
COPY . .
# Run the complete release check while the source tree is still pristine.
# prepare-edge-deploy intentionally rewrites build-time anchors afterward, so
# source-anchor tests belong here rather than in Railway's post-build phase.
RUN npm run check
