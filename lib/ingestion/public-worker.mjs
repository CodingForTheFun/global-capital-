...
const configuredDraftKingsSite = text(process.env.AUTOSCOUT_DRAFTKINGS_SITE || 'US-VA-SB').toUpperCase();
const DK_SITE = /^US-(?:[A-Z]{2}-SB|DK)$/.test(configuredDraftKingsSite) ? configuredDraftKingsSite : 'US-VA-SB';
...
async function fetchJson(fetcher, url) {
  const response = await fetcher(url, {
    headers: {
      ...
      referer: `https://sportsbook.draftkings.com/?intendedSiteExp=${encodeURIComponent(DK_SITE)}`,
...
function draftKingsUrl(sport) {
  ...
  return group ? `${DK_BASE}/sites/${encodeURIComponent(DK_SITE)}/api/v5/eventgroups/${encodeURIComponent(group)}/?format=json` : null;
}
...
export function createPublicIngestionRunner({
  ...
  async function ingestDraftKings(sport) {
    ...
    try {
      ...
      const result = await persistSnapshot(source, rows, observedAt);
      await status(source, { status: rows.length ? 'available' : 'no_props', rows: rows.length, fetchedAt: observedAt, written: Number(result?.written || 0), httpStatus: 200 });
      return { source, persisted: true, rows: rows.length, written: Number(result?.written || 0), httpStatus: 200 };
    } catch (error) {
