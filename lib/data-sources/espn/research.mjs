...
import { PUBLIC_LEAGUES, LEAGUE_AGNOSTIC, ANY_LEAGUE_PATH, canonicalSport, marketContract, normalizeStatColumns, numeric } from './stat-contract.mjs';
import { resolvePublicAthlete, resolveRosterAthlete, teamKey, samePublicTeam as sameTeam, matchesTeamRecord, sameClub, matchesClubRecord } from './identity.mjs';
import { createDefensePosition } from './defense-position.mjs';
import { createTeammates } from './teammates.mjs';
import { fetchPublicMatchup } from './matchup.mjs';
...
const STAT_OUTPUT = {
  Points:'points',Rebounds:'rebounds',OffensiveRebounds:'offensiveRebounds',DefensiveRebounds:'defensiveRebounds',Assists:'assists',Steals:'steals',BlockedShots:'blocks',Turnovers:'turnovers',ThreePointersMade:'threes',ThreePointersAttempted:'threePointersAttempted',
  FieldGoalsAttempted:'fieldGoalsAttempted',FreeThrowsMade:'freeThrowsMade',FreeThrowsAttempted:'freeThrowsAttempted',PersonalFouls:'personalFouls',
  ...
  SoccerGoals:'goals',SoccerAssists:'assists',Shots:'shots',ShotsOnTarget:'shotsOnTarget',Fouls:'fouls',GoalsConceded:'goalsConceded',
};
const MESSAGES = {
...
function seasonType(group, category, soccer = false, anyCompetition = false) {
  ...
  if (soccer) return /playoff|knockout|round of|quarter.final|semi.final|final$/i.test(title) ? 3
    : /regular season|league phase|group stage|group phase|premier league/i.test(title) ? 2
    // ESPN heads a domestic season with the competition's own name rather than
    // a phrase this list can match: "2026-27 Spanish LaLiga", "2026-27 Italian
    // Serie A". For a prop that named no competition there is nothing to
    // recognise, so the heading is not the gate — every event is still checked
    // against the single competition the log came back with, which is stricter
    // than reading a title. Friendlies and preseason are already gone above.
    : anyCompetition ? 2 : null;
...
// A prop that named no competition still must not mix them: a hit rate is only
// meaningful over one. ESPN answers a league-agnostic game log with a single
// competition, so that competition is read back off the events themselves and
// anything else in the payload is counted foreign, exactly as a wrong league
// would be for EPL or MLS.
function reportedLeague(payload) {
  ...
  let best=null,top=0;
  for(const [league,count] of counts)if(count>top){top=count;best=league;}
  return best;
}
function leagueMatches(event,sport,reported=null) {
  const league=String(event.leagueAbbreviation||'').toUpperCase();
  if(LEAGUE_AGNOSTIC.has(sport))return Boolean(reported)&&league===reported;
...
export function inspectGameLog(payload, params = {}) {
  ...
  const now=params.now??Date.now(),anyCompetition=LEAGUE_AGNOSTIC.has(sport),reported=anyCompetition?reportedLeague(payload):null;
  for(const group of payload.seasonTypes||[])for(const cat of group.categories||[]) {
    const type=seasonType(group,cat,family==='soccer',anyCompetition);
    ...
    for(const row of cat.events||[]) {
      ...
      if(!leagueMatches(event,sport,reported)){foreignLeagueEvents++;continue;}
...
export function createPublicResearch({ fetchImpl = (...args) => fetch(...args), now = () => Date.now() } = {}) {
  ...
  async function request(path, ttl) {
    ...
    const work = (async () => {
      ...
      try {
        ...
        if (response.ok) result = { data: await response.json(), cached: false, retrievedAt:new Date(now()).toISOString() };
      } catch { /* Generic absence only; no raw provider error reaches the API. */ }
      finally { const next=queue.shift(); if(next)next(); else active--; }
      const emptySearch=path.startsWith('/search/')&&!result.data?.results?.some(g=>g.type==='player'&&g.contents?.length);
      const expires = !result.data ? now()+30_000 : emptySearch ? now()+60_000 : ttl === Infinity ? Infinity : now()+ttl;
  ...
  const fetchResearch = async function(params = {}) {
    const sport=canonicalSport(params.sport),contract=marketContract({...params,sport});
    ...
    const anyLeague=LEAGUE_AGNOSTIC.has(sport),pathLeague=leaguePath||ANY_LEAGUE_PATH;
    ...
    if(!athlete&&(params.team||params.homeTeam||params.awayTeam)) {
      ...
      for(const t of scope){
        const roster=await request(`/site/v2/sports/${sportPath}/${pathLeague}/teams/${t.id}/roster`,60*60_000);
    ...
    const path=`/common/v3/sports/${sportPath}/${pathLeague}/athletes/${athlete.id}/gamelog`;
    ...
    // league parameter and independently validate every event's league. A prop
    // that named no competition has no parameter to send, so the log comes back
    // on the athlete's own competition and inspectGameLog holds every event to
    // that one instead.
    if(sportPath==='soccer'&&!anyLeague)query.set('league',leaguePath);
    const logPath=()=>path+(query.size?'?'+query:'');
    // No competition, no scoreboard to read a season off. ESPN already defaults
    // a league-agnostic log to the athlete's current season, so there is
    // nothing to correct and nothing worth spending a request to learn.
    const clock=anyLeague?{data:null}:await request(`/site/v2/sports/${sportPath}/${leaguePath}/scoreboard?limit=1`,GAME_LOG_TTL);
    ...
    const historyYears=Math.min(5,Math.max(1,Math.floor(Number(params.historyYears)||1)));
    let logs=current.rows,olderResults=[],olderFailed=false;
    const count=Math.max(15,Math.min(historyYears>1?100:40,Math.floor(Number(params.games)||20)));
    const filter=response.data.filters?.find(f=>f.name==='season');
    const year=Number(filter?.value);
    const listedYears=[...new Set((filter?.options||[]).map(o=>Number(o?.value)).filter(value=>Number.isInteger(value)&&value>1900&&value<year))].sort((a,b)=>b-a);
    const fallbackYears=Number.isInteger(year)&&year>1900
      ? Array.from({length:Math.max(1,historyYears-1)},(_,index)=>year-index-1)
      : [];
    const olderYears=historyYears>1
      ? (listedYears.length?listedYears:fallbackYears).slice(0,historyYears-1)
      : logs.length<count&&fallbackYears.length?[listedYears[0]||fallbackYears[0]]:[];
    for(const olderYear of olderYears) {
      query.set('season',String(olderYear));
      const older=await request(logPath(),GAME_LOG_TTL);
      cached=cached&&Boolean(older.cached);
      if(!older.data){olderFailed=true;break;}
      const inspected=inspectGameLog(older.data,{...params,sport,now:now()});
      olderResults.push(inspected);
      // A provider ignoring the season parameter or crossing competitions must
      // never masquerade as long-range history. Stop instead of repeating the
      // same bad payload across more requested years.
      if(String(inspected.season)!==String(olderYear)||inspected.foreignLeagueEvents>0){olderFailed=true;break;}
      logs=[...logs,...inspected.rows.filter(r=>r.seasonType===2)];
    }
    logs=[...new Map(logs.sort((a,b)=>Date.parse(b.date)-Date.parse(a.date)).map(r=>[r.gameId,r])).values()];
    if(!logs.length)return unavailable(olderFailed?'RESEARCH_PROVIDER_ERROR':current.code==='STAT_NOT_AVAILABLE'||olderResults.some(result=>result?.code==='STAT_NOT_AVAILABLE')?'STAT_NOT_AVAILABLE':'NO_GAME_LOG_DATA');
    ...
    const isSoccer=sportPath==='soccer';
    const knownTeam=value=>sameTeam(value,logs[0].team,sport)||sameTeam(value,athlete.teamName,sport)||matchesTeamRecord(value,currentTeam,sport)
      ||isSoccer&&(sameClub(value,athlete.teamName)||sameClub(value,logs[0].teamName)||matchesClubRecord(value,currentTeam));
    ...
    if(requestedTeams.length&&!requestedTeams.some(knownTeam)) {
      const profile=await request(`/common/v3/sports/${sportPath}/${pathLeague}/athletes/${athlete.id}`,60*60_000);
    ...
    const homeKnown=Boolean(params.homeTeam&&knownTeam(params.homeTeam)),awayKnown=Boolean(params.awayTeam&&knownTeam(params.awayTeam));
    const isHome=homeKnown!==awayKnown?homeKnown:null;
    const sackKind=current.sackKind||olderResults.find(result=>result?.sackKind)?.sackKind;
    const returnedSeasons=[...new Set(logs.map(row=>String(row.season||'')).filter(Boolean))].sort((a,b)=>Number(b)-Number(a));
    return {ok:true,available:true,source:'Historical stats',cached,isHome,opponent:matchedOpponent?.opponent||matchupOpponent,opponentId:matchedOpponent?.opponentId||null,
      ...
      coverage:{seasonComplete:current.complete,backfilledGames:logs.filter(r=>String(r.season)!==String(year)).length,backfilled:logs.some(r=>String(r.season)!==String(year)),
        currentSeasonGames:current.rows.filter(r=>r.seasonType===2).length,historyPartial:olderFailed,historyYearsRequested:historyYears,returnedSeasons},season:filter?.value||null};
  };
  fetchResearch.defensePosition = createDefensePosition({request,teamDirectory,now});
  fetchResearch.teammates = createTeammates({request,teamDirectory});
  // The same cached, read-only public requests for the model's box-score features.
  fetchResearch.espn = {request,teamDirectory};
  fetchResearch.matchup = params => fetchPublicMatchup(params,{request,now});
  // A prop's Opponent filter needs the full verified league directory, not only
  // clubs that happen to appear in that player's returned game log. Reuse the
  // same public ESPN team endpoint and 24h cache already used for identity
  // verification; this is read-only public metadata and never widens paid
  // provider polling.
  fetchResearch.teams = async inputSport => {
    const sport=canonicalSport(inputSport);
    if(!PUBLIC_LEAGUES[sport]?.[1])return [];
    const rows=await teamDirectory(sport),seen=new Set(),out=[];
    for(const team of rows){
      ...
      if(!/^\d+$/.test(id)||!abbreviation||!name)continue;
      const key=id+'|'+abbreviation.toUpperCase();
      if(seen.has(key))continue;
      seen.add(key);
      out.push({id,abbreviation,name});
    }
...
export const fetchPublicTeamDirectory = sport => typeof fetchPublicResearch.teams==='function' ? fetchPublicResearch.teams(sport) : Promise.resolve([]);
export const fetchMatchupResearch = params => fetchPublicResearch.matchup(params);
...
export const fetchDefensePosition = params => fetchPublicResearch.defensePosition(params);
export const fetchTeammates = params => fetchPublicResearch.teammates(params);
