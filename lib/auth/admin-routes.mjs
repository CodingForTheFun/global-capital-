...
export async function handleAdminRoutes(req, res, url, { sessions, json, secret, log = console }) {
  ...
  try {
    ...
    if (path === '/api/admin/session/revoke') {
      ...
      await appendAudit({ actorId: user.id, actorRole: ROLES.OWNER, action: 'owner.session.revoke', outcome: revoked ? 'success' : 'no-op' });
    ...
    if (path === '/api/admin/member/role') {
      ...
      if (body.role === ROLES.OWNER) {
        json(res, 400, { ok: false, code: 'OWNER_PROTECTED', message: 'Owner access can only be assigned by the server owner identity.' });
        return true;
      }
