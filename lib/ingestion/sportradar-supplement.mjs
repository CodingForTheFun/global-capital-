...
export function sportradarSupplementPolicy() {
  return {
    ...
    entitlementBackoffSeconds: clamp(process.env.SPORTRADAR_PROP_ENTITLEMENT_BACKOFF_SECONDS, 21_600, 900, 86_400),
...
export async function maybeRefreshSportradarSupplement({ now = Date.now } = {}) {
  ...
  try {
    ...
    const entitlementDenied = ['SPORTRADAR_FORBIDDEN','SPORTRADAR_UNAUTHORIZED'].includes(state.lastError);
    const waitSeconds = entitlementDenied
      ? policy.entitlementBackoffSeconds
      : state.lastError === 'SPORTRADAR_RATE_LIMITED'
        ? 5 * 60
        : policy.intervalSeconds;
    state.nextAt = current + waitSeconds * 1000;
    return {
      skipped: false,
      sport: state.lastSport,
      error: state.lastError,
      reason: entitlementDenied ? 'player_props_not_entitled' : null,
