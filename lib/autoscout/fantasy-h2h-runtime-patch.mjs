...
function researchParams(g,line,side){var q=new URLSearchParams({sport:g.sport,playerName:g.playerName,market:g.market,marketId:researchMarketKey(g,line,side)||'',line:String(line==null?'':line),side:side||defaultSide(g),games:'40',providerPlayerId:g.providerPlayerId||'',homeTeam:g.homeTeam||'',awayTeam:g.awayTeam||'',team:g.team||''});return q.toString();}`,
  ...
  // The base patch deliberately blocks all fantasy history. Open only formula +
  // source combinations the server now reconstructs without guessing.
  out = replaceOnce(
    out,
    ...
    "if(/\\bfantasy(?:\\s+(?:score|points?))?\\b/i.test(market+' '+marketId)){var pp=(g.rows||[]).some(r=>r.sportsbookKey==='prizepicks');var basketball=['NBA','WNBA','NCAAB'].includes(selectedSport)&&marketId==='player_fantasy_score';var football=['NFL','NCAAF'].includes(selectedSport)&&marketId==='player_fantasy_score';var kicker=selectedSport==='NFL'&&(marketId==='player_kicker_fantasy_score'||/Kicker Fantasy Score/i.test(market));var baseball=selectedSport==='MLB'&&(/hitter_fantasy_score/.test(marketId)||/pitcher_fantasy_score/.test(marketId)||/^(?:Hitter|Pitcher) Fantasy Score$/i.test(market));var goalie=['SOCCER','MLS','EPL','UCL'].includes(selectedSport)&&(marketId==='player_goalie_fantasy_score'||/Goal(?:ie|keeper) Fantasy Score/i.test(market));var exact=pp&&(basketball||football||kicker||baseball||goalie);if(!exact)return {ok:true,available:false,lineOnly:true,retryable:false,code:'FANTASY_SCORING_UNVERIFIED',message:'Live fantasy line available. Trend data is not available for this scoring market yet.'};}",
    'client fantasy gate',
  );
