...
function flexiblePlayerPattern(value) {
 return String(value||'').normalize('NFKC').replace(/\u00a0/g,' ').trim().split(/\s+/).filter(Boolean)
  .map(escapeMarketPattern).join('[\\s\\u00a0._-]+');
}
function displayNorm(value) {
 return String(value||'').normalize('NFKC').toLowerCase().replace(/[’']/g,'').replace(/[^a-z0-9]+/g,' ').trim();
}
function stripPlayerFromMarket(value,playerName) {
...
function labelFromMarketKey(key) {
 const bare=String(key||'').replace(/^(?:player|batter|pitcher|team|sgo|propline|sportsgameodds)_/,'');
 if(!bare||!/^[a-z0-9_]+$/.test(bare))return '';
 const tokenLabels={yds:'Yards',tds:'Touchdowns',pts:'Points',reb:'Rebounds',rebs:'Rebounds',ast:'Assists',asts:'Assists',rbi:'RBI',rbis:'RBIs',fg:'FG',ft:'FT',qb:'QB',hr:'HR',hrs:'HRs',pra:'PRA'};
...
export function cleanMarketLabel(value,{playerName='',marketId='',statId='',sport=''}={}) {
 const key=normalizedMarketKey(marketId||statId||value);
 if(['NFL','NCAAF'].includes(String(sport||'').toUpperCase())&&key==='player_assists')return 'Assisted tackles';
 if(LABELS[key])return LABELS[key];

 let label=stripPlayerFromMarket(value,playerName);
 label=label
...
function teamAliasProfile(value) {
 ...
 if(words.length>1){
  ...
  const city=words.slice(0,-1),nickname=words[words.length-1]||'';
  const cityCode=city.length===1&&city[0].length<=3?city[0]:city.map(word=>word[0]).join('');
...
function cardTeamIndex(groups=[]) {
 const teamsByName=new Map();
 for(const group of groups){
  const name=norm(group?.playerName),team=norm(group?.team);
  if(!name||!team)continue;
  const bucket=teamsByName.get(name)||new Set();
  bucket.add(team);teamsByName.set(name,bucket);
 }
 const out=new Map();
 for(const [name,bucket] of teamsByName){
  const teams=[...bucket],profiles=teams.map(teamAliasProfile),parent=teams.map((_,i)=>i);
  const root=i=>{while(parent[i]!==i){parent[i]=parent[parent[i]];i=parent[i];}return i;};
  const join=(a,b)=>{a=root(a);b=root(b);if(a!==b)parent[b]=a;};
  // Full labels can safely merge on strong aliases such as NYG, LAR or NYM.
  // Weak city-only aliases are deliberately excluded here so New York Giants
  // and New York Jets can never collapse merely because both are "NY".
  for(let i=0;i<teams.length;i++)for(let j=i+1;j<teams.length;j++){
   if(!profiles[i].short&&!profiles[j].short&&intersects(profiles[i].strong,profiles[j].strong))join(i,j);
  }
  // A short provider code may use only the city code (KC, SD, OKC). Attach it
  // only when that code resolves to one already-distinct full-label cluster.
  for(let i=0;i<teams.length;i++){
   if(!profiles[i].short)continue;
   const matches=new Set();
   for(let j=0;j<teams.length;j++){
    if(i===j||profiles[j].short)continue;
    if(profiles[j].strong.has(profiles[i].raw)||profiles[j].weak.has(profiles[i].raw))matches.add(root(j));
   }
   if(matches.size===1)join(i,[...matches][0]);
  }
  const members=new Map();
  for(let i=0;i<teams.length;i++){const r=root(i),list=members.get(r)||[];list.push(teams[i]);members.set(r,list);}
  const clusterKey=new Map();
  for(const [r,list] of members)clusterKey.set(r,[...list].sort((a,b)=>a.length-b.length||a.localeCompare(b))[0]);
  const keyByTeam=new Map();
  for(let i=0;i<teams.length;i++)keyByTeam.set(teams[i],clusterKey.get(root(i)));
  out.set(name,{size:members.size,keyByTeam});
 }
 return out;
}
export function playerCardKey(group,index=null) {
 // Provider IDs can differ by book, while provider team labels can differ by
 // spelling (NYG vs New York Giants). Use the normalized player name by
 // default, and add a team discriminator only when the board proves that name
 // belongs to multiple distinct team clusters.
 const name=norm(group.playerName);
 const sport=String(group.sport||'').toUpperCase();
 const entity=group.entityType||'player';
 if(name){
  const team=norm(group.team),entry=index instanceof Map?index.get(name):null;
  const clustered=entry&&!(entry instanceof Set)&&entry.keyByTeam instanceof Map;
  const contested=clustered?entry.size>1:(entry instanceof Set?entry.size>1:false);
  const teamKey=clustered?(entry.keyByTeam.get(team)||team):team;
  return [sport,entity,'name:'+name,contested&&teamKey?'team:'+teamKey:''].join('|');
 }
...
export function categoryOptions(groups,sport) {
 const map=new Map();
 const index=cardTeamIndex(groups);
