...
// Documented game-log fields beyond the stat itself (result, score_for,
// score_against, season_type) were previously dropped here, which left every
// archive-sourced history unable to answer a win/loss or game-type filter.
...
/**
 * Set score of a completed tennis match from the player's side. `sets_won` is
 * the anchor; the opponent's sets come from score_for/score_against only when
 * score_for is that same set count (it may otherwise be games), or from the
 * number of per-set game rows. The winner must have reached 2 (best of 3) or
 * 3 (best of 5) sets; retirements, conflicts and partial scores return null.
 */
export function tennisSetScore(row) {
  const stats = row?.stats && typeof row.stats === 'object' ? row.stats : {};
  const setsWon = numeric(stats.sets_won);
  if (setsWon === null) return null;
  const setRows = [1, 2, 3, 4, 5].filter(n => numeric(stats[`set_${n}_games`]) !== null).length || null;
  const scoreFor = numeric(row?.score_for), scoreAgainst = numeric(row?.score_against);
  let setsLost = null;
  if (scoreFor !== null && scoreAgainst !== null && scoreFor === setsWon) setsLost = scoreAgainst;
  else if (setRows !== null && setRows >= setsWon) setsLost = setRows - setsWon;
...
/** Result from the stated code, cross-checked against the score; a conflict is unknown. */
function gameResultOf(row, sport, sets) {
  const stated = statedResult(row?.result);
  let derived = null;
  if (sport === 'TENNIS') derived = sets?.result || null;
  else {
    const scoreFor = numeric(row?.score_for), scoreAgainst = numeric(row?.score_against);
    if (scoreFor !== null && scoreAgainst !== null) derived = scoreFor === scoreAgainst ? 'T' : scoreFor > scoreAgainst ? 'W' : 'L';
  }
  if (stated && derived && stated !== derived) return null;
...
export function normalizeGameArchive(payload, params = {}, now = Date.now()) {
  ...
  for (const row of payload.games.slice(0, 100)) {
    ...
    const sets = sport === 'TENNIS' ? tennisSetScore(row) : null;
    const detail = {
      gameResult: gameResultOf(row, sport, sets),
      ...
      ...(sport === 'TENNIS'
        ...
        : { scoreFor: numeric(row.score_for), scoreAgainst: numeric(row.score_against) }),
    };
    rows.set(eventId, {
