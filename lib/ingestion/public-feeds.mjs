...
export function createPublicFeeds({fetcher=globalThis.fetch,now=Date.now,storeDir=null,mappings=[],feeds=PUBLIC_FEEDS}={}){
 ...
 async function get(feed,url){
  const response=await ingestionFetch(url,{headers:headers(feed),signal:AbortSignal.timeout(12000),redirect:'follow'},fetcher);
 ...
 async function refreshOne(feed){
  ...
  const task=(async()=>{
   try{
    const {data,partial,endpoint}=await fetchPages(feed);assertIngestionActive();let parsed=empty();
 ...
 function refresh(){if(refreshPromise)return refreshPromise;refreshPromise=(async()=>{await load();for(const feed of feeds){try{await withIngestionDeadline(`feed-refresh:${feed.id}`,()=>refreshOne(feed),B.feeds);}catch(error){assertIngestionActive();console.warn('[AutoScout public feed retained]',JSON.stringify({source:feed.id,code:error?.code||'PUBLIC_FEED_FAILED'}));}}})().finally(()=>{refreshPromise=null;});return refreshPromise;}
