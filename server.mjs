...
async function saveResult(result) {
  ...
  history.unshift({ scannedAt: result.scannedAt, mode: result.mode, totalReviewed: result.totalReviewed, qualifiedCount: result.qualifiedCount, rejectedCount: result.rejectedCount, bestConfidence: Math.max(0, ...(result.picks || []).filter((p) => p.qualified).map((p) => Number(p.confidence) || 0)) });
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/api/status' && req.method === 'GET') {
    const latest = await readJson(latestPath, null); const connection = await getPickFinderConnectionState();
    return json(res, 200, { running, lastError, connection, demoMode: String(process.env.DEMO_MODE ?? 'true').toLowerCase() === 'true', autoScanMinutes: intervalMinutes, payoutMultiplier: process.env.PAYOUT_MULTIPLIER || null, progress: scanProgress, latest });
  }
  ...
  if (url.pathname === '/api/connect' && req.method === 'POST') {
    ...
    try { const body = await readJsonBody(req, 16_000); const { verifyPickFinderConnection } = await import('./scanner/pickfinder.mjs'); const result = await verifyPickFinderConnection({ email: body.email, password: body.password }); return json(res, 200, { ok: true, connection: result }); }
  ...
  if (url.pathname === '/api/disconnect' && req.method === 'POST') {
    ...
    try { const { disconnectPickFinder } = await import('./scanner/pickfinder.mjs'); return json(res, 200, { ok: true, connection: await disconnectPickFinder() }); }
  ...
  if (url.pathname === '/api/scan' && req.method === 'POST') {
    ...
    scanNow().catch(() => {}); return json(res, 202, { ok: true, message: 'Scan started.' });
  }
  if (await serveStatic(req, res)) return; res.writeHead(404, { 'content-type': 'text/plain; charset=utf-8' }); res.end('Not found');
});
server.listen(port, () => { console.log(`AutoProp Scout Pro running on http://localhost:${port}`); console.log(`Mode: ${String(process.env.DEMO_MODE ?? 'true').toLowerCase() === 'true' ? 'demo' : 'live'}`); console.log(`Dashboard gate: ${authRequired ? 'enabled' : 'disabled'}`); });
