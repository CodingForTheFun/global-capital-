...
import { fetchProplineResearch } from '../data-sources/propline/research.mjs';
...
// PropLine is the first choice for game history; ESPN and ClearSports stay
// behind it as backups. The switch exists so the order can be reverted from
// the environment without a deploy - it defaults on, and only an explicit
// off value turns it back off.
function proplineResearchPrimary() {
  const raw = String(process.env.PROPLINE_RESEARCH_PRIMARY ?? '').trim().toLowerCase();
  if (!raw) return true;
  return !['0', 'false', 'off', 'no'].includes(raw);
}

// Ask PropLine, and answer with it only when it came back with usable graded
// history. Every other outcome - not configured, sport not carried, wrong
// player, market not graded, too few games, an error - returns null so the
// caller falls through to the source chain that was there before.
async function proplineHistory(params) {
  if (!proplineResearchPrimary()) return null;
  try {
    const attempt = await fetchProplineResearch(params);
    return attempt?.available ? attempt : null;
  } catch (error) {
    console.error('[research] PropLine history failed', String(error?.code || 'PROPLINE_UNAVAILABLE').slice(0, 80));
    return null;
  }
}
...
async function researchUncached(params = {}) {
  ...
  // PropLine first. It grades the same market keys the board prices, so when it
  // has the player it is the closer match to the line being researched.
  const propline = await proplineHistory(params);
  if (propline) return finalizePublicHistory(propline, params);
