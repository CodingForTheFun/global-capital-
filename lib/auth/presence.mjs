...
// Emergency presence only. If the persistent volume is full, a successfully
// authenticated customer may still receive a signed session while this process
// remembers the device in RAM. A restart drops these rows and therefore logs
// those sessions out (fail-closed) instead of creating permanently untracked
// devices. Account-wide revocation remains authoritative via sessionVersion.
...
export async function startSession({ userId, ip = null, userAgent = null }) {
  ...
  try {
    await transaction(async (sessions) => {
      ...
      sessions.length = 0;
      sessions.push(...live, record);
      return { value: id };
    });
  } catch (error) {
    if (error?.code !== 'ENOSPC') throw error;
    ephemeralSessions.set(id, record);
    console.warn('[Scout Pro auth] data volume full; device presence is temporarily in memory');
  }
  return id;
}
...
export async function touchSession(sessionId, { ip = null } = {}) {
  ...
  const ephemeral = ephemeralSessions.get(sessionId);
  if (ephemeral && !ephemeral.revokedAt) {
    ephemeral.lastSeenAt = new Date(now).toISOString();
    if (ip) ephemeral.ip = String(ip).slice(0, 64);
    return;
  }
  ...
  try {
    ...
    // Presence heartbeat is metadata, not authorization. A full volume must not
    // turn a valid signed session into a 500. Individual-device validity still
    // comes from the existing persisted record; only its last-seen timestamp may
    // temporarily lag.
...
export async function isSessionActive(sessionId) {
  ...
  const persisted = (await readAll()).find((row) => row.id === sessionId);
  if (persisted) return Boolean(!persisted.revokedAt && alive(persisted, Date.now()));
  const ephemeral = ephemeralSessions.get(sessionId);
  return Boolean(ephemeral && !ephemeral.revokedAt && alive(ephemeral, Date.now()));
}
...
export async function revokeSession(sessionId) {
  const ephemeral = ephemeralSessions.get(sessionId);
  if (ephemeral && !ephemeral.revokedAt) {
    ephemeral.revokedAt = new Date().toISOString();
    return true;
  }
...
export async function revokeAllForUser(userId) {
  const count = await transaction(async (sessions) => {
    let changed = 0;
    const stamp = new Date().toISOString();
    for (const session of sessions) {
      if (session.userId === String(userId) && !session.revokedAt) { session.revokedAt = stamp; changed++; }
    }
...
function combinedSessions(persisted) {
  const byId = new Map(persisted.map((session) => [session.id, session]));
  for (const session of ephemeralSessions.values()) if (!byId.has(session.id)) byId.set(session.id, session);
  return [...byId.values()];
}

/** Live sessions for one user, newest first. */
export async function sessionsForUser(userId, { now = Date.now() } = {}) {
  return combinedSessions(await readAll())
