...
function detailedGameLog(sport, market, rows = []) {
...
export async function researchPlayerProp({
  ...
  if (!fieldsFor(leagueKey, selectedMarket)) {
  ...
  const rawGames = detailedGameLog(leagueKey, selectedMarket, logResult.data).slice(0, take);
  if (!rawGames.length) {
  ...
  return {
    ...
    coverage: {
      ...
      mappedFields: fieldsFor(leagueKey, selectedMarket),
    },
  };
}
