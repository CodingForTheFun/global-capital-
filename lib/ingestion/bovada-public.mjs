...
const SPORT_PATHS = Object.freeze({
  ...
  NCAAF: 'football/college-football', NCAAB: 'basketball/college-basketball', TENNIS: 'tennis',
});
const TENNIS_PLAYER_MARKETS = new Set(['Aces', 'Double Faults', 'Games Won', 'Sets Won']);
...
function playerFromDescription(description) {
  ...
  const value = parts[1].replace(/\s*\([^)]*\)\s*$/, '').trim();
...
function parseEvent(event, sport) {
  ...
  for (const group of list(event?.displayGroups)) {
    ...
    for (const market of list(group?.markets)) {
      ...
      const label = marketLabel(description.split(/\s[-–—]\s/, 1)[0]);
      if (!playerName || !label || (sport === 'TENNIS' && !TENNIS_PLAYER_MARKETS.has(label))) continue;
