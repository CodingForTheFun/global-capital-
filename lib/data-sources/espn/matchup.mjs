...
/**
 * One scoreboard from several single-day ones. An event listed on two days is
 * kept once only when both copies carry the same start; a conflicting copy
 * stays so the exactly-one rule below rejects the game.
 */
export function mergeScoreboards(boards){
 const seen=new Set(),events=[];
 for(const board of boards)for(const event of list(board?.events)){
  const key=String(event?.id)+'|'+String(event?.date);
  if(seen.has(key))continue;
  seen.add(key);events.push(event);
 }
 return {leagues:list(boards[0]?.leagues),events};
}
...
export function normalizeMatchupSummary(summary,target,sourceEventId,{now=Date.now(),retrievedAt}={}){
 ...
 const teamRows=pair.map(({side,teamId,team,record,probables,curatedRank})=>{
  ...
  const bench=roster.length===1?list(roster[0].roster).filter(r=>r.starter===false&&id(r.athlete?.id)&&clean(r.athlete?.displayName)).map(r=>({playerId:`history:${target.sport}:${r.athlete.id}`,playerName:clean(r.athlete.displayName),position:clean(r.position?.abbreviation||r.athlete.position?.abbreviation),status:clean(r.status)||'Status not reported'})).slice(0,70):[];
  ...
  return {teamId,side,name:clean(team.displayName),abbreviation:clean(team.abbreviation),
   ...
   lineup:{available:starters.length>0,starters,bench,probables:probablePlayers}};
 });
 const quote=pregame?list(summary.pickcenter).find(q=>clean(q.provider?.name)&&q.homeTeamOdds&&q.awayTeamOdds):null;
 const american=v=>{const n=numeric(v);return n!==null&&Math.abs(n)>=100?n:null;};
 const odds=quote?{available:true,book:clean(quote.provider.name),homeMoneyline:american(quote.homeTeamOdds.moneyLine),awayMoneyline:american(quote.awayTeamOdds.moneyLine),spread:clean(quote.details)||null,total:numeric(quote.overUnder),retrievedAt,expiresAt:new Date(Math.min(start,retrieved+MAX_AGE)).toISOString()}: {available:false,message:'Game odds have not been published for this matchup.'};
 ...
 return {ok:true,available:true,sport:target.sport,eventId:target.eventId,sourceEventId,gameStartTime:target.gameStartTime,
  source:'ESPN',sourceUrl,retrievedAt,expiresAt:new Date(retrieved+MAX_AGE).toISOString(),pregame,prediction,odds,teams:teamRows,
...
export async function fetchPublicMatchup(input,{request,now=Date.now}={}){
 ...
 if(!PUBLIC_LEAGUES[target.sport]?.[1]||!clean(target.eventId)||!clean(target.homeTeam)||!clean(target.awayTeam)||!Number.isFinite(start)
 ...
 // ESPN rejects scoreboard date ranges, so the day before, of and after the
 // start are read one by one. Every day must answer: a missing day could
 // hide a second match that makes the game ambiguous.
 const boards=await Promise.all([-1,0,1].map(d=>request(`${prefix}/scoreboard?dates=${date(start+d*86400_000)}&limit=1000`,TTL)));
 if(boards.some(b=>!b.data))return absent('MATCHUP_SOURCE_UNAVAILABLE','Game context could not load. Try again shortly.');
 const event=resolveMatchupEvent(mergeScoreboards(boards.map(b=>b.data)),target);
