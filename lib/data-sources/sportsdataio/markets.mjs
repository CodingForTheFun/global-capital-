...
const BASKETBALL_MARKETS = Object.freeze({
  'points': ['Points'], 'rebounds': ['Rebounds'], 'offensive rebounds': ['OffensiveRebounds'], 'defensive rebounds': ['DefensiveRebounds'], 'assists': ['Assists'],
  'steals': ['Steals'], 'blocks': ['BlockedShots'], 'blocked shots': ['BlockedShots'], 'personal fouls': ['PersonalFouls'],
  'fg made': ['FieldGoalsMade'], 'field goals made': ['FieldGoalsMade'], 'fg attempted': ['FieldGoalsAttempted'], 'field goals attempted': ['FieldGoalsAttempted'],
  'free throws made': ['FreeThrowsMade'], 'ft made': ['FreeThrowsMade'], 'free throws attempted': ['FreeThrowsAttempted'], 'ft attempted': ['FreeThrowsAttempted'],
  'turnovers': ['Turnovers'], '3-pt made': ['ThreePointersMade'], '3-pt attempted': ['ThreePointersAttempted'], 'three pointers attempted': ['ThreePointersAttempted'],
  ...
  'fantasy score': ['FantasyPoints'], 'fantasy points': ['FantasyPoints'],
});
...
export const MARKETS = Object.freeze({
  ...
  MLB: {
    ...
    // The feeds post the singular abbreviation, which matched nothing: 330 of
    // MLB's quotes were falling through to "Line only" on a mainstream market.
    'rbi': ['RunsBattedIn'],
    ...
    'batter strikeouts': ['Strikeouts'], 'pitcher strikeouts': ['Strikeouts'], 'singles': ['Singles'], 'doubles': ['Doubles'], 'triples': ['Triples'],
    ...
    'pitching outs': ['PitchingOuts'], 'outs recorded': ['PitchingOuts'], 'pitcher outs': ['PitchingOuts'],
  ...
  NHL: {
    ...
    'blocked shots': ['BlockedShots'], 'power play points': ['PowerPlayPoints'], 'fantasy score': ['FantasyPoints'], 'fantasy points': ['FantasyPoints'],
  },
});

export const PROVIDER_MARKET_KEYS = Object.freeze({
  player_points: 'points', player_rebounds: 'rebounds', player_offensive_rebounds: 'offensive rebounds', player_defensive_rebounds: 'defensive rebounds', player_assists: 'assists', player_steals: 'steals',
  player_blocks: 'blocks', player_turnovers: 'turnovers', player_threes: 'threes', player_three_pointers_attempted: 'three pointers attempted',
  player_field_goals_made: 'field goals made', player_field_goals_attempted: 'field goals attempted', player_fg_attempted: 'fg attempted',
  player_free_throws_made: 'free throws made', player_free_throws_attempted: 'free throws attempted', player_ft_made: 'ft made', player_ft_attempted: 'ft attempted', player_personal_fouls: 'personal fouls',
  ...
  player_fantasy_points: 'fantasy score', player_fantasy_score: 'fantasy score', fantasy_score: 'fantasy score', fantasy_points: 'fantasy score',
...
export function marketFromProviderKey(providerKey) {
  const raw = String(providerKey || '').trim().toLowerCase();
  if (!raw) return null;
  // Book-qualified IDs such as "underdog:player_fantasy_points" keep the
  // platform identity upstream. Historical stat mapping is book-agnostic.
  const canonical = raw.split(':').filter(Boolean).pop() || raw;
  const candidates = [raw, canonical, canonical.replace(/^market[_:-]?/, '')];
  for (const key of candidates) {
    const mapped = PROVIDER_MARKET_KEYS[key];
