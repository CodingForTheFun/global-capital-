...
/**
 * Parse only source-labelled PrizePicks alternate variants.
 *
 * We never classify an alternate by whether its number looks easier/harder.
 * If outcome-level metadata is present, it wins. Older/current projection
 * payloads also expose odds_type=goblin|demon on the projection itself; that
 * designation historically labels the More outcome, so only that labelled
 * side is emitted unless the feed explicitly labels Less as well.
 */
export function normalizePrizePicksSpecials(payload) {
  ...
  for (const row of payload.data) {
    ...
    if (!explicit.size && projectionVariant) explicit.set('OVER', projectionVariant);
    for (const [direction, type] of explicit) {
      out.push({ ...base, side: direction, specialType: type, isAlternate: true, specialVerified: true,
        specialSourceId: text(row.id), specialTypeSource: explicitSideVariants(a).size ? 'outcome_metadata' : 'projection_odds_type' });
    }
  }
  return out;
}
...
export function attachPrizePicksSpecialRows(board, specials, ingestedAt = new Date().toISOString()) {
  ...
  for (const special of list(specials)) {
    if (special?.specialVerified !== true || special?.isAlternate !== true || !['goblin','demon'].includes(special?.specialType) || !['OVER','UNDER'].includes(special?.side)) continue;
  ...
  if (!attached.length) return board;
  return { ...board, props: [...regular, ...attached] };
}
