...
// SportsDataIO uses the same player-stat field names for the college feeds as
// for the pro ones, so one table serves both. Whether a given league actually
// returns game logs is a provider-entitlement question, answered at fetch time.
const BASKETBALL_MARKETS = Object.freeze({
    ...
    'blocks steals': ['BlockedShots', 'Steals'],
...
const FOOTBALL_MARKETS = Object.freeze({
    ...
    'reception yards': ['ReceivingYards'],
...
export const MARKETS = Object.freeze({
  ...
  CBB: BASKETBALL_MARKETS,
  NFL: FOOTBALL_MARKETS,
  NCAAF: FOOTBALL_MARKETS,
  CFB: FOOTBALL_MARKETS,
  MLB: {
    ...
    'hits runs rbis': ['Hits', 'Runs', 'RunsBattedIn'],
    'home runs': ['HomeRuns'], 'hits allowed': ['PitchingHits'],
    'pitching hits allowed': ['PitchingHits'],
...
/**
 * The odds provider's stable market keys.
 *
 * Display labels drift — "Receiving Yards" arrives as "Reception Yards", and
 * "Pass TDs" is not "Passing TDs" — which silently unmapped roughly half the
 * live board. These keys do not drift, so they are preferred over the label.
 *
 * A market with no honest game-log equivalent is deliberately absent:
 * quarter-scoped markets (player_points_q1) cannot be answered from full-game
 * logs, and kicking points would need weighted scoring the summing model here
 * cannot express. Both stay unmapped rather than answered wrongly.
 */
...
/** Canonical market for a provider key, or null when we have no mapping. */
 ...
 *
 * `providerKey` is the odds provider's stable market key and is tried first,
 * because display labels change without notice.
 */
