 ...
 *
 * Railway's edge sets X-Real-IP to the connecting client and documents it as
 * the client address; the domain is not behind another proxy. The first
 * X-Forwarded-For entry is whatever the client chose to send, so preferring it
 * let anyone rotate a fake header to escape every per-IP limit. It remains
 * only as a fallback for local runs without the edge.
 */
export function clientIp(req) {
  const real = String(req?.headers?.['x-real-ip'] || '').trim();
  if (real && real.length <= 64 && /^[0-9a-f.:]+$/i.test(real)) return real;
  const forwarded = String(req?.headers?.['x-forwarded-for'] || '').split(',')[0].trim();
  return (forwarded || req?.socket?.remoteAddress || '').slice(0, 64) || null;
}

export function clientKey(req, bucket) {
  return `${bucket}:${clientIp(req) || 'unknown'}`;
}
