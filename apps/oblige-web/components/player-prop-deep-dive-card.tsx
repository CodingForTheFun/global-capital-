...
function visibleBookName(name: string) {
  return sourceLikeBook(name) ? 'Verified line' : name;
}

function bookBadge(name: string) {
  return sourceLikeBook(name) ? 'LINE' : initials(name);
}
...
export function PlayerPropDeepDiveCard({
  ...
  const [sample, setSample] = React.useState<'l5' | 'l10' | 'l15' | 'season' | 'h2h'>('l15');
  ...
  const windowRows = React.useMemo(() => [
    ...
    computeWindow(played, targetLine, side, 'season', 'Season'),
  ...
  const chartRows = React.useMemo<ChartRow[]>(() => {
    ...
    const magnitude = Math.max(Math.abs(targetLine), ...observed.map(value => Math.abs(value)), 1);
    const dnpHeight = Math.max(magnitude * 0.08, 0.5);
  ...
  }, [chartSampleRows, targetLine]);
  const chartDomain = React.useMemo<[number, number]>(() => {
    const values = chartRows.map(row => finite(row.value)).filter((value): value is number => value !== null);
    const low = Math.min(0, targetLine, ...values);
    const high = Math.max(0, targetLine, ...values);
    const span = Math.max(high - low, 1);
    return [low < 0 ? low - span * 0.12 : 0, high + span * 0.12];
  }, [chartRows, targetLine]);
  ...
  const chartHeading = sample === 'l15' ? 'Last 15 Games' : sample === 'season' ? 'Season Games' : sample === 'h2h' ? 'Head-to-Head Games' : `Last ${sample.slice(1)} Games`;
  ...
  function choosePosted(value: string) {
    if (numericPosted && onLine) { onLine(Number(value)); return; }
  ...
  return <section className="research-reference min-h-screen w-full min-w-0 overflow-x-hidden bg-[#070b12] px-2 py-3 text-slate-100 sm:px-4 lg:px-6" data-design="premium-player-research-v1" data-deep-dive="player-prop-deep-dive-v1" data-research-route={routeKind}>
    ...
    <div className="mx-auto w-full min-w-0 max-w-[1180px] overflow-hidden rounded-2xl border border-slate-800/90 bg-[#0b101a] shadow-[0_30px_90px_rgba(0,0,0,0.45)]">
      ...
      <div className="space-y-2 p-2.5 sm:space-y-3 sm:p-4 lg:p-5">
        <div data-research-hero className="rounded-2xl border border-slate-800 bg-[radial-gradient(circle_at_15%_10%,rgba(59,130,246,0.13),transparent_30%),linear-gradient(180deg,rgba(19,27,46,0.9),rgba(10,15,24,0.96))] p-2.5 sm:p-4">
          <div className="flex items-start justify-between gap-3">
            <div className="flex min-w-0 items-center gap-3">
              <div className="relative grid h-14 w-14 shrink-0 place-items-center sm:h-[66px] sm:w-[66px] rounded-full bg-[linear-gradient(135deg,#38bdf8,#2563eb,#22c55e)] p-[2px] shadow-[0_0_28px_rgba(56,189,248,0.28)]"><div className="h-full w-full overflow-hidden rounded-full bg-[#101827]"><PlayerHeadshot sport={player.sport} name={player.name} providerPlayerId={player.playerId} team={team}/></div><span className="absolute -bottom-1 -right-1 rounded-full border border-slate-700 bg-[#111827] px-1.5 py-0.5 text-[9px] font-black text-sky-300">{sportName(player.sport)}</span></div>
              <div className="min-w-0"><div className="flex min-w-0 flex-wrap items-center gap-1.5"><h1 className="max-w-full truncate text-[17px] font-black leading-tight tracking-tight text-white sm:text-2xl">{player.name}</h1>{position && <span className="rounded-md border border-slate-700 bg-slate-900/90 px-1.5 py-0.5 text-[10px] font-black text-slate-400">{position}</span>}</div><p className="mt-0.5 text-[11px] font-semibold leading-4 text-slate-400 sm:mt-1 sm:text-xs"><span className="block truncate sm:inline">{game}</span><span className="block sm:ml-1 sm:inline"> {kickoffLabel(player.startsAt)}</span></p><div className="hidden sm:mt-2 sm:flex sm:flex-wrap sm:items-center sm:gap-2 sm:text-[10px] sm:text-slate-500"><span>{team || history?.player?.team || 'Team unavailable'}</span>{analysis.loading ? <><span>•</span><span>Loading history…</span></> : analysis.unavailable ? <><span>•</span><span className="text-amber-300">History unavailable</span></> : <><span>•</span><span className="text-emerald-400">Verified history</span></>}</div></div>
            </div>
            <button type="button" onClick={onFavourite} disabled={!canFollow} aria-label={favourite ? `Unfollow ${player.name}` : `Follow ${player.name}`} aria-pressed={favourite} className={`inline-flex h-9 w-9 shrink-0 items-center justify-center gap-2 rounded-lg border px-0 text-xs font-bold transition sm:w-auto sm:px-3 ${favourite ? 'border-amber-400/40 bg-amber-400/10 text-amber-300' : 'border-slate-700 bg-slate-900/80 text-slate-300 hover:border-slate-600 hover:text-white'}`}><Star className={`h-3.5 w-3.5 ${favourite ? 'fill-current' : ''}`}/><span className="hidden sm:inline">{favourite ? 'Watching' : 'Add to Watchlist'}</span></button>
          </div>
          <div className="mt-2 grid gap-1.5 rounded-xl border border-sky-500/20 bg-sky-500/[0.07] p-1.5 sm:mt-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center sm:gap-2 sm:p-2.5"><div className="flex min-w-0 items-center gap-2"><span className="shrink-0 rounded-md bg-sky-500 px-2 py-1 text-[10px] font-black tracking-wide text-white">{bookBadge(selected.bookName)}</span><div className="min-w-0"><div className="flex min-w-0 items-center gap-2"><h2 className="truncate text-xs font-bold text-white sm:text-sm">{title}</h2>{variantLabel && <span data-variant={selected.dfsOddsType} className="shrink-0 rounded-md border border-emerald-400/20 bg-emerald-400/10 px-1.5 py-0.5 text-[8px] font-black uppercase tracking-wide text-emerald-300">{variantLabel}</span>}</div><div className="hidden text-[10px] text-slate-500 sm:block">Main prop · {period}</div></div></div><div className="flex min-w-0 items-center justify-between gap-2 sm:justify-end"><label className="relative block min-w-0"><span className="sr-only">Selected book</span><select aria-label="Selected book" value={currentBook || ''} onChange={event => onBook(event.target.value)} className="h-7 max-w-[118px] appearance-none truncate rounded-lg border border-sky-400/20 bg-slate-950/60 pl-2 pr-5 text-[9px] font-bold text-slate-300 outline-none sm:h-8 sm:max-w-[190px] sm:pr-6 sm:text-[10px]">{allowBestPrices && <option value="">Best price</option>}{applicableBooks.map(book => <option key={book.key} value={book.key}>{visibleBookName(book.name)}</option>)}</select><ChevronDown className="pointer-events-none absolute right-1.5 top-2 h-3 w-3 text-slate-500"/></label><div className="flex shrink-0 items-center gap-1.5 whitespace-nowrap text-sm font-black tabular-nums"><span className={selected.side === 'UNDER' ? 'text-rose-400' : 'text-emerald-400'}>{selected.side === 'OVER' ? 'O' : selected.side === 'UNDER' ? 'U' : selected.choice} {selected.line ?? ''}</span><span className="text-slate-400">{offerPrice(selected)}</span></div></div></div>
        </div>
        ...
        <div className="op-sample-caption flex min-h-5 items-center justify-between gap-3 text-[10px] text-slate-500"><span className="op-sample-count">{analysis.unavailable ? 'History unavailable' : `${played.length} of ${verifiedTotal} verified games`}</span></div>
        <div className="grid gap-3 lg:grid-cols-[300px_1fr]">
          <div className="rounded-2xl border border-slate-800 bg-[#0d1420] p-2.5"><div className="flex items-center justify-between gap-2"><div className="text-[10px] font-bold uppercase tracking-[0.16em] text-slate-500">Target line</div><select aria-label="Posted line or outcome" value={numericPosted ? String(selected.line ?? postedLines[0] ?? '') : selected.key} onChange={event => choosePosted(event.target.value)} className="h-7 max-w-[130px] rounded-lg border border-slate-800 bg-slate-950/60 px-2 text-[10px] font-bold text-slate-400 outline-none">{numericPosted ? postedLines.map(line => <option key={line} value={line}>{line}</option>) : activePostedOffers.map(offer => <option key={offer.key} value={offer.key}>{offer.choice}</option>)}</select></div><div className="mt-1.5 grid grid-cols-[40px_1fr_40px] overflow-hidden rounded-xl border border-slate-700 bg-slate-950/70"><button type="button" onClick={() => { const next=round1(targetLine-0.5); setTargetLine(next); analysis.onLineChange?.(next); }} className="grid h-10 place-items-center border-r border-slate-800 text-slate-400 transition hover:bg-slate-900 hover:text-white" aria-label="Lower research line"><Minus className="h-4 w-4"/></button><output className="op-line-number grid h-10 place-items-center text-lg font-black tabular-nums text-white" aria-live="polite">{Number.isInteger(targetLine) ? targetLine : targetLine.toFixed(1)}</output><button type="button" onClick={() => { const next=round1(targetLine+0.5); setTargetLine(next); analysis.onLineChange?.(next); }} className="grid h-10 place-items-center border-l border-slate-800 text-slate-400 transition hover:bg-slate-900 hover:text-white" aria-label="Raise research line"><Plus className="h-4 w-4"/></button></div><div className="mt-1.5 flex items-center justify-between gap-2 rounded-lg bg-slate-950/60 px-2.5 py-1.5 text-[10px]"><span className="truncate font-bold text-slate-300">{visibleBookName(selected.bookName)}</span><div className="flex gap-2 font-black tabular-nums"><span className="text-emerald-400">O {offerPrice(selectedPair.over)}</span><span className="text-rose-400">U {offerPrice(selectedPair.under)}</span></div></div></div>
          {!analysis.loading && !analysis.unavailable && <div className="grid grid-cols-5 overflow-hidden rounded-2xl border border-slate-800 bg-[#0d1420]">{windowRows.map((item, index) => <button type="button" key={item.id} className={`op-sample min-w-0 px-1.5 py-2.5 text-center transition hover:bg-slate-900/50 sm:px-3 ${sample === item.id ? 'bg-sky-500/[0.06]' : ''} ${index !== windowRows.length - 1 ? 'border-r border-slate-800' : ''}`} aria-pressed={sample === item.id} onClick={() => setSample(item.id as typeof sample)}><div className="truncate text-[9px] font-bold uppercase tracking-wide text-slate-500 sm:text-[10px]">{item.label}{item.id === 'season' ? '' : ''}</div>{item.id === 'season' && <div className="mt-0.5 truncate text-[8px] text-slate-600 sm:text-[9px]">{text(history?.season) || 'Available'}</div>}{item.id === 'h2h' && <div className="mt-0.5 truncate text-[8px] text-slate-600 sm:text-[9px]">{currentOpponent ? `vs ${currentOpponent}` : 'No opponent'}</div>}<div className={`mt-1 text-base font-black tabular-nums sm:text-xl ${item.hitRate !== null && item.hitRate >= 60 ? 'text-emerald-400' : 'text-slate-200'}`}>{item.hitRate === null ? '—' : `${item.hitRate}%`}</div><div className="mt-1 truncate text-[8px] text-slate-500 sm:text-[10px]">Avg {item.average === null ? '—' : item.average}</div></button>)}</div>}
        </div>

        <div className="op-chart-section min-w-0 overflow-hidden rounded-2xl border border-slate-800 bg-[#0d1420] p-3 sm:p-4" id="analysis-chart">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex items-center gap-2"><Flame className="h-4 w-4 text-orange-400"/><h2 className="text-sm font-black text-white sm:text-base">{chartHeading} – {title}</h2></div><p className="mt-1 text-[10px] text-slate-500">Current target: {targetLine.toFixed(1)} · {period}</p></div><div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[9px] font-semibold text-slate-400 sm:text-[10px]"><span className="inline-flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-[#22c55e]"/>Over</span><span className="inline-flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-[#f43f5e]"/>Under</span><span className="inline-flex items-center gap-1.5"><span className="h-2 w-2 rounded-sm border border-slate-500 bg-slate-800"/>DNP</span><span className="inline-flex items-center gap-1.5"><span className="w-3 border-t border-dashed border-amber-400"/>Prop Line</span></div></div>
          {analysis.loading ? <div className="grid h-[310px] place-items-center text-sm text-slate-500">Loading verified game history…</div> : analysis.unavailable ? <div className="op-no-history flex min-h-[310px] flex-col items-center justify-center px-6 text-center text-sm text-slate-500"><div>{analysis.unavailable}</div>{historyRetry && <div className="mt-3">{historyRetry}</div>}</div> : chartRows.length === 0 ? <div className="grid h-[310px] place-items-center text-sm text-slate-500">No verified games match these filters.</div> : <div className="mt-4 h-[310px] w-full sm:h-[340px]"><ResponsiveContainer width="100%" height="100%"><BarChart data={chartRows} margin={{top:26,right:14,left:-18,bottom:26}} barCategoryGap="18%"><defs><pattern id={`dnpHatch-${patternId}`} width="6" height="6" patternUnits="userSpaceOnUse" patternTransform="rotate(45)"><rect width="6" height="6" fill="#111827"/><line x1="0" y1="0" x2="0" y2="6" stroke="#64748b" strokeWidth="2"/></pattern></defs><CartesianGrid stroke="#1e293b" vertical={false} strokeDasharray="3 3"/><XAxis dataKey="chartKey" tickLine={false} axisLine={{stroke:'#263244'}} interval={0} height={46} tick={({x,y,index}) => { const row=chartRows[index]; return <g transform={`translate(${x},${y})`}><text x={0} y={13} textAnchor="middle" fill="#94a3b8" fontSize="9" fontWeight="700">{row?.displayDate}</text><text x={0} y={26} textAnchor="middle" fill="#475569" fontSize="8">{row?.isHome === false ? '@' : 'vs'}{text(row?.opponent)}</text></g>; }}/><YAxis domain={chartDomain} tickLine={false} axisLine={false} tick={{fill:'#64748b',fontSize:9}}/><Tooltip cursor={{fill:'rgba(30,41,59,0.22)'}} content={<ChartTooltip line={targetLine} marketLabel={title}/>}/><ReferenceLine y={targetLine} stroke="#fbbf24" strokeWidth={1.5} strokeDasharray="6 5" label={{value:targetLine.toFixed(1),position:'insideTopRight',fill:'#fbbf24',fontSize:10,fontWeight:800}}/><Bar dataKey="chartValue" radius={[5,5,2,2]} maxBarSize={30} isAnimationActive={false}>{chartRows.map(row => { const value=finite(row.value); const dnp=row.dnp===true||row.didNotPlay===true||value===null; const fill=dnp?`url(#dnpHatch-${patternId})`:value>targetLine?'#22c55e':value<targetLine?'#f43f5e':'#94a3b8'; return <Cell key={row.chartKey} fill={fill}/>; })}<LabelList dataKey="chartValue" content={renderBarLabel}/></Bar></BarChart></ResponsiveContainer></div>}
        </div>
        ...
        <div className="rounded-2xl border border-slate-800 bg-[#0d1420] p-3 sm:p-4"><div className="mb-2 text-[10px] font-black uppercase tracking-[0.16em] text-slate-500">Model choice</div><div>{model}</div></div>

        <div className="rounded-2xl border border-slate-800 bg-[#0d1420] p-3 sm:p-4"><div className="mb-3 flex items-center justify-between gap-3"><div><h3 className="text-sm font-black text-white">Sportsbook Prices</h3><p className="mt-0.5 text-[10px] text-slate-500">Exact posted quotes at {targetLine.toFixed(1)} {title.toLowerCase()}</p></div><span className="rounded-full border border-emerald-400/20 bg-emerald-400/10 px-2 py-1 text-[9px] font-black text-emerald-300">{booksShown.length} BOOK{booksShown.length === 1 ? '' : 'S'}</span></div><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-5">{booksShown.map(book => { const base=chooseOffer(market,book.key,selected.line ?? targetLine,side); const bookLine=base?.line ?? targetLine; const pair=pricePair(market,book.key,bookLine); const active=book.key===selected.book; return <div key={book.key} className={`rounded-xl border p-2.5 transition ${active?'border-sky-500/40 bg-sky-500/[0.06]':'border-slate-800 bg-slate-950/40 hover:border-slate-700'}`}><button type="button" disabled={!base} aria-label={`Select ${visibleBookName(book.name)}`} onClick={() => base && onOffer(base)} className="flex w-full items-center justify-between gap-2 text-left disabled:cursor-default"><span className="truncate text-[11px] font-black text-white">{visibleBookName(book.name)}</span><span className="rounded bg-slate-800 px-1.5 py-0.5 text-[8px] font-black text-slate-400">{bookBadge(book.name)}</span></button><div className="mt-1 text-[8px] font-bold text-slate-600">{base?.line === null || base?.line === undefined ? 'Outcome' : `Line ${base.line}`}</div><div className="mt-2 grid grid-cols-2 gap-1.5"><button type="button" disabled={!pair.over} onClick={() => pair.over && onOffer(pair.over)} className="rounded-lg border border-emerald-400/15 bg-emerald-400/[0.06] px-2 py-2 text-left disabled:opacity-45"><div className="text-[8px] font-bold uppercase text-emerald-500/80">Over</div><div className="mt-0.5 text-sm font-black tabular-nums text-emerald-400">{offerPrice(pair.over)}</div></button><button type="button" disabled={!pair.under} onClick={() => pair.under && onOffer(pair.under)} className="rounded-lg border border-rose-400/15 bg-rose-400/[0.06] px-2 py-2 text-left disabled:opacity-45"><div className="text-[8px] font-bold uppercase text-rose-500/80">Under</div><div className="mt-0.5 text-sm font-black tabular-nums text-rose-400">{offerPrice(pair.under)}</div></button></div>{!base && <div className="mt-2 text-[9px] text-slate-600">No quote available</div>}</div>; })}</div></div>

        {quoteHistory && <div className="rounded-2xl border border-slate-800 bg-[#0d1420] p-3 sm:p-4">{quoteHistory}</div>}
        ...
        {showFullResearch && <div className="space-y-4 rounded-2xl border border-slate-800 bg-[#0d1420] p-3 sm:p-4" id="full-research"><div>{research}</div>{supporting && <div>{supporting}</div>}{analysisSections}{gameLog}</div>}
      </div>
    </div>
  </section>;
}