...
function SelectFilter({ label, ariaLabel, value, options, onChange }: { label: string; ariaLabel?: string; value: string; options: FilterOption[]; onChange(value: string): void }) {
  return <label className="relative flex h-9 shrink-0 items-center gap-1 rounded-lg border border-slate-700/80 bg-slate-900/80 px-2.5 text-[11px] text-slate-300 transition hover:border-slate-600">
    ...
    <select aria-label={ariaLabel || label} value={value} onChange={event => onChange(event.target.value)} className="appearance-none bg-transparent pr-4 font-medium text-slate-200 outline-none">
    ...
    <div className="mx-auto max-w-[1180px] overflow-hidden rounded-2xl border border-slate-800/90 bg-[#0b101a] shadow-[0_30px_90px_rgba(0,0,0,0.45)]">
      ...
      <div className="space-y-4 p-3 sm:p-4 lg:p-5">
        ...
        <div className="grid gap-3 lg:grid-cols-[300px_1fr]">
          <div className="rounded-2xl border border-slate-800 bg-[#0d1420] p-3"><div className="text-[10px] font-bold uppercase tracking-[0.16em] text-slate-500">Target line</div><div className="mt-2 grid grid-cols-[44px_1fr_44px] overflow-hidden rounded-xl border border-slate-700 bg-slate-950/70"><button type="button" onClick={() => setTargetLine(value => round1(value - 0.5))} className="grid h-12 place-items-center border-r border-slate-800 text-slate-400 transition hover:bg-slate-900 hover:text-white" aria-label="Lower research line"><Minus className="h-4 w-4"/></button><output className="op-line-number grid h-12 place-items-center text-xl font-black tabular-nums text-white" aria-live="polite">{Number.isInteger(targetLine) ? targetLine : targetLine.toFixed(1)}</output><button type="button" onClick={() => setTargetLine(value => round1(value + 0.5))} className="grid h-12 place-items-center border-l border-slate-800 text-slate-400 transition hover:bg-slate-900 hover:text-white" aria-label="Raise research line"><Plus className="h-4 w-4"/></button></div><div className="mt-2 flex items-center justify-between gap-2 rounded-lg bg-slate-950/60 px-2.5 py-2 text-[10px]"><span className="font-bold text-slate-300">{selected.bookName}</span><div className="flex gap-2 font-black tabular-nums"><span className="text-emerald-400">O {offerPrice(selectedPair.over)}</span><span className="text-rose-400">U {offerPrice(selectedPair.under)}</span></div></div><select aria-label="Posted line or outcome" value={numericPosted ? String(selected.line ?? postedLines[0] ?? '') : selected.key} onChange={event => choosePosted(event.target.value)} className="mt-2 h-8 w-full rounded-lg border border-slate-800 bg-slate-950/60 px-2 text-[10px] font-bold text-slate-400 outline-none">{numericPosted ? postedLines.map(line => <option key={line} value={line}>{line}</option>) : activePostedOffers.map(offer => <option key={offer.key} value={offer.key}>{offer.choice}</option>)}</select></div>
          {!analysis.loading && !analysis.unavailable && <div className="grid grid-cols-5 overflow-hidden rounded-2xl border border-slate-800 bg-[#0d1420]">{windowRows.map((item, index) => <button type="button" key={item.id} className={`op-sample min-w-0 px-1.5 py-3 text-center transition hover:bg-slate-900/50 sm:px-3 ${sample === item.id ? 'bg-sky-500/[0.06]' : ''} ${index !== windowRows.length - 1 ? 'border-r border-slate-800' : ''}`} aria-pressed={sample === item.id} onClick={() => setSample(item.id as typeof sample)}><div className="truncate text-[9px] font-bold uppercase tracking-wide text-slate-500 sm:text-[10px]">{item.label}{item.id === 'season' ? '' : ''}</div>{item.id === 'season' && <div className="mt-0.5 truncate text-[8px] text-slate-600 sm:text-[9px]">{text(history?.season) || 'Available'}</div>}{item.id === 'h2h' && <div className="mt-0.5 truncate text-[8px] text-slate-600 sm:text-[9px]">{currentOpponent ? `vs ${currentOpponent}` : 'No opponent'}</div>}<div className={`mt-1 text-base font-black tabular-nums sm:text-xl ${item.hitRate !== null && item.hitRate >= 60 ? 'text-emerald-400' : 'text-slate-200'}`}>{item.hitRate === null ? '—' : `${item.hitRate}%`}</div><div className="mt-1 truncate text-[8px] text-slate-500 sm:text-[10px]">Avg {item.average === null ? '—' : item.average}</div></button>)}</div>}
        </div>
