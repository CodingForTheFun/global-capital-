import { headTags, siteOrigin, robotsTxt, sitemapXml } from './public-surface.mjs';
...
export function seoRobotsTxt(origin = siteOrigin()) {
  const base = robotsTxt(origin);
  const allows = SEO_PATHS.map(path => `Allow: ${path}`).join('\n');
  return base.replace('Allow: /pricing', `Allow: /pricing\n${allows}`);
}

export function seoSitemapXml(origin = siteOrigin(), now = new Date()) {
  const base = sitemapXml(origin, now);
  const day = now.toISOString().slice(0, 10);
  const extra = SEO_PATHS.map(path => `  <url><loc>${origin}${path}</loc><lastmod>${day}</lastmod><priority>0.8</priority></url>`).join('\n');
  return base.replace('</urlset>', `${extra}\n</urlset>`);
}

export function serveSeoPage(req, res, { origin = siteOrigin() } = {}) {
  if (req.method !== 'GET' && req.method !== 'HEAD') return false;
  const pathname = new URL(req.url || '/', 'http://localhost').pathname.replace(/\/+$/, '') || '/';
  let type = 'text/html; charset=utf-8';
  let body = null;
  if (SEO_PATHS.includes(pathname)) body = seoPage(pathname, { origin });
  else if (pathname === '/robots.txt') { type = 'text/plain; charset=utf-8'; body = seoRobotsTxt(origin); }
  else if (pathname === '/sitemap.xml') { type = 'application/xml; charset=utf-8'; body = seoSitemapXml(origin); }
  else return false;
