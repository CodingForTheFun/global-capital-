...
export function createMLStore({file = path.resolve(process.env.DATA_DIR || './data','ml/predictions.json'), clock = Date.now, pollMs = 5000,
  research = null, global = null, adaptiveBudgetMs = 1500, pendingBudgetMs = 2500} = {}) {
  let cached = null, checked = -Infinity, inflight = null;
  // One adaptive run per exact prop (sport, event, player, market, book,
  // start and line: the full target key), shared by concurrent and repeated
  // lookups. A cold history can take seconds; the board is answered
  // "pending" and asks again, and the retry reads this run's result. A
  // failed run is dropped at once so the next ask starts fresh.
  const adaptiveRuns = new Map();
  const ADAPTIVE_TTL_MS = 5 * 60_000, ADAPTIVE_MAX = 2000;
  function adaptiveFor(t) {
    const key = targetKey(t), now = clock();
    const hit = adaptiveRuns.get(key);
    if (hit && now - hit.at < ADAPTIVE_TTL_MS) return hit.promise;
    const promise = adaptive(t);
    adaptiveRuns.set(key, {at: now, promise});
    promise.then(result => {
      if (!result || result.code === 'MODEL_FEED_UNAVAILABLE') adaptiveRuns.delete(key);
    }, () => adaptiveRuns.delete(key));
    while (adaptiveRuns.size > ADAPTIVE_MAX) adaptiveRuns.delete(adaptiveRuns.keys().next().value);
    return promise;
  }
  const PENDING = Symbol('pending');
  const withinBudget = (task, ms) => {
    let timer;
    return Promise.race([task, new Promise(resolve => { timer = setTimeout(() => resolve(PENDING), ms); timer.unref?.(); })])
  ...
  async function lookup(input) {
    ...
    const adaptiveTask = adaptiveFor(t);
    if (global) {
      let quick = null;
    ...
    // Nothing quick to offer: wait a short budget for the verified history,
    // then say it is still loading rather than "no model". The run carries
    // on and the board's retry picks up its answer.
    const learned = typeof research === 'function' ? await withinBudget(adaptiveTask, pendingBudgetMs) : await adaptiveTask;
    if (learned === PENDING) {
      return {available:false,modelled:true,engine:'Auto Scout Adaptive',code:'MODEL_PENDING',retryable:true,retryAfterMs:3000,
        message:'The model is still loading this player\'s verified history. It will be ready in a moment.'};
    }
