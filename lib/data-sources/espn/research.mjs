...
export function resolveHistoryEventIdentity(gameLog, params = {}, {maxDriftMs=48*60*60_000} = {}) {
  const sport=canonicalSport(params.sport),start=Date.parse(params.gameStartTime||''),home=String(params.homeTeam||'').trim(),away=String(params.awayTeam||'').trim();
  if(!PUBLIC_LEAGUES[sport]||!Number.isFinite(start)||!home||!away)return {ok:true,available:false,code:'MATCHUP_IDENTITY_UNVERIFIED'};
  const equivalent=(a,b)=>sport==='SOCCER'?sameClub(a,b):sameTeam(a,b,sport);
  const candidates=[];let incompleteNear=false;
  for(const row of Array.isArray(gameLog)?gameLog:[]) {
    const when=Date.parse(row?.date||''),gameId=String(row?.gameId||'').trim();
    if(!Number.isFinite(when)||Math.abs(when-start)>maxDriftMs)continue;
    const teams=[row?.teamName,row?.team].filter(Boolean),opponents=[row?.opponentName,row?.opponent].filter(Boolean);
    if(!teams.length||!opponents.length||!gameId){incompleteNear=true;continue;}
    const forward=teams.some(v=>equivalent(v,home))&&opponents.some(v=>equivalent(v,away));
    const reverse=teams.some(v=>equivalent(v,away))&&opponents.some(v=>equivalent(v,home));
    if(forward||reverse)candidates.push({gameId,sourceEventId:gameId.includes(':')?gameId.split(':').at(-1):null,driftMs:Math.abs(when-start),when});
  }
  candidates.sort((a,b)=>a.driftMs-b.driftMs||a.when-b.when||a.gameId.localeCompare(b.gameId));
  // Team/matchup identity alone cannot distinguish doubleheaders, daily series,
  // or duplicate same-club fixtures. Never guess among two plausible source games.
  if(incompleteNear||candidates.length>1)return {ok:true,available:false,code:'MATCHUP_IDENTITY_UNVERIFIED'};
  if(!candidates.length)return {ok:true,available:true,source:'ESPN',gameId:null,sourceEventId:null,matched:false};
  return {ok:true,available:true,source:'ESPN',...candidates[0],matched:true};
}
...
export function createPublicResearch({ fetchImpl = (...args) => fetch(...args), now = () => Date.now() } = {}) {
  ...
  fetchResearch.eventIdentity = params => resolveHistoryEventIdentity(params?.gameLog,{...params,sport:canonicalSport(params?.sport)});
