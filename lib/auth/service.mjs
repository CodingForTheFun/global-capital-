...
import { consumePendingAccountNotice } from './account-notices.mjs';
...
export async function register({ email, password }, { log = console, fetchImpl } = {}) {
  ...
  // Beta mode: no mail provider exists, so there is no code to wait for. The
  // account is created usable and the caller is told to sign it in. See
  // beta.mjs for what this trades away.
  if (betaOpenSignup()) {
    if (existing) {
      return existing.disabled
        ? { ok: false, code: 'AUTH_REGISTER_FAILED', message: 'That account is not available.' }
        : { ok: false, code: 'AUTH_EMAIL_TAKEN', message: 'That email already has an account. Sign in instead.' };
    }
    const betaRole = roleForNewAccount(normalized, await countUsers());
    const betaHash = await hashPassword(password);
    const created = await createUser({ email: normalized, passwordHash: betaHash, role: betaRole, emailVerified: true });
    const activated = await updateUser(created.user.id, () => ({ lastLoginAt: new Date().toISOString() }));
    const user = activated ?? created.user;
    return {
      ok: true,
      ...
      // The route reads this and issues a session cookie immediately.
      signedIn: true,
    };
  }
...
export async function login({ email, password, ip = null }, { log = console } = {}) {
  ...
  if (user.disabled) {
    // Do not reveal the disabled account unless the person proves they know its
    // password. A valid blocked sign-in may consume exactly one owner notice;
    // no session or product access is issued.
    const blockedPassword = await verifyPassword(password, user.passwordHash);
    if (!blockedPassword.ok) {
      return { ok: false, code: 'AUTH_INVALID_CREDENTIALS', message: GENERIC_CREDENTIALS };
    }
    const notice = await consumePendingAccountNotice(user.id, { types: ['account-disabled'] });
    return notice
      ? { ok: false, code: 'AUTH_ACCOUNT_DISABLED_NOTICE', message: notice.message }
      : { ok: false, code: 'AUTH_ACCOUNT_DISABLED', message: 'This account is currently disabled.' };
  }
  if (isLocked(user)) {
...
const BOARD_PREFERENCE_SPORTS = new Set(['NFL', 'NBA', 'MLB', 'NHL', 'NCAAF', 'NCAAB', 'WNBA', 'SOCCER', 'TENNIS']);
const BOARD_PREFERENCE_SORTS = new Set(['hit', 'line', 'name']);
const BOARD_PREFERENCE_WINDOWS = new Set(['l5', 'l10', 'l15', 'h2h', 'streak']);
const BOARD_PREFERENCE_FILTERS = ['market', 'team', 'opponent', 'book', 'game', 'date', 'modifier'];
const BOARD_PREFERENCE_VALUE_MAX = 160;

function cleanBoardPreferenceValue(value) {
  const raw = String(value ?? '').trim();
  if (!raw || raw.length > BOARD_PREFERENCE_VALUE_MAX || /[\u0000-\u001f\u007f]/.test(raw)) return 'ALL';
  return raw;
}

function cleanBoardFilterSet(value) {
  const source = value && typeof value === 'object' ? value : {};
  const safe = {
    sort: BOARD_PREFERENCE_SORTS.has(source.sort) ? source.sort : 'line',
    hitWindow: BOARD_PREFERENCE_WINDOWS.has(source.hitWindow) ? source.hitWindow : 'l10',
  };
  for (const key of BOARD_PREFERENCE_FILTERS) safe[key] = cleanBoardPreferenceValue(source[key]);
  return safe;
}

/**
 * Account-scoped board preferences. They intentionally contain only product
 * filter labels and never account secrets, provider identifiers, or prop data.
 * Existing accounts with no saved view receive an empty by-sport map.
 */
export function publicPreferences(user) {
  const stored = user?.preferences?.boardFilters;
  const activeSport = BOARD_PREFERENCE_SPORTS.has(stored?.activeSport) ? stored.activeSport : 'NFL';
  const bySport = {};
  const source = stored?.bySport && typeof stored.bySport === 'object' ? stored.bySport : {};
  for (const [sport, filters] of Object.entries(source)) {
    if (!BOARD_PREFERENCE_SPORTS.has(sport)) continue;
    bySport[sport] = cleanBoardFilterSet(filters);
  }
  return { boardFilters: { activeSport, bySport } };
}

/**
 * Persist one explicit "Save view" action. This is deliberately not called on
 * every filter click: account storage lives on the persistent volume, so user
 * intent — not UI churn — is what creates a write.
 */
export async function saveBoardPreferences({ userId, sport, filters }) {
  const normalizedSport = String(sport || '').trim().toUpperCase();
  if (!BOARD_PREFERENCE_SPORTS.has(normalizedSport)) {
    return { ok: false, code: 'PREFERENCES_INVALID', message: 'That sport cannot be saved.' };
  }
  const safeFilters = cleanBoardFilterSet(filters);
  const updated = await updateUser(userId, (current) => {
    const existing = publicPreferences(current).boardFilters;
    return {
      preferences: {
        ...(current.preferences && typeof current.preferences === 'object' ? current.preferences : {}),
        boardFilters: {
          activeSport: normalizedSport,
          bySport: { ...existing.bySport, [normalizedSport]: safeFilters },
        },
      },
    };
  });
  if (!updated) return { ok: false, code: 'AUTH_REQUIRED', message: 'Sign in again.' };
  return { ok: true, preferences: publicPreferences(updated) };
}
