...
export function playerCardKey(group) {
 // Book/DFS feeds often assign different internal IDs to the same athlete. For
 // presentation, group the same normalized player name into one card so provider
 // IDs do not create duplicate cards. Exact event/market rows remain separate
 // choices and research still runs against the selected exact prop.
 const name=norm(group.playerName);
 if(name)return [String(group.sport||'').toUpperCase(),group.entityType||'player','name:'+name].join('|');
 return [String(group.sport||'').toUpperCase(),group.entityType||'player','id:'+String(group.playerId||group.providerPlayerId||'unknown')].join('|');
}
