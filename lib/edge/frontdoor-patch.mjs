...
export function patchEdgeFrontdoor(source) {
  ...
  const retiredRedirectHelper = [
    "const RETIRED_CUSTOMER_HOSTS = new Set(['obligepay.com', 'www.obligepay.com', 'obligeprops.com']);",
    "function normalizeCustomerHost(value) {",
    "  return String(value || '').toLowerCase().split(':')[0].trim().replace(/\\.$/, '');",
    "}",
    "function requestCustomerHost(req) {",
    "  const forwarded = normalizeCustomerHost(String(req.headers['x-forwarded-host'] || '').split(',')[0]);",
    "  const direct = normalizeCustomerHost(req.headers.host);",
    "  if (RETIRED_CUSTOMER_HOSTS.has(forwarded)) return forwarded;",
    "  if (RETIRED_CUSTOMER_HOSTS.has(direct)) return direct;",
    "  return forwarded || direct;",
    "}",
    "function maybeRedirectRetiredNavigation(req, res) {",
    "  const host = requestCustomerHost(req);",
    "  if (!RETIRED_CUSTOMER_HOSTS.has(host)) return false;",
    "  const rawUrl = String(req.url || '/');",
    "  if (rawUrl.includes('\\r') || rawUrl.includes('\\n')) return false;",
    "  let pathname = '/';",
    "  try { pathname = new URL(rawUrl, 'http://localhost').pathname; } catch { return false; }",
    "  if (pathname === '/api' || pathname.startsWith('/api/')) return false;",
    "  let canonicalHost = '';",
    "  try { canonicalHost = new URL(SITE_ORIGIN).host.toLowerCase(); } catch { return false; }",
    "  if (!canonicalHost || canonicalHost === host) return false;",
    "  const pathAndQuery = rawUrl.startsWith('/') ? rawUrl : '/' + rawUrl.replace(/^\\/+/, '');",
    "  const status = req.method === 'GET' || req.method === 'HEAD' ? 301 : 308;",
    "  res.writeHead(status, {",
    "    location: SITE_ORIGIN + pathAndQuery,",
