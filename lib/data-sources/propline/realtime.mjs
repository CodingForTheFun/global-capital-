...
function persistState() {
  try {
    ...
    console.log(`[PropLine realtime] state persist failed code=${text(error?.code || 'WRITE_FAILED').slice(0, 60)}`);
  }
}
...
async function api(pathname, { method = 'GET', body = null, timeoutMs = 15_000 } = {}) {
  ...
  if (!response.ok) {
    const detail = text(payload?.detail?.error || payload?.error || payload?.code || `HTTP_${response.status}`);
    throw Object.assign(new Error('PropLine webhook management request failed.'), {
      code: `PROPLINE_WEBHOOK_${detail || response.status}`,
...
function createBody() {
  return {
    ...
    filter_market_key: playerMarkets.join(','),
...
export async function ensureProplineRealtimeSubscription() {
  ...
  try {
    ...
    if (!current || !hasSecret()) {
      ...
      const created = await api('/v1/webhooks', { method: 'POST', body: createBody() });
      ...
      if (needsPatch) {
        current = await api(`/v1/webhooks/${encodeURIComponent(current.id)}`, {
          method: 'PATCH',
          body: { ...createBody(), active: true },
        });
      }
      state.subscription = {
        ...state.subscription,
