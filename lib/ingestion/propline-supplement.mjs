...
function prioritizeProplineQuote(row, source) {
  ...
  const merged = {
    ...
    isAlternate: false,
    // Keep ObligeProps' canonical event/player joins stable. Native provider
    // identity remains attached through providerPlayerId/propline* metadata.
...
export function mergeCachedPropline(board, sport, { now = Date.now } = {}) {
  ...
  // Rebuilt rows come last so the exact canonical customer rows win any id
  // collision with provider-native normalized rows.
...
export function proplineSupplementHealth() {
  ...
  return {
    ...
    role: 'primary-current-quote',
    secondaryProvider: 'sportsgameodds',
