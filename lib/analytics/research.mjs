...
export function analyzeResearch(base = {}, line = base.line, side = base.side, venue = 'all') {
  ...
  const seasonRows = rows.filter(row => row.season == null || String(row.season) === String(base.season));
  if (complete) analytics.windows.season = rollingAnalytics(seasonRows, threshold, direction).windows.season;
  ...
  return {
    ...
    context: { ...(base.context || {}), ...(complete ? {
      seasonAverage: analytics.windows.season.average,
      averageMinutes: seasonRows.length && seasonRows.every(row => num(row.minutes) !== null)
 ...
 *
 * The season average is the intended basis, but it is null whenever season
 * coverage is incomplete. Rather than report nothing, fall back to the widest
 * window that actually has games and say which one was used, so the card can
 * label the number instead of implying a full season behind it.
 */
