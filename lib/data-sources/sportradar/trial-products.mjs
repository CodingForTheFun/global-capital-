...
const PROBE_STATE_DIR = path.join(text(process.env.DATA_DIR || '/app/data'), 'autoscout');
const SPORTRADAR_TRIAL_STATE_FILE = path.join(PROBE_STATE_DIR, 'sportradar-trial-products-v1.json');
const SPORTRADAR_TRIAL_LOCK_FILE = path.join(PROBE_STATE_DIR, 'sportradar-trial-products-v1.lock');
const PROBE_STATE_MAX_AGE_MS = 6 * 60 * 60_000;
const PROBE_LOCK_STALE_MS = 3 * 60_000;
...
function loadPersistedProbeState() {
  try {
    const payload = JSON.parse(fs.readFileSync(SPORTRADAR_TRIAL_STATE_FILE, 'utf8'));
    const at = Date.parse(payload?.testedAt || '');
    if (!Number.isFinite(at) || Date.now() - at > PROBE_STATE_MAX_AGE_MS) return false;
    for (const row of Array.isArray(payload?.products) ? payload.products : []) {
      if (!row?.productId) continue;
      state.set(row.productId, {
        available: row.available === true,
        status: Number(row.status) || null,
        code: row.code || null,
        testedAt: row.testedAt || payload.testedAt,
        latencyMs: Number(row.latencyMs) || null,
        sampleCount: Number.isFinite(Number(row.sampleCount)) ? Number(row.sampleCount) : null,
      });
    }
    return state.size > 0;
  } catch {
...
function persistProbeState(rows) {
  try {
    fs.mkdirSync(PROBE_STATE_DIR, { recursive: true });
    ...
    const payload = {
      ...
      products: rows.map((row) => ({
        ...
        code: row.code ?? null,
        testedAt: row.testedAt ?? testedAt,
        latencyMs: row.latencyMs ?? null,
        sampleCount: row.sampleCount ?? null,
      })),
    };
    const temp = SPORTRADAR_TRIAL_STATE_FILE + '.tmp';
    fs.writeFileSync(temp, JSON.stringify(payload));
    fs.renameSync(temp, SPORTRADAR_TRIAL_STATE_FILE);
  } catch {}
}
...
function acquireProbeLock() {
  try {
    fs.mkdirSync(PROBE_STATE_DIR, { recursive: true });
    return fs.openSync(SPORTRADAR_TRIAL_LOCK_FILE, 'wx');
  } catch (error) {
    if (error?.code !== 'EEXIST') return null;
    try {
      const stat = fs.statSync(SPORTRADAR_TRIAL_LOCK_FILE);
      if (Date.now() - Number(stat.mtimeMs) > PROBE_LOCK_STALE_MS) {
        fs.unlinkSync(SPORTRADAR_TRIAL_LOCK_FILE);
        return fs.openSync(SPORTRADAR_TRIAL_LOCK_FILE, 'wx');
      }
    } catch {}
    return null;
...
function releaseProbeLock(fd) {
  try { if (fd !== null && fd !== undefined) fs.closeSync(fd); } catch {}
  try { fs.unlinkSync(SPORTRADAR_TRIAL_LOCK_FILE); } catch {}
}
...
async function scheduleProductRequest(productId, task) {
  ...
  const tracked = run.finally(() => {
    if (queues.get(productId) === tracked) queues.delete(productId);
  });
  queues.set(productId, tracked);
  return run;
}
...
export function sportradarTrialProductAvailable(productId) {
  if (!state.size) loadPersistedProbeState();
...
export async function probeAllSportradarTrials({ delayMs = 1250 } = {}) {
  if (probePromise) return probePromise;
  probePromise = (async () => {
    const lock = acquireProbeLock();
    if (lock === null) {
      // Another ObligeProps process owns the shared trial audit. Give it time
      // to finish, then consume the safe persisted entitlement state.
      await sleep(35_000);
      loadPersistedProbeState();
      return Object.keys(SPORTRADAR_TRIAL_PRODUCTS).map(sportradarTrialProductState);
    }
    ...
    try {
      ...
      for (let index = 0; index < ids.length; index += 1) {
        ...
        let row = await probeSportradarTrialProduct(productId);
        if (row.status === 429) {
          await sleep(5_000);
          row = await probeSportradarTrialProduct(productId);
        }
        rows.push(row);
        if (index < ids.length - 1) await sleep(Math.max(1200, Number(delayMs) || 1250));
      }
      persistProbeState(rows);
      return rows;
    } finally {
