...
export function proplineTrafficEnabled(env = process.env) {
  const mode = text(env.OBLIGE_PROP_PROVIDER_MODE || env.PROP_PROVIDER_MODE || 'auto').toLowerCase();
  return !['sportradar','radar','sr','sportsgameodds','sgo','sports-game-odds'].includes(mode);
}
function automaticProplineVerifyEnabled() {
  return proplineTrafficEnabled();
}
...
if (proplineConfigured() && automaticProplineVerifyEnabled()) {
