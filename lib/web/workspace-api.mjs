...
export function createWorkspaceHandler({authenticate:auth=authenticate,read:provider=read,predict:prediction=predict,historyFallback=publicHistory,wnbaHistoryFallback=wnbaArchiveHistory,now=Date.now}={}) {
    ...
    try {
      ...
      if(action==='history') {
        ...
        if(fallback?.available===true&&Array.isArray(fallback.gameLog)&&fallback.gameLog.length) {
          send(res,200,{ok:true,...fallback,sourceProvider:fallback.source||'ESPN',fallback:true,primaryCode:primary.code,attemptedSources});return true;
        }
