...
async function fetchCsv(url,{fetcher=globalThis.fetch,gzip=false}={}){
 ...
 try{
  const response=await fetcher(url,{method:'GET',redirect:'follow',credentials:'omit',headers:{accept:gzip?'application/gzip':'text/csv'},signal:controller.signal});
...
export async function fetchSportsDataverseCfbResearch(params={},dependencies={}){
 ...
 for(const year of [season,season-1]){
  try{const snap=await loadSeason(year,dependencies);cached=cached&&snap.cached;loaded.push(snap);}catch(error){loadError||=error;continue;}
