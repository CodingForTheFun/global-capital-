...
function sampleStd(values){
  ...
  const m=mean(values);return Math.sqrt(values.reduce((s,v)=>s+(v-m)**2,0)/(values.length-1));
}
function cleanValues(logs,key){return logs.map(row=>Number(row?.[key])).filter(Number.isFinite);}
function sourceKey(feature){
  const match=feature.match(/^(attempts|completions|passing_tds|receptions)_(?:lag\d+|mean\d+|std\d+)$/);
  return match?STAT_KEYS[match[1]]:null;
}
function featuresFor(model,logs,target){
  ...
  for(const name of model.features){
    ...
    const avg=name.match(/_mean(\d+)$/);if(avg){const n=Number(avg[1]),slice=values.slice(0,n);out[name]=slice.length>=3?mean(slice):null;continue;}
    const std=name.match(/_std(\d+)$/);if(std){out[name]=sampleStd(values.slice(0,Number(std[1])));continue;}
    out[name]=null;
  }
  return out;
}
function score(model,featureMap){
  let value=model.intercept;
  for(let i=0;i<model.features.length;i++){
    let x=featureMap[model.features[i]];
    if(!finite(x))x=model.imputerMedian[i];
    const scale=model.scale[i]||1;
    value+=model.coef[i]*((x-model.mean[i])/scale);
  }
  return Math.max(0,value);
}
function residualCdf(quantiles,x){
  if(!Array.isArray(quantiles)||quantiles.length<2)return null;
  if(x<=quantiles[0])return 0;
  const last=quantiles.length-1;if(x>=quantiles[last])return 1;
  for(let i=1;i<=last;i++)if(x<=quantiles[i]){
    const lo=quantiles[i-1],hi=quantiles[i],fraction=hi===lo?0.5:(x-lo)/(hi-lo);
...
export function createResearchProjectionStore({clock=Date.now}={}){
  ...
  return {async lookup(input){
    ...
    const hit=cache.get(key);if(hit&&hit.expires>clock())return hit.value;
    ...
    const pending=(async()=>{
      ...
      const fmap=featuresFor(model,history.gameLog,target),projection=score(model,fmap),cdf=residualCdf(model.validation?.residualQuantiles,target.line-projection);
