...
export function predictionTarget(input) {
  ...
  // Optional: the board's consensus no-vig over probability at this exact line.
  // It feeds the global model and the market fallback; it is not an identity field.
  const market = input.marketOverProbability;
  const marketOverProbability = finite(market) && market > 0 && market < 1 ? market : undefined;
  // Optional: the game's two teams, for the global model's context features.
  // Not identity fields either; a target without them is still valid.
  const team = v => (text(v,100) ? v.trim() : undefined);
  const homeTeam = team(input.homeTeam), awayTeam = team(input.awayTeam);
  return {sport, eventId, playerId, playerName, marketId, sportsbookKey, gameStartTime:new Date(timestamp(gameStartTime)).toISOString(), line,
    ...
    ...(marketOverProbability === undefined ? {} : {marketOverProbability}),
