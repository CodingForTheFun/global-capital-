import {createOddsRequestGate,oddsRequestKey} from './odds-request-gate.mjs';
...
const refreshAfter = new Map();
const alternateBoards = new Map();
const catalogInflight = new Map();
const lastRefreshFailureLog = new Map();
...
const requestGate = createOddsRequestGate({
  onState: recordProviderControl,
  concurrency: intEnv('THE_ODDS_API_CONCURRENCY',2,1,2),
  spacingMs: intEnv('THE_ODDS_API_REQUEST_SPACING_MS',250,100,5000),
});

async function apiGet(path, params = {}, { sport = null, event = null } = {}) {
  if (!apiKey()) throw Object.assign(new Error('The Odds API is not configured.'), { code: 'NOT_CONFIGURED' });
  requestGate.restore(snapshotDiagnostics().providerControl);
  return requestGate.run(oddsRequestKey(path,params),async signal=>{
    ...
    catch(error){recordProviderError({provider:'the-odds-api',endpoint:path,sport,event,code:error?.name});throw error;}
    recordProviderRequest({used:numberOrNull(response.headers.get('x-requests-used')),
      remaining:numberOrNull(response.headers.get('x-requests-remaining')),cost:numberOrNull(response.headers.get('x-requests-last'))});
    const body=await response.json().catch(()=>null);
    if(!response.ok){
      const code=response.status===429?'RATE_LIMITED':body?.error_code||body?.code||'ODDS_API_ERROR';
      recordProviderError({provider:'the-odds-api',endpoint:path,sport,event,status:response.status,code});
      throw Object.assign(new Error(`Odds provider HTTP ${response.status}`),{status:response.status,code,retryAfter:response.headers.get('retry-after')});
    }
    return body;
  });
}
...
async function discoverMarkets(sportKey, eventId, scope, { signal, sport, budget, includeAlternates = false } = {}) {
  // The upstream catalog is identical for regular and alternate views. Keep
  // their filters separate, but pay for discovery only once per scope/event.
  const key = oddsRequestKey(`/sports/${sportKey}/events/${eventId}/markets`, scope.params);
  const select = catalog => catalog.filter(row => isPropMarket(row.key, includeAlternates));
  const cached = marketCache.get(key);
  if (cached) {
    const catalog = select(cached.catalog);
    const ttl = catalog.length ? 30 * 60_000 : 5 * 60_000;
    if (cached.fetchedAt + ttl > Date.now()) return catalog;
  }
  if(catalogInflight.has(key))return select(await catalogInflight.get(key));
  const pending=(async()=>{
  ...
  const catalog = marketCatalog(body.bookmakers, key => isPropMarket(key, true));
  ...
  marketCache.set(key, { catalog, fetchedAt: Date.now() });
  while (marketCache.size > 1000) marketCache.delete(marketCache.keys().next().value);
  return catalog;
  })();
  catalogInflight.set(key,pending);
  try{return select(await pending);}finally{catalogInflight.delete(key);}
}
...
export async function fetchBoard(sport, { force = false, includeAlternates = false, cacheOnly = false, respectFresh = false } = {}) {
  const selected=text(sport).toUpperCase(),sportKey=SPORT_KEYS[selected];
  if(!SUPPORTED_SPORTS.includes(selected)||!sportKey)throw Object.assign(new Error('Unsupported sport.'),{code:'UNSUPPORTED_SPORT'});
  if(!apiKey())throw Object.assign(new Error('The Odds API is not configured.'),{code:'NOT_CONFIGURED'});
  const scope=oddsScope(),signature=coverageSignature(scope);
  const key=selected+'|'+signature+'|'+includeAlternates;
  const alt=alternateBoards.get(key);
  const cached=includeAlternates?(alt?{...alt.value,meta:{...alt.value.meta,stale:alt.expiresAt<=Date.now(),cacheHit:true}}:null):await readCachedBoard(selected,{allowStale:true});
  const compatible=cached?.meta?.coverageSignature===signature;
  if(compatible&&(!force||respectFresh)&&!cached.meta.stale)return cached;
  if(cacheOnly)return cached||{props:[],data:{events:[],players:[],props:[],lines:[]},meta:{sport:selected,cacheHit:true,stale:true,provider:'The Odds API'}};
  if(compatible&&(refreshAfter.get(key)||0)>Date.now())return cached;
  const start=()=>{
    if(boardInflight.has(key)){sharedRefreshes++;return boardInflight.get(key);}
    const pending=refreshBoard(selected,{includeAlternates,scope,signature,sportKey});
    boardInflight.set(key,pending);
    pending.then(board=>{
      boardInflight.delete(key);
      // Negative results and failures are temporary, including forced cycles.
      const delay=board.meta?.stale?30000:board.props?.length?0:300000;
      refreshAfter.set(key,Date.now()+delay);
      if(includeAlternates)alternateBoards.set(key,{value:board,expiresAt:Date.now()+(board.meta?.cacheSeconds||300)*1000});
    },()=>{boardInflight.delete(key);refreshAfter.set(key,Date.now()+30000);});
    return pending;
  };
  if(compatible&&!force){void start().catch(()=>{});return {...cached,meta:{...cached.meta,stale:true,revalidating:true}};}
  if((refreshAfter.get(key)||0)>Date.now())throw Object.assign(new Error('Provider refresh is deferred.'),{code:'PROVIDER_COOLDOWN'});
  return start();
}

async function refreshBoard(selected,{includeAlternates,scope,signature,sportKey}) {
  // Shared work has its own bounded request timeouts; one disconnected browser
  // must not abort refreshes used by other readers or ingestion.
  ...
  try {
    ...
    const catalogResults = await mapLimit(sourceEvents, 4, async rawEvent => {
      try {
        ...
        return { catalog: [], error: String(error?.code || error?.name || 'REQUEST_FAILED') };
      }
    });
    ...
    const results = await mapLimit(sourceEvents, 4, async (rawEvent, index) => {
      ...
      try {
        ...
        return { ...empty, error: String(error?.code || error?.name || 'REQUEST_FAILED') };
      } finally {
    ...
    if (!lines.length && !coverage.complete && !includeAlternates) {
      if (Date.now() - (lastRefreshFailureLog.get(selected) || 0) > 60000) {
        lastRefreshFailureLog.set(selected, Date.now());
        const known = new Set(['COVERAGE_BUDGET','INVALID_CATALOG','INVALID_ODDS','INVALID_API_KEY','OUT_OF_USAGE_CREDITS','INVALID_REGION','INVALID_REGIONS','INVALID_BOOKMAKERS','INVALID_MARKET','INVALID_MARKETS','INVALID_REQUEST','NO_ACCESS','RATE_LIMITED','ODDS_API_ERROR','AbortError','TimeoutError','TypeError','ReferenceError','REQUEST_FAILED']);
        const errors = [...catalogResults, ...results].map(r => r.error).filter(Boolean);
        const counts = {};
        for (const error of errors) { const code = known.has(error) ? error : 'OTHER'; counts[code] = (counts[code] || 0) + 1; }
        // Safe operator classification also covers failures before an HTTP request.
        console.warn('[AutoScout refresh failure]', JSON.stringify({ sport: selected, errors: counts, remainingCredits: snapshotDiagnostics().quota.remaining, refreshBudget: maxRefreshCredits(), checkedEvents: coverage.checkedEvents }));
      }
    ...
    const ttlSeconds = lines.length ? policyTtl(events) : 300;
...
export const theOddsApiProvider = Object.freeze({
  ...
  health: () => ({
    ...
    projectedCredits: projectedCreditsPerRefresh(),
    maxEvents: maxEvents(), maxMarketsPerEvent: maxMarkets(),
    monthlyCredits: monthlyCredits(), diagnostics: snapshotDiagnostics(), requestControl: requestGate.state(), refreshesInFlight: boardInflight.size, sharedRefreshes,
  }),
});
