...
//
// The central rule: a hit rate is measured against THIS PROP'S LINE, per game,
// not against the player's average. "Last 10 hit rate 80%" means the player
// beat this exact line in 8 of their last 10 games, in the direction of this
// exact pick. OVER and UNDER are different questions and are answered
// separately.
//
// A push (value exactly equal to the line) counts as neither a hit nor a miss
// and is excluded from the denominator, which is how sportsbooks settle them.
...
/**
 * Game log entries -> per-window analytics for one line and side.
 *
 * @param {Array<{value:number,date?:string,opponent?:string,isHome?:boolean}>} games  most recent FIRST
 */
export function rollingAnalytics(games = [], line, side = 'OVER') {
  ...
  for (const window of WINDOWS) {
    ...
    windows[window.id] = {
      ...
      // Null, not 0, when there is no line or every game pushed.
      hitRate: lineValue === null || decided === 0 ? null : Math.round((hits / decided) * 100),
    };
  }
  ...
  return {
    ...
    windows,
    trend,
    splits: venueSplits(usable, lineValue, direction),
...
function venueSplits(games, line, direction) {
...
/** Fill hit-rate gaps from computed windows without overwriting PickFinder's own. */
export function mergeHitRates(existing = {}, analytics) {
  const windows = analytics?.windows || {};
  const out = { ...existing };
  for (const id of ['l5', 'l10', 'l20', 'season']) {
