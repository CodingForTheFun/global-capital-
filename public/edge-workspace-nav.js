...
(() => {
  ...
  async function load(sport) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(response.status === 401 ? 'Sign in to view Taco props.' : 'Promotion data is temporarily unavailable.');
      ...
      if (!active || requestVersion !== version) return;
      message = controller.signal.aborted ? 'Taco props request timed out. Try refreshing.' : error.message;
    } finally {
  ...
  window.addEventListener('pagehide', () => { observer.disconnect(); version++; pending?.abort(); clearTimeout(expiryTimer); }, { once: true });
  mount();
})();
