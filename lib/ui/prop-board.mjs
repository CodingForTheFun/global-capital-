...
function teamCodes(value) {
 const words=String(value||'').normalize('NFKC').replace(/[^A-Za-z0-9]+/g,' ').trim().split(/\s+/).filter(Boolean);
 if(!words.length)return new Set();
 const compact=words.join('').toLowerCase();
 const allInitials=words.map(w=>w[0]).join('').toLowerCase();
 const placeInitials=words.length>1?words.slice(0,-1).map(w=>w[0]).join('').toLowerCase():'';
 const firstPlusRest=words.length>1&&words[0].length<=3?(words[0]+words.slice(1).map(w=>w[0]).join('')).toLowerCase():'';
 return new Set([compact,allInitials,placeInitials,firstPlusRest].filter(Boolean));
}
function teamMatches(candidate,value) {
 const a=teamCodes(candidate),b=teamCodes(value);
...
function canonicalTeam(group) {
 ...
 if(!team)return '';
 // Provider rows frequently mix a short team code with the full matchup name
 // (NYG vs New York Giants, LAR vs Los Angeles Rams). Anchor that presentation
 // alias to the verified matchup team before deciding whether a same-named
 // athlete is a genuine namesake.
 for(const candidate of [group?.homeTeam,group?.awayTeam]){
  if(candidate&&teamMatches(team,candidate))return norm(candidate);
 }
 return norm(team);
}
export function teamIndex(groups=[]) {
 ...
 for(const group of groups){
  ...
  if(!name)continue;
  const team=canonicalTeam(group);
  if(!team)continue;
...
export function playerCardKey(group,index=null) {
 ...
 // provider IDs cannot key a card. Group by normalized name and use team only
 // when the same name genuinely resolves to more than one team.
 //
 // Team spellings are canonicalized against the matchup first, so NYG and
 // New York Giants do not create duplicate cards. Genuine namesakes such as
 // same-name athletes on BUF and JAX still split, and a teamless row in that
 // collision remains separate rather than being guessed onto either athlete.
 const name=norm(group.playerName);
