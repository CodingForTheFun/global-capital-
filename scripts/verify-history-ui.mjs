...
// Run: CHROMIUM_PATH=/usr/bin/chromium node scripts/verify-history-ui.mjs
...
try{
 for(const [name,viewport] of [['desktop',{width:1440,height:1000}],['mobile',{width:390,height:844}]]){
  ...
  const errors=[],requests=[];let held=null,firstNFL=true;
  page.on('pageerror',e=>errors.push(e.message));
  await page.route('**/*',async route=>{
   const req=route.request(),u=new URL(req.url());
   if(u.hostname!=='127.0.0.1')return route.abort();
   const json=data=>route.fulfill({contentType:'application/json',body:JSON.stringify(data)});
   if(u.pathname==='/')return route.fulfill({contentType:'text/html',body:'<!doctype html><meta name="viewport" content="width=device-width, initial-scale=1"><title>Isolated history regression replay</title><script src="/scout-ui-v5.js"></script>'});
   if(u.pathname==='/api/apex/props'){
    const s=selections[u.searchParams.get('sport')];
    return json({props:s?['OVER','UNDER'].map(side=>({...s,log:undefined,id:side,side,eventId:'fixture',gameStartTime:'2026-09-13T18:00:00Z',sportsbook:'PrizePicks',sportsbookKey:'prizepicks',isAlternate:false,providerUpdatedAt:'2026-09-12T22:00:00Z'})):[],data:{players:[],lines:[]},meta:{warning:'ISOLATED QA REPLAY — example lines, recorded game results, not a live board.'}});
   }
   ...
   const relative=u.pathname==='/scout-ui-v5.js'?'apex-v2/scout-ui-v5.js':u.pathname==='/assets/autoscout-research.css'?'apex-v2/autoscout-research.css':u.pathname.replace(/^\/assets\//,'');
   const file=resolve(root,relative);
   if(!file.startsWith(root+'/'))return route.abort();
   try{return route.fulfill({contentType:extname(file)==='.css'?'text/css':'text/javascript',body:await readFile(file,'utf8')});}
  ...
  await page.waitForFunction(()=>document.querySelector('#asBatch')?.disabled===true||document.body.innerText.includes('Loading research'));
  ...
  assert.deepEqual(errors,[]);console.log(name,'PASS: sport-switch recovery, research cards, side/line recalculation, no page errors');
  await context.close();
 }
