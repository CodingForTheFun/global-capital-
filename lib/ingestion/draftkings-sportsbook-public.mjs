...
function parseDocument(sport, doc) {
  ...
  for (const marketRow of list(doc?.markets)) {
    ...
    // More than one complete line means an alternate ladder is present and the
    // response does not tell us which threshold is the main line. Fail closed.
...
export async function fetchDraftKingsSportsbookPublic(sport, { fetcher = globalThis.fetch, force = false } = {}) {
  ...
  const task = (async () => {
    const metaUrl = `${BASE}/leagues/${leagueId}`;
    const meta = await fetchJson(fetcher, metaUrl);
