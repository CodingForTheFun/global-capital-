...
function sportState(state, sport, now) {
  ...
  s.floor = Math.max(now - TRAINING_POLICY.lookbackDays * DAY, Date.parse(s.windowStart || '') || 0);
  return s;
}
...
export async function runTrainingCycle({
  ...
  async function pull(sport, since, until) {
    ...
    if (result.meta?.windowStart) s.windowStart = result.meta.windowStart;
  ...
  state.lastCycle = { at: iso(now), exportCalls: calls, stop, pulls: log.slice(-80), trained };
...
export function startGlobalTraining({ env = process.env, onCycle = () => {} } = {}) {
  ...
  const tick = async () => {
    if (running) return;
    running = true;
    try { onCycle(await runTrainingCycle()); } catch (error) { onCycle({ error: String(error?.message || error).slice(0, 200) }); }
    finally { running = false; }
  };
  timer = setTimeout(function loop() {
    void tick();
    timer = setTimeout(loop, TRAINING_POLICY.intervalMs);
    timer.unref?.();
