...
export async function fetchPublicFeedJson(url, {
  ...
  const timeout = Object.assign(new Error('PUBLIC_FEED_TIMEOUT'), { code: 'PUBLIC_FEED_TIMEOUT', endpoint });
  ...
  try {
    ...
    // Never include URL query strings, response bodies, or arbitrary error text.
    if (error && typeof error === 'object') {
      error.endpoint ||= endpoint;
      if (!error.code && error.name === 'TimeoutError') error.code = 'PUBLIC_FEED_TIMEOUT';
    }
    throw error;
  } finally {
    clearTimeout(timer);
