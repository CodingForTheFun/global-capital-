...
function SelectFilter({
  ...
  return (
    <label data-filter={label} className="deep-dive-filter relative flex h-10 min-w-[112px] shrink-0 items-center gap-1 rounded-[6px] border border-slate-700/80 bg-slate-900/80 px-2.5 pr-7 text-[12px] text-slate-400">
...
function Chart({
  ...
  if (!rows.length) {
    return (
      <div className="grid min-h-[230px] place-items-center rounded-[10px] border border-dashed border-slate-800 px-5 text-center text-xs text-slate-500">
    ...
    <div className="overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
      <div className="min-w-[520px]">
        <div className="relative h-[220px] border-b border-slate-700 bg-[repeating-linear-gradient(to_top,transparent_0,transparent_54px,rgba(71,85,105,.16)_54px,rgba(71,85,105,.16)_55px)]">
          ...
          >
            <span className="absolute right-0 -top-5 rounded bg-[var(--surface-2)] px-1.5 py-0.5 text-[12px] font-black text-amber-300">
              {line}
            </span>
          </div>
          <div
            ...
            {rows.map((game, index) => {
              ...
              return (
                <div key={`${game.gameId || game.date || index}-${index}`} className="relative flex h-full items-end">
                  <span
                    className="absolute inset-x-0 text-center text-[12px] font-bold text-slate-300"
        ...
        >
          {rows.map((game, index) => (
            <span key={`label-${game.gameId || game.date || index}-${index}`} className="min-w-0 overflow-hidden text-[11px] text-slate-500">
...
export function PlayerPropDeepDive({
  ...
  return (
    <section
      className="deep-dive-card mx-auto w-full max-w-[1180px] overflow-hidden rounded-[10px] border border-slate-800/90 bg-[var(--surface-2)] text-slate-100"
      data-design="player-prop-deep-dive-restored"
      data-release="history-model-inline-20260921d"
    >
      <div className="border-b border-slate-800/90 bg-[var(--surface)] p-3 sm:p-4">
        <div className="flex items-start justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3">
            <div className="relative grid h-14 w-14 shrink-0 place-items-center rounded-full bg-[var(--surface)] p-[2px] sm:h-[66px] sm:w-[66px]">
              ...
              <span className="absolute -bottom-1 -right-1 rounded-full border border-slate-700 bg-[var(--surface-2)] px-1.5 py-0.5 text-[11px] font-black text-sky-300">
                {group.sport}
              </span>
            </div>
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-1.5">
                ...
                {group.position ? (
                  <span className="rounded-[3px] border border-slate-700 bg-slate-900/90 px-1.5 py-0.5 text-[12px] font-black text-slate-400">
                    {group.position}
                  </span>
                ) : null}
              </div>
          ...
          <button
            ...
            className={[
              'inline-flex h-9 w-9 shrink-0 items-center justify-center gap-2 rounded-[6px] border text-xs font-bold transition sm:w-auto sm:px-3',
              favourite
        ...
        <div className="mt-2 flex items-center justify-between gap-2 rounded-[10px] border border-sky-500/20 bg-sky-500/[.07] p-2 sm:mt-4 sm:p-2.5">
          <div className="flex min-w-0 items-center gap-2">
            <span className="rounded-[3px] bg-sky-500 px-2 py-1 text-[12px] font-black tracking-wide text-white">
            ...
            <div className="min-w-0">
              ...
              <p className="hidden text-[12px] text-slate-500 sm:block">Main prop · {periodLabel(group.period)}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <label className="relative block">
              ...
              <select
                ...
                className="h-8 max-w-[112px] appearance-none rounded-[6px] border border-sky-400/20 bg-slate-950/60 pl-2 pr-6 text-[12px] font-bold text-slate-300 outline-none"
              >
      ...
      <div className="space-y-3 p-2.5 sm:p-4">
            ...
            {categoryLabels.map((entry) => {
              ...
              return (
                <button
                  ...
                  className={[
                    'h-9 shrink-0 rounded-[6px] border px-3 text-[12px] font-black tracking-wide transition',
                    active
          ...
          {periods.map((entry) => {
            ...
            return (
              <button
                ...
                className={[
                  'h-8 shrink-0 rounded-full border px-3 text-[12px] font-bold transition',
                  active
                    ? 'border-blue-400/40 bg-blue-500 text-white'
                    : 'border-slate-800 bg-[var(--surface-2)] text-slate-500 hover:text-slate-300',
                ].join(' ')}
              >
        ...
        <div className="flex min-h-5 items-center justify-between gap-3 text-[12px] text-slate-500">
        ...
        <div className="grid gap-3 lg:grid-cols-[300px_1fr]">
          <div className="rounded-[10px] border border-slate-800 bg-[var(--surface-2)] p-2.5">
            <div className="flex items-center justify-between gap-2">
              <div className="text-[12px] font-bold uppercase tracking-[.16em] text-slate-500">Target line</div>
              {lines.length > 1 ? (
                <select
                  aria-label="Posted line"
                  value={group.line}
                  onChange={(event) => choosePostedLine(Number(event.target.value))}
                  className="h-7 max-w-[130px] rounded-[6px] border border-slate-800 bg-slate-950/60 px-2 text-[12px] font-bold text-slate-400 outline-none"
                >
                  {lines.map((line) => <option key={line} value={line}>{line}</option>)}
                </select>
              ) : (
                <span className="text-[12px] text-slate-600">Posted {group.line}</span>
              )}
            </div>
            <div className="mt-1.5 grid grid-cols-[40px_1fr_40px] overflow-hidden rounded-[10px] border border-slate-700 bg-slate-950/70">
              <button
                type="button"
            ...
            <div className="mt-1.5 grid grid-cols-2 overflow-hidden rounded-[6px] border border-slate-800 bg-slate-950/60">
              <button
                type="button"
              ...
              >
                <span className="block text-[11px] font-bold uppercase text-emerald-500/80">Over</span>
              ...
              >
                <span className="block text-[11px] font-bold uppercase text-rose-500/80">Under</span>
                <strong className="mt-0.5 block text-sm font-black tabular-nums text-rose-400">{odds(under?.price)}</strong>
              </button>
            </div>
            <p className="deep-dive-price-note mt-1.5 text-[11px] leading-relaxed text-slate-600">Book prices reflect posted line {group.line}; adjusted target line recalculates verified history only.</p>
          </div>

          {!loading && research?.available !== false ? (
            <div className="grid grid-cols-6 overflow-hidden rounded-[10px] border border-slate-800 bg-[var(--surface-2)]">
              {windows.map((item, index) => (
                ...
                >
                  <div className="truncate text-[11px] font-bold uppercase tracking-wide text-slate-500 sm:text-[12px]">{item.label}</div>
                  ...
                  <div className="mt-1 truncate text-[11px] text-slate-500">Avg {item.average ?? '—'}</div>
                  <div className="truncate text-[11px] text-slate-600">{item.games ? `${item.hits}/${item.games}` : 'No sample'}</div>
                </button>
              ))}
            </div>
          ) : (
            <div className="grid min-h-[122px] place-items-center rounded-[10px] border border-slate-800 bg-[var(--surface-2)] px-4 text-center text-xs text-slate-500">
              {loading ? 'Loading verified history…' : research?.message || 'Verified history unavailable'}
            </div>
          )}
        </div>

        <section className="deep-dive-chart-section rounded-[10px] border border-slate-800 bg-[var(--surface-2)] p-3 sm:p-4" id="analysis-chart">
          <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-[12px] text-slate-500">Current target: {state.line} · {periodLabel(group.period)}</p>
            </div>
        ...
        <section className="rounded-[10px] border border-slate-800 bg-[var(--surface-2)] p-3 sm:p-4">
          <div className="mb-3 text-[12px] font-black uppercase tracking-[.16em] text-slate-500">Supporting stats</div>
          <div className="grid grid-cols-4 overflow-hidden rounded-[10px] border border-slate-800">
            {supporting.map((stat, index) => (
              <div key={stat.label} className={`bg-slate-950/40 px-2 py-3 text-center ${index ? 'border-l border-slate-800' : ''}`}>
                <div className="truncate text-[11px] font-bold uppercase tracking-wide text-slate-500">{stat.label}</div>
        ...
        <section className="rounded-[10px] border border-slate-800 bg-[var(--surface-2)] p-3 sm:p-4" aria-label="History model">
          <div className="mb-3 text-[12px] font-black uppercase tracking-[.16em] text-slate-500">Model choice</div>
          <div className="rounded-[10px] border border-slate-800 bg-slate-950/35 p-3 sm:p-4">
            ...
            <div className="mt-3 grid grid-cols-2 gap-x-5 gap-y-4">
              <div>
                <div className="text-[12px] font-semibold text-slate-500">Projection</div>
              ...
              <div>
                <div className="text-[12px] font-semibold text-slate-500">Over</div>
              ...
              <div>
                <div className="text-[12px] font-semibold text-slate-500">Under</div>
              ...
              <div>
                <div className="text-[12px] font-semibold text-slate-500">{selectedEv?.source === 'model' ? 'Selected-quote model EV' : 'Selected-quote market EV'}</div>
            ...
            <p className="mt-4 text-[12px] leading-relaxed text-slate-500 sm:text-[11px]">
            ...
            <button
              ...
              className="mt-3 min-h-10 rounded-[6px] border border-slate-700 bg-slate-900/80 px-3 text-[11px] font-bold text-white transition hover:border-slate-600 disabled:cursor-wait disabled:opacity-60"
            >
        ...
        <section className="rounded-[10px] border border-slate-800 bg-[var(--surface-2)] p-3 sm:p-4">
          <div className="mb-3 flex items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-0.5 text-[12px] text-slate-500">Exact real quotes for posted line {group.line}</p>
            </div>
            <span className="rounded-full border border-emerald-400/20 bg-emerald-400/10 px-2 py-1 text-[11px] font-black text-emerald-300">
          ...
          {availableBooks.length ? (
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
              {availableBooks.map((book) => (
                <article
                  key={book.key}
                  className={`rounded-[10px] border p-2.5 ${book.key === state.book ? 'border-sky-500/40 bg-sky-500/[.06]' : 'border-slate-800 bg-slate-950/40'}`}
                >
                  <button
                    type="button"
                  ...
                  >
                    <span className="truncate text-[12px] font-black text-white">{book.name}</span>
                    <span className="rounded bg-slate-800 px-1.5 py-0.5 text-[11px] font-black text-slate-400">{bookInitials(book.name)}</span>
                  </button>
                  <div className="mt-1 text-[11px] font-bold text-slate-600">Line {group.line}</div>
                  <div className="mt-2 grid grid-cols-2 gap-1.5">
                    <button
                      type="button"
                      disabled={!book.over}
                      onClick={() => onState({ ...state, book: book.key, side: 'OVER' })}
                      className="rounded-[6px] border border-emerald-400/15 bg-emerald-400/[.06] px-2 py-2 text-left disabled:opacity-35"
                    >
                      <span className="block text-[11px] font-bold uppercase text-emerald-500/80">Over</span>
                    ...
                    <button
                      ...
                      className="rounded-[6px] border border-rose-400/15 bg-rose-400/[.06] px-2 py-2 text-left disabled:opacity-35"
                    >
                      <span className="block text-[11px] font-bold uppercase text-rose-500/80">Under</span>
