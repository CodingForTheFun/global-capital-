...
/* The compact per-prop selector above is the source of truth. The older book
   card rail duplicated PrizePicks/Underdog and left a large dead block below
   every prop, so hide that redundant presentation without touching its data. */
...
;(function(){
 ...
 function modal(){
  ...
  var style=document.createElement('style');style.id='oblige-account-notice-style';style.textContent='\n#obligeAccountNotice{position:fixed;inset:0;z-index:2147483000;display:none;align-items:center;justify-content:center;padding:20px;background:rgba(1,5,14,.78);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}#obligeAccountNotice.on{display:flex}#obligeAccountNotice .oanCard{width:min(460px,100%);border:1px solid rgba(92,139,201,.52);border-radius:20px;background:linear-gradient(155deg,#0d2038,#061222);box-shadow:0 28px 80px rgba(0,0,0,.55),inset 0 1px 0 rgba(255,255,255,.06);padding:24px;color:#eef6ff;font-family:Inter,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}#obligeAccountNotice .oanEyebrow{font-size:10px;font-weight:850;letter-spacing:.14em;text-transform:uppercase;color:#68a5ff}#obligeAccountNotice h2{margin:8px 0 9px;font-size:23px;line-height:1.15;letter-spacing:-.03em}#obligeAccountNotice p{margin:0;color:#b7c8db;font-size:14px;line-height:1.6}#obligeAccountNotice .oanCount{display:block;margin-top:14px;color:#6f89a8;font-size:11px}#obligeAccountNotice button{width:100%;height:46px;margin-top:20px;border:1px solid #438fff;border-radius:12px;background:linear-gradient(145deg,#1687ff,#1763ef);color:#fff;font:800 14px/1 inherit;cursor:pointer;box-shadow:0 8px 24px rgba(21,112,255,.26)}#obligeAccountNotice button:disabled{opacity:.6;cursor:wait}#obligeAccountNotice .oanError{display:block;min-height:16px;margin-top:8px;color:#ff9dab;font-size:11px}\n';(document.head||document.documentElement).appendChild(style);
