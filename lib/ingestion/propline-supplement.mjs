...
function prioritizeProplineQuote(row, source) {
  ...
  const merged = {
    ...
    isAlternate: false,
    // Preserve ObligeProps canonical joins. PropLine-native identifiers remain
    // separately attached through the propline* / provider* metadata fields.
...
export function mergeCachedPropline(board, sport, { now = Date.now } = {}) {
  ...
  // Rebuilt rows come last so canonical ids represented by the exact flat
  // customer rows win any collision with provider-native normalized rows.
