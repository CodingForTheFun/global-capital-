...
import { readArtifact, readObservations, readEnrich, globalModelDir } from './store.mjs';
...
let mlbEnricher = null;
async function defaultStarterHand(input) {
  if (!mlbEnricher) {
    const [{ fetchPublicResearch }, { createMlbEnricher }] = await Promise.all([
      import('../../data-sources/espn/research.mjs'), import('./mlb-enrich.mjs'),
    ]);
    mlbEnricher = createMlbEnricher(fetchPublicResearch.espn);
  }
  return mlbEnricher.opposingStarterHand(input);
}

export function createGlobalPredictor({ dir = globalModelDir(), clock = Date.now, starterHand = defaultStarterHand, starterBudgetMs = 1500 } = {}) {
  ...
  async function load(sport) {
    ...
    const loading = (async () => {
      ...
      if (artifact && artifact.trainedAt !== cached?.artifact?.trainedAt) {
        history = new Map();
        context = createContextIndex({ enrich: sport === 'MLB' ? await readEnrich(sport, dir).catch(() => ({})) : null });
  ...
  async function predict(target, base = null) {
    ...
    if (artifact?.regimes?.[regime] && informative) {
      const describe = (opposingHand = null) => context?.describe({ playerName: target.playerName, homeTeam: target.homeTeam, awayTeam: target.awayTeam, market: target.marketId, t: start, e: target.eventId, opposingHand }) || null;
      let ctx = describe();
      // MLB batters: today's opposing probable starter's hand, when the model
      // uses it and it answers quickly; otherwise that feature stays off.
      if (sport === 'MLB' && artifact.weights.length > 15 && ctx?.team && target.homeTeam && target.awayTeam) {
        const team = [target.homeTeam, target.awayTeam].find((label) => nameKey(label) === ctx.team);
        const opponent = [target.homeTeam, target.awayTeam].find((label) => label !== team);
        let timer;
        const thrown = await Promise.race([
          Promise.resolve(starterHand({ team, opponent, t: start })).catch(() => null),
          new Promise((resolve) => { timer = setTimeout(() => resolve(null), starterBudgetMs); timer.unref?.(); }),
        ]).finally(() => clearTimeout(timer));
        if (thrown === 'L' || thrown === 'R') ctx = describe(thrown);
      }
      ...
      if (push !== null) {
        ...
        return {
          ...
          message: artifact.source === 'pooled'
            ? 'Global model estimate, trained on resolved props across every sport and checked on later games of this sport it never saw. Not a guarantee.'
            : 'Global model estimate for this sport, trained on resolved props and checked on later games it never saw. Not a guarantee.',
        };
      }
    }
