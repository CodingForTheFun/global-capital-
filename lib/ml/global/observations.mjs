...
export function createObservationReducer({ sportKey, maxGroups = 250_000 } = {}) {
  ...
  function add(row) {
    ...
    if (!group) {
      ...
      // The game's two teams and final score: context for the model (home or
      // away, the opponent, scoring environment). Blank when the export omits them.
      group = { e: text(row.event_id), t: commence, p: player, m: market, a: actual, bad: false, close: new Map(), open: new Map(), dfs: new Map(),
        h: text(row.home_team) || null, v: text(row.away_team) || null, hs: num(row.home_score), vs: num(row.away_score) };
  ...
  function finish() {
    ...
    for (const group of groups.values()) {
      ...
      out.push({
        ...
        dl: dfsLine,
        h: group.h, v: group.v, hs: group.hs, vs: group.vs,
      });
    }
