...
const football = {
  ...
  Sacks: ['sacks', 'sacks'], TacklesAssists: ['totalTackles', 'tacklesAssists'],
...
const COLUMNS = {
  ...
  MLB: { Hits:['hits','hits'], Runs:['runs','runs'], RunsBattedIn:['RBIs','runsBattedIn'],
    ...
    EarnedRuns:['earnedRuns','earnedRuns'], HitsAllowed:['hits','hitsAllowed'], PitchingWalks:['walks','pitchingWalks'],
    PitchingOuts:['innings','pitchingOuts'], TotalBases:['totalBases','totalBases'], Singles:['singles','singles'] },
  NHL: { Goals:['goals','goals'], Assists:['assists','assists'], ShotsOnGoal:['shotsTotal','shotsOnGoal'],
    Saves:['saves','saves'], GoalsAgainst:['goalsAgainst','goalsAgainst'], Hits:['hits','hits'], BlockedShots:['blockedShots','blocks'],
...
function marketFields(sport, market, key) {
  const extra = { batter_home_runs:['HomeRuns'], batter_strikeouts:['Strikeouts'], pitcher_hits_allowed:['HitsAllowed'],
    pitcher_earned_runs:['EarnedRuns'], pitcher_walks:['PitchingWalks'], pitcher_outs:['PitchingOuts'],
    pitcher_outs_recorded:['PitchingOuts'], player_sacks:['Sacks'], player_tackles_assists:['TacklesAssists'],
    player_field_goals:['FieldGoalsMade'], player_goals_against:['GoalsAgainst'], player_saves:['Saves'] };
...
function categoryFor(sport, fields, key) {
  return sport !== 'MLB' ? null : String(key || '').startsWith('pitcher_')
    || (!String(key || '').startsWith('batter_') && fields?.some(f => ['Strikeouts','EarnedRuns','HitsAllowed','PitchingWalks','PitchingOuts'].includes(f))) ? 'pitching' : 'batting';
}
const TEAM_ALIASES = {
  NBA: { NY: 'NYK', GS: 'GSW', SA: 'SAS', NO: 'NOP', UTAH: 'UTA', WSH: 'WAS', PHO:'PHX', BRK:'BKN' },
  NFL: { WSH:'WAS', JAC:'JAX', LA:'LAR', GNB:'GB', KAN:'KC', NWE:'NE', NOR:'NO', SFO:'SF', TAM:'TB', LVR:'LV' },
  MLB: { NYY:'NYY', SD:'SD', SDP:'SD', SFG:'SF', KCR:'KC', TBR:'TB', WSH:'WSH', WAS:'WSH' },
...
function pairPart(value, index) {
  const m = typeof value === 'string' && value.trim().match(/^(-?\d+(?:\.\d+)?)-(-?\d+(?:\.\d+)?)$/);
  return m ? Number(m[index + 1]) : null;
}
...
function mappedValue(field, name, values) {
  const direct = numeric(values[name]);
  if (direct !== null) return direct;
  if (field === 'PassingCompletions') return pairPart(values['completions-passingAttempts'], 0);
  if (field === 'PassingAttempts') return pairPart(values['completions-passingAttempts'], 1);
  if (field === 'Sacks') return pairPart(values['sacks-sackYardsLost'], 0);
  if (field === 'FieldGoalsMade') return pairPart(values['fieldGoalsMade-fieldGoalAttempts'], 0);
  if (field === 'PitchingOuts') return inningsToOuts(values.innings);
  return null;
}
    ...
    for (const cat of group.categories || []) {
      ...
      for (const row of cat.events || []) {
        ...
        for (const [field, [name, key]] of Object.entries(columns)) {
          ...
          const value = field === 'ThreePointersMade' ? (pair && Number(pair[1]) <= Number(pair[2]) ? Number(pair[1]) : null)
            : mappedValue(field, name, values);
...
export function createPublicResearch({ fetchImpl = (...args) => fetch(...args), now = () => Date.now() } = {}) {
  ...
  return async function fetchResearch(params = {}) {
    ...
    return { ok: true, available: true, source: 'Historical stats', cached, opponent, opponentId,
      ...
      coverage: { seasonComplete: current.complete, currentSeasonGames: current.rows.length,
        backfilledGames: Math.max(0, unique.length - current.rows.length) }, season: filter?.value || null };
  };
}
