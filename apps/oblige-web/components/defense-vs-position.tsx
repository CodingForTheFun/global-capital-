...
export function DefenseVsPosition({sport,opponent,position,onMetric,data:provided}:{sport:string;opponent?:string|null;position?:string|null;onMetric?:(metric:string)=>void;data?:DvpData}) {
  ...
  const defaultTeam=teams.find(t=>sameTeamLabel(t.abbreviation,opponent)||sameTeamLabel(t.name,opponent))?.id||teams[0]?.id||'';
  const activeTeam=teams.some(t=>t.id===team)?team:defaultTeam;
  const activePosition=positions.includes(selectedPosition)?selectedPosition:positions.includes(position||'')?position!:positions[0]||'';
