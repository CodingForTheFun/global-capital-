...
import {adaptivePrediction,ADAPTIVE_SPORTS,cleanAdaptiveHistory} from './adaptive.mjs';
...
export function createMLStore({file = path.resolve(process.env.DATA_DIR || './data','ml/predictions.json'), clock = Date.now, pollMs = 5000,
  ...
  async function adaptive(t) {
    ...
    try {
      ...
      const base = {
        sport:t.sport,playerName:t.playerName,providerPlayerId:t.playerId,
      ...
      // Recovery and fitting must agree: nulls, target/future games and
      // conflicting duplicate records cannot satisfy the minimum sample.
      const usableRows = value => cleanAdaptiveHistory(value?.gameLog, t, clock()).length;
