...
export function playerCardKey(group,index=null) {
 // A player's team text is provider presentation data, not a stable identity.
 // The same athlete can arrive as "NYG" from one quote source and
 // "New York Giants" from another. Treating those spellings as distinct teams
 // split one real athlete into duplicate cards and defeated the quote merge.
 //
 // Prefer the normalized name inside a sport/entity surface. The upstream
 // normalized player id remains the fallback when a name is genuinely absent.
 // Namesake disambiguation belongs in the identity normalizer where a verified
 // roster id can prove two people are different; the card renderer must not
 // guess from provider-specific team labels.
 void index;
 const name=norm(group.playerName);
 const sport=String(group.sport||'').toUpperCase();
 const entity=group.entityType||'player';
 if(name)return [sport,entity,'name:'+name].join('|');
