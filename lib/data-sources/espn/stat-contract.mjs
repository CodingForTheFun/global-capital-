...
export function marketContract({sport,market,providerMarketKey} = {}) {
  ...
  // Public DFS adapters may use stable fallback ids (for example
  // player_3_pt_made) that are not provider-native keys. In that case the
  // verified display label is still allowed to resolve an existing exact stat
  // contract. Unsupported labels remain unavailable rather than being guessed.
  else if(!fields) fields=fieldsFor(sport,market,key);
  if(!fields) fields=family==='football'?footballLabels[label]:null;
  if(!fields&&sport==='MLB') fields=({'pitching outs':['PitchingOuts'],'outs recorded':['PitchingOuts'],
  ...
  if(!fields&&sport==='NHL') fields=({'goals against':['GoalsAgainst'],'goals allowed':['GoalsAgainst']})[label];
  if(!fields&&family==='basketball') fields=({'blk+stl':['BlockedShots','Steals'],'pts':['Points'],'reb':['Rebounds'],'ast':['Assists'],
    '3 pt made':['ThreePointersMade'],'3pt made':['ThreePointersMade'],'three pointers made':['ThreePointersMade']})[label];
