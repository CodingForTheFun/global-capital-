...
const PUBLIC_SNAPSHOT_BATCH_SIZE = 100;
...
export async function persistPublicSnapshot(source, rows, observedAt = new Date().toISOString()) {
  ...
  let written = 0;
  // A full PrizePicks board is several thousand rows. One giant RPC can exceed
  // free-tier PostgREST/compute timeouts and then starve every smaller provider.
  // Write bounded batches first and prune omitted rows only after every batch
  // succeeds. If any batch fails, the previous last-good rows remain intact.
  for (let i = 0; i < snapshotRows.length; i += PUBLIC_SNAPSHOT_BATCH_SIZE) {
    const result = await store('props', {
      source,
      observed_at: observedAt,
      rows: snapshotRows.slice(i, i + PUBLIC_SNAPSHOT_BATCH_SIZE),
      chunked: true,
      finalize: false,
    });
