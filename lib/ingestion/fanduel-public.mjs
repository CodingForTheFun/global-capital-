...
const MARKET_PATTERNS = Object.freeze([
  ...
  [/pitches?\s+thrown|total\s+pitches?/i, 'Pitches Thrown'], [/(?:strikeouts?|k'?s)\s*$/i, 'Strikeouts'],
...
async function fetchJson(fetcher, url) {
  const response = await fetcher(url, { headers: headers(), signal: ingestionSignal(AbortSignal.timeout(20_000)), redirect: 'follow' });
...
function parseEventPage(payload, sport, fallbackEvent) {
  ...
  for (const group of groups.values()) {
    ...
    if (parsed && isVerifiedPlayerPropRow({
      ...parsed,
      sportsbookKey: 'fanduel',
      provider: 'fanduel',
      marketId: parsed.fallbackMarketKey,
    })) records.push(parsed);
  }
  return records;
}
