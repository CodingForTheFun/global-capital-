...
export function proplineSupplementEnabled() {
  return providerIsEnabled('propline') && truthy(process.env.PROPLINE_SUPPLEMENT_ENABLED) && proplineConfigured();
}
...
function sameQuote(left, right) {
  ...
  const leftPrice = comparablePrice(left?.price);
  const rightPrice = comparablePrice(right?.price);
  // A missing price is a coverage gap, not a conflicting quote. Keep the
  // canonical row identity and let PropLine fill the missing price below.
  return leftPrice === null || rightPrice === null || leftPrice === rightPrice;
}
...
function enrichFromPropline(row, source) {
  ...
  for (const key of ['price','impliedProbability','fairOdds','fairLine','consensusLine']) {
...
function prioritizeProplineQuote(row, source) {
  if (!row || !source) return row;
  const merged = {
    ...row,
    source: source.source || 'PropLine',
    provider: 'propline',
    line: source.line,
    price: source.price,
    ...
    providerUpdatedAt: source.providerUpdatedAt || source.lastChangeAt || row.providerUpdatedAt || null,
    updatedAt: source.updatedAt || source.providerUpdatedAt || source.lastChangeAt || row.updatedAt || null,
    isAlternate: false,
    eventId: row.eventId || source.eventId,
    playerId: row.playerId || source.playerId,
    providerPlayerId: row.providerPlayerId || source.providerPlayerId,
    team: row.team || source.team,
    position: row.position || source.position,
    homeTeam: row.homeTeam || source.homeTeam,
    awayTeam: row.awayTeam || source.awayTeam,
    gameStartTime: row.gameStartTime || source.gameStartTime,
    period: row.period || source.period || 'game',
    marketId: row.marketId || source.marketId,
    market: row.market || source.market,
  };
  return enrichFromPropline(merged, source);
}

export function mergeCachedPropline(board, sport, { now = Date.now, primary = providerIsPrimary('propline') } = {}) {
  ...
  const transformed = rawBaseProps.map((row) => {
    ...
    // PropLine only replaces a quote when it is explicitly selected as the
    // primary quote source. In mesh/fallback mode, SportsGameOdds keeps the
    // current quote and PropLine may only enrich an exact match or add a
    // completely missing slot below.
    if (row?.isAlternate !== true) {
      const slot = identity(row);
      const sourceQuote = slot ? proplineBySlot.get(slot) : null;
      if (sourceQuote) {
        if (!sameQuote(row, sourceQuote)) {
          if (primary) {
            prioritized += 1;
            return prioritizeProplineQuote(merged, sourceQuote);
          }
          return merged;
        }
        const refreshed = enrichFromPropline(merged, sourceQuote);
  ...
  const lines = mergeById(baseData.lines || [], supplementData.lines || [], rebuilt.lines)
    .filter((row) => finalLineIds.has(text(row?.id)));
  const propIds = new Set(lines.map((row) => text(row?.propId)).filter(Boolean));
  const normalizedProps = mergeById(baseData.props || [], supplementData.props || [], rebuilt.props)
  ...
  const players = mergeById(baseData.players || [], supplementData.players || [], rebuilt.players)
    .filter((row) => playerIds.has(text(row?.id)));
  const events = mergeById(baseData.events || [], supplementData.events || [], rebuilt.events)
...
export function proplineSupplementHealth() {
  ...
  return {
    ...
    // Make provider precedence and freshness semantics inspectable without
    // exposing credentials or requiring a customer-facing source badge.
    mode: propProviderMode(),
    role: providerIsPrimary('propline') ? 'primary-current-quote' : 'fallback',
    secondaryProvider: providerIsPrimary('propline') ? 'sportsgameodds' : null,
    presenceRefreshCreatesHistory: false,
