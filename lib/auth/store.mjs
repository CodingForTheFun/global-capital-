...
/**
 * The shape stored on disk. `passwordHash`, passkey public-key records and code
 * hashes never leave the server; publicUser() below is the only thing an API
 * may return.
 */
function newUser({ email, passwordHash, role = 'member', emailVerified = false }) {
  ...
  return {
    ...
    pendingCodes: {},
    // Owner-only phishing-resistant credentials. Existing accounts that predate
    // this field simply behave as an empty list until the owner enrolls one.
    passkeys: [],
    // Offline one-time recovery codes are stored only as keyed hashes.
    recoveryCodeHashes: [],
  };
}

/** Everything an API response may contain. Never includes a hash or public key. */
export function publicUser(user) {
  if (!user) return null;
  const passkeys = Array.isArray(user.passkeys) ? user.passkeys : [];
  const recovery = Array.isArray(user.recoveryCodeHashes) ? user.recoveryCodeHashes : [];
  return {
    id: user.id,
    email: user.email,
    role: user.role,
    ...
    passkeyEnabled: user.role === 'owner' && passkeys.length > 0,
    passkeyCount: user.role === 'owner' ? passkeys.length : 0,
    recoveryCodesRemaining: user.role === 'owner' ? recovery.length : 0,
  };
}
...
/** Server-only lookup for a WebAuthn credential id. Never expose the record. */
export async function findByPasskeyCredentialId(credentialId) {
  const id = String(credentialId ?? '').trim();
  if (!id) return null;
  return (await readAll()).find((user) => (Array.isArray(user.passkeys) ? user.passkeys : [])
    .some((row) => row?.credentialId === id)) ?? null;
}
