...
function flattenBoard(data, ingestedAt) {
  ...
  for (const line of data.lines || []) {
    ...
    const multiplier = line.payoutMultiplier == null || typeof line.payoutMultiplier === 'boolean' || text(line.payoutMultiplier) === '' ? null : Number(line.payoutMultiplier);
    const underdogModifier = line.bookmakerKey === 'underdog' && Number.isFinite(multiplier) && multiplier > 0 && multiplier !== 1;
    output.push({
      id: line.id,
      source: 'PropLine',
      provider: 'propline',
      ...
      proplineEventId: event.providerEventId,
      playerId: player.id,
      providerPlayerId: player.providerPlayerId || '',
      proplinePlayerId: player.providerPlayerId || null,
      ...
      proplineOutcomeId: line.providerOutcomeId || null,
