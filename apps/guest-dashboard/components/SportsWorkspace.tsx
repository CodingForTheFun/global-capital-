...
import PropInsight from '@/components/PropInsight';
...
export default function SportsWorkspace() {
  const [boardMode, setBoardMode] = useState<'games'|'props'>('games');
  const [insight, setInsight] = useState<Quote|null>(null);
  ...
  useEffect(() => {
    const syncHash = () => { const id = window.location.hash.slice(1); if(id==='tacos'){window.location.replace('/apex#tacos');return;} setView(VIEWS.some(v => v.id === id) ? id as View : 'sports'); setPage(1); if(id==='props')setBoardMode(id); else if(id==='sports'||!id)setBoardMode('games'); };
  ...
  useEffect(() => {
    if (checking || !account || boardMode !== 'props') return;
    ...
    fetch(`/api/apex/props?sport=${encodeURIComponent(sport)}`, { credentials: 'same-origin', cache: 'no-store', signal: controller.signal })
      ...
      .then(data => {
        ...
        setCoverageNote(data.meta?.warning || 'All returned books are shown. Source coverage can be partial; unavailable props are never invented.');
  ...
  }, [checking, account, sport, refresh, boardMode]);
  ...
  const visibleBooks = book === 'all' ? books : books.filter(([key]) => key === book);
  ...
  function pickSport(value: string) {
    ...
    generation.current++; setInsight(null); setRows([]); setLoadedAt(''); setSport(value); setPage(1); setBook('all'); setMarket('all');
  ...
  return <div className="min-h-screen bg-[#090d16] text-slate-100">
    ...
    <div className="mx-auto max-w-[1700px] px-3 py-4 pb-24 sm:px-5 lg:px-6">
      ...
      <div className="grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1fr)_330px] xl:grid-cols-[minmax(0,1fr)_360px]">
        <main id="workspace-main" className={`min-w-0 ${boardMode==='games'&&(view==='sports'||view==='live')?'lg:col-span-2':''}`}>
          <section className="mb-5 grid gap-3 md:grid-cols-[minmax(0,1fr)_210px]">
            <div className="relative overflow-hidden rounded-2xl border border-emerald-500/20 bg-gradient-to-br from-emerald-950/60 via-[#101c24] to-[#111726] p-6"><Radar aria-hidden="true" size={150} className="pointer-events-none absolute -right-6 -top-5 text-emerald-500/10"/><p className="text-[10px] font-bold tracking-[.2em] text-emerald-400">OBLIGEPAY SPORTSBOOK + AUTO SCOUT</p><h1 className="relative mt-2 text-2xl font-extrabold tracking-tight sm:text-3xl">Game lines. Player props. Auto Scout.</h1><p className="relative mt-3 max-w-xl text-sm leading-6 text-slate-400">Browse game moneylines, spreads and totals. Explore player props and projected stats. Open Auto Scout beside Tools without changing accounts.</p></div>
            <div className={`${panel} flex flex-col justify-center p-5`}><span className="text-xs font-semibold text-slate-400">{boardMode==='games'?'Game market types':'Available research'}</span><strong className="mt-2 text-4xl font-extrabold tabular-nums text-emerald-400">{boardMode==='games'?'3':account ? groups.length.toLocaleString() : '—'}</strong><span className="mt-1 text-xs text-slate-500">{boardMode==='games'?'Spread · Moneyline · Total':`prop markets across ${books.length} books`}</span><p className="mt-3 text-[10px] text-slate-500">{boardMode==='games'?'Select real quotes to calculate returns':loadedAt ? `Board received ${loadedAt} · cached feed may be older` : 'No invented statistics'}</p></div>
          </section>
          <div className="mb-4 flex items-start gap-2 text-xs leading-5 text-slate-500"><ShieldCheck size={16} className="mt-0.5 shrink-0 text-emerald-500"/><p>Game odds and player props in one place. Wager processing is not connected: selections calculate returns but do not place bets.</p></div>
          {(view==='sports'||view==='live') && <div className="mb-5 flex gap-2 overflow-x-auto" aria-label="Market board type">{([['games','Game lines'],['props','Player props']] as const).map(([id,label])=><button key={id} aria-pressed={boardMode===id} onClick={()=>{setBoardMode(id);window.history.replaceState(null,'',window.location.pathname+'#'+(id==='games'?'sports':id));}} className={`whitespace-nowrap rounded-xl px-4 py-3 text-xs font-bold transition ${boardMode===id?'bg-emerald-500 text-[#06130f]':'border border-slate-700 text-slate-400 hover:text-white'}`}>{label}</button>)}</div>}
          ...
          {(view==='sports'||view==='live') && boardMode==='games' ? <GameBoard sport={sport} accountId={account.id} liveOnly={view==='live'}/> : view === 'tools' ? <ResearchTools/> : view === 'analytics' ? <section className={`${panel} p-5`}><h2 className="flex items-center gap-2 text-lg font-bold"><BarChart3 size={20} className="text-emerald-400"/>Coverage analytics</h2><p className="mt-2 text-sm text-slate-500">Counts reflect this loaded {sport} board—not prediction accuracy, ROI, or profit.</p><div className="mt-6 space-y-4">{books.map(([key, label]) => { const count = rows.filter(q => q.sportsbookKey === key).length; return <div key={key}><div className="mb-2 flex justify-between text-sm"><span>{label}</span><span className="text-slate-500">{count} quotes</span></div><div className="h-2 rounded-full bg-slate-800"><div className="h-2 rounded-full bg-emerald-500" style={{ width: `${rows.length ? count / rows.length * 100 : 0}%` }}/></div></div>; })}{!books.length && <p className="text-sm text-slate-500">Load an available sport to see its coverage.</p>}</div></section> : view === 'research' ? <section className={`${panel} p-5`}><h2 className="text-lg font-bold">My research snapshots</h2><p className="mt-2 text-sm text-slate-500">Your session list. These selections are not synced into Auto Scout’s Saved Props and are not placed bets.</p><div className="mt-5 space-y-2">{draft.map(q => <div key={q.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-800 p-4"><div><b className="text-sm">{q.playerName}</b><p className="mt-1 text-xs text-slate-400">{q.market} · {q.side} {q.line} · {q.sportsbook}</p></div><button onClick={() => setDraft(d => d.filter(x => x.id !== q.id))} className={secondary}><Trash2 size={14}/>Remove</button></div>)}{!draft.length && <p className="py-8 text-center text-sm text-slate-500">No research selections yet. Choose a line on the Sports board.</p>}</div></section> : <>
            ...
            <p role="status" className="mb-3 text-xs leading-5 text-amber-200/70">{coverageNote}</p>
            <section className={`${panel} overflow-hidden`} aria-label="Sportsbook prop comparison"><div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800 p-4"><h2 className="text-sm font-bold">{view === 'live' ? 'In-play player props' : 'Player props'} <span className="ml-2 text-slate-500">{filtered.length}</span></h2><button disabled={!rows.length} onClick={() => download()} className="inline-flex items-center gap-2 text-xs text-slate-400 hover:text-emerald-400"><Download size={14}/>Export board</button></div>
              ...
              <div className="flex items-center justify-between gap-2 border-t border-slate-800 p-4 text-xs text-slate-500"><span>Page {currentPage} of {pages}{book === 'all' ? ` · ${books.length} books; scroll to compare` : ''}</span><div className="flex gap-2"><button aria-label="Previous page" disabled={currentPage <= 1} onClick={() => setPage(p => p - 1)} className={secondary}><ChevronLeft size={15}/></button><button aria-label="Next page" disabled={currentPage >= pages} onClick={() => setPage(p => p + 1)} className={secondary}><ChevronRight size={15}/></button></div></div>
            </section>
          </>}
    ...
    {insight&&<PropInsight key={insight.id} quote={insight} onClose={()=>setInsight(null)}/>}
