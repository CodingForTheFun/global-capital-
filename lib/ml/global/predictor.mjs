...
import { featureVector, predictRow } from './model.mjs';
...
export function createGlobalPredictor({ dir = globalModelDir(), clock = Date.now } = {}) {
  ...
  async function load(sport) {
    ...
    const loading = (async () => {
      ...
      // Rebuild the history index only when a new model (and so new data) landed.
      if (artifact && artifact.trainedAt !== cached?.artifact?.trainedAt) {
        history = new Map();
        const floor = clock() - 400 * 86_400_000;
        for (const o of await readObservations(sport, dir).catch(() => [])) {
      ...
      const entry = { at: clock(), artifact, history, loading: null };
  ...
  async function predict(target, base = null) {
    ...
    const { artifact, history } = await load(sport);
    ...
    if (artifact?.regimes?.[regime]) {
      const over = predictRow(artifact.weights, featureVector({ history: past, line, marketP }));
      const push = pushFor();
      if (push !== null) {
        ...
        return {
          ...
          sourceKind: 'global-model', inputs: { market: marketP !== null, historyGames: past.length },
