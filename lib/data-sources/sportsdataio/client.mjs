...
function researchOnlyAllowed(purpose = 'default') {
  return purpose === 'research' && process.env.AUTOSCOUT_SPORTSDATAIO_RESEARCH_ONLY === 'true';
}

/** Read the key for a league. Per-league keys win, then the global key. */
export function resolveKey(sport, { purpose = 'default' } = {}) {
  if (process.env.AUTOSCOUT_DISABLE_SPORTSDATAIO === 'true' && !researchOnlyAllowed(purpose)) return '';
  const scoped = sport ? process.env[`SPORTSDATAIO_KEY_${String(sport).toUpperCase()}`] : '';
  return scoped || process.env.SPORTSDATAIO_API_KEY || '';
}

export function isConfigured({ purpose = 'default' } = {}) {
  if (process.env.AUTOSCOUT_DISABLE_SPORTSDATAIO === 'true' && !researchOnlyAllowed(purpose)) return false;
  return Boolean(process.env.SPORTSDATAIO_API_KEY
    || Object.keys(process.env).some((key) => key.startsWith('SPORTSDATAIO_KEY_') && process.env[key]));
}
...
export function createClient({
  ...
  async function get(url, path, { ttlMs = 0, sport = null, allowStale = true } = {}) {
    // Check before caches too: disabled provider paths cannot leak old results.
    if (process.env.AUTOSCOUT_DISABLE_SPORTSDATAIO === 'true' && !researchOnlyAllowed(purpose)) {
      return { ok: false, status: 0, reason: 'disabled', data: null, cached: false };
    }
    ...
    const apiKey = resolveKey(sport, { purpose });
