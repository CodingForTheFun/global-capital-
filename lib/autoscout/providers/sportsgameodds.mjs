...
export async function fetchSportsGameOddsBoard(sport, options = {}) {
  ...
  const sportID = SPORTSGAMEODDS_SPORT_IDS[selected] || null;
  ...
  const task = (async () => {
    ...
    try {
      ...
      const catalog = await fetchSportsGameOddsCatalog({ signal: options.signal || null });
      ...
      if (leagues.length && catalog.leagues.length) {
        const allowed = new Set(catalog.leagues.map((row) => row.id));
        const entitled = leagues.filter((league) => allowed.has(league));
        if (!entitled.length) {
          return emptyBoard(selected, {
            supported: false,
            entitlementMissing: true,
