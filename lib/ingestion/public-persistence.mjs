...
export async function claimPublicCycle(intervalSeconds = publicIntervalSeconds()) {
  ...
  return store('claim', { interval_seconds: publicIntervalSeconds(intervalSeconds) }, { allowDirect: true, timeoutMs: 10_000 });
}
export async function releasePublicCycle(owner) {
  if (!owner) return {};
  // Release is idempotent. REST stays primary; the direct fallback is allowed
  // only for transient Data API failures so a recovered owner is not stranded
  // during an API Gateway incident.
  return store('release', { owner }, { allowDirect: true, timeoutMs: 2_500 });
}
