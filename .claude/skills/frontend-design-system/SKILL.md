...
description: Use when building UI in a frontend codebase on top of an existing design system or component library — discovering its tokens and components, reusing them correctly, and binding to semantic tokens instead of hardcoding. Works with ANY system (MUI, Chakra, shadcn, Radix, an internal @org/ui, or local components) — or a design system that lives in Figma, passed as a reference URL, from which it extracts tokens, component appearance, and the real icons (as SVG) rather than fabricating them. If none is present, detect what's available and offer it, or ask which to adopt; always verify the source before building and never invent it. Triggers on requests to build a screen, page, component, or feature.
---
