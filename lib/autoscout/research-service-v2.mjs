...
import { tennisContextHealth } from '../data-sources/sportradar/tennis-context.mjs';
...
export function researchHealth() {
  return { configured: true, clearSports: clearSportsSeasonHealth(), sportradar: sportradarResearchHealth(), tennisContext: tennisContextHealth(), sportsDataIo: sportsDataIoResearchHealth() };
}
...
async function researchUncached(params = {}) {
  ...
  // Fantasy charts used to fail closed here before the normal provider
  // fallback chain could run. Keep platform-native formulas first, then allow
  // SportsGameOdds to supply only its exact finalized/graded fantasy market
  // score. This is generic across sports and does not change the live line or
  // sportsbook quote displayed on the card.
  const fantasyRequested = `${String(params.providerMarketKey || '')} ${String(params.market || '')}`
    .toLowerCase()
    .includes('fantasy');
  if (fantasyRequested) {
    const sportsGameOdds = await fetchSportsGameOddsResearch(params).catch(() => null);
    if (sportsGameOdds?.available) {
      return finalizePublicHistory(sportsGameOdds, params, {
        fallback: { used: true, primary: 'Platform-native fantasy history', source: sportsGameOdds.source },
      });
    }
  }

  // Keep existing verified-formula failures (including retryable transport
  // errors) intact when no verified fallback can supply a complete sample.
