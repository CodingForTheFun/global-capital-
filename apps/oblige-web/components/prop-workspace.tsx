...
function WorkspaceDetail({sport,eventId,playerKey,categoryKey}:{sport:string;eventId:string;playerKey:string;categoryKey:string}){
 ...
 return <PlayerAnalysisPage analysis={{group:displayGroup,history,line:analysis.line,loading:historyLoading,unavailable,onLineChange:line=>setAnalysis(previous=>({...previous,line}))}} player={player} market={market} selected={selected} side={analysis.side} favourite={favourite} canFollow={!!group} onCategory={setCategory} onBook={selectBook} onOffer={select} onFavourite={toggleFavourite}
  research={displayGroup?<PropExplorer group={displayGroup} games={unavailable?[]:history?.gameLog||[]} loading={historyLoading} unavailableReason={unavailable} currentOpponent={history?.matchup?.opponent ?? null} season={history?.season} hideBookFilter state={analysis} onState={next=>{if(next.side!==analysis.side){const quote=chooseOffer(market,selected.book,selected.line,next.side);if(quote?.side===next.side)setSelected(quote);}setAnalysis({...next,book:selected.book});}} favourite={favourite} onFavourite={toggleFavourite}/>:<p className={styles.status}>This is a {selected.choice} outcome, not a numeric Over/Under line. Its quote is preserved; a numeric history chart is not substituted.</p>}
  historyRetry={historyError?<button className={styles.button} onClick={()=>setRetry(n=>n+1)}>Retry history</button>:null}
