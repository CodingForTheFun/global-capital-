...
const LIMIT_PREF = 'scout-pro:prop-limit';
const LIMITS = new Set([100, 250, 500]);
...
function storedLimit() {
  try {
    ...
    return LIMITS.has(value) ? value : 500;
  } catch { return 500; }
}
...
function readFiltersFromUrl() {
  ...
  return {
    ...
    timeWindow: p.get('timeWindow') || 'TODAY',
    minScore: number('minScore'),
    minHitRate: number('minHitRate'),
    hitRateWindow: p.get('hitRateWindow') || 'l10',
    minProjectionEdge: number('minProjectionEdge'),
    minExpectedMinutes: number('minExpectedMinutes'),
    startersOnly: p.get('startersOnly') === '1',
    excludeInjured: p.get('excludeInjured') === '1',
    applyScoutRules: p.has('rules') ? p.get('rules') === '1' : storedRules(),
  };
}

const urlLimit = Number(paramsAtLoad.get('limit'));
const state = {
  mode: paramsAtLoad.get('mode') === 'best' ? 'best' : 'all',
  sort: paramsAtLoad.get('sort') || 'score-desc',
  filters: readFiltersFromUrl(),
  offset: 0,
  limit: LIMITS.has(urlLimit) ? urlLimit : storedLimit(),
  loading: false,
  lastPayload: null,
  sports: new Set(),
  debounce: null,
...
function providerLabel(data) {
  ...
  if (universe?.sourceMode === 'provider-native') {
    const count = finiteOrNull(universe.providerNative) ?? 0;
    return `SportsDataIO board · ${count.toLocaleString()} source rows`;
  }
  if (universe?.sourceMode === 'pickfinder-fallback') return 'PickFinder fallback board';
  return 'Data source checking';
}
...
async function loadProps({ reset = false, append = false } = {}) {
  ...
  try {
    ...
    for (const row of data?.universe?.providerCoverage || []) {
      if ((finiteOrNull(row.offerCount) ?? 0) > 0 && row.sport) state.sports.add(String(row.sport).toUpperCase());
    }

    renderSportChips();
    renderSummary(data, rows, { append });
    renderProps(rows, { append });
    renderFilterChips();
    syncUrl();

    state.renderedCount = append ? state.renderedCount + rows.length : rows.length;
    const totalFiltered = finiteOrNull(data?.counts?.filtered) ?? state.renderedCount;
    $('showingCount').textContent = state.renderedCount.toLocaleString();
    $('loadMoreBtn').hidden = state.mode === 'best' || state.renderedCount >= totalFiltered || rows.length === 0;
  } catch (error) {
    showNotice(error?.message || 'Props could not be loaded.', true);
    if (reset) $('propGrid').innerHTML = '<div class="empty-state"><b>Prop board unavailable</b>The last valid board could not be loaded. Retry or check provider health.</div>';
  } finally {
    state.loading = false;
...
function renderSummary(data, rows, { append = false } = {}) {
  const counts = data?.counts || {};
  const available = finiteOrNull(counts.matching) ?? 0;
  const qualified = finiteOrNull(counts.qualifiers) ?? 0;
  const filtered = finiteOrNull(counts.filtered) ?? rows.length;
  $('availableCount').textContent = available.toLocaleString();
  $('qualifiedCount').textContent = qualified.toLocaleString();
  $('resultCount').textContent = `${filtered.toLocaleString()} ${filtered === 1 ? 'prop' : 'props'}`;
  $('sourceStatus').textContent = providerLabel(data);
  $('lastScan').textContent = data?.universe?.providerFetchedAt ? fmtTime(data.universe.providerFetchedAt) : (data?.scannedAt ? fmtTime(data.scannedAt) : '—');
  ...
  if (!append && state.filters.applyScoutRules && available > 0 && filtered === 0) {
    showNotice(`${available.toLocaleString()} props are available. 0 currently pass Scout Rules. Turn Rules OFF to browse the full board.`, false, true);
    return;
  }
  if (!append && data?.emptyReason && filtered === 0) {
  ...
  if (!append && data?.universe?.providerError) {
    showNotice('Provider-native props are temporarily degraded. Showing the best available fallback data.', false);
    return;
  }
  $('notice').hidden = true;
}
...
function showNotice(message, isError = false, offerRulesOff = false) {
  ...
  $('viewAllPropsAction')?.addEventListener('click', () => {
    state.filters.applyScoutRules = false;
    savePreferences();
...
function renderSportChips() {
  const selected = new Set((state.filters.sports || []).map((sport) => String(sport).toUpperCase()));
  const sports = [...state.sports].sort();
  $('sportChips').innerHTML = `<button class="quick-chip ${selected.size === 0 ? 'active' : ''}" data-sport="" type="button">All</button>` +
...
function renderProps(rows, { append = false } = {}) {
  ...
  if (!rows.length && !append) {
    grid.innerHTML = '<div class="empty-state"><b>No props to show</b>Change a filter, turn Scout Rules off, or refresh the board.</div>';
    return;
  }
  const markup = renderPropCardsMarkup(rows, { mode: state.mode, offset: state.offset });
  if (append) grid.insertAdjacentHTML('beforeend', markup); else grid.innerHTML = markup;
}
...
function syncControlsFromState() {
  ...
  $('resultLimit').value = String(state.limit);
  renderFilterChips();
  renderSportChips();
}
...
async function openDetail(id) {
  ...
  $('detailBody').innerHTML = '<div class="detail-loading">Loading player intelligence…</div>';
  if (!dialog.open) dialog.showModal();
  try {
    renderDetail(await api(`/api/props/detail?id=${encodeURIComponent(id)}`));
  } catch (error) {
...
function renderDetail(data) {
  ...
  const failures = Array.isArray(prop?.ruleResults?.failures) ? prop.ruleResults.failures : [];
  const source = prop.sourceLabel || prop.sportsbook || prop.provider || 'Source unavailable';

  $('detailBody').innerHTML = `
    <div class="detail-hero"><span class="eyebrow">${esc(prop.sport || 'PROP')} · ${prop.gameStartTime ? esc(fmtDateTime(prop.gameStartTime)) : 'TIME —'}</span><h2>${esc(prop.playerName || 'Player')}</h2><p>${esc(prop.marketDisplayName || prop.market || 'Prop')} ${display(prop.line)} · ${esc(prop.side || '')}${prop.opponent ? ` · vs ${esc(prop.opponent)}` : ''}</p><small>${esc(source)} · updated ${prop.updatedAt ? esc(fmtDateTime(prop.updatedAt)) : '—'}</small></div>
    <div class="detail-score-row"><div class="detail-score"><strong>${display(prop.score)}</strong><span>Scout Score</span></div><div><b>${esc(prop.qualityTier || 'Evidence incomplete')}</b><p>${breakdown.dataCoverage !== null && breakdown.dataCoverage !== undefined ? `${esc(breakdown.dataCoverage)}% model coverage` : 'Coverage unavailable'}</p></div></div>
    <section class="detail-section"><h3>Current intelligence</h3><div class="detail-metrics">
      <div><span>Line</span><b>${display(prop.line)}</b><small>${esc(prop.side || '')}</small></div>
      <div><span>Live stat</span><b>${display(prop.liveStat)}</b><small>${esc(prop.liveStatus || 'Not live')}</small></div>
...
function bind() {
  ...
  $('resultLimit').addEventListener('change', () => {
    ...
    state.limit = LIMITS.has(value) ? value : 500;
    savePreferences();
    loadProps({ reset: true });
  });
  $('searchInput').addEventListener('input', () => {
    ...
    state.debounce = setTimeout(() => loadProps({ reset: true }), 220);
  });
  ...
  $('loadMoreBtn').addEventListener('click', async () => {
    state.offset = state.renderedCount;
    await loadProps({ append: true });
  });
  ...
  $('propGrid').addEventListener('keydown', (event) => {
    if (!['Enter', ' '].includes(event.key)) return;
    const card = event.target.closest('[data-prop-id]');
    if (card) { event.preventDefault(); openDetail(card.dataset.propId); }
  });
