...
var { createIntelligenceUI } = await import('/assets/lib/ui/intelligence-panel.mjs');
var intelligenceUI = null;
...
function shell(){
 document.body.insertAdjacentHTML('beforeend',styles()+`<link rel="stylesheet" href="/assets/autoscout-research.css"><link rel="stylesheet" href="/assets/autoscout-intelligence.css"><div class="as5" id="as5">
 ...
 <section id="asIntelligenceRadar" aria-label="Prop change radar"></section>
 ...
 intelligenceUI=createIntelligenceUI({
  openProp:g=>{openDrawer(g);drawerState.panel='intelligence';renderDrawer();document.getElementById('asTab-intelligence')?.focus();},
  changeLine:value=>{if(!drawerState||value===null)return;drawerState.line=value;renderDrawer();},
  openPanel:panel=>{if(!drawerState)return;drawerState.panel=panel;renderDrawer();document.getElementById('asTab-'+panel)?.focus();},
  loadHistory:ctx=>readStoredLineHistory(ctx.propId,ctx.book,ctx.side),notify:toast,
 });
 document.getElementById('asIntelligenceRadar').dataset.expanded=String(window.matchMedia('(min-width:1000px)').matches);
...
function closeDrawer(){intelligenceUI?.close();document.getElementById('asDrawerBg')?.classList.remove('on');document.body.style.overflow='';document.querySelectorAll('.asMain,.asTop,.asNav').forEach(x=>x.inert=false);drawerState=null;if(lastFocus?.isConnected)lastFocus.focus();}
...
function renderListLight(){
 ...
 intelligenceUI?.renderRadar(cardList(),{sport,stale:payload.meta?.stale===true});
...
async function historyHtml(g,book,side){
 ...
 var j;
 try{j=await readStoredLineHistory(pid,book,side);
...
function intelligenceContext(g,base,line,side){return {group:g,base:base||{},line,side,projection:projectionFor(g,line),propId:propIdForRow(g.rows[0])||g.propId||null,book:drawerState?.historyBook||books(g)[0]};}
async function readStoredLineHistory(pid,book,side){
 if(!pid||!book)return {configured:false,rows:[]};
 var key=[pid,book,side].join('|'),entry=historyCache.get(key);
 if(entry&&entry.expires>Date.now())return entry.promise;
 var promise=nativeFetch('/api/apex/line-history?'+new URLSearchParams({propId:pid,bookmaker:book,side,limit:1000}),{signal:AbortSignal.timeout(15000)}).then(async response=>{if(!response.ok)throw Error('Stored history unavailable');return response.json();});
 historyCache.set(key,{promise,expires:Date.now()+60000});
 while(historyCache.size>100)historyCache.delete(historyCache.keys().next().value);
 return promise;
}
function renderDrawer(){
 ...
 var panel=drawerState.panel||'overview', tabs=[['overview','Overview'],['games','Game log'],['lines','Compare lines'],['sandbox','Scenario'],['intelligence','Intelligence'],['ask','Ask']];
 ...
 var panels={intelligence:panel==='intelligence'?intelligenceUI.render(intelligenceContext(g,r||base,line,side)):'',sandbox:section('Scenario sandbox',sandboxPanel(g,line,side),'Simulated'),
 ...
 var historyBook=document.getElementById('asHistoryBook');if(historyBook){historyBook.onchange=e=>{drawerState.historyBook=e.target.value;loadHistoryIntoDrawer();};loadHistoryIntoDrawer();}if(panel==='intelligence')intelligenceUI.bind(intelligenceContext(g,r||base,line,side));restoreFocus(focused);
}
...
async function load(){
 ...
 try{var response=await nativeFetch('/api/apex/props?sport='+encodeURIComponent(selected),{signal:loadController.signal}),j=await response.json();if(!response.ok||!Array.isArray(j?.props))throw Error();if(generation!==loadGeneration)return;payload=j;payloadSport=selected;intelligenceUI?.observe(groups(),{sport:selected,at:new Date().toISOString(),stale:j.meta?.stale===true,complete:j.meta?.complete===true});}
