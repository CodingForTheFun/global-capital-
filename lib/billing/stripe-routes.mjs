  ...
  return async function handleStripeRoutes(req, res, url) {
    ...
    if (req.method === 'GET' && path === '/api/billing/stripe/config') {
      json(res, 200, {
        ...
        // Reported so setup problems are diagnosable without exposing a key.
    ...
    if (req.method === 'POST' && path === '/api/billing/stripe/checkout') {
      ...
      const auth = await currentAccount(req, sessions).catch(() => ({ user: null }));
      if (!auth?.user?.id) { json(res, 401, { ok: false, code: 'SIGN_IN_REQUIRED' }); return true; }
      if (!csrfValid(req, secret)) { json(res, 403, { ok: false, code: 'CSRF_INVALID' }); return true; }
      ...
      try {
        const session = await createCheckoutSession({
          ...
          // A double-clicked button must not open two subscriptions.
          idempotencyKey: `checkout:${auth.user.id}:${Math.floor(Date.now() / 60_000)}`,
        });
    ...
    if (path === '/api/billing/stripe/webhook') {
      ...
      try {
        ...
        let subscriptionId = text(object?.subscription || (type.startsWith('customer.subscription') ? object?.id : ''));
