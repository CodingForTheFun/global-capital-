...
export function startFrugalPersistence() {
  ...
  console.log(`[AutoScout persistence] single public-feed scheduler, interval=${ingestSeconds} seconds, sports-per-cycle=${perCycle}/${PUBLIC_PERSISTENCE_SPORTS.length}, snapshot-interval=${snapshotSeconds} seconds, primary-paid-refresh=off, selected-provider-cache=enabled, propline-supplement=mode-gated, sportradar-supplement=mode-gated, sportsgameodds-gap-fill=usage-aware`);
  return timer;
}
