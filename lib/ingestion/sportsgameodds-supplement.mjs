...
const DEFAULT_SPORTS = Object.freeze(['NFL','NBA','WNBA','MLB','NHL','NCAAF','NCAAB','TENNIS','SOCCER']);
...
export function sportsGameOddsSupplementPolicy(usage = sportsGameOddsHealth().usage) {
...
export async function maybeRefreshSportsGameOddsSupplement({ now = Date.now } = {}) {
  ...
  try {
    ...
    if (!sport) {
      state.nextAt = current + Math.min(15 * 60, policy.intervalSeconds) * 1000;
...
export function mergeCachedSportsGameOdds(board, sport, { now = Date.now } = {}) {
  ...
  const baseProps = rawBaseProps.map((row) => {
    ...
    return source ? overlayMetadata(row, source) : row;
  });
...
export function sportsGameOddsIdentityFor({ sport, playerName, team = '', gameStartTime = '' } = {}) {
  const selected = text(sport).toUpperCase();
