...
const PUBLIC_PERSISTENCE_SPORTS = Object.freeze([...AUTOMATIC_SPORTS, 'SOCCER','MLS','EPL','UCL','TENNIS']);
  ...
  try {
    ...
    if (publicWorkerConfigured()) {
      try {
        ...
        deferDatabaseWrites = !publicClaimed;
        ...
        // A failed lease/health probe means the database is already degraded.
    ...
    if (deferDatabaseWrites) {
      console.log(`[AutoScout persistence] ${reason} database-degraded-or-unclaimed; canonical board and retention writes deferred`);
      return;
    }
