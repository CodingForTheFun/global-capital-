...
Feature values use only each athlete's earlier completed games. Hyperparameters are
selected before a final chronological holdout. A cell publishes only when clustered
holdout evidence establishes lower MSE than the same player's rolling-10 mean.
A malformed/sparse market is withheld independently instead of disabling the sport.
"""
...
def fit_market(sport,market,rows):
    ...
    events=int(hold.gameId.nunique())
    if min(len(dev),len(sel),len(hold))<180 or events<50:return None,{'marketId':market,'status':'INSUFFICIENT_HOLDOUT','development':len(dev),'selection':len(sel),'heldout':len(hold),'heldoutEvents':events}
    choices=[]
    for alpha in [10.,50.,200.,800.]:
        p=make_pipeline(SimpleImputer(strategy='median',keep_empty_features=True),StandardScaler(),Ridge(alpha=alpha));p.fit(dev[FEATURES],dev.value);pred=p.predict(sel[FEATURES]);choices.append((metric(sel.value.to_numpy(),pred),alpha))
    alpha=min(choices)[1];pipe=make_pipeline(SimpleImputer(strategy='median',keep_empty_features=True),StandardScaler(),Ridge(alpha=alpha));pipe.fit(train[FEATURES],train.value);pred=pipe.predict(hold[FEATURES]);act=hold.value.to_numpy();base=hold.baseline.to_numpy();rmse=metric(act,pred);brmse=metric(act,base);ci=cluster_ci(act,pred,base,hold.gameId.to_numpy())
    gate=bool(ci and np.isfinite(rmse) and np.isfinite(brmse) and brmse>0 and rmse<brmse and ci[1]<0)
    evidence={'marketId':market,'status':'PASSED' if gate else 'WITHHELD','trainingRows':len(train),'trainingEvents':int(train.gameId.nunique()),'heldoutRows':len(hold),'heldoutEvents':events,'rmse':rmse,'baselineRmse':brmse,'improvement':1-rmse/brmse if brmse else 0,'mseDelta95':ci,'alpha':alpha,'holdoutStart':hold.date.min().isoformat(),'holdoutEnd':hold.date.max().isoformat()}
    ...
    model={'id':f'{sport.lower()}-{market}-ridge-v1','marketId':market,'statKey':'value','features':FEATURES,'imputerMedian':np.asarray(imp.statistics_).round(10).tolist(),'mean':np.asarray(sc.mean_).round(10).tolist(),'scale':np.asarray(sc.scale_).round(10).tolist(),'coef':np.asarray(rg.coef_).round(12).tolist(),'intercept':round(float(rg.intercept_),12),'trainingRows':len(train),'trainingEvents':int(train.gameId.nunique()),'validation':{'method':'chronological-stat-holdout','observations':len(hold),'events':events,'start':hold.date.min().isoformat(),'end':hold.date.max().isoformat(),'rmse':round(rmse,6),'baselineRmse':round(brmse,6),'rmseImprovement':round(1-rmse/brmse,6),'mseDelta95':[round(x,6) for x in ci],'residualQuantiles':q}}
    return model,evidence

def main():
    ap=argparse.ArgumentParser();ap.add_argument('--input',required=True);ap.add_argument('--output',required=True);args=ap.parse_args();src=json.loads(Path(args.input).read_text());sport=src['stats']['sport'];df=pd.DataFrame(src.get('records') or []);models=[];evidence=[]
    if not df.empty:
        df['date']=pd.to_datetime(df.date,utc=True,errors='raise');df['value']=pd.to_numeric(df.value,errors='raise')
        for market,rows in df.groupby('marketId'):
            try:model,ev=fit_market(sport,market,rows.copy())
            except Exception as exc:
                model=None;ev={'marketId':market,'status':'TRAINING_ERROR','errorType':type(exc).__name__,'message':str(exc)[:240]};traceback.print_exc()
            evidence.append(ev)
