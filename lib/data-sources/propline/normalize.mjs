...
export function normalizeEventOdds(payload, { sport = null, ingestedAt = new Date().toISOString(), provider = 'propline' } = {}) {
  ...
  // PropLine documents names as display strings, not durable join keys. Keep
  // its canonical team/event identity metadata alongside the normalized
  // contract so callers can survive spelling corrections and event merges.
  if (text(payload?.home_team_key)) event.homeTeamKey = text(payload.home_team_key);
  if (text(payload?.away_team_key)) event.awayTeamKey = text(payload.away_team_key);
  if (text(payload?.home_team_id)) event.homeTeamProviderId = text(payload.home_team_id);
  if (text(payload?.away_team_id)) event.awayTeamProviderId = text(payload.away_team_id);
  if (text(payload?.home_team_logo_url)) event.homeTeamLogoUrl = text(payload.home_team_logo_url);
  if (text(payload?.away_team_logo_url)) event.awayTeamLogoUrl = text(payload.away_team_logo_url);
  if (text(payload?.espn_event_id)) event.espnEventId = text(payload.espn_event_id);
  if (Array.isArray(payload?.merged_from_event_ids) && payload.merged_from_event_ids.length) {
    event.mergedFromProviderEventIds = payload.merged_from_event_ids.map(text).filter(Boolean);
  }
    ...
    for (const market of list(bookmaker?.markets)) {
      ...
      for (const outcome of outcomes) {
        try {
          ...
          if (text(outcome?.dfs_odds_type)) row.dfsOddsType = text(outcome.dfs_odds_type).toLowerCase();
          if (text(outcome?.outcome_id)) row.providerOutcomeId = text(outcome.outcome_id);
          if (text(outcome?.book_outcome_id)) row.bookOutcomeId = text(outcome.book_outcome_id);
          if (text(bookmaker?.book_event_id)) row.bookEventId = text(bookmaker.book_event_id);
          const payoutMultiplier = numberOrNull(outcome?.payout_multiplier);
          if (payoutMultiplier !== null) row.payoutMultiplier = payoutMultiplier;
          const lineGap = numberOrNull(outcome?.line_gap);
          if (lineGap !== null) row.lineGap = lineGap;
          const liquidity = numberOrNull(outcome?.liquidity);
          if (liquidity !== null) row.liquidity = liquidity;
          if (text(outcome?.liquidity_updated_at)) row.liquidityUpdatedAt = text(outcome.liquidity_updated_at);
          if (text(outcome?.book_updated_at)) row.bookUpdatedAt = text(outcome.book_updated_at);
          if (text(outcome?.last_change_at)) row.lastChangeAt = text(outcome.last_change_at);
          if (text(outcome?.app_link || bookmaker?.app_link)) row.appLink = text(outcome?.app_link || bookmaker?.app_link);
