  ...
  for (const group of payload?.seasonTypes || []) {
    for (const category of group?.categories || []) {
      ...
      for (const event of category?.events || []) {
        ...
        const values = Object.fromEntries(names.map((name, index) => [name, event.stats[index]]));
        // Football game logs can expose passing, rushing, receiving and return
        // categories separately for the same event. Merge them so an exact
        // fantasy formula can see every verified component instead of allowing
        // the last category to overwrite the others.
        rows.set(eventId, { ...(rows.get(eventId) || {}), ...values });
      }
    }
  }
  return rows;
