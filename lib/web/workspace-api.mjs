import {normalizeEv,normalizeProjections} from '../data-sources/propline/insights.mjs';
...
async function publicHistory(params) {
  ...
  return fetchPublicResearch(params);
}
async function sportsGameOddsHistory(params) {
  const {fetchSportsGameOddsResearch} = await import('../data-sources/sportsgameodds/research.mjs');
  return fetchSportsGameOddsResearch(params);
}
async function wnbaArchiveHistory(params) {
  const {fetchWnbaStatsArchiveResearch} = await import('../data-sources/sportsdataverse/wnba-game-research.mjs');
  return fetchWnbaStatsArchiveResearch(params);
}
async function nflverseHistory(params) {
  const {fetchNflverseResearch} = await import('../data-sources/nflverse/nfl-game-research.mjs');
  return fetchNflverseResearch(params);
}
async function cfbArchiveHistory(params) {
  const {fetchSportsDataverseCfbResearch} = await import('../data-sources/sportsdataverse/cfb-game-research.mjs');
  return fetchSportsDataverseCfbResearch(params);
}
...
export function normalizeMarketReference(projectionPayload,evPayload,player,market,offer) {
  if (market.period || market.variant !== 'standard' || offer.conflict || offer.line === null || (offer.multiplier !== null && offer.multiplier !== 1)) return null;
  const samePlayer=row=>{
    if (player.playerId && row.playerId) return text(player.playerId) === text(row.playerId);
    return nameKey(row.playerName) === nameKey(player.name);
  };
  const sameMarket=row=>text(row.marketKey).toLowerCase()===market.marketKey.toLowerCase();
  const projections=normalizeProjections(projectionPayload).projections.filter(row=>samePlayer(row)&&sameMarket(row));
  // Conflicting source records are unavailable, never an arbitrary first row.
  const projectionRow=projections.length && new Set(projections.map(row=>row.projection)).size===1 ? projections[0] : null;
  const projection=projectionRow?.projection??null;
  const evRows=offer.dfs?[]:normalizeEv(evPayload).plays.filter(row=>samePlayer(row)&&sameMarket(row)
    && row.line===offer.line && row.side===offer.side && row.bookmakerKey===offer.book
    && row.price!==null && row.price===offer.price && row.evPercent!==null);
  const exact=evRows.length && new Set(evRows.map(row=>JSON.stringify([row.evPercent,row.fairProbability]))).size===1 ? evRows[0] : null;
  if(projection===null&&!exact)return null;
  return {
    available:true,projection,
    evPercent:exact?.evPercent??null,
    fairProbability:exact?.fairProbability??null,
    fairPrice:exact?.fairPrice??null,
    bookmaker:exact?.bookmakerKey??null,
    side:offer.side,line:offer.line,price:offer.price,
    updatedAt:exact?.updatedAt||projectionRow?.updatedAt||null,
    booksContributing:projectionRow?.booksContributing??null,
    basis:'PropLine market-implied / no-vig',
  };
}

export function createWorkspaceHandler({authenticate:auth=authenticate,read:provider=read,predict:prediction=predict,sportsGameOddsHistoryFallback=sportsGameOddsHistory,historyFallback=publicHistory,wnbaHistoryFallback=wnbaArchiveHistory,nflHistoryFallback=nflverseHistory,ncaafHistoryFallback=cfbArchiveHistory,now=Date.now}={}) {
  ...
  return async function handleWorkspace(req,res) {
    ...
    try {
      ...
      if(action==='history') {
        ...
        if(primary.available) { send(res,200,{ok:true,...primary});return true; }
        const historyParams={
          ...
          eventId:player.eventId,games:20,now:now(),
        };
        const attemptedSources=['PropLine'];
        let sportsGameOdds=null;
        try { sportsGameOdds=await sportsGameOddsHistoryFallback(historyParams); } catch { sportsGameOdds=null; }
        if(sportsGameOdds) attemptedSources.push('SportsGameOdds');
        if(sportsGameOdds?.available===true&&Array.isArray(sportsGameOdds.gameLog)&&sportsGameOdds.gameLog.length) {
          send(res,200,{ok:true,...sportsGameOdds,sourceProvider:sportsGameOdds.source||'SportsGameOdds results',fallback:true,primaryCode:primary.code,attemptedSources});return true;
        }
        // Period and fantasy markets must never borrow full-game/public-stat
        // history. SportsGameOdds above is allowed only because it requires the
        // exact player + market + period (and graded fantasy score when needed).
        if(market.period||/fantasy/i.test(market.marketKey)) {
          send(res,200,{ok:true,...primary,attemptedSources});return true;
        }
        attemptedSources.push('ESPN');
        let fallback=null;
        try { fallback=await historyFallback(historyParams); } catch { fallback=null; }
        if(fallback?.available===true&&Array.isArray(fallback.gameLog)&&fallback.gameLog.length) {
          send(res,200,{ok:true,...fallback,sourceProvider:'ESPN',fallback:true,primaryCode:primary.code,attemptedSources});return true;
        }
        if(historyParams.sport==='WNBA') {
          attemptedSources.push('WNBA Stats archive');
          let wnbaArchive=null;
          try { wnbaArchive=await wnbaHistoryFallback(historyParams); } catch { wnbaArchive=null; }
          if(wnbaArchive?.available===true&&Array.isArray(wnbaArchive.gameLog)&&wnbaArchive.gameLog.length) {
            send(res,200,{ok:true,...wnbaArchive,sourceProvider:wnbaArchive.source||'WNBA Stats archive',fallback:true,primaryCode:primary.code,attemptedSources});return true;
          }
        }
        if(historyParams.sport==='NFL') {
          attemptedSources.push('nflverse');
          let nflArchive=null;
          try { nflArchive=await nflHistoryFallback(historyParams); } catch { nflArchive=null; }
          if(nflArchive?.available===true&&Array.isArray(nflArchive.gameLog)&&nflArchive.gameLog.length) {
            send(res,200,{ok:true,...nflArchive,sourceProvider:nflArchive.source||'nflverse',fallback:true,primaryCode:primary.code,attemptedSources});return true;
          }
        }
        if(historyParams.sport==='NCAAF') {
          attemptedSources.push('SportsDataverse CFB');
          let cfbArchive=null;
          try { cfbArchive=await ncaafHistoryFallback(historyParams); } catch { cfbArchive=null; }
          if(cfbArchive?.available===true&&Array.isArray(cfbArchive.gameLog)&&cfbArchive.gameLog.length) {
            send(res,200,{ok:true,...cfbArchive,sourceProvider:cfbArchive.source||'SportsDataverse CFB',fallback:true,primaryCode:primary.code,attemptedSources});return true;
          }
        }
        send(res,200,{ok:true,...primary,attemptedSources});return true;
      }
      ...
      if(!valid&&!offer.conflict&&offer.line!==null&&!market.period&&market.variant==='standard'&&(offer.multiplier===null||offer.multiplier===1)) {
        ...
        if(marketReference)marketReference.expiresAt=new Date(now()+60000).toISOString();
      }
