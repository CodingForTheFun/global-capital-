...
import { gateAskQuestion, OFF_TOPIC_MESSAGE } from '../jev/ask-gate.mjs';
...
export async function askAboutProp({ question, prop = {}, history = [], fetchImpl = (...args) => fetch(...args), jevFetchImpl = (...args) => fetch(...args) } = {}) {
  ...
  // Cheap Jev pre-check: a clearly off-topic question is declined here instead
  // of spending a paid model request. Fails open — see lib/jev/ask-gate.mjs.
  const gate = await gateAskQuestion({ question: text, payload, turns }, { fetchImpl: jevFetchImpl });
  if (gate.decline) return unavailable('ASK_OFF_TOPIC', OFF_TOPIC_MESSAGE);
