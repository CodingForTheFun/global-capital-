...
export async function errorReason(response) {
  ...
  return detail?.reason || error.status || null;
}
...
export async function generate({ system, payload, schema, timeoutMs = 150_000, fetchImpl = (...args) => fetch(...args) } = {}) {
    ...
    response = await fetchImpl(ENDPOINT, {
      ...
      body: JSON.stringify({
        ...
        response_format: {
          ...
          schema: toResponseSchema(schema),
        },
      }),
  ...
  if (!response.ok) {
    // An invalid key comes back as 400 INVALID_ARGUMENT here, not 401 or 403.
    // Verified against the live endpoint — mapping it to a generic provider
    // error would tell users the feature is "temporarily unavailable" forever
    // while hiding a misconfiguration only the operator can fix.
    ...
    // Status and reason code only. An error body can echo the request back.
    console.error('[projections] gemini error', String(response.status).slice(0, 4), String(reason || '').slice(0, 40));
    return { ok: false, code: 'PROJECTION_PROVIDER_ERROR' };
  }
  ...
  if (!parsed) {
    // A safety block returns 200 with no usable text. Treated as a decline
    // rather than an error, because nothing went wrong on the wire.
