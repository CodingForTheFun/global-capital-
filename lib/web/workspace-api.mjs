...
async function publicHistory(params) {
  ...
  const start=Date.parse(params?.gameStartTime||''),clock=Number(params?.now);
  if(Number.isFinite(start)&&Number.isFinite(clock)&&start<=clock) {
    const identity=await fetchPublicResearch.eventIdentity(params);
    ...
    return {...history,selectedSourceGameId:identity.gameId,selectedSourceEventId:identity.sourceEventId};
  }
  return {...history,selectedSourceGameId:null,selectedSourceEventId:null};
}
