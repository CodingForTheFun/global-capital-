...
// Keep the full live working set in memory. Disk only needs a compact warm-start
// tail; persisting all 37k objects every half-second is wasteful and can fill a
// small Railway volume even though the stream itself is healthy.
const PERSIST_EVENTS = 160;
const PERSIST_QUOTES = 1_200;
const PERSIST_SEEN = 1_000;
const EMERGENCY_EVENTS = 24;
const EMERGENCY_QUOTES = 160;
const EMERGENCY_SEEN = 128;
...
function persistenceSnapshot({ emergency = false } = {}) {
  return {
    version: 1,
    cursor,
    latestAt,
    persistedAt,
    events: events.slice(0, emergency ? EMERGENCY_EVENTS : PERSIST_EVENTS),
    seen: seen.slice(0, emergency ? EMERGENCY_SEEN : PERSIST_SEEN),
    quotes: [...quotes.values()].slice(-(emergency ? EMERGENCY_QUOTES : PERSIST_QUOTES)),
  };
}
...
function persistNow() {
  ...
  try {
    ...
    writeFileSync(tmp, JSON.stringify(persistenceSnapshot()), { encoding: 'utf8', mode: 0o600 });
    renameSync(tmp, STATE_PATH);
    persistSuccessLogs += 1;
    if (persistSuccessLogs <= 3) {
      console.log(`[market-core] persist ok cycle=${persistSuccessLogs} sequence=${cursor} events=${Math.min(events.length,PERSIST_EVENTS)} quotes=${Math.min(quotes.size,PERSIST_QUOTES)} persistedAt=${persistedAt}`);
    }
  } catch (error) {
    const code = text(error?.code || 'WRITE_FAILED').slice(0, 60);
    try { rmSync(tmp, { force: true }); } catch {}
    if (code === 'ENOSPC') {
      // If atomic replacement cannot allocate a second file, shrink only this
      // rebuildable cache in place. Durable customer/history storage is untouched.
      try {
        persistedAt = new Date().toISOString();
        writeFileSync(STATE_PATH, JSON.stringify(persistenceSnapshot({ emergency: true })), { encoding: 'utf8', mode: 0o600 });
        console.log('[market-core] state compacted after ENOSPC');
        return;
      } catch (fallbackError) {
        console.log(`[market-core] persist failed code=${text(fallbackError?.code || code).slice(0, 60)}`);
        return;
      }
    }
    console.log(`[market-core] persist failed code=${code}`);
  }
}
...
function restore() {
  ...
  try {
    ...
    console.log(`[market-core] restored sequence=${cursor} events=${events.length} quotes=${quotes.size} persistedAt=${persistedAt||'none'}`);
  } catch {}
}
restore();
