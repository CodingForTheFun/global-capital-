...
export function AnalysisGameLog({games,line,side,sport,market,loading}:{games:GameLogRow[];line:number;side:Side;sport:string;market:string;loading?:boolean}) {
  ...
  const columns=[['date','Date'],['opponent','Matchup'],['gameResult','Result'],...((['basketball','hockey'].includes(sportFamily(sport))||recent.some(g=>finite(g.minutes)!==null))?[['minutes','MIN']]:[]),...gameColumns(sport,recent).map(key=>[key,statLabels[key]]),['value',market]];
