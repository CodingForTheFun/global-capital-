...
import { fetchPinnaclePublic, pinnacleSupportedSports } from './pinnacle-public-compat.mjs';
...
import { fetchDraftKingsSportsbookPublic, draftKingsSportsbookSupportedSports } from './draftkings-controldata-public.mjs';
...
async function ingest(book, sport, fetcher, source = `${book}:${sport}`) {
  ...
  try {
    ...
    // longer in-process cache. The sources below are zero-credit public feeds.
    const snapshot = await fetcher(sport, { force: true });
