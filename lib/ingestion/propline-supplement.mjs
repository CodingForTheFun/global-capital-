...
function prioritizeProplineQuote(row, source) {
  if (!row || !source) return row;
  // Quote/value fields come from PropLine, but ObligeProps' canonical entity
  // joins stay stable. Native provider identity is attached separately through
  // propline* / provider* metadata so saved props, history and player research
  // do not jump to a different entity merely because the preferred provider
  // supplied a newer line.
  const merged = {
    ...row,
    ...
    isAlternate: false,
    // Preserve canonical ObligeProps joins.
...
export function mergeCachedPropline(board, sport, { now = Date.now } = {}) {
    ...
    if (row?.isAlternate !== true) {
      ...
      if (primary) {
        if (!sameQuote(row, primary)) {
          prioritized += 1;
          return prioritizeProplineQuote(merged, primary);
        }
  ...
  // Rebuilt rows come last so the canonical ids represented by the exact flat
  // customer rows win any id collision with a provider-native normalized row.
