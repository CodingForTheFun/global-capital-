 ...
 */
export function analyzeLineHistory(rows = [], { now = Date.now(), windowDays = 7, readable = null } = {}) {
  ...
  // Reading zero rows is only evidence of a stopped writer when the reader was
  // actually allowed to see rows. RLS on line_snapshots grants select to the
  // `authenticated` role, so an anon-key read returns an empty set rather than
  // an error -- identical, from here, to a genuinely empty table. Saying
  // "failing" on that would be inventing a result, so it is reported as its own
  // state and the caller decides.
  const unverifiable = !usable.length && readable === false;

  return {
    ok: failures.length === 0 && !unverifiable,
    unverifiable,
    failures: unverifiable ? ['the configured credential cannot read line_snapshots, so zero rows proves nothing'] : failures,
    windowDays,
    ...
    provenForWindow: !unverifiable && daysCovered >= windowDays && longestSpanMs >= (windowDays - 1) * DAY,
  };
}
...
export function formatLineHistoryReport(report) {
  const lines = [];
  const verdict = report.unverifiable ? 'UNVERIFIABLE' : report.ok ? 'HEALTHY' : 'FAILING';
  lines.push(`Line history — ${verdict} (window ${report.windowDays}d, evaluated ${report.evaluatedAt})`);
  lines.push('');
  ...
  if (!report.ok) {
    lines.push('');
    lines.push(report.unverifiable ? '  unverifiable because:' : '  failing because:');
