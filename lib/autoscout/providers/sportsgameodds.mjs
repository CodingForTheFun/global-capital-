...
export async function fetchSportsGameOddsBoard(sport, options = {}) {
  ...
  const task = (async () => {
    ...
    try {
      ...
      let payload;
      let tennisLeagueFallback = false;
      try {
        payload = await sportsGameOddsGet('/events', standardParams, eventOptions);
      } catch (error) {
        if (selected !== 'TENNIS' || error?.code !== 'SPORTSGAMEODDS_HTTP_400') throw error;

        // The official Tennis examples use a minimal per-tour request. If the
        // broader production filter set is rejected for Tennis, retry only the
        // entitled tours with documented parameters. This also isolates a
        // single bad/unsupported tour instead of losing all Tennis enrichment.
        tennisLeagueFallback = true;
        const merged = new Map();
        for (const leagueID of requestedLeagues) {
          try {
            const page = await sportsGameOddsGet('/events', {
              leagueID,
              oddsAvailable: 'true',
              includeAltLines: includeAlternates ? 'true' : undefined,
              limit: eventLimit,
            }, eventOptions);
            for (const row of list(page?.data)) {
              const id = text(row?.eventID || row?.id);
              if (id) merged.set(id, row);
            }
          } catch (leagueError) {
            if (leagueError?.code !== 'SPORTSGAMEODDS_HTTP_400') throw leagueError;
          }
        }
        payload = { success: true, data: [...merged.values()] };
      }
