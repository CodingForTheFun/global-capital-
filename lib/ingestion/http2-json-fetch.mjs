...
export async function http2JsonFetch(url, { timeoutMs = 15_000, headers = {} } = {}) {
  ...
  try {
    const result = await new Promise((resolve, reject) => {
      ...
      req.on('data', chunk => {
        ...
        if (total > MAX_BYTES) {
          ...
          reject(Object.assign(new Error('PUBLIC_HTTP2_TOO_LARGE'), { code: 'PUBLIC_HTTP2_TOO_LARGE' }));
          return;
        }
    ...
    if (decoded.length > MAX_BYTES) throw Object.assign(new Error('PUBLIC_HTTP2_TOO_LARGE'), { code: 'PUBLIC_HTTP2_TOO_LARGE' });
