...
export default function SportsWorkspace() {
  const [boardMode, setBoardMode] = useState<'games'|'props'|'tacos'>('games');
  ...
  useEffect(() => {
    const syncHash = () => { const id = window.location.hash.slice(1); setView(VIEWS.some(v => v.id === id) ? id as View : 'sports'); setPage(1); if(id==='props'||id==='tacos')setBoardMode(id); else if(id==='sports'||!id)setBoardMode('games'); };
    ...
    <div className="mx-auto max-w-[1700px] px-3 py-4 pb-24 sm:px-5 lg:px-6">
      ...
      <div className="grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1fr)_330px] xl:grid-cols-[minmax(0,1fr)_360px]">
        <main id="workspace-main" className={`min-w-0 ${boardMode==='games'&&(view==='sports'||view==='live')?'lg:col-span-2':''}`}>
          ...
          {(view==='sports'||view==='live') && <div className="mb-5 flex gap-2 overflow-x-auto" aria-label="Market board type">{([['games','Game lines'],['props','Player props'],['tacos','🌮 Tacos']] as const).map(([id,label])=><button key={id} aria-pressed={boardMode===id} onClick={()=>{setBoardMode(id);window.history.replaceState(null,'',window.location.pathname+'#'+(id==='games'?'sports':id));}} className={`whitespace-nowrap rounded-xl px-4 py-3 text-xs font-bold transition ${boardMode===id?'bg-emerald-500 text-[#06130f]':'border border-slate-700 text-slate-400 hover:text-white'}`}>{label}</button>)}</div>}
