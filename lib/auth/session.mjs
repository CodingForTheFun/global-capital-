...
export function createAccountSessions({ secret, ttlMs = DEFAULT_TTL_MS, now = Date.now } = {}) {
   ...
   */
  function issue(userId, sessionVersion, sessionId = '', options = {}) {
    ...
    const tokenTtlMs = Number.isFinite(requestedTtlMs) && requestedTtlMs > 0 ? requestedTtlMs : ttlMs;
    const expires = now() + tokenTtlMs;
 ...
 * cross-site POSTs still cannot ride it. Hosted production always sets Secure,
 * even if a reverse proxy omits or rewrites x-forwarded-proto.
 */
export function accountCookie(req, token, { ttlMs = DEFAULT_TTL_MS } = {}) {
  const secure = secureCookieRequest(req);
  return `${ACCOUNT_COOKIE}=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Lax; Priority=High; Max-Age=${Math.floor(ttlMs / 1000)}${secure ? '; Secure' : ''}`;
}

export function clearAccountCookie(req) {
  const secure = secureCookieRequest(req);
  return `${ACCOUNT_COOKIE}=; Path=/; HttpOnly; SameSite=Lax; Priority=High; Max-Age=0${secure ? '; Secure' : ''}`;
}
