...
async function loadRawSeasons(history, spec, params, options) {
  ...
  if (!family || !league || !id) return new Map();
...
export async function fetchFantasyResearch(params = {}, options = {}) {
  ...
  const raw = await loadRawSeasons(history, spec, params, options);
  const scored = [];
  let excluded = 0;
  ...
  if (!scored.length) {
    return {
      ...
      message: 'No completed games returned every component required by this platform’s fantasy scoring formula.',
      gameLog: [],
  ...
  return {
    ...
    coverage: {
      ...(history.coverage || {}),
      seasonComplete: history.coverage?.seasonComplete === true && excluded === 0,
      fantasyGamesScored: scored.length,
      fantasyGamesExcluded: excluded,
      ...(excluded ? { partial: true, reason: `${excluded} games excluded because required fantasy scoring components were missing.` } : {}),
    },
  };
}
