...
async function loadStats(season,{now=Date.now,cacheEnabled=true,...deps}={}){
  ...
  const required=['player_id','player_display_name','season','week','team','opponent_team'];
...
function chooseIdentity(rows,params){
  ...
  if(!team)return null;
  const teamIds=[...new Set(named.filter(r=>samePublicTeam(r.team,team,'NFL')).map(r=>text(r.player_id)).filter(Boolean))];
...
function findGame(row,index){
  ...
  return index.byMatch.get(JSON.stringify([Number(row.season),Number(row.week),text(row.team),text(row.opponent_team)]))||null;
}
...
function normalizeRows(rows,params,fields,schedule,before){
  ...
  for(const row of rows){
    ...
    const team=text(row.team),away=text(game.away_team),home=text(game.home_team);
