...
import { PROPLINE_BASE, proplineConfigured, proplineGet, proplineNoteQuota, proplineQuota, proplineReserve, proplineTrafficEnabled } from './client.mjs';
...
function assertProplineTrafficEnabled() {
  if (!proplineTrafficEnabled()) {
    throw Object.assign(new Error('PropLine traffic is disabled by the active provider mode.'), { code: 'PROPLINE_DISABLED_BY_PROVIDER_MODE' });
  }
}
...
// PropLine explicitly documents /freshness as no-auth. Keep that one entirely
// outside the paid allowance; other "Free" endpoints still run through the
// authenticated shared client so their free-tier semantics are unambiguous.
async function publicGet(path, params = {}, ttlSeconds = 60) {
  assertProplineTrafficEnabled();
...
export async function fetchDfsPayouts({ platform = 'prizepicks', legWinProb = null } = {}) {
  const p = await proplineGet('/v1/dfs/payouts', { platform: text(platform).toLowerCase() || 'prizepicks', leg_win_prob: legWinProb === null || legWinProb === '' ? undefined : Number(legWinProb) }, { ttlSeconds: 3600 });
...
export async function fetchMarketHitRates({ days = 28, bookmaker = 'bovada' } = {}) {
  const p = await proplineGet('/v1/markets/hit-rates', { days: clamp(days, 28, 1, 60), bookmaker: text(bookmaker).toLowerCase() || 'bovada' }, { ttlSeconds: 600 }), source = p?.markets, markets = Object.create(null);
...
export async function fetchResolutionSummary({ days = 30 } = {}) {
  const p = await proplineGet('/v1/markets/resolution-summary', { days: clamp(days, 30, 1, 90) }, { ttlSeconds: 3600 });
...
export function startProplineFreshnessMonitor({ intervalMs = 300_000 } = {}) {
  if (!proplineTrafficEnabled()) {
    console.log('[PropLine freshness] disabled by provider mode');
    return null;
  }
