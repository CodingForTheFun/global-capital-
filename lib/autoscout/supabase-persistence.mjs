...
function mapBoard(board, { includeSnapshots = true } = {}) {
  ...
  const snapshotRows = includeSnapshots
    ? lines.map((row) => ({ prop_id: row.propId, bookmaker_key: row.bookmakerKey, side: row.side, line: row.line, price: row.price, provider_updated_at: iso(row.providerUpdatedAt), ingested_at: iso(row.ingestedAt) }))
    : [];
