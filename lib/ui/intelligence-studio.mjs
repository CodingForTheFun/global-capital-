...
const STYLE=`
#as5 .asDrawer .asResearchTabs{max-width:100%;min-width:0;overflow-x:auto;overscroll-behavior-x:contain;scrollbar-width:thin}
#as5 .asDrawer .asResearchTab{flex:0 0 auto;white-space:nowrap}
...
export function mountBoard({ root, onOpen }) {
  ...
  root.querySelector('.asHero')?.style.setProperty('display','none','important');
  boardRoot?.remove();
...
function sensitivityHTML(base,line,side,window,venue) {
  ...
  if(!map.available)return selector+empty('No verified sample is available for this selection. Change the research window or check the Results tab.');
...
export function renderDetails(root,{group:g,base={},line,side,venue='all',getHistory}){
  ...
  const hasScenario=scenarioLab(base,{sport:g.sport,targetMinutes:30,usagePercent:0}).available;
  const hasDependencies=playerDependencies(base).some(row=>row.difference!==null);
  const kinds=[['sensitivity','Line sensitivity'],['quality','Data quality'],['scenario','Scenario lab'],['disagreement','Book disagreement'],['timeline','Timeline'],['dependencies','Player dependencies'],['brief','Research brief'],['radar-detail','Change radar']].filter(([id])=>(id!=='scenario'||hasScenario)&&(id!=='dependencies'||hasDependencies));
  ...
  root.innerHTML=`<p class="asi-eyebrow">Research, with its workings shown</p><h2>Intelligence studio</h2><p class="asi-lede">${esc(g.playerName)} · ${esc(g.market)} · ${esc(side)} ${fmt(line)}. These tools reuse your existing board and game logs.</p><div class="asi-menu" aria-label="Intelligence tools">${kinds.map(([id,label])=>`<button data-asi-section="${id}">${label}</button>`).join('')}</div><div class="asi-grid">`+
    ...
    (hasScenario?panel('scenario','Scenario Lab',scenarioHTML(base,g,state),{wide:true,badge:'Explicit what-if assumptions'}):'')+
    ...
    (hasDependencies?panel('dependencies','Player Dependency Graph',dependencyHTML(base,g),{wide:true,badge:'Measured association only'}):'')+
  ...
  if(hasScenario)root.querySelector('#asi-scenario-form').onsubmit=event=>{event.preventDefault();state.minutes=root.querySelector('#asi-minutes').value;state.usage=root.querySelector('#asi-usage').value;root.querySelector('#asi-scenario-result').innerHTML=scenarioResultHTML(scenarioLab(base,{sport:g.sport,targetMinutes:state.minutes,usagePercent:state.usage}));};
