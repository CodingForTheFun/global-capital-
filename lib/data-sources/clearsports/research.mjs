...
import { clearSportsStatAliases } from './season-research.mjs';
...
// Reuse the provider's established stat schema for both season and game rows.
const FIELD_ALIASES = Object.fromEntries(Object.entries(LEGACY_FIELD_ALIASES).map(
  ([field, aliases]) => [field, [...new Set([...aliases, ...(clearSportsStatAliases[field] || [])])]],
));
...
function unwrapRows(payload) {
  ...
  const candidates = ['data', 'results', 'player_stats', 'playerStats', 'stats', 'players', 'items',
    'football_player_stats', 'basketball_player_stats', 'hockey_player_stats',
    'games', 'football_games', 'basketball_games', 'hockey_games',
    'injury_stats', 'injuries', 'football_injury_stats', 'basketball_injury_stats', 'hockey_injury_stats'];
...
export async function fetchClearSportsResearch({ sport, playerName, team = null, market, providerMarketKey = null, games = 20 } = {}) {
  ...
  if (!statsResult.cached) {
    const rows = unwrapRows(statsResult.data);
    console.info('[research-feed]', JSON.stringify({
      provider: 'ClearSports', sport: league,
      statsStatus: statsResult.status || null, gamesStatus: gamesResult.status || null, injuriesStatus: injuriesResult.status || null,
      rows: rows.length, gameIdentifiedRows: rows.filter(row => rowGameId(row)).length,
      datedRows: rows.filter(row => rowDate(row)).length,
      mappedRows: rows.filter(row => statValue(league, selectedMarket, row, providerMarketKey) !== null).length,
    }));
  }
