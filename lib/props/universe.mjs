...
function validDisplayPlayer(prop = {}) {
  const name = String(prop.playerName || '').trim();
  const normalized = name.toLowerCase();
  if (!name || ['pickfinder', 'unknown', 'n/a', 'na', 'player'].includes(normalized)) return false;
  // Opaque sportsbook/UI identifiers were previously leaking through the
  // Participant field (for example 16-character mixed alpha-numeric tokens).
  // Real player names may contain numbers rarely, but a long no-space token
  // containing digits is not safe display identity and is excluded.
  if (!name.includes(' ') && name.length >= 10 && /[a-z]/i.test(name) && /\d/.test(name)) return false;
  return toNumberOrNull(prop.line) !== null && Boolean(String(prop.market || '').trim());
}
...
export async function buildPropUniverse(scanResult, { sports = [], forceProvider = false } = {}) {
  const pickFinderProps = fromScanResult(scanResult).filter(validDisplayPlayer);
  ...
  try {
    ...
    providerProps = fromSportsDataIoOffers(providerBoard.offers, { fetchedAt: providerBoard.fetchedAt }).filter(validDisplayPlayer);
  } catch (error) {
...
export async function buildUnifiedPropViewAsync(scanResult, options = {}) {
  ...
  // Game-log hydration is explicitly opt-in for bounded list pages. Detail and
  // live routes must not accidentally fan out one provider request per prop.
  const hydratedProps = options.hydrateHistory === true
    ? await hydrateVisibleHistory(view.props)
    : view.props;
