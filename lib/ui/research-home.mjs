...
export function researchClient(source) {
  ...
  // The owner entry is derived only from the server-issued capability returned
  // by /api/account/me. Ordinary members never receive viewOwnerConsole=true,
  // and every /owner route independently checks requireOwner() server-side.
  // Remove any prior control link first so role/session changes fail closed.
