...
export function createClearSportsClient({ timeoutMs = DEFAULT_TIMEOUT_MS, maxEntries = 500 } = {}) {
  ...
  function prune() {
    ...
    for (const [key, row] of failures) if (row.expiresAt <= now) failures.delete(key);
    while (failures.size > maxEntries) failures.delete(failures.keys().next().value);
  }

  async function fetchFresh(cleanPath, query, id, ttlMs, key) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        failures.set(id, { value: failure, expiresAt: Date.now() + ([401,403,429].includes(response.status) ? 15 * 60_000 : 60_000) });
        prune();
        return failure;
      }
      ...
      const failure = { ok: false, status: 0, code: error?.name === 'AbortError' ? 'TIMEOUT' : 'NETWORK_ERROR', data: null, cached: false, latencyMs: metrics.lastLatencyMs };
      failures.set(id, { value: failure, expiresAt: Date.now() + 60_000 });
      prune();
      return failure;
    } finally {
      clearTimeout(timer);
  ...
  async function get(path, { query = {}, ttlMs = 120_000 } = {}) {
    ...
    if (failed && failed.expiresAt > Date.now()) return { ...failed.value, cached: true };
    failures.delete(id);
    ...
    if (inflight.has(id)) {
      metrics.coalesced += 1;
      const shared = await inflight.get(id);
      return shared?.ok ? { ...shared, cached: true, latencyMs: 0, coalesced: true } : shared;
    }

    const request = fetchFresh(cleanPath, query, id, ttlMs, key);
  ...
  return {
    get,
    probe,
    stats: () => ({ ...metrics, cacheEntries: cache.size, inflight: inflight.size }),
  };
}

// Both adapters consume the same upstream resources; share their cache and queue.
export const researchClient = createClearSportsClient({ timeoutMs: 9000, maxEntries: 300 });
export function researchTtlMs() {
  const configured = Number(process.env.CLEARSPORTS_STATS_TTL_MS);
  return Number.isFinite(configured) && configured >= 60_000 ? configured : 6 * 60 * 60_000;
}
