...
function researchParams(g,line,side){var q=new URLSearchParams({sport:g.sport,playerName:g.playerName,market:g.market,marketId:researchMarketKey(g,line,side)||'',eventId:g.eventId||'',gameStartTime:g.gameStartTime||'',period:g.period||'',position:g.position||'',line:String(line==null?'':line),side:side||defaultSide(g),games:'40',providerPlayerId:g.providerPlayerId||'',homeTeam:g.homeTeam||'',awayTeam:g.awayTeam||'',team:g.team||''});return q.toString();}`,
  ...
  out = replaceOnce(
    ...
    "providerPlayerId:g.providerPlayerId||'',line:line,side:side,team:g.team||'',homeTeam:g.homeTeam||'',awayTeam:g.awayTeam||'',marketId:researchMarketKey(g,line,side)||'',eventId:g.eventId||'',gameStartTime:g.gameStartTime||'',period:g.period||'',position:g.position||'',games:40};",
  ...
  out = replaceOnce(out,
    ...
    "function researchKey(g,line,side){return [g.key,g.team,g.homeTeam,g.awayTeam,num(line),side||'OVER',researchMarketKey(g,line,side),g.eventId||'',g.gameStartTime||'',g.period||''].join('|');}", 'platform-aware research cache');
  ...
  out = replaceOnce(out, "({g,line:valueLine,side:valueSide,key,resolve});",
    "({g,line:valueLine,side:valueSide,key,query:researchParams(g,valueLine,valueSide),resolve});", 'queued platform snapshot');
