...
export const PROJECTION_OUTPUT_SCHEMA = Object.freeze({
  ...
  properties: {
    ...
    primary_driver: {
      type: 'string',
      description: 'One sentence naming the single strongest reason, citing a figure from the payload.',
    },
    data_gaps: {
      type: 'array',
      items: { type: 'string' },
  ...
  required: ['projection', 'probability_over', 'confidence', 'primary_driver', 'data_gaps'],
...
export function deriveProjection(model = {}, market = {}) {
  ...
  return {
    ...
    primaryDriver: typeof model.primary_driver === 'string' ? model.primary_driver.slice(0, 400) : null,
