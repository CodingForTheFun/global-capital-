...
import { parseCookies, createRateLimiter, clientKey, clientIp } from '../session.mjs';
import { internalDetail, GENERIC_MESSAGE } from '../safe-error.mjs';
import { listWatchlist, upsertWatchlistItem, removeWatchlistItem } from './watchlist.mjs';
...
const LIMITS = Object.freeze({
  ...
  watchlist: { max: 90, windowMs: 60_000 },
  preferences: { max: 30, windowMs: 60_000 },
});
...
export async function handleAccountRoutes(req, res, url, { sessions, json, secret, log = console }) {
  ...
  try {
    ...
    if (path === '/api/account/watchlist' && req.method === 'GET') {
      ...
      json(res, 200, {
        ...
        items,
        count: items.length,
        csrfToken: csrfTokenFor(token, secret),
      });
      return true;
    }
    ...
    if (path === '/api/account/preferences' && req.method === 'GET') {
      ...
      if (!user) return unauthenticated(res, json);
      json(res, 200, { ok: true, preferences: service.publicPreferences(user) });
      return true;
    }
    ...
    if (path === '/api/account/preferences') {
      ...
      if (!allow(req, 'preferences', user.email)) return tooMany(res, json);
      const result = await service.saveBoardPreferences({
        userId: user.id,
        sport: body.sport,
    ...
    if (path === '/api/account/watchlist') {
      ...
      if (!user) return unauthenticated(res, json);
      if (!csrfValid(token, req.headers['x-csrf-token'], secret)) return badCsrf(res, json);
      if (!allow(req, 'watchlist', user.email)) return tooMany(res, json);
      const action = body.action === 'remove' ? 'remove' : body.action === 'upsert' ? 'upsert' : null;
      if (!action) {
        json(res, 400, { ok: false, code: 'REQUEST_INVALID', message: 'Choose a valid watchlist action.' });
        return true;
      }
      const items = action === 'remove'
        ? await removeWatchlistItem(user.id, body.key)
        : await upsertWatchlistItem(user.id, body.item);
    ...
    if (path === '/api/account/logout') {
      // Self sign-out must always clear the local auth cookie. Presence cleanup
      // is useful bookkeeping, but it cannot be allowed to trap a customer in a
      // session when the data volume is full or otherwise temporarily degraded.
      try {
        const token = parseCookies(req)[ACCOUNT_COOKIE];
        const session = token ? sessions.read(token) : null;
        if (session?.valid && session.sessionId) await revokeSession(session.sessionId);
      } catch (error) {
        log?.warn?.('[Scout Pro auth] sign-out presence cleanup failed', JSON.stringify(internalDetail(error, {
          stage: 'logout-presence-revoke',
        })));
      }
