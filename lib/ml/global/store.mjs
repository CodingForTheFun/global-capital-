...
//   enrich-<sport>.json box-score facts per game (MLB lineup spots, starter hands)
...
/** Box-score facts per resolved game: { [eventId]: { f: facts | null, at } }. */
export async function readEnrich(sport, dir = globalModelDir()) {
  const raw = await readSmall(path.join(dir, `enrich-${safeSport(sport)}.json`));
  if (!raw) return {};
  try {
...
export async function writeEnrich(sport, map, dir = globalModelDir()) {
  await atomicWrite(path.join(dir, `enrich-${safeSport(sport)}.json`), JSON.stringify(map));
}
