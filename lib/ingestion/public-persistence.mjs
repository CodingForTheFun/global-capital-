...
async function rpcStore(functionName, action, payload = {}) {
  ...
  try {
    ...
    return directStore(functionName, action, payload);
  }
  const body = await response.json().catch(() => null);
  if (!response.ok && TRANSIENT_DATA_API_STATUSES.has(Number(response.status))) return directStore(functionName, action, payload);
...
async function store(action, payload = {}) {
  return rpcStore('autoscout_public_store', action, payload);
}

async function historyStore(action, payload = {}) {
  ...
  return rpcStore('autoscout_public_history_store', action, payload);
}
