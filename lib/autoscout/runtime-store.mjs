...
import { SUPPORTED_SPORTS, numberOrNull } from './models.mjs';
...
const runtime = {
  providerControl: {blockedUntil:0},
...
export async function loadPersistedDiagnostics() {
  try {
    ...
    runtime.providerControl = {blockedUntil:Number(stored?.providerControl?.blockedUntil)||0};
...
let diagnosticsWrite=null,diagnosticsDirty=false;
function persistDiagnostics() {
  diagnosticsDirty=true;
  if(diagnosticsWrite)return diagnosticsWrite;
  diagnosticsWrite=(async()=>{
    while(diagnosticsDirty){diagnosticsDirty=false;await atomicWrite(STATE_FILE,snapshotDiagnostics()).catch(()=>{});}
  })().finally(()=>{diagnosticsWrite=null;});
...
export async function readCachedBoard(sport, { allowStale = false } = {}) {
  ...
  if (!entry) {
    if(!boardReads.has(selected)){
      const pending=fs.readFile(boardFile(selected),'utf8').then(raw=>{
        const value=JSON.parse(raw);if(value?.value)memoryBoards.set(selected,value);return value;
      }).catch(()=>null);
      boardReads.set(selected,pending);pending.then(()=>boardReads.delete(selected));
    }
    entry=await boardReads.get(selected);
  }
  if (!entry?.value) {
...
export function recordProviderRequest({ cost = null, remaining = null, used = null } = {}) {
  ...
  if (numberOrNull(cost) !== null) runtime.observedCreditsToday += Number(cost);
  if (numberOrNull(used) !== null) runtime.quota.used = Number(used);
  if (numberOrNull(remaining) !== null) runtime.quota.remaining = Number(remaining);
  if (numberOrNull(cost) !== null) runtime.quota.lastCost = Number(cost);
...
export function recordProviderError({ provider = 'unknown', endpoint = null, sport = null, event = null, status = null, reason = null, code = null } = {}) {
  ...
  // Operator-only, rate-limited failure classification; never log URLs, messages or credentials.
  const safeCode = ['INVALID_API_KEY','OUT_OF_USAGE_CREDITS','INVALID_REGION','INVALID_REGIONS','INVALID_BOOKMAKERS','INVALID_MARKET','INVALID_MARKETS','INVALID_REQUEST','NO_ACCESS','RATE_LIMITED','ODDS_API_ERROR'].includes(code) ? code : 'OTHER';
  const previous = runtime.errors[0];
  if (!previous || previous.provider !== provider || previous.status !== (Number(status)||null) || previous.code !== code || Date.now()-Date.parse(previous.timestamp)>60000) {
    console.warn('[AutoScout provider failure]',JSON.stringify({provider:provider==='the-odds-api'?provider:'other',status:Number(status)||null,code:safeCode}));
  }
...
export function recordProviderControl(value){runtime.providerControl={blockedUntil:Number(value.blockedUntil)||0};void persistDiagnostics();}
