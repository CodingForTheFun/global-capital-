    ...
    try {
      ...
      if ((!snapshot || !SUCCESS.has(snapshot.status) || snapshot.partial) && [403, 426].includes(Number(snapshot?.httpStatus))) {
        try {
          ...
          await status(source, { status: 'unavailable', retained: true, transport: 'browser-public', code: text(error?.code || 'PUBLIC_BROWSER_FAILED').slice(0, 80) });
        }
      }
      if (!snapshot || !SUCCESS.has(snapshot.status) || snapshot.partial) {
        await status(source, { status: snapshot?.partial ? 'partial' : snapshot?.status || 'unavailable', retained: true, fetchedAt: snapshot?.fetchedAt || null, httpStatus: snapshot?.httpStatus ?? null });
        return { source, persisted: false, retained: true, httpStatus: snapshot?.httpStatus ?? null };
      }
