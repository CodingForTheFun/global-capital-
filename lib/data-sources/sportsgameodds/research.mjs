...
function scoreFromResults(event, playerId, statIds, periodId = 'game') {
  if (!Array.isArray(statIds) || !statIds.length) return null;
...
export async function fetchSportsGameOddsResearch(params = {}) {
  ...
  // Platform-native fantasy history remains preferred by the research service.
  // This fallback is sport-agnostic, so every supported player chart uses the
  // same verified grading rule when its primary historical source has a gap.
  // When it is missing, SportsGameOdds can fill the chart only from the exact
  // finalized fantasy market's dedicated grading score. We intentionally do
  // not reinterpret a generic raw results.fantasyScore as a DFS platform score.
  const marketIdentity = `${text(params.providerMarketKey)} ${text(params.market)}`.toLowerCase();
  const fantasyMarket = marketIdentity.includes('fantasy');

  const periodId = sportsGameOddsPeriodId(params.period);
  if (!periodId) return null;

  // Prefer an exact identity already attached to the live normalized row. This
  // avoids a second name/team/time match on the research request and works even
  // when the in-memory SportsGameOdds supplement cache has not warmed yet.
  const explicitPlayerId = text(params.sportsGameOddsPlayerId);
  const identity = explicitPlayerId
    ? {
        playerId: explicitPlayerId,
        eventId: text(params.sportsGameOddsEventId) || null,
        leagueId: text(params.sportsGameOddsLeagueId) || null,
        statId: text(params.sportsGameOddsStatId) || null,
        playerName: text(params.playerName) || null,
        team: text(params.team) || null,
      }
  ...
  const exactStat = currentStat || (statIds.length === 1 ? statIds[0] : '');
  ...
  const query = {
    ...
    oddID: exactStat ? exactStat + '-' + identity.playerId + '-' + periodId + '-ou-over' : undefined,
    includeOpposingOdds: exactStat ? 'true' : undefined,
  ...
  for (const event of list(payload?.data)) {
    ...
    let value = exactStat ? scoreFromOdds(event, identity.playerId, exactStat, periodId) : null;
    if (value === null && !fantasyMarket) {
      const direct = exactStat ? scoreFromResults(event, identity.playerId, [exactStat], periodId) : null;
  ...
  return {
    ...
    season: null,
    entityType: 'player',
    statKind: exactStat || statIds.join('+'),
    ...
    coverage: {
      ...
      exactStatId: exactStat || null,
      periodId,
      gradedMarketOnly: fantasyMarket,
