...
import { fetchNcaaMfbResearch } from '../data-sources/sportsdataverse/ncaaf-game-research.mjs';
...
async function researchUncached(params = {}) {
  ...
  if (PUBLIC_LEAGUES[String(params.sport || '').toUpperCase()]) {
    ...
    if (!history.available) {
      ...
      // NCAAF gets current stats.ncaa.org-derived player-game data from the
      // SportsDataverse archive after PropLine and ESPN cannot verify the exact stat.
      if (params.sport === 'NCAAF') {
        let ncaaArchive = null;
        try { ncaaArchive = await fetchNcaaMfbResearch(params); }
        catch { ncaaArchive = null; }
        if (ncaaArchive?.available) {
          return finalizePublicHistory(ncaaArchive, params, {
            fallback: { used: true, primary: 'PropLine raw game archive', source: ncaaArchive.source },
          });
        }
      }
