...
export function createPublicIngestionRunner({ now = Date.now, fetcher = globalThis.fetch, feeds = publicFeeds, claim = claimPublicCycle, release = releasePublicCycle, persistSnapshot = persistPublicSnapshot, recordStatus = recordPublicStatus } = {}) {
  ...
  async function ingestDfs(source) {
    try {
      ...
      // A partial source response is not permission to replace a complete stored board.
      // Keep the last-good database snapshot and wait for the next scheduled refresh.
      ...
      const blocked = () => (!snapshot || !SUCCESS.has(snapshot.status)) && [0, 403, 426].includes(Number(snapshot?.httpStatus || 0));
      if (blocked()) {
      ...
      if (!snapshot || !SUCCESS.has(snapshot.status)) {
