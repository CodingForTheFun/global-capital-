...
async function researchUncached(params = {}) {
  ...
  if (period && !['full', 'full_game', 'game', 'match', 'single_stat'].includes(period)) {
    return { ok: true, available: false, code: 'PERIOD_HISTORY_UNAVAILABLE', message: 'Verified game logs for this exact period are not available. Full-game totals are not used for a period prop.', gameLog: [], lineOnly: true, retryable: false };
  }
...
export async function researchPlayerProp(params = {}) {
  const key = JSON.stringify(['sport','playerName','providerPlayerId','team','homeTeam','awayTeam','opponent','market','providerMarketKey','line','side','games','historyYears','eventId','gameStartTime','period'].map(k => params[k] ?? null));
