...
export function record(input){
 ...
 if(!input.sourceId||!input.nativePlayerId||!input.playerName||!market||line===null||!start||!fallbackMarketKey||contract?.entityType==='team')return null;
...
export function normalizeUnderdog(payload){
 ...
 for(const row of payload.over_under_lines){
  ...
  if(!p||!appearance||!g||off(row)||off(p))continue;
...
export function normalizedFeedBoard(records,reference={props:[]},at=new Date().toISOString()){
 ...
 for(const r of records){
  ...
  const inferredKey=r.contract?canonicalKeys.find(key=>contractKey(marketContract({sport:r.sport,market:r.market,providerMarketKey:key}))===contractKey(r.contract)):null;
