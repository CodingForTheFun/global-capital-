...
export function legalIdentity(env = process.env) {
  return {
    ...
    contact: text(env.LEGAL_CONTACT_EMAIL) || text(env.ACCOUNT_OWNER_EMAIL) || 'support@obligeprops.com',
...
<div class="callout">
<p><b>Oblige Props is a research tool, not a sportsbook.</b> It does not accept wagers, hold funds, or process bets. It shows publicly available lines, historical game logs and calculations derived from them.</p>
...
<h2>1. Who we are</h2>
<p>Oblige Props (&ldquo;the service&rdquo;) is operated by ${escape(entity)}. You can reach us at <a href="mailto:${escape(contact)}">${escape(contact)}</a>.</p>
...
<div class="callout">
<p><b>Nothing on Oblige Props is a prediction, a tip, or advice to place a bet.</b> A hit rate is a count of what already happened. A fair value is what a sportsbook&rsquo;s own prices imply. Neither tells you what will happen tonight.</p>
...
const PAGES = Object.freeze({
  '/terms': { title: 'Terms of Service', description: 'The terms that apply to using Oblige Props.', body: termsBody },
  '/privacy': { title: 'Privacy Policy', description: 'What Oblige Props collects, why, and what it never does.', body: privacyBody },
  '/responsible-gaming': { title: 'Responsible gaming', description: 'Oblige Props is research, not advice — and where to get help.', body: responsibleBody },
});
