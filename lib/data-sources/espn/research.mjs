...
export function inspectGameLog(payload, params = {}) {
  ...
  const seen=new Set(),expected=new Set(),output=[];let validSeason=false,unresolved=false,sackKind=null;
  ...
  for(const group of payload.seasonTypes||[])for(const cat of group.categories||[]) {
    ...
    for(const row of cat.events||[]) {
      ...
      if(!Number.isFinite(date)||date>now||!leagueMatches(event,sport)||event.team?.isAllStar||event.opponent?.isAllStar
      ...
      if(contract.fields.some(f=>numeric(mapped[f])===null))continue;
  ...
  return {rows:output.sort((a,b)=>Date.parse(b.date)-Date.parse(a.date)),season,sackKind,seenEvents:expected.size,
...
export function createPublicResearch({ fetchImpl = (...args) => fetch(...args), now = () => Date.now() } = {}) {
  ...
  return async function fetchResearch(params = {}) {
    ...
    const response=await request(logPath(),GAME_LOG_TTL);
    ...
    return {ok:true,available:true,source:'Historical stats',cached,opponent:matchedOpponent?.opponent||matchupOpponent,opponentId:matchedOpponent?.opponentId||null,
      ...
      coverage:{seasonComplete:current.complete,backfilled:logs.some(r=>String(r.season)!==String(year)),
