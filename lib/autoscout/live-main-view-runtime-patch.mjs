...
;(function obligePropsInlineLiveScores(){
  ...
  function boot(){
    ...
    style.textContent=[
      ...
      '#as5 .asLiveInlineFilters{display:flex;align-items:center;gap:3px;padding:2px;border:1px solid rgba(86,113,155,.28);border-radius:9px;background:#080b10}',
      '#as5 .asLiveFilter{height:26px;border:0;background:transparent;color:#8f9bae;border-radius:7px;padding:0 9px;font-size:8px;font-weight:800;display:inline-flex;align-items:center;gap:5px}',
      '#as5 .asLiveFilter.on{background:#1b2432;color:#fff}',
      '#as5 .asLiveFilter i{width:6px;height:6px;border-radius:50%;background:#626d7c}',
      '#as5 .asLiveFilter.liveNow i{background:#ef4444;box-shadow:0 0 0 3px rgba(239,68,68,.08)}',
      '#as5 .asLiveScorePanel{overflow:hidden;border:1px solid rgba(91,120,166,.28);border-radius:12px;background:#0f1115}',
      '#as5 .asLiveScoreCoverage{padding:7px 12px;border-bottom:1px solid rgba(91,120,166,.2);color:#69788e;font-size:7px;text-align:right}',
      '#as5 .asLiveScoreRows{display:block}',
      '#as5 .asScoreRow{display:grid;grid-template-columns:80px minmax(0,1fr) 64px minmax(0,1fr) 90px;align-items:center;min-height:48px;padding:7px 12px;border-bottom:1px solid rgba(91,120,166,.16);transition:background .15s}',
      '#as5 .asScoreRow:last-child{border-bottom:0}',
      '#as5 .asScoreRow:hover{background:#151921}',
      '#as5 .asScoreStatus{font-size:9px;color:#929db0;font-variant-numeric:tabular-nums}',
      '#as5 .asScoreStatus.live{color:#ef4444;font-weight:900}',
      '#as5 .asScoreStatus.live i{display:inline-block;width:6px;height:6px;margin-right:5px;border-radius:50%;background:#ef4444}',
      '#as5 .asScoreTeam{display:flex;align-items:center;gap:7px;min-width:0}',
      '#as5 .asScoreTeam.away{justify-content:flex-end;text-align:right;padding-right:10px}',
      '#as5 .asScoreTeam.home{justify-content:flex-start;text-align:left;padding-left:10px}',
      '#as5 .asScoreTeam span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:10px;color:#d5dbe5}',
      '#as5 .asScoreTeam.win span{color:#fff;font-weight:900}',
      '#as5 .asScoreLogo{width:18px;height:18px;object-fit:contain;flex:0 0 18px}',
      '#as5 .asScoreLogoFallback{display:grid;place-items:center;width:18px;height:18px;border-radius:50%;background:#1b2432;color:#8fa0b8;font-size:6px;font-weight:900}',
      '#as5 .asScorePill{width:58px;margin:auto;padding:4px 2px;text-align:center;border:1px solid #252d39;border-radius:7px;background:#090c11;font-size:10px;font-weight:900;font-variant-numeric:tabular-nums}',
      '#as5 .asScorePill.live{color:#34d399}',
      '#as5 .asScorePill.pending{color:#667085;font-size:8px}',
      '#as5 .asScoreContext{text-align:right;color:#68778b;font-size:7px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}',
      ...
      '@media(max-width:650px){#as5 .asLiveEmbedded{padding:10px 8px 88px}#as5 .asScoreRow{grid-template-columns:64px minmax(0,1fr) 54px minmax(0,1fr);padding:8px 7px}#as5 .asScoreContext{display:none}#as5 .asScoreTeam{gap:5px}#as5 .asScoreTeam span{font-size:9px}#as5 .asScorePill{width:50px;font-size:9px}#as5 .asLiveInlineToolbar{flex-wrap:wrap}#as5 .asLiveInlineFilters{order:3;width:100%;justify-content:center}#as5 .asLiveInlineHead{align-items:flex-end;gap:8px}#as5 .asLiveInlineHead h1{font-size:25px}#as5 .asLiveInlineHead p{display:none}#as5 .asLiveToday{height:21px;font-size:7px}#as5 .asLiveInlineFresh{min-width:94px;padding:7px 8px}#as5 .asLiveInlineFresh strong{font-size:11px}#as5 .asLiveInlineStats{gap:5px}#as5 .asLiveInlineStats>div{padding:8px}#as5 .asLiveInlineStats strong{font-size:16px}#as5 .asLiveInlineToolbar{top:6px;padding:6px}#as5 .asLiveGameGrid{grid-template-columns:1fr;gap:7px}#as5 .asLiveGame{padding:11px 12px 9px;border-radius:15px}#as5 .asLiveTeamRow{min-height:46px}#as5 .asLiveScore{font-size:22px}#as5 .asNavScores{display:grid!important;grid-template-rows:22px 13px!important;place-items:center!important}}',
    ...
    panel.innerHTML=[
      ...
      '<section class="asLiveInlineToolbar"><div id="asLiveInlineSports" class="asLiveInlineSports"></div><div id="asLiveInlineFilters" class="asLiveInlineFilters"><button class="asLiveFilter on" data-inline-live-filter="all" type="button">All</button><button class="asLiveFilter" data-inline-live-filter="live" type="button"><i></i>Live <span id="asLiveFilterCount"></span></button><button class="asLiveFilter" data-inline-live-filter="finished" type="button">Finished</button></div><button id="asLiveInlineRefresh" class="asLiveInlineRefresh" type="button">Refresh</button></section>',
      '<div id="asLiveInlineNotice" class="asLiveInlineNotice" hidden></div>',
      '<section class="asLiveScorePanel"><div id="asLiveInlineCoverage" class="asLiveScoreCoverage">Checking feeds</div><div id="asLiveScoreRows" class="asLiveScoreRows"></div></section>',
      '<span id="asLiveNowSection" hidden></span><span id="asLiveUpcomingSection" hidden></span><span id="asLiveFinalSection" hidden></span>',
      '<section id="asLiveEmpty" class="asLiveInlineSection" hidden><div class="asLiveEmpty"><b>No games in this view</b>Choose another sport or filter.</div></section>'
    ].join('');
    main.insertAdjacentElement('afterend',panel);

    var sportsStrip=root.querySelector('.asSports');
    var state={active:false,sport:'NFL',filter:'all',timer:null,loading:false,lastLiveCount:0,previousView:'research',snapshot:null};
    var sportOrder=['NFL','NBA','SOCCER','NHL','MLB'];
    var sportLabels={NFL:'NFL',NBA:'NBA',SOCCER:'EPL',NHL:'NHL',MLB:'MLB'};
    ...
    function logoHtml(game,side){var logo=game[side+'Logo'],name=game[side+'Name']||game[side+'Team']||'?';return logo?'<img class="asScoreLogo" src="'+esc(logo)+'" alt="" loading="lazy" referrerpolicy="no-referrer">':'<span class="asScoreLogoFallback">'+esc(String(name).slice(0,2).toUpperCase())+'</span>';}
    function winner(game,side){var a=num(game.awayScore),h=num(game.homeScore);if(game.status!=='FINAL'||a==null||h==null||a===h)return false;return side==='away'?a>h:h>a;}
    function scoreRowHtml(game){var live=game.status==='LIVE',final=game.status==='FINAL',status=gameStatus(game),away=game.awayName||game.awayTeam||'Away',home=game.homeName||game.homeTeam||'Home',hasScore=(game.awayScore!==null&&game.awayScore!==undefined&&game.homeScore!==null&&game.homeScore!==undefined),scoreHtml=hasScore?esc(game.awayScore)+' - '+esc(game.homeScore):'VS',context=game.venue||game.broadcast||game.league||'';return '<div class="asScoreRow" data-score-match="'+esc(game.id||'')+'"><div class="asScoreStatus '+(live?'live':'')+'">'+(live?'<i></i>':'')+esc(final?'FT':status)+'</div><div class="asScoreTeam away '+(winner(game,'away')?'win':'')+'"><span>'+esc(away)+'</span>'+logoHtml(game,'away')+'</div><div class="asScorePill '+(live?'live':hasScore?'':'pending')+'">'+scoreHtml+'</div><div class="asScoreTeam home '+(winner(game,'home')?'win':'')+'">'+logoHtml(game,'home')+'<span>'+esc(home)+'</span></div><div class="asScoreContext">'+esc(context)+'</div></div>';}
    ...
    function renderSports(){el('asLiveInlineSports').innerHTML=sportOrder.map(function(s){return '<button class="asLiveInlineChip '+(state.sport===s?'on':'')+'" data-inline-live-sport="'+esc(s)+'" type="button">'+esc(sportLabels[s]||s)+'</button>';}).join('');}
    function renderCurrent(){var snapshot=state.snapshot;if(!snapshot)return;var all=filterRows(snapshot.games||[]),live=all.filter(function(g){return g.status==='LIVE';}),upcoming=all.filter(function(g){return g.status==='SCHEDULED';}),finals=all.filter(function(g){return g.status==='FINAL';});state.lastLiveCount=live.length;el('asLiveInlineNow').textContent=String(live.length);el('asLiveInlineUpcoming').textContent=String(upcoming.length);el('asLiveInlineFinal').textContent=String(finals.length);el('asLiveInlineUpdated').textContent=time(snapshot.fetchedAt);var rows=state.filter==='live'?live:state.filter==='finished'?finals:all;el('asLiveScoreRows').innerHTML=rows.map(scoreRowHtml).join('');el('asLiveEmpty').hidden=rows.length>0;el('asLiveFilterCount').textContent=live.length?String(live.length):'';var liveTab=document.querySelector('[data-inline-live-filter="live"]');if(liveTab)liveTab.classList.toggle('liveNow',live.length>0);document.querySelectorAll('[data-inline-live-filter]').forEach(function(button){button.classList.toggle('on',button.dataset.inlineLiveFilter===state.filter);});}
    ...
    async function load(force){if(!state.active||state.loading)return;state.loading=true;el('asLiveInlineRefresh').disabled=true;try{var query='sports=NFL,NBA,SOCCER,NHL,MLB'+(force?'&force=1':'');var snapshot=await api('/api/live?'+query);state.snapshot=snapshot;renderSports();renderCoverage(snapshot);renderCurrent();}catch(error){var notice=el('asLiveInlineNotice');notice.hidden=false;notice.textContent=error&&error.message||'Scores could not be loaded.';el('asLiveScoreRows').innerHTML='';el('asLiveEmpty').hidden=false;el('asLiveEmpty').innerHTML='<div class="asLiveEmpty"><b>Scores temporarily unavailable</b>Oblige Props will retry automatically.</div>';}finally{state.loading=false;el('asLiveInlineRefresh').disabled=false;schedule();}}
    ...
    el('asLiveInlineSports').addEventListener('click',function(event){var button=event.target.closest('[data-inline-live-sport]');if(!button)return;state.sport=String(button.dataset.inlineLiveSport||'NFL').toUpperCase();renderSports();renderCurrent();});
    el('asLiveInlineFilters').addEventListener('click',function(event){var button=event.target.closest('[data-inline-live-filter]');if(!button)return;state.filter=String(button.dataset.inlineLiveFilter||'all');renderCurrent();});
