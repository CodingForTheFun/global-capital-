...
export function canonicalSportsGameOddsMarketKey(statID, sport = '') {
  ...
  if (!raw) return null;
  const key = slug(raw);
  if (String(sport).toUpperCase() === 'NHL' && key === 'assists') return 'player_assists';
...
export function normalizeSportsGameOddsEvents(events = [], {
  ...
  for (const rawEvent of list(events)) {
    ...
    const sport = sportsGameOddsSportForLeague(rawEvent?.leagueID, rawEvent?.sportID) || text(requestedSport).toUpperCase();
