...
async function providerUsage() {
  ...
  try {
    ...
    const response = await fetch(url, { headers: { accept: 'application/json' }, signal: ingestionSignal(AbortSignal.timeout(10_000)) });
...
async function requestMode(targetUrl, { targetHeaders = {}, browser = false, returnPageSource = false, estimatedCost = 1, transport }) {
  ...
  try {
    ...
    const response = await fetch(url, { headers, signal: ingestionSignal(AbortSignal.timeout(browser ? 45_000 : 25_000)) });
