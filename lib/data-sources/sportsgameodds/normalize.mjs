...
const MARKET_ALIASES = Object.freeze({
  ...
  fantasypoints: 'player_fantasy_score',
...
const DIRECT_STAT_BY_MARKET = Object.freeze({
  ...
  player_fantasy_score: 'fantasyScore',
  // Legacy alias kept so persisted/pre-fix rows remain research-addressable.
...
export function canonicalSportsGameOddsMarketKey(statID, sport = '') {
  ...
  if (['NFL','NCAAF'].includes(selectedSport) && key === 'points') return 'sgo_points';
  if (selectedSport === 'MLB' && key === 'points') return 'batter_runs';
  if (selectedSport === 'TENNIS' && key === 'points') return 'player_sets_won';
  if (selectedSport === 'NHL' && key === 'assists') return 'player_assists';
  if (selectedSport === 'NHL' && key === 'blocks') return 'player_blocked_shots';
  if (selectedSport === 'NHL' && key === 'points') return 'player_goals';
  if (selectedSport === 'NHL' && key === 'goalsassists') return 'sgo_goals_assists';
...
function sourceQualifiedFantasyMarketKey({ marketKey, bookmakerID, sport, player, marketName = '' } = {}) {
  const key = text(marketKey).toLowerCase();
  const book = bookId(bookmakerID);
  if (book !== 'prizepicks' || !['player_fantasy_score','player_fantasy_points'].includes(key)) return key;

  // Fantasy scoring is platform-specific. Match the exact market identities
  // already produced by the direct PrizePicks ingestion path so an SGO line
  // fills the same quote slot instead of creating a duplicate category.
  const selectedSport = text(sport).toUpperCase();
  const role = text(player?.position).toUpperCase();
  const label = text(marketName).toLowerCase();
  if (selectedSport === 'MLB') {
    if (/^(?:P|SP|RP|LHP|RHP)$/.test(role) || /\bpitcher\b/.test(label)) {
      return 'prizepicks:player_pitcher_fantasy_score';
    }
    if (role || /\b(?:hitter|batter)\b/.test(label)) {
      return 'prizepicks:player_hitter_fantasy_score';
    }
  }
  return 'prizepicks:player_fantasy_score';
}
...
function eventScores(event) {
  ...
  const home = numberOrNull(game?.home?.points ?? game?.home ?? event?.scores?.home ?? event?.teams?.home?.score ?? event?.status?.score?.home);
  const away = numberOrNull(game?.away?.points ?? game?.away ?? event?.scores?.away ?? event?.teams?.away?.score ?? event?.status?.score?.away);
  return { home, away };
}
...
function flatRow({ event, player, prop, line, odd, book, home, away, ingestedAt }) {
  ...
  return {
    ...
    sportsGameOddsLeagueId: event.league || null,
    playerId: player.id,
...
export function normalizeSportsGameOddsEvents(events = [], {
  ...
  for (const rawEvent of list(events)) {
    ...
    // The board request owns the customer-facing sport bucket. This keeps an
    // aggregate request such as CRICKET, RUGBY, BASEBALL or SOCCER together
    // even when the provider returns many leagueIDs underneath it.
    const sport = requested || mappedSport;
    ...
    for (const odd of Object.values(object(rawEvent?.odds))) {
      ...
      for (const [bookmakerID, bookmaker] of Object.entries(bookmakers)) {
        ...
        const bookMarketKey = sourceQualifiedFantasyMarketKey({
          marketKey,
          bookmakerID,
          sport,
          player,
        ...
        // Only this bookmaker's own number. `bookOverUnder` is the consensus and
        // `fairOverUnder` is derived — this file keeps both, correctly, as
        // `consensusLine` and `fairLine` on the prop. Falling back to them here
        // published one of them AS this book's line while pairing it with this
        // book's real price, so a card could read "DraftKings 45.5" for a line
        // DraftKings never posted, or show a modelled number as a PrizePicks
        // projection. Only `ou` markets reach this loop, so `overUnder` is the
        // field every row here is supposed to have; its absence means the book
        // has no line to show, which is what `noLine` already reports.
        const mainLine = numberOrNull(bookmaker?.overUnder);
