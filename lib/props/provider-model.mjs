...
const SLATE_TZ = process.env.SCAN_TIME_ZONE || 'America/Chicago';
...
function isSlateToday(value) {
  const event = dayKey(value);
  return event ? event === dayKey(new Date()) : null;
}
...
/** Collapse identical real lines from multiple books without averaging them. */
...
export function fromSportsDataIoOffer(offer, { fetchedAt = null } = {}) {
  ...
  const today = isSlateToday(offer.gameStartTime);
