...
export const PUBLIC_FEEDS=Object.freeze([
 {id:'prizepicks',url:'https://api.prizepicks.com/projections?per_page=250',ttl:THREE_MINUTES,origin:'https://app.prizepicks.com',referer:'https://app.prizepicks.com/'},
 {id:'underdog',url:'https://api.underdogfantasy.com/beta/v5/over_under_lines',ttl:THREE_MINUTES,origin:'https://underdogfantasy.com',referer:'https://underdogfantasy.com/'},
 ...
 async function refreshOne(feed){
  ...
  const task=(async()=>{
   try{
    ...
    state.set(feed.id,{...parsed,fetchedAt:new Date(now()).toISOString(),nextAt:now()+feed.ttl,failures:0,status:parsed.records.length?'available':feed.id==='sleeper'?'roster_only':parsed.contracts.length?'unmapped_contracts':'no_props',partial,httpStatus:200});
