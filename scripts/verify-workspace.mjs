...
try{
 ...
 await page.waitForURL(base+'/sportsbooks');await page.getByRole('button',{name:'Player props',exact:true}).click();
 ...
 await page.setViewportSize({width:390,height:844});await nav().getByRole('link',{name:'Sports',exact:true}).click();await page.getByRole('button',{name:'Player props',exact:true}).click();await first.waitFor();check(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1),'Workspace fits mobile viewport');await page.screenshot({path:'artifacts/workspace-mobile.png',fullPage:true});await page.getByRole('button',{name:'Research (1)',exact:true}).click();await page.getByRole('dialog',{name:'Mobile research slip',exact:true}).waitFor();check(true,'Mobile research drawer');await page.getByRole('button',{name:'Close research slip',exact:true}).click();await page.getByRole('dialog',{name:'Mobile research slip',exact:true}).waitFor({state:'hidden'});
