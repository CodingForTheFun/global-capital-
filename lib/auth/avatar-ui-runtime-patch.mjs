...
const PROFILE_HEADINGS = [
  '<strong>Oblige Props account</strong>',
  '<strong>Auto Scout account</strong>',
];
...
export function patchProfileAvatarUi(source) {
  ...
  const profileHeading = PROFILE_HEADINGS.find(anchor => input.includes(anchor));
  if (!profileHeading) throw new Error('Profile avatar patch could not locate the account dropdown heading.');
  ...
  let patched = input
    ...
    .replace(profileHeading, PROFILE_HEADING_PATCHED)
