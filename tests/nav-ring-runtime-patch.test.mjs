...
test('nav/ring runtime patch keeps research basis and installs compact glass navigation', () => {
  ...
  assert.match(output, /fitNewsNav/, 'runtime nav fitting must preserve the News tab after later visual overrides');
  assert.match(output, /setProperty\('grid-template-columns','repeat\(7,minmax\(0,1fr\)\)','important'\)/, 'mobile runtime must force a seven-tab grid');
