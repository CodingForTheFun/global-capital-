...
export function createWorkspaceHandler({authenticate:auth=authenticate,read:provider=read,predict:prediction=predict,historyFallback=publicHistory,wnbaHistoryFallback=wnbaArchiveHistory,nflHistoryFallback=nflverseHistory,ncaafHistoryFallback=cfbArchiveHistory,now=Date.now}={}) {
  ...
  return async function handleWorkspace(req,res) {
    ...
    try {
      ...
      if(action==='history') {
        ...
        const attemptedSources=['PropLine','ESPN'];
        let fallback=null;
