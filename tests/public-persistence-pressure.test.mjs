...
test('direct-primary background writes retry direct once and never spill into PostgREST', async () => {
  const restore = {};
  const originalFetch = globalThis.fetch;
  const urls = [];
  try {
    ...
    assert.equal(urls.length, 2, 'one direct attempt plus one direct retry');
    assert.ok(urls.every((url) => url.endsWith('/functions/v1/autoscout-db-direct')));
    assert.ok(urls.every((url) => !url.includes('/rest/v1/rpc/')));
  } finally {
    globalThis.fetch = originalFetch;
    restoreEnv(restore);
  }
