...
export function researchClient(source) {
  ...
  // Keep the already-tested account/auth controls exactly as implemented by v5,
  // but move the menu out of the crowded research toolbar and pin it to the app
  // root so it remains reachable in the top-right corner on iPhone and desktop.
  const headerHook = " document.getElementById('asRefresh').onclick=()=>load();";
  const mobileMenuMarker = "profileMenu.classList.add('asHeaderAccount')";
  if (!client.includes(mobileMenuMarker) && client.includes(headerHook)) {
    client = client.replace(headerHook,
      " var profileMenu=document.getElementById('asProfileMenu'),accountRoot=document.getElementById('as5');if(profileMenu&&accountRoot){profileMenu.classList.add('asHeaderAccount');accountRoot.appendChild(profileMenu);}\n" + headerHook);
  }
