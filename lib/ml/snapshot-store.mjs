...
export function createMLStore({file = path.resolve(process.env.DATA_DIR || './data','ml/predictions.json'), clock = Date.now, pollMs = 5000,
  ...
  async function lookup(input) {
    ...
    // supplies probabilities; the adaptive model keeps supplying projections.
    if (global) {
      try {
        const g = await global.predict(t, learned);
        if (g) return g;
      } catch { /* the adaptive or snapshot answer stands */ }
    }
