...
const MODEL_DIR = path.resolve('data/ml/research');
const DIRECT_KEYS = Object.freeze({
  attempts:'passingAttempts',completions:'passingCompletions',passing_tds:'passingTouchdowns',receptions:'receptions',
  PTS:'points',REB:'rebounds',AST:'assists',FG3M:'threes',BLK:'blocks',STL:'steals',TOV:'turnovers',value:'value',
});
const COMBO_KEYS = Object.freeze({PRA:['points','rebounds','assists'],PR:['points','rebounds'],PA:['points','assists'],BS:['blocks','steals']});
const modelPromises=new Map();
async function loadModels(sport){
  sport=String(sport||'').toUpperCase();if(!/^[A-Z0-9]{2,8}$/.test(sport))return null;
  if(modelPromises.has(sport))return modelPromises.get(sport);
  const work=readFile(path.join(MODEL_DIR,`${sport.toLowerCase()}-ridge-v1.json`),'utf8').then(JSON.parse).then(pack=>{
    if(pack?.version!==1||pack?.engine!=='Auto Scout ML'||pack?.sport!==sport||!Array.isArray(pack.models))throw Error('bad research model pack');
    return new Map(pack.models.map(model=>[model.marketId,model]));
  }).catch(error=>{if(error?.code==='ENOENT')return null;throw error;});
  modelPromises.set(sport,work);return work;
}
...
function statFromRow(row,key){
  if(COMBO_KEYS[key]){const values=COMBO_KEYS[key].map(k=>Number(row?.[k]));return values.every(Number.isFinite)?values.reduce((a,b)=>a+b,0):null;}
  const mapped=DIRECT_KEYS[key]||key;const value=Number(row?.[mapped]);return Number.isFinite(value)?value:null;
}
function cleanValues(logs,key){return logs.map(row=>statFromRow(row,key)).filter(Number.isFinite);}
function featureSource(feature){const match=feature.match(/^(.+?)_(?:lag\d+|mean\d+|std\d+)$/);return match?.[1]||null;}
export function buildResearchFeatures(model,logs,target){
  ...
  for(const name of model.features){
    ...
    const key=featureSource(name);if(!key){out[name]=null;continue;}
...
export function createResearchProjectionStore({clock=Date.now}={}){
  ...
  return {async lookup(input){
    ...
    const models=await loadModels(target.sport);if(!models)return unavailable('SPORT_NOT_SUPPORTED','A validated Auto Scout research model pack has not been published for this sport yet.');
    const model=models.get(target.marketId);if(!model)return unavailable('MARKET_NOT_SUPPORTED','No validated Auto Scout model is available for this prop type yet.');
    ...
    const pending=(async()=>{
      const history=await researchPlayerProp({sport:target.sport,playerName:target.playerName,providerMarketKey:target.marketId,market:target.marketId,line:target.line,side:'OVER',games:40});
