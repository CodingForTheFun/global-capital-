...
async function request(url, apiKey, { fetchImpl = fetch, timeoutMs = 8000 } = {}) {
 ...
 *   nflSeason / nflWeek  required only for NFL, which is week-addressed.
  ...
  return {
    ...
    async fetchEnrichment({ props = [] } = {}) {
      ...
      for (const [sport, sportProps] of bySport) {
        ...
        let url;
        if (league.by === 'week') {
          // NFL is addressed by season and week, not by date.
          if (!nflSeason || !nflWeek) continue;
          url = `${BASE}/${league.path}/projections/json/PlayerGameProjectionStatsByWeek/${encodeURIComponent(nflSeason)}/${encodeURIComponent(nflWeek)}`;
        } else {
