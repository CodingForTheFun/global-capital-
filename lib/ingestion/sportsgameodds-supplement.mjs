...
// DFS fantasy market ids can be source-qualified upstream (for example
// "underdog:player_fantasy_score") while SportsGameOdds exposes the same
// settled stat as the canonical fantasy-score market. Treat only that fantasy
// family as equivalent; every other market still requires an exact id match.
function researchMarketIdentity(value) {
  const raw = text(value).toLowerCase();
  if (!raw.includes('fantasy')) return raw;
  const key = raw.includes(':') ? raw.slice(raw.lastIndexOf(':') + 1) : raw;
  if ([
    'player_fantasy_points',
    'player_pitcher_fantasy_score',
    'player_hitter_fantasy_score',
  ].includes(key)) return 'player_fantasy_score';
  return key;
}
...
export function sportsGameOddsIdentityFor({ sport, playerName, team = '', gameStartTime = '', marketKey: wantedMarketKey = '', marketName = '' } = {}) {
  ...
  const matches = board.props.filter((row) => {
    ...
    if (wantedMarket) {
      const rowMarket = marketKey(row);
      if (
        rowMarket !== wantedMarket
        && researchMarketIdentity(rowMarket) !== researchMarketIdentity(wantedMarket)
      ) return false;
    }
