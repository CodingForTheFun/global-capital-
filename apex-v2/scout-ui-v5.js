...
function shell(){
 ...
 document.querySelectorAll('[data-view]').forEach(b=>b.onclick=async()=>{closeDrawer();activeView=b.dataset.view;page=1;document.querySelectorAll('[data-view]').forEach(x=>x.classList.toggle('on',x===b));document.getElementById('asPageTitle').textContent=BOARD_VIEWS[activeView]||'Prop Research';renderControls();renderAdvanced();renderList();if(activeView==='saved'){b.disabled=true;try{await loadSaved();if(activeView==='saved'){renderControls();renderAdvanced();renderList();}}finally{b.disabled=false;}}});
 ...
 document.getElementById('asMenuDiscord').onclick=()=>document.getElementById('asDiscord').click();
