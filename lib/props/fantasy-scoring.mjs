...
export function scoreFantasyRow(row, spec, raw = null) {
  ...
  if (spec.id === 'nfl_offense') {
    const values = {
      ...
      twoPointConversions: rawCount(raw, 'twoPointConversions', 'twoPointConversion', 'twoPtConversions'),
      offensiveFumbleRecoveryTouchdowns: rawCount(raw, 'offensiveFumbleRecoveryTouchdowns', 'fumbleRecoveryTouchdowns'),
      returnTouchdowns: rawCount(raw, 'kickPuntFieldGoalReturnTouchdowns', 'returnTouchdowns', 'kickReturnTouchdowns'),
    };
    if (Object.values(values).some(value => value === null)) return null;
    return Number((
