...
const PUBLIC_PROP_TTL=150000;
const TEN_MINUTES=600000;
export const PUBLIC_FEEDS=Object.freeze([
 {id:'prizepicks',url:'https://api.prizepicks.com/projections?per_page=250',ttl:PUBLIC_PROP_TTL},
 {id:'underdog',url:'https://api.underdogfantasy.com/beta/v5/over_under_lines',ttl:PUBLIC_PROP_TTL},
...
export function createPublicFeeds({fetcher=globalThis.fetch,now=Date.now,storeDir=null,mappings=[],feeds=PUBLIC_FEEDS}={}){
 ...
 async function get(url){
  const response=await fetcher(url,{headers:{accept:'application/json'},signal:AbortSignal.timeout(12000),redirect:'error'});
