...
export function patchEdgeFrontdoor(source) {
  ...
  // /props/:id is a real shareable research URL. Internally reuse the existing
  // authenticated /apex shell so the API and auth contracts stay unchanged.
  output = output.replace(hook, `  {\n    const propUrl = new URL(req.url || '/', 'http://localhost');\n    if ((req.method === 'GET' || req.method === 'HEAD') && propUrl.pathname.startsWith('/props/')) req.url = '/apex';\n  }\n  if (await edgeGuest(req, res)) return;\n${hook}`);
  output = output.replace(injection, 'const injection = `<link rel="stylesheet" href="/assets/autoscout-home.css"><link rel="stylesheet" href="/assets/autoscout-reference-ui.css?v=20260913c"><link rel="stylesheet" href="/assets/autoscout-reference-ui-fixes.css?v=20260913c"><link rel="stylesheet" href="/assets/zero-credit-ui.css?v=20260913a"><link rel="manifest" href="/manifest.webmanifest"><script>${APEX_SHELL}</script><script src="/assets/autoscout-home.js" defer></script><script src="/assets/autoscout-tacos.js" defer></script>`;');
