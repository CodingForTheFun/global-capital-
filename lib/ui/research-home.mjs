...
export function researchClient(source) {
  ...
  const newProfile = '<details class="asProfileMenu" id="asProfileMenu"><summary aria-label="Account &amp; security menu" title="Account &amp; security"><svg class="asAccountIcon" viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="8" r="4"></circle><path d="M4.5 20c.8-4.1 3.3-6.2 7.5-6.2s6.7 2.1 7.5 6.2"></path></svg><span class="asProfileText">Account</span></summary><div class="asProfileDropdown"><strong>Oblige Props account</strong><button class="asBtn" id="asAccount">Account &amp; security</button><a class="asBtn" id="asFounding" href="/pricing">Founding Pro</a><button class="asBtn" id="asSecurity">Security &amp; devices</button><button class="asBtn" id="asMenuReset">Reset password</button><button class="asBtn" id="asMenuSaved" data-view="saved">Saved props</button><button class="asBtn" id="asSettings">Display settings</button><button class="asBtn asProfileSignOut" id="asMenuSignOut">Sign out</button></div></details>';
  const profileAnchor = oldProfiles.find(profile => client.includes(profile));
  if (profileAnchor) client = client.replace(profileAnchor, newProfile);
  ...
  if (!client.includes(secureBoot)) {
    if (client.includes(insecureBoot)) client = client.replace(insecureBoot, secureBoot);
    else if (client.includes('loadAccount(') || client.includes('loadSaved(')) {
      throw new Error('Oblige Props account boot changed; refusing to serve an unverified prop board.');
    }
  }

  // A just-opened prop can briefly disappear from groups() while Saved/filter state
  // reconciles. Keep the already-open verified group for the matching route instead
  // of ejecting the reader. Direct links still require a current or saved group
  // because drawerState is empty until a real card has been opened.
  const routeLookup = "var current=groups().find(x=>x.key===route.key),saved=savedRecords.get(route.key);\n var g=current||(saved?{...saved,archived:true}:null);";
  const resilientRouteLookup = "var current=groups().find(x=>x.key===route.key),saved=savedRecords.get(route.key),open=drawerState?.g?.key===route.key?drawerState.g:null;\n var g=current||(saved?{...saved,archived:true}:null)||open;";
  if (!client.includes(resilientRouteLookup)) {
    if (client.includes(routeLookup)) client = client.replace(routeLookup, resilientRouteLookup);
    else if (client.includes('function syncPropRoute(){')) {
      throw new Error('Oblige Props prop-route lookup changed; review transient route reconciliation.');
    }
  }
  // On a direct #prop URL, account/saved state can finish before the initial
  // sportsbook board. Do not classify the route as missing until that sport's
  // board has actually completed its first load. load() calls syncPropRoute()
  // again after payloadSport is set, so a genuinely absent route still fails
  // closed immediately after the authoritative board arrives.
  const prematureRouteReady = "if(!g&&!savedRouteReady)return;";
  const boardAwareRouteReady = "if(!g&&(!savedRouteReady||payloadSport!==route.sport))return;";
  if (!client.includes(boardAwareRouteReady)) {
    if (client.includes(prematureRouteReady)) client = client.replace(prematureRouteReady, boardAwareRouteReady);
    else if (client.includes('function syncPropRoute(){')) {
      throw new Error('Oblige Props route readiness guard changed; review direct-route boot ordering.');
    }
  }
  ...
  if (!client.includes(secureAfterAuth)) {
    if (client.includes(insecureAfterAuth)) client = client.replace(insecureAfterAuth, secureAfterAuth);
    else if (client.includes('async function afterAuthChange')) {
      throw new Error('Oblige Props auth-change flow changed; review the fail-closed redirect.');
    }
  }
...
const FOUNDING_LANDING = '<section aria-label="Founding Pro" style="margin:34px 0 0;padding:18px 20px;border:1px solid #2f6b52;border-radius:16px;background:linear-gradient(145deg,#102b24,#0b1a2c);display:flex;align-items:center;gap:18px;flex-wrap:wrap">'
  + '<div style="flex:1 1 320px"><span style="display:block;color:#7ef0bd;font-size:11px;font-weight:800;letter-spacing:.14em;text-transform:uppercase;margin-bottom:6px">FOUNDING PRO</span>'
  + '<b style="display:block;font-size:18px;margin-bottom:4px">Help build faster live data.</b>'
  + '<span style="display:block;color:#b8cadd;font-size:13px;line-height:1.6">Founding memberships help fund broader sportsbook feeds, permanent line history, infrastructure and deeper verified research.</span></div>'
  + '<a href="/pricing" style="display:inline-flex;min-height:44px;align-items:center;justify-content:center;padding:0 16px;border-radius:10px;background:#306fee;color:#fff;text-decoration:none;font-weight:750;white-space:nowrap">Explore Founding Pro</a>'
  + '</section>';
...
export function researchLanding(html) {
  ...
  return html.replace('<body>', '<body class="asResearchGate">')
    ...
    .replace('<div class="features">', `${FOUNDING_LANDING}<div class="features">`)
