import {normalizeCatalog,normalizeEvents,normalizeOffers,historyForMarket,sportCode,text} from './prop-workspace.mjs';
...
export function createWorkspaceHandler({authenticate:auth=authenticate,read:provider=read,predict:prediction=predict,now=Date.now}={}) {
  ...
  return async function handleWorkspace(req,res) {
    ...
    try {
      ...
      if(action==='history') {
        ...
        send(res,200,{ok:true,...historyForMarket(body,player,market)});return true;
      }
      const offer=market.offers.find(o=>o.key===url.searchParams.get('offer'));
      if(!offer||offer.line===null||!offer.side||!player.playerId||market.period||market.variant!=='standard'||(offer.multiplier!==null&&offer.multiplier!==1)) {
        send(res,200,{ok:true,prediction:{available:false,code:'TARGET_UNVERIFIED',message:'A validated model is not available for this exact selection.'}});return true;
      }
      const model=await prediction({sport:sportCode(sport),eventId:player.eventId,playerId:player.playerId,playerName:player.name,marketId:market.marketKey,sportsbookKey:offer.book,gameStartTime:player.startsAt,line:offer.line,entityType:'player',live:Date.parse(player.startsAt)<=now(),isAlternate:false});
      ...
      send(res,200,{ok:true,prediction:valid?model:{available:false,code:model?.code||'MODEL_NOT_READY',message:'No trained model has passed validation for this exact sport, market and line yet.'}});
    } catch(error) {
