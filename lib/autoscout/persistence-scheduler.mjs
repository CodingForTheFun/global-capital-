...
function persistenceSportsForMode() {
  if (propProviderMode() !== 'sportsgameodds') return PUBLIC_PERSISTENCE_SPORTS;
...
async function persistBoardsCycle(reason, deps, budgets) {
  ...
  try {
    ...
    const sgoOnly = propProviderMode() === 'sportsgameodds';
    if (!sgoOnly && publicWorkerConfigured()) {
      try {
    ...
    const perCycle = clampInt(process.env.AUTOSCOUT_PERSIST_SPORTS_PER_CYCLE, activePersistenceSports.length, 1, activePersistenceSports.length);
    for (const sport of nextSports(activePersistenceSports, perCycle)) {
      try {
        ...
        console.log(`[AutoScout persistence] ${reason} ${sport} ${sgoOnly ? 'sportsgameodds-only' : 'public-only'} persisted=${Boolean(result?.persisted)} events=${counts.events || 0} props=${counts.props || 0} lines=${counts.lines || 0} snapshots=${counts.snapshots || 0}`);
...
export function startSportsGameOddsLocalRefresh() {
  ...
  const primary = propProviderMode() === 'sportsgameodds';
...
export function startFrugalPersistence() {
  ...
  const perCycle = clampInt(process.env.AUTOSCOUT_PERSIST_SPORTS_PER_CYCLE, startupPersistenceSports.length, 1, startupPersistenceSports.length);
  const snapshotSeconds = clampInt(process.env.AUTOSCOUT_SNAPSHOT_INTERVAL_SECONDS, 300, 45, 3600);
  ...
  console.log(`[AutoScout persistence] scheduler interval=${ingestSeconds}s, sports-per-cycle=${perCycle}/${PUBLIC_PERSISTENCE_SPORTS.length}, snapshot-interval=${snapshotSeconds}s, provider-mode=${propProviderMode()}, public-feeds=${propProviderMode()==='sportsgameodds'?'off':'on'}, selected-provider-cache=enabled`);
  return timer;
}
