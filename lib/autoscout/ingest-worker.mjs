 ...
 */
async function executeIngestCycle({
  sports, fetchBoard, decorate = (b) => b, persist, log = console, stagger = async()=>{},
} = {}) {
  ...
  const targets = [...((config.sports.length ? config.sports : sports) || [])].sort((a,b)=>{
    const x=state.leagues.get(a),y=state.leagues.get(b);
    const priority=row=>row?.lastEvents>0?0:row?.emptyStreak?2:1;
    return priority(x)-priority(y)||(x?.nextStart||Infinity)-(y?.nextStart||Infinity);
  });
  ...
  for (const sport of targets) {
    ...
    try {
      ...
      const board = decorate(await fetchBoard(sport, { force: true, respectFresh: true }));
      const events = Number(board?.meta?.events || 0);
      const useful=board?.meta?.lineCount==null?events:Number(board.meta.lineCount);
      recordLeague(sport, { events:useful });
      const starts=(board?.data?.events||[]).map(e=>Date.parse(e.commenceTime)).filter(n=>Number.isFinite(n)&&n>Date.now()-6*3600000);
      state.leagues.get(sport).nextStart=starts.length?Math.min(...starts):null;

      if (!events || !useful || board?.meta?.stale || board?.meta?.cacheHit) {
...
export async function runIngestCycle(deps={}){
  if(state.cycleRunning)return {skipped:true,reason:'cycle already running'};
  state.cycleRunning=true;
  try{return await executeIngestCycle(deps);}finally{state.cycleRunning=false;}
}
...
export function startIngestWorker({ sports, fetchBoard, decorate, persist, persistenceConfigured, log = console } = {}) {
  ...
  const tick = async () => {
    try {
      const cycle = await runIngestCycle({ sports, fetchBoard, decorate, persist, log, stagger:()=>new Promise(resolve=>setTimeout(resolve,1000)) });
      if (!cycle.skipped) {
  ...
  state.timer = timer;
  state.bootstrapTimer=setTimeout(() => void tick(), 30_000);state.bootstrapTimer.unref?.();
...
export function stopIngestWorker() {
  ...
  clearTimeout(state.bootstrapTimer);state.bootstrapTimer=null;
  state.timer = null;
  state.running = false;
}
