...
for (const [name,mutate] of [
  ['player name',r=>r.player_name='Different Player'],
  ['season',r=>r.season='2025'],
  ['season type',r=>r.season_type=3],
  ['source',r=>r.source='OTHER'],
]) test(`persisted ${name} mismatch fails durable read-back`,async()=>{
  const p=plan(),stored=row();stored.source='ESPN';mutate(stored);
  const r=await runHistoryMaintenance(p,{mode:'apply',approveSnapshot:p.cohortHash,...dependencies(),
    readRows:async()=>[stored],insertRows:async()=>{throw Error('must not overwrite');}});
  assert.equal(r.outcomes[0].durableVerified,false);
  assert.equal(r.outcomes[0].status,'EXISTING_HISTORY_CONFLICT');
});
test('valid persisted ESPN row including richer extra stats remains accepted',async()=>{
  const p=plan(),stored=row();stored.source='ESPN';stored.stats.extraVerifiedField=123;
  const r=await runHistoryMaintenance(p,{mode:'apply',approveSnapshot:p.cohortHash,...dependencies(),
    readRows:async()=>[stored],insertRows:async()=>{throw Error('no write expected');}});
  assert.equal(r.outcomes[0].durableVerified,true);
  assert.equal(r.outcomes[0].status,'ALREADY_PERSISTED_AND_VERIFIED');
});
...
test('migration is isolated, active-board based, token-protected and insert-only',async()=>{
  ...
  assert.match(sql,/p_action is null or p_action not in \('read_keys','insert_verified'\)/);
});
