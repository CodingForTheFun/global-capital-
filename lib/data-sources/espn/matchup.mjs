...
export function normalizeMatchupSummary(summary,target,sourceEventId,{now=Date.now(),retrievedAt}={}){
 ...
 if(!Number.isFinite(retrieved)||retrieved>now||now-retrieved>=MAX_AGE)return absent('MATCHUP_STALE','The game summary needs a fresh observation.');
 ...
 const sourceUrl=`https://www.espn.com/${({NCAAF:'college-football',NCAAB:'mens-college-basketball'}[target.sport]||target.sport.toLowerCase())}/game/_/gameId/${sourceEventId}`;
