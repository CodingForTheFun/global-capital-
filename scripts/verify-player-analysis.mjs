...
try{for(const width of [390,1440])for(const sport of ['NBA','NFL','TENNIS']){
 ...
 if(tennis){await page.getByRole('heading',{name:'Opponent matchup',exact:true}).waitFor();assert.equal(await page.getByLabel('Defense team').count(),0);assert.equal(await page.getByRole('heading',{name:'Lineups & depth chart',exact:true}).count(),0);assert.ok((await log.innerText()).includes('Aces'));assert.ok(!(await log.innerText()).includes('PTS'));}
 else {await page.getByLabel('Defense team').selectOption('11');await page.getByRole('group',{name:'Opponent position'}).getByRole('button',{name:'C',exact:true}).click();assert.ok((await page.getByRole('region',{name:'Defense vs position',exact:true}).innerText()).includes('26th / 30'));}
