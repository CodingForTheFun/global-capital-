...
var bookFilter='all';
var selectedBooks=bookSelection(readStored('autoscout-selected-books',null));
var sideFilter='all';
var sortBy='shuffle';
restoreFilters();
...
function esc(v){return String(v==null?'':v).replace(/game logs?/gi,'historical results').replace(/[&<>\"]/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'}[c];});}
...
function researchSourceLabel(r){
 if(!r)return null;
 var candidates=[r.source,r.provider,r.coverage?.source,r.fallback?.source];
 for(var value of candidates){var label=canonicalSourceLabel(value,null);if(label)return label;}
 return null;
}
function freshnessLabelFromRows(rows){
 var values=(rows||[]).map(function(row){return Date.parse(row?.providerUpdatedAt||row?.updatedAt||row?.ingestedAt||'');}).filter(Number.isFinite);
 if(!values.length)return {text:'Update time unavailable',stale:false,updatedAt:null};
 var latest=Math.max(...values), age=Math.max(0,Date.now()-latest), seconds=Math.floor(age/1000);
 var text=seconds<60?'Updated '+seconds+'s ago':seconds<3600?'Updated '+Math.floor(seconds/60)+'m ago':seconds<86400?'Updated '+Math.floor(seconds/3600)+'h ago':'Updated '+Math.floor(seconds/86400)+'d ago';
 return {text,stale:age>10*60*1000,updatedAt:new Date(latest).toISOString()};
}
function sourceMeta(g,r){
 var rows=(g?.comparisonOffers||g?.rows||[]);
 var labels=uniq(rows.map(function(row){return canonicalSourceLabel(row?.source||row?.provider,row);}).filter(Boolean));
 var research=researchSourceLabel(r);if(research&&!labels.includes(research))labels.push(research);
 var label=labels.length>1?'Multi-source':labels[0]||'Verified data';
 var kind=label==='PropLine'?'propline':label==='SportsGameOdds'?'sportsgameodds':label==='ESPN'?'espn':label==='Multi-source'?'multi':'verified';
 return {labels,label,kind,count:labels.length,...freshnessLabelFromRows(rows)};
}
...
function mobileRefEsc(value){return String(value==null?'':value).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/"/g,'&quot;');}
function mobileRefOptionHtml(source){if(!source)return '';return Array.from(source.options||[]).map(function(o){return '<option value="'+mobileRefEsc(o.value)+'" '+(o.selected?'selected':'')+'>'+mobileRefEsc(o.textContent||'')+'</option>';}).join('');}
function mobileRefShortBook(name){return String(name||'').split(/\s+/).filter(Boolean).map(function(part){return part.charAt(0);}).join('').slice(0,2).toUpperCase()||'•';}
function mobileRefProxySelect(id,label,icon,source){
 var value=source?source.value:'',selected=source&&source.options?source.options[source.selectedIndex]:null,active=!!(value&&value!=='all'),shown=active?(selected?selected.textContent:value):label;
 return '<label class="asMobileRefFilter" data-ref-filter="'+id+'" data-active="'+active+'"><span class="asMobileRefFilterIcon" aria-hidden="true">'+icon+'</span><span class="asMobileRefFilterLabel">'+mobileRefEsc(shown||label)+'</span><span aria-hidden="true">⌄</span><select aria-label="'+mobileRefEsc(label)+'">'+mobileRefOptionHtml(source)+'</select></label>';
}
function renderMobileRefMenu(){
 var root=document.getElementById('as5'),menu=document.getElementById('asMobileRefMenu');if(!root||!menu)return;
 var sportButtons=Array.from(document.querySelectorAll('#asSports [data-sport]')),sportsRail=document.getElementById('asMobileRefSportsRail'),sportCount=document.getElementById('asMobileRefSportCount');
 if(sportCount)sportCount.textContent=sportButtons.length+' SPORTS';
 if(sportsRail){
  sportsRail.innerHTML=sportButtons.map(function(src){var name=src.dataset.sport||src.textContent||'';return '<button type="button" class="'+(src.classList.contains('on')?'on':'')+'" data-ref-sport="'+mobileRefEsc(name)+'" aria-pressed="'+(src.classList.contains('on')?'true':'false')+'">'+mobileRefEsc(name)+'</button>';}).join('');
  sportsRail.querySelectorAll('[data-ref-sport]').forEach(function(btn){btn.onclick=function(){var source=sportButtons.find(function(item){return item.dataset.sport===btn.dataset.refSport;});if(source)source.click();sportsRail.setAttribute('hidden','');var toggle=document.getElementById('asMobileRefSportsToggle');if(toggle)toggle.setAttribute('aria-expanded','false');};});
 }
 var books=Array.isArray(BOOKS)?BOOKS.map(function(book){return book.name||book.id;}).filter(Boolean):[],badges=document.getElementById('asMobileRefBookBadges'),more=document.getElementById('asMobileRefMore');
 if(badges)badges.innerHTML=books.slice(0,3).map(function(name){return '<span class="asMobileRefBookBadge" title="'+mobileRefEsc(name)+'">'+mobileRefEsc(mobileRefShortBook(name))+'</span>';}).join('');
 if(more){var remaining=Math.max(0,books.length-3);more.hidden=!remaining;more.textContent=remaining?'+ '+remaining:'';}
 var team=document.querySelector('#asAdvanced [data-advanced="team"]'),opponent=document.querySelector('#asAdvanced [data-advanced="opponent"]'),game=document.querySelector('#asAdvanced [data-advanced="game"]'),market=document.getElementById('asMarket'),side=document.getElementById('asSide'),day=document.querySelector('#asAdvanced [data-advanced="day"]'),dayValue=day&&day.value||'',filterRail=document.getElementById('asMobileRefFilterRail');
 if(filterRail){
  filterRail.innerHTML=mobileRefProxySelect('team','Teams','◈',team)+mobileRefProxySelect('opponent','Opponent','⚔',opponent)+mobileRefProxySelect('market','Stats','▥',market)+mobileRefProxySelect('game','Games','⚔',game)+'<label class="asMobileRefFilter" data-ref-filter="day" data-active="'+!!dayValue+'"><span class="asMobileRefFilterIcon" aria-hidden="true">▣</span><span class="asMobileRefFilterLabel">'+mobileRefEsc(dayValue||'Date')+'</span><span aria-hidden="true">⌄</span><input type="date" aria-label="Date" value="'+mobileRefEsc(dayValue)+'"></label>'+mobileRefProxySelect('side','Side','◒',side)+'<button class="asMobileRefFilter asMobileRefFilterButton" data-ref-more type="button"><span class="asMobileRefFilterIcon" aria-hidden="true">☷</span><span class="asMobileRefFilterLabel">More</span><span aria-hidden="true">⌄</span></button>';
  filterRail.querySelectorAll('select').forEach(function(proxy){proxy.onchange=function(){var kind=proxy.parentElement.dataset.refFilter,source=kind==='market'?document.getElementById('asMarket'):kind==='side'?document.getElementById('asSide'):document.querySelector('#asAdvanced [data-advanced="'+kind+'"]');if(!source)return;source.value=proxy.value;source.dispatchEvent(new Event('change',{bubbles:true}));setTimeout(renderMobileRefMenu,0);};});
  var dateProxy=filterRail.querySelector('[data-ref-filter="day"] input');if(dateProxy)dateProxy.onchange=function(){var source=document.querySelector('#asAdvanced [data-advanced="day"]');if(!source)return;source.value=dateProxy.value;source.dispatchEvent(new Event('change',{bubbles:true}));setTimeout(renderMobileRefMenu,0);};
  var moreBtn=filterRail.querySelector('[data-ref-more]');if(moreBtn)moreBtn.onclick=function(){var details=document.getElementById('asBoardFilterMenu');if(details)details.open=true;var trigger=document.getElementById('asAdvancedToggle');if(trigger)trigger.click();};
 }
 var metricRail=document.getElementById('asMobileRefMetricRail'),sort=document.getElementById('asSort');
 if(metricRail&&sort){
  var current=sort.value,filterText=(document.getElementById('asFilterCount')||{}).textContent||'',activeCount=(filterText.match(/\d+/)||[])[0]||'',defs=[['projection','IP'],['l5','L5'],['l10','L10'],['l15','L15'],['h2h','H2H'],['streak','Strk'],['books','Books']];
  metricRail.innerHTML=defs.map(function(def){if(!Array.from(sort.options).some(function(o){return o.value===def[0];}))return '';var on=current===def[0];return '<button class="asMobileRefMetric '+(on?'on':'')+'" type="button" data-ref-sort="'+def[0]+'" aria-pressed="'+(on?'true':'false')+'"><span>'+def[1]+'</span>'+(on&&activeCount?'<span class="asMobileRefMetricCount">'+activeCount+'</span>':'')+'<span class="asMobileRefSortGlyph" aria-hidden="true">↧</span></button>';}).join('');
  metricRail.querySelectorAll('[data-ref-sort]').forEach(function(btn){btn.onclick=function(){sort.value=btn.dataset.refSort;sort.dispatchEvent(new Event('change',{bubbles:true}));renderMobileRefMenu();};});
 }
 var sourceSearch=document.getElementById('asSearch'),proxySearch=document.getElementById('asMobileRefSearchInput');if(sourceSearch&&proxySearch&&proxySearch!==document.activeElement&&proxySearch.value!==sourceSearch.value)proxySearch.value=sourceSearch.value;
 var saved=document.querySelector('#as5 [data-view="saved"]'),savedButton=document.getElementById('asMobileRefSaved');if(saved&&savedButton)savedButton.setAttribute('aria-pressed',String(saved.classList.contains('on')));
 root.classList.add('asMobileRefReady');
}
function initMobileRefMenu(){
 var menu=document.getElementById('asMobileRefMenu');if(!menu||menu.dataset.bound==='1')return;menu.dataset.bound='1';
 var sportsToggle=document.getElementById('asMobileRefSportsToggle');if(sportsToggle)sportsToggle.onclick=function(){var rail=document.getElementById('asMobileRefSportsRail');if(!rail)return;var open=rail.hasAttribute('hidden');if(open)rail.removeAttribute('hidden');else rail.setAttribute('hidden','');sportsToggle.setAttribute('aria-expanded',String(open));};
 var profile=document.getElementById('asMobileRefProfile');if(profile)profile.onclick=function(){var source=document.getElementById('asProfileMenu');if(source)source.open=!source.open;};
 var apps=document.getElementById('asMobileRefApps');if(apps)apps.onclick=function(){var details=document.getElementById('asBoardFilterMenu');if(details)details.open=true;var source=document.getElementById('asBooksOpen');if(source)source.click();};
 var saved=document.getElementById('asMobileRefSaved');if(saved)saved.onclick=function(){var source=document.querySelector('#as5 [data-view="saved"]');if(source)source.click();setTimeout(renderMobileRefMenu,0);};
 var searchToggle=document.getElementById('asMobileRefSearchToggle'),searchRow=document.getElementById('asMobileRefSearch'),searchInput=document.getElementById('asMobileRefSearchInput');
 if(searchToggle&&searchRow&&searchInput)searchToggle.onclick=function(){var open=searchRow.hasAttribute('hidden');if(open){searchRow.removeAttribute('hidden');setTimeout(function(){searchInput.focus();},0);}else searchRow.setAttribute('hidden','');searchToggle.setAttribute('aria-pressed',String(open));};
 if(searchInput)searchInput.oninput=function(){var source=document.getElementById('asSearch');if(!source)return;if(source.value!==searchInput.value)source.value=searchInput.value;source.dispatchEvent(new Event('input',{bubbles:true}));};
 document.addEventListener('keydown',function(event){if(!(event.ctrlKey||event.metaKey)||String(event.key||'').toLowerCase()!=='k'||!window.matchMedia('(max-width:760px)').matches)return;event.preventDefault();event.stopImmediatePropagation();if(searchRow&&searchRow.hasAttribute('hidden'))searchRow.removeAttribute('hidden');if(searchToggle)searchToggle.setAttribute('aria-pressed','true');if(searchInput)searchInput.focus();},true);
 renderMobileRefMenu();
}
...
<style id="as-mobile-reference-menu-style">
#as5 .asMobileRefMenu{display:none}
@media(max-width:760px){
 #as5.asMobileRefReady .asMobileRefMenu{display:grid;min-width:0;gap:9px;padding:0 10px 10px;background:#070a18;color:#f7f8ff}
 #as5.asMobileRefReady .asTop{position:sticky!important;top:0!important;z-index:86!important;padding:0!important;border-bottom:1px solid #20294a!important;background:#070a18!important;box-shadow:0 16px 34px rgba(0,0,0,.34)!important;overflow:visible!important}
 #as5.asMobileRefReady .asTop>.asBar{position:absolute!important;inset:0 auto auto 0!important;width:0!important;height:0!important;min-height:0!important;padding:0!important;margin:0!important;overflow:visible!important;grid-template-columns:0!important;grid-template-areas:none!important}
 #as5.asMobileRefReady .asTop>.asBar>.asIdentity,#as5.asMobileRefReady .asTop>.asBar>.asHeaderSearch,#as5.asMobileRefReady .asTop>.asBar>.asSports,#as5.asMobileRefReady .asTop>.asBar>.asGrow,#as5.asMobileRefReady .asTop>.asBar>.asStatus,#as5.asMobileRefReady .asTop>.asBar>#asRefresh,#as5.asMobileRefReady .asTop>.asBar>.asRight>.asProfileMenu>summary{display:none!important}
 #as5.asMobileRefReady .asRight{position:static!important;width:0!important;height:0!important;min-width:0!important;max-width:0!important;margin:0!important}
 #as5.asMobileRefReady #asProfileMenu{position:fixed!important;top:0!important;right:0!important;width:0!important;height:0!important;margin:0!important;z-index:121!important;overflow:visible!important}
 #as5.asMobileRefReady #asProfileMenu>summary{display:none!important}
 #as5.asMobileRefReady #asProfileMenu[open] .asProfileDropdown{display:grid!important;position:fixed!important;top:68px!important;right:10px!important;left:auto!important;width:min(300px,calc(100vw - 20px))!important;z-index:122!important;visibility:visible!important;opacity:1!important;pointer-events:auto!important}
 #as5 .asMobileRefHead{display:flex;min-width:0;min-height:78px;align-items:center;justify-content:space-between;gap:12px;margin:0 -10px;padding:max(14px,env(safe-area-inset-top)) 13px 12px;background:radial-gradient(420px 130px at 8% 115%,rgba(72,77,255,.78),transparent 72%),linear-gradient(180deg,#070a1b 0%,#101538 56%,#3038e9 100%)}
 #as5 .asMobileRefTitle{display:flex;min-width:0;align-items:center;gap:10px}
 #as5 .asMobileRefBoardIcon{display:grid;width:27px;height:27px;place-items:center;flex:none;border:2px solid #eef1ff;border-radius:5px;color:#eef1ff;font-size:15px;line-height:1}
 #as5 .asMobileRefTitle>b{font-size:22px;font-weight:850;letter-spacing:-.025em}
 #as5 .asMobileRefSlash{color:#a5abc9;font-size:25px;font-weight:700}
 #as5 .asMobileRefSportsToggle{display:inline-flex;min-height:42px;align-items:center;gap:8px;padding:0 14px;border:1px solid #7c92c7;border-radius:18px;background:rgba(80,104,161,.34);color:#f4f6ff;font:800 13px/1 inherit;letter-spacing:.015em;white-space:nowrap}
 #as5 .asMobileRefProfile{display:grid;width:44px;height:44px;place-items:center;flex:none;padding:0;border:0;border-radius:999px;background:linear-gradient(145deg,#784fff,#4b62ff);color:#fff;box-shadow:0 8px 22px rgba(31,35,119,.38)}
 #as5 .asMobileRefSportsRail,#as5 .asMobileRefFilterRail,#as5 .asMobileRefMetricRail{display:flex;min-width:0;gap:8px;overflow-x:auto;overflow-y:hidden;overscroll-behavior-inline:contain;scrollbar-width:none;-webkit-overflow-scrolling:touch;scroll-snap-type:x proximity;touch-action:pan-x}
 #as5 .asMobileRefSportsRail::-webkit-scrollbar,#as5 .asMobileRefFilterRail::-webkit-scrollbar,#as5 .asMobileRefMetricRail::-webkit-scrollbar{display:none}
 #as5 .asMobileRefSportsRail[hidden]{display:none}
 #as5 .asMobileRefSportsRail button{min-height:36px;flex:0 0 auto;padding:0 13px;border:1px solid #293355;border-radius:12px;background:#11172a;color:#aeb7d2;font:750 11px/1 inherit;scroll-snap-align:start}
 #as5 .asMobileRefSportsRail button.on{border-color:#6d95ff;background:#182344;color:#fff}
 #as5 .asMobileRefAppsRow{display:grid;grid-template-columns:minmax(0,1fr) 52px 52px;gap:8px}
 #as5 .asMobileRefApps,#as5 .asMobileRefSquare{min-height:56px;border:1px solid #252d49;border-radius:17px;background:#171d33;color:#f1f3fb;box-shadow:inset 0 1px 0 rgba(255,255,255,.025)}
 #as5 .asMobileRefApps{display:flex;min-width:0;align-items:center;gap:8px;padding:0 14px;text-align:left}
 #as5 .asMobileRefApps>b{margin-right:auto;font-size:15px;font-weight:850}
 #as5 .asMobileRefBookBadges{display:flex;align-items:center;gap:4px;flex:none}
 #as5 .asMobileRefBookBadge{display:grid;width:27px;height:27px;place-items:center;border:1px solid rgba(255,255,255,.08);border-radius:9px;color:#fff;font:900 8px/1 inherit;letter-spacing:-.02em}
 #as5 .asMobileRefBookBadge:nth-child(1){background:linear-gradient(145deg,#273451,#45a9a5)}
 #as5 .asMobileRefBookBadge:nth-child(2){background:linear-gradient(145deg,#6d2cff,#aa36ff)}
 #as5 .asMobileRefBookBadge:nth-child(3){background:linear-gradient(145deg,#c6a629,#3d361f)}
 #as5 .asMobileRefMore{flex:none;padding:6px 9px;border:1px solid #30384e;border-radius:999px;background:#242a38;color:#f5f6fa;font:750 11px/1 inherit}
 #as5 .asMobileRefSquare{display:grid;width:52px;place-items:center;padding:0;color:#aab2cb}
 #as5 .asMobileRefSquare[aria-pressed="true"]{border-color:#608dff;background:#192344;color:#f8f9ff}
 #as5 .asMobileRefSearch{display:grid;grid-template-columns:18px minmax(0,1fr);align-items:center;gap:8px;min-height:46px;padding:0 12px;border:1px solid #2b3658;border-radius:14px;background:#11172a;color:#929cb9}
 #as5 .asMobileRefSearch[hidden]{display:none}
 #as5 .asMobileRefSearch input{width:100%;height:42px;min-width:0;padding:0;border:0;outline:0;background:transparent;color:#f5f7ff;font:650 13px/1.2 inherit}
 #as5 .asMobileRefSearch input::placeholder{color:#77839e}
 #as5 .asMobileRefFilterRail{margin:0 -10px;padding:0 10px 2px}
 #as5 .asMobileRefFilter{position:relative;display:inline-flex;min-width:138px;max-width:190px;min-height:54px;align-items:center;gap:9px;flex:0 0 auto;padding:0 13px;border:1px solid #252d48;border-radius:16px;background:#1a1f34;color:#aab1c7;scroll-snap-align:start;overflow:hidden}
 #as5 .asMobileRefFilter[data-active="true"]{border-color:#648fff;background:#1a2748;color:#f4f6ff}
 #as5 .asMobileRefFilterIcon{font-size:18px;line-height:1;flex:none}
 #as5 .asMobileRefFilterLabel{min-width:0;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:13px;font-weight:760}
 #as5 .asMobileRefFilter>select,#as5 .asMobileRefFilter>input{position:absolute;inset:0;width:100%;height:100%;opacity:0;cursor:pointer}
 #as5 .asMobileRefFilterButton{cursor:pointer}
 #as5 .asMobileRefMetricRail{margin:0 -10px;padding:2px 10px 0;border-top:1px solid #1f2740}
 #as5 .asMobileRefMetric{display:inline-flex;min-width:91px;min-height:46px;align-items:center;justify-content:center;gap:7px;flex:0 0 auto;padding:0 11px;border:1px solid #222b47;border-radius:14px;background:#0c1120;color:#9ea7c0;font:760 13px/1 inherit;scroll-snap-align:start}
 #as5 .asMobileRefMetric.on{border-color:#5d8dff;background:#17223d;color:#f8f9ff;box-shadow:inset 0 0 0 1px rgba(75,126,255,.16)}
 #as5 .asMobileRefMetricCount{display:grid;min-width:22px;height:22px;place-items:center;padding:0 5px;border-radius:999px;background:#4f7ff5;color:#fff;font:900 10px/1 inherit}
 #as5 .asMobileRefSortGlyph{color:#9099b3;font-size:18px}
 #as5.asMobileRefReady .asHero,#as5.asMobileRefReady .asPropTypes,#as5.asMobileRefReady .asQuick,#as5.asMobileRefReady .asSummary{display:none!important}
 #as5.asMobileRefReady .asBoardFilterMenu{display:block!important;height:0!important;min-height:0!important;margin:0!important;padding:0!important;border:0!important;overflow:visible!important}
 #as5.asMobileRefReady .asBoardFilterMenu>summary,#as5.asMobileRefReady .asBoardFilterMenu>.asFilters,#as5.asMobileRefReady .asBoardFilterMenu>.asToolbar{display:none!important}
 #as5.asMobileRefReady .asMain{padding-top:9px!important}
}
@media(max-width:420px){
 #as5 .asMobileRefTitle{gap:8px}
 #as5 .asMobileRefTitle>b{font-size:20px}
 #as5 .asMobileRefSlash{font-size:22px}
 #as5 .asMobileRefSportsToggle{min-height:40px;padding:0 11px;font-size:12px}
 #as5 .asMobileRefAppsRow{grid-template-columns:minmax(0,1fr) 48px 48px}
 #as5 .asMobileRefSquare{width:48px}
 #as5 .asMobileRefBookBadge:nth-child(3){display:none}
 #as5 .asMobileRefFilter{min-width:132px}
 #as5 .asMobileRefMetric{min-width:86px}
}
</style><header class="asTop"><div class="asBar"><a class="asIdentity" href="/apex" aria-label="Auto Scout Research"><div class="asLogo">A</div><span class="asBrand">Props</span></a><div class="asSports" id="asSports" aria-label="Sports"></div><span class="asGrow"></span><span class="asStatus" id="asStatus">Current sportsbook lines</span><button class="asBtn" id="asRefresh">Refresh</button></div><div class="asMobileRefMenu" id="asMobileRefMenu">
 <div class="asMobileRefHead">
  <div class="asMobileRefTitle"><span class="asMobileRefBoardIcon" aria-hidden="true">▥</span><b>Props</b><span class="asMobileRefSlash">/</span><button class="asMobileRefSportsToggle" id="asMobileRefSportsToggle" type="button" aria-expanded="false"><span id="asMobileRefSportCount">SPORTS</span><span aria-hidden="true">⌄</span></button></div>
  <button class="asMobileRefProfile" id="asMobileRefProfile" type="button" aria-label="Open account menu"><svg viewBox="0 0 24 24" width="25" height="25" fill="currentColor" aria-hidden="true"><circle cx="12" cy="8" r="4"></circle><path d="M4 21c.7-4.3 3.2-6.5 8-6.5s7.3 2.2 8 6.5Z"></path></svg></button>
 </div>
 <div class="asMobileRefSportsRail" id="asMobileRefSportsRail" hidden></div>
 <div class="asMobileRefAppsRow">
  <button class="asMobileRefApps" id="asMobileRefApps" type="button"><b>Apps</b><span class="asMobileRefBookBadges" id="asMobileRefBookBadges"></span><span class="asMobileRefMore" id="asMobileRefMore" hidden></span><span aria-hidden="true">⌄</span></button>
  <button class="asMobileRefSquare" id="asMobileRefSearchToggle" type="button" aria-label="Search props" aria-pressed="false"><svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><circle cx="11" cy="11" r="7"></circle><path d="m20 20-4.2-4.2"></path></svg></button>
  <button class="asMobileRefSquare" id="asMobileRefSaved" type="button" aria-label="Saved props" aria-pressed="false"><svg viewBox="0 0 24 24" width="21" height="21" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><path d="M6 4h12v17l-6-4-6 4V4Z" stroke-linejoin="round"></path></svg></button>
 </div>
 <label class="asMobileRefSearch" id="asMobileRefSearch" hidden><svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><circle cx="11" cy="11" r="7"></circle><path d="m20 20-4.2-4.2"></path></svg><input id="asMobileRefSearchInput" type="search" placeholder="Search players, teams, or props…" autocomplete="off" aria-label="Search player, market or team"></label>
 <div class="asMobileRefFilterRail" id="asMobileRefFilterRail" aria-label="Prop filters"></div>
 <div class="asMobileRefMetricRail" id="asMobileRefMetricRail" role="group" aria-label="Sort props"></div>
</div></header>
 ...
 <details class="asBoardFilterMenu" id="asBoardFilterMenu"><summary>Find props &amp; filters</summary><section class="asFilters" aria-label="Filter player props"><label class="asSearchLabel"><span class="asSrOnly">Search player, market or team</span><input class="asControl" id="asSearch" placeholder="Search player, market, team…" type="search"></label><label><span class="asSrOnly">Market</span><select class="asControl" id="asMarket"></select></label><button class="asControl" id="asBooksOpen" aria-haspopup="dialog">All books</button><label><span class="asSrOnly">Over or Under</span><select class="asControl" id="asSide"><option value="all">Over + Under</option><option value="OVER">Over</option><option value="UNDER">Under</option></select></label><button class="asBtn asFilterTrigger" id="asAdvancedToggle" aria-expanded="false">Filters <span id="asFilterCount"></span></button></section>
 <dialog id="asBooksDialog" class="asFilterSheet asBooksDialog" aria-labelledby="asBooksTitle"><div class="asSheetHead"><h2 id="asBooksTitle">Platforms</h2><button class="asBtn" id="asBooksDone">Done</button></div><div id="asBooksChoices"></div></dialog><dialog id="asFilterSheet" class="asFilterSheet" aria-labelledby="asFilterTitle"><div class="asSheetHead"><h2 id="asFilterTitle">Research filters</h2><button class="asBtn" id="asFilterDone">Done</button></div><section id="asAdvanced" class="asAdvanced"></section></dialog><div class="asToolbar"><span id="asResultCount" aria-live="polite"></span><button class="asBtn" id="asRules" role="switch" aria-checked="true">Rules on</button><button class="asBtn" id="asColumns">Columns</button><details class="asProfileMenu" id="asProfileMenu"><summary aria-label="Account menu">Account</summary><div class="asProfileDropdown"><strong>Auto Scout account</strong><button class="asBtn" id="asAccount">Manage account &amp; billing</button><button class="asBtn" data-view="saved">Saved props</button><button class="asBtn" id="asSettings">Display settings</button><button class="asBtn" id="asMenuSignOut">Sign out</button></div></details><button class="asBtn" id="asResearchBatch">Load research</button><label>Sort <select class="asControl" id="asSort"><option value="shuffle">Shuffled (no order)</option><option value="research">Research coverage</option><option value="recent">Recent hit rate</option><option value="l5">L5 hit rate</option><option value="l10">L10 hit rate</option><option value="l15">L15 hit rate</option><option value="l20">L20 hit rate</option><option value="season">Season hit rate</option><option value="h2h">H2H hit rate</option><option value="streak">Streak</option><option value="projection">Projection difference</option><option value="books">Most books</option><option value="player">Player A–Z</option><option value="time">Game time</option></select></label></div>
 </details><p class="asNotice" id="asViewNote" hidden></p><section class="asQuick" id="asQuick" aria-label="Quick filters"></section><section class="asSummary" id="asSummary" aria-label="Board summary"></section><div class="asTableViewport"><div class="asHeaderRow"><span>Player / Market</span><span>Line</span><span>Projection</span><span>L5</span><span>L10</span><span>L15</span><span>Season</span><span>H2H</span><span>Average</span><span>Books</span></div><section class="asList" id="asList" aria-label="Player props"></section></div><div id="asMore"></div><p class="asCoverageNote">Browse by prop type. Each player appears once; use the card selector to switch props or games. Statistics use verified historical results only. N/A means no verified sample for that split; it is not zero.</p></main>
 <nav class="asNav" aria-label="Main navigation"><button data-view="research" class="on" aria-current="page" data-icon="props"><span class="asNavIcon" aria-hidden="true">☲</span><span>Props</span></button><button data-view="players" data-icon="players"><span class="asNavIcon" aria-hidden="true">●</span><span>Players</span></button><button data-view="popular" data-icon="popular"><span class="asNavIcon" aria-hidden="true">▲</span><span>Popular</span></button><button data-view="discrepancies" data-icon="trend" aria-label="Compare line discrepancies"><span class="asNavIcon" aria-hidden="true">↗</span><span>Compare</span></button><button data-view="saved" data-icon="saved" aria-label="Saved props"><span class="asNavIcon" aria-hidden="true"><svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M6 4h12v17l-6-4-6 4V4Z" stroke-linejoin="round"/></svg></span><span>Saved</span></button></nav>
 <div class="asDetailPage" id="asDrawerBg" hidden><main class="asDrawer asAnalyticsPage" aria-labelledby="asDrawerTitle" tabindex="-1"><div class="asDrawerHead"><div class="asDrawerAvatar"><img id="asDrawerImg" alt=""></div><div><h2 id="asDrawerTitle">Player research</h2><span id="asInjuryBadge" class="asInjuryBadge" hidden></span><p id="asDrawerSub"></p></div><button class="asClose" id="asClose" aria-label="Back to research"><span class="asBackText">Back</span><span aria-hidden="true">×</span></button></div><div class="asDrawerBody" id="asDrawerBody"></div></main></div>
 ...
 document.getElementById('asBooksOpen').onclick=()=>{renderBookDrawer();document.getElementById('asBooksDialog').showModal();};
 document.getElementById('asBooksDone').onclick=()=>document.getElementById('asBooksDialog').close();
 document.getElementById('asBooksDialog').onclose=()=>document.getElementById('asBooksOpen').focus();
 ...
 document.getElementById('asClose').onclick=()=>{if(history.state?.asPropRoute)history.back();else closeDrawer();};
 document.querySelectorAll('[data-view]').forEach(b=>b.onclick=async()=>{closeDrawer();activeView=b.dataset.view;page=1;document.querySelectorAll('[data-view]').forEach(x=>{var selected=x.dataset.view===activeView;x.classList.toggle('on',selected);if(selected)x.setAttribute('aria-current','page');else x.removeAttribute('aria-current');});document.getElementById('asPageTitle').textContent=BOARD_VIEWS[activeView]||'Prop Research';renderControls();renderAdvanced();renderList();if(activeView==='saved'){b.disabled=true;try{await loadSaved();if(activeView==='saved'){renderControls();renderAdvanced();renderList();}}finally{b.disabled=false;}}});
 document.getElementById('asAccount').onclick=accountPanel;
 document.getElementById('asSettings').onclick=settingsPanel;
 document.getElementById('asMenuSignOut').onclick=async()=>{var result=await accountPost('/api/account/logout');if(result.status===200)await afterAuthChange('Signed out.');else toast('Sign out could not complete. Try again.');};
 document.querySelectorAll('.asProfileDropdown button').forEach(b=>b.addEventListener('click',()=>{document.getElementById('asProfileMenu').open=false;}));
 document.addEventListener('click',e=>{var m=document.getElementById('asProfileMenu');if(m&&!m.contains(e.target))m.open=false;});
 document.addEventListener('keydown',e=>{if(e.key==='Escape'&&document.getElementById('asProfileMenu').open){document.getElementById('asProfileMenu').open=false;document.querySelector('.asProfileMenu summary').focus();}});
 document.addEventListener('keydown',e=>{if(drawerState&&e.key==='Escape'){e.preventDefault();document.getElementById('asClose').click();}});
 window.addEventListener('popstate',syncPropRoute);window.addEventListener('hashchange',syncPropRoute);
...
function closeDrawer(clearRoute=true){
 var wasOpen=!!drawerState;intelligence?.disposeDetails();document.getElementById('asDrawerBg').hidden=true;
 document.getElementById('as5').classList.remove('asDetailActive');drawerState=null;
 if(clearRoute&&propRoute())history.replaceState(null,'',location.pathname+location.search);
 if(wasOpen){window.scrollTo(0,boardScroll);if(lastFocus?.isConnected)lastFocus.focus({preventScroll:true});}
}
function syncPropRoute(){
...
function renderControls(){var gs=viewGroups(),markets=uniq([...gs.map(g=>g.market),marketFilter==='all'?null:marketFilter]).sort();document.getElementById('asMarket').innerHTML='<option value="all">All players · grouped props</option>'+markets.map(x=>'<option value="'+esc(x)+'" '+(x===marketFilter?'selected':'')+'>'+esc(x)+'</option>').join('');document.getElementById('asBooksOpen').textContent=selectedBooks===null?'All books':selectedBooks.length===1?bookInfo(selectedBooks[0]).name:selectedBooks.length+' books';}
function setBooks(value){selectedBooks=bookSelection(value);bookFilter='all';storeLocal('autoscout-selected-books',selectedBooks);page=1;staleCache.clear();renderControls();renderAdvanced();renderList();renderBookDrawer();}
function renderBookDrawer(){
 var live=new Map();groups(true).filter(g=>g.sport===sport).forEach(g=>g.rows.forEach(r=>live.set(bookId(r.sportsbookKey),r.sportsbook)));
 var registry=[...BOOKS,...[...live].filter(([id])=>!BOOKS.some(b=>b.id===id)).map(([id,name])=>({...bookInfo(id),name}))];
 var host=document.getElementById('asBooksChoices');host.innerHTML='<label class="asBooksAll"><input id="asBooksAll" type="checkbox" '+(selectedBooks===null?'checked':'')+'> Select all platforms</label><p class="asNotice">Selection filters the available lines. Some platforms may have no current player props.</p>'+registry.map(b=>'<div class="asBookChoice"><label><input type="checkbox" data-book-check="'+esc(b.id)+'" '+(selectedBooks===null||selectedBooks.includes(b.id)?'checked':'')+'><span class="asBookBadge" style="--book-color:'+b.badgeColor+'">'+esc(b.name.slice(0,2).toUpperCase())+'</span><span>'+esc(b.name)+'<small>'+esc(live.has(b.id)?'On this board':'No current props')+'</small></span></label><button class="asBtn" data-book-only="'+esc(b.id)+'" aria-label="Only '+esc(b.name)+'">ONLY</button></div>').join('');
 var all=host.querySelector('#asBooksAll');all.indeterminate=selectedBooks!==null&&selectedBooks.length>0;all.onchange=()=>setBooks(all.checked?null:[]);
 host.querySelectorAll('[data-book-check]').forEach(el=>el.onchange=()=>{var next=new Set(selectedBooks===null?registry.map(b=>b.id):selectedBooks);el.checked?next.add(el.dataset.bookCheck):next.delete(el.dataset.bookCheck);setBooks([...next]);host.querySelector('[data-book-check="'+el.dataset.bookCheck+'"]').focus();});
 host.querySelectorAll('[data-book-only]').forEach(el=>el.onclick=()=>{setBooks([el.dataset.bookOnly]);host.querySelector('[data-book-only="'+el.dataset.bookOnly+'"]').focus();});
}
window.addEventListener('storage',e=>{if(e.key==='autoscout-selected-books'){selectedBooks=bookSelection(readStored(e.key,null));bookFilter='all';closeDrawer();renderControls();renderAdvanced();renderList();if(document.getElementById('asBooksDialog').open)renderBookDrawer();}});
...
function ringGauge(rates,r){
 if(!rates)return '<div class="asRing asRingEmpty" title="'+esc(r?.message||researchState(r))+'">'
  +(!r&&!hydrateFailed?'<span class="asStatSkeleton" aria-label="Loading historical results"></span>':'<b>N/A</b>')
...
// numbers come from real historical results, these come from a model reading them.
//
...
function sandboxPanel(g,line,side){
 ...
 if(!roster||!roster.available){
  return '<p class="asNotice">'+esc(roster?.reason==='TEAM_UNKNOWN'?'Select a player with a verified team to load teammates.':roster?.reason==='ROSTER_ACCESS_DENIED'?'The connected sports data plan does not provide this team roster.':'Teammate data is temporarily unavailable for this team.')+'</p>';
 }
 if(!roster.teammates.length)return '<p class="asNotice">No team-mates were returned for this team.</p>';
 var chips=roster.teammates.map(function(mate){
 ...
 return '<p class="asNotice">Mark a team-mate out and re-estimate. The model re-reads the same historical results with that absence noted'
...
var GAP_COPY={
 ...
 gameLog:'a full historical results',
};
...
function projectionCard(g,line){
 ...
 if(!entry){
  ...
  // whose historical results was too short to measure a spread from, which is the only
  ...
  return '<section class="asSection asProjection">'+head+'<div class="asSectionBody">'
   +'<p class="asNotice">'+(other
     ? 'This prop\u2019s historical results was too short to re-price the estimate at a new line. Run it again to estimate against this line directly.'
 ...
 return '<section class="asSection asProjection">'+head+'<div class="asSectionBody">'
  ...
  +'<p class="asProjFootnote">'+esc(entry.repriced
     ? 'Projection of '+dec(entry.projection,1)+' from the model run at '+dec(entry.modelLine,1)+', re-priced against this line from the spread of the player\u2019s own historical results. Run it again to re-estimate the projection itself.'
...
async function loadRoster(g){
 var rosterTeam=displayTeam(g.team||drawerState?.base?.player?.team||drawerState?.base?.context?.team||'');
 if(!rosterTeam){sandbox.roster={available:false,reason:'TEAM_UNKNOWN'};return;}
 if(sandbox.key===g.key&&((sandbox.roster&&sandbox.roster.reason!=='TEAM_UNKNOWN')||sandbox.loading))return;
 sandbox={key:g.key,out:new Set(),roster:null,loading:true};
 renderDrawer();
 var query=new URLSearchParams({sport:g.sport,team:rosterTeam,playerName:g.playerName});
 var out=null;
 try{
...
function calibrationNote(entry){
 ...
 if(!c||!c.applied)return 'Model estimate — not a measured statistic. The historical results was too short to calibrate against.';
 ...
 return '<article class="asRow asCard" data-open="'+esc(g.key)+'" tabindex="0" aria-label="Research '+esc(g.playerName+' '+g.market)+'">'
  +'<div class="asCardHead">'
   ...
   +'<div class="asCardId"><div class="asCardName"><span class="asPlayer">'+esc(r?.entityType==='team'?r.player?.playerName||g.playerName:g.playerName)+'</span>'
    ...
    +(position?'<span class="asPos">'+esc(position)+'</span>':'')+'<span class="asLeagueBadge">'+esc(g.sport)+'</span>'
    +'<span class="asSourceChip '+esc(provenance.kind)+'" title="'+esc(provenance.labels.join(' + ')||provenance.label)+'">'+esc(provenance.label)+'</span>'
    +(provenance.count>1?'<span class="asSourceCount">'+provenance.count+' sources</span>':'')+'</div>'
   ...
   +'</div>'
   +'<div class="asCardGauge">'+ringGauge(gaugeRates(r,side),r)+'</div>'
  +'</div>'
  +matchupPills(r)
  +badgeStrip(g,r,side)
  +staleBadge(g)
  ...
  +'<div class="asRowActions"><span class="asResearchState '+(r&&r.available?'ready':'')+'">'
   ...
   +'<span class="asBookCount">'+books(g).length+' book'+(books(g).length===1?'':'s')+'</span>'
   +'<span class="asSourceFresh '+(provenance.stale?'stale':'')+'">'+esc(g.archived?'Saved '+shortDate(g.savedAt):provenance.text)+'</span>'
...
function renderBatchControl(){
 ...
 batch.textContent=hydrating?'Loading historical results…':retry?'Retry research':pending?'Load visible research':'Visible research loaded';
...
// entirely. Every figure here is averaged from the real historical results, and a metric
...
function openDrawer(g,{fromRoute=false}={}){
 if(!drawerState){lastFocus=document.activeElement;boardScroll=window.scrollY;}
 if(!fromRoute){var url=location.pathname+location.search+'#prop/'+g.sport+'/'+encodeURIComponent(g.key);history[drawerState?'replaceState':'pushState']({asPropRoute:true},'',url);}
 if(sandbox.key!==g.key)sandbox={key:g.key,out:new Set(),roster:null,loading:false};
 drawerState={g,line:boardLine(g),side:defaultSide(g),window:'l10',filter:'all',propFilters:{},base:null,panel:'overview',historyBook:bookFilter!=='all'?bookFilter:books(g)[0]};
 document.getElementById('asDrawerBg').hidden=false;document.getElementById('as5').classList.add('asDetailActive');renderDrawer();window.scrollTo(0,0);document.getElementById('asClose').focus({preventScroll:true});
...
function renderDrawer(){
 if(!drawerState)return;var keepOpen=['.asMorePropFilters','.asDetailModels'].filter(selector=>document.querySelector(selector)?.open);var focused=document.getElementById('asDrawerBody').contains(document.activeElement)?focusToken(document.activeElement):null;var {g,line,side}=drawerState,rawBase=researchWithMatchupContext(drawerState.base||researchFor(g,line,side),matchupClient.peek(g)),base=filterPropResearch(rawBase,drawerState.propFilters),r=base?recalcFromGameLog(base,line,side):null;
 ...
 if(panel==='sandbox'&&(!sandbox.roster||sandbox.roster.reason==='TEAM_UNKNOWN')&&!sandbox.loading)loadRoster(g);
 document.querySelectorAll('[data-market-key]').forEach(b=>b.onclick=()=>{var next=playerMarketGroups(g).find(x=>x.key===b.dataset.marketKey);if(next)openDrawer(next);});
 document.getElementById('asDetailSave').onclick=async e=>{e.currentTarget.disabled=true;await toggleSaved(g.key);renderListLight();if(drawerState?.g.key===g.key)renderDrawer();};
 document.getElementById('asMarketSwitch').onchange=e=>{var next=playerMarketGroups(g).find(x=>x.key===e.target.value);if(next)openDrawer(next);};
 ...
 document.getElementById('asClearPropFilters').onclick=()=>{drawerState.propFilters={};drawerState.filter='all';renderDrawer();};
...
function restoreFilters(){var f=readStored('autoscout-filters-'+sport,{});if(!f||typeof f!=='object')f={};query=typeof f.query==='string'?f.query:'';marketFilter=typeof f.market==='string'?f.market:'all';bookFilter='all';if(readStored('autoscout-selected-books',undefined)===undefined&&typeof f.book==='string'&&f.book!=='all'){selectedBooks=[bookId(f.book)];storeLocal('autoscout-selected-books',selectedBooks);}sideFilter=['all','OVER','UNDER'].includes(f.side)?f.side:'all';sortBy=['shuffle','research','recent','l5','l10','l15','l20','season','h2h','projection','books','player','time'].includes(f.sort)?f.sort:'shuffle';advanced=f.advanced&&typeof f.advanced==='object'&&!Array.isArray(f.advanced)?f.advanced:{};rulesEnabled=f.rulesEnabled!==false;}
...
function renderAdvanced(){
 ...
 document.getElementById('asClear').onclick=()=>{advanced={};query='';marketFilter=bookFilter=sideFilter='all';selectedBooks=null;storeLocal('autoscout-selected-books',null);document.getElementById('asSearch').value='';document.getElementById('asSide').value='all';document.getElementById('asFilterCount').textContent='';renderControls();renderAdvanced();renderList();};
}
