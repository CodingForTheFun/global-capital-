...
const aliases = {
  ...
  Points:['points','pts'], Rebounds:['totalRebounds','rebounds','reb'], OffensiveRebounds:['offensiveRebounds','offRebounds','oreb'], DefensiveRebounds:['defensiveRebounds','defRebounds','dreb'], Assists:['assists','ast'],
  Steals:['steals','stl'], BlockedShots:['blocks','blockedShots','blk'], Turnovers:['turnovers','tov'],
  ThreePointersMade:['threePointFieldGoalsMade','threePointersMade','threes'], ThreePointersAttempted:['threePointFieldGoalsAttempted','threePointersAttempted','threePointAttempts'],
  FieldGoalsAttempted:['fieldGoalsAttempted','fga'], FreeThrowsMade:['freeThrowsMade','ftm'], FreeThrowsAttempted:['freeThrowsAttempted','fta'], PersonalFouls:['personalFouls','fouls','pf'],
  ...
  FieldGoalsMade:['fieldGoalsMade','fgm'], ExtraPointsMade:['extraPointsMade'], KickingPoints:['totalKickingPoints','kickingPoints'],
...
const extra = {
  ...
  MLB:{batter_runs_scored:['Runs'],batter_strikeouts:['Strikeouts'],batter_triples:['Triples'],
    player_hits_runs_sb:['Hits','Runs','StolenBases'],player_total_bases_from_hits:['TotalBases'],
    player_strikeouts_thrown:['Strikeouts'],player_hitter_strikeouts:['Strikeouts'],
    player_extra_base_hits:['Doubles','Triples','HomeRuns'],
...
export function marketContract({sport,market,providerMarketKey} = {}) {
  ...
  const category=sport==='MLB' ? (key.startsWith('pitcher_') || key==='player_strikeouts_thrown' || !key.startsWith('batter_') &&
...
export function normalizeStatColumns(values={}, {sport,category}={}) {
  ...
  if(PUBLIC_LEAGUES[sport]?.[0]==='basketball') {
    out.FieldGoalsMade ??= made(values['fieldGoalsMade-fieldGoalsAttempted']);
    out.FieldGoalsAttempted ??= pairPart(values['fieldGoalsMade-fieldGoalsAttempted'],1);
    out.FreeThrowsMade ??= made(values['freeThrowsMade-freeThrowsAttempted']);
    out.FreeThrowsAttempted ??= pairPart(values['freeThrowsMade-freeThrowsAttempted'],1);
    out.ThreePointersAttempted ??= pairPart(values['threePointFieldGoalsMade-threePointFieldGoalsAttempted'],1);
  }
