...
import { maybeRefreshSportsGameOddsSupplement } from '../ingestion/sportsgameodds-supplement.mjs';
...
const dependencies = { persistenceHealth, publicWorkerConfigured, runPublicIngestionCycle,
  ...
  persistNormalizedBoard, retentionConfig, pruneLineSnapshots, startPropLineRefresh, startSportsGameOddsRefresh };
...
async function persistBoardsCycle(reason, deps, budgets) {
  const { publicWorkerConfigured, runPublicIngestionCycle, readPublicSchedulerState,
    ...
    retentionConfig, pruneLineSnapshots, startPropLineRefresh, startSportsGameOddsRefresh } = deps;
...
export function startSportsGameOddsLocalRefresh() {
  ...
  const seconds = clampInt(process.env.SPORTSGAMEODDS_LOCAL_REFRESH_SECONDS, 60, 30, 300);
  ...
  console.log(`[AutoScout SportsGameOdds supplement] local demand refresh interval=${seconds}s; paid calls occur only after local board demand`);
...
export function startFrugalPersistence() {
  ...
  console.log(`[AutoScout persistence] single public-feed scheduler, interval=${ingestSeconds} seconds, sports-per-cycle=${perCycle}/${PUBLIC_PERSISTENCE_SPORTS.length}, snapshot-interval=${snapshotSeconds} seconds, primary-paid-refresh=off, propline-supplement=quota-aware, sportsgameodds-gap-fill=usage-aware`);
  return timer;
}
