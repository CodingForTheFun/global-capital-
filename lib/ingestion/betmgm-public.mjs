...
function headers() {
  return { accept: 'application/json,text/plain,*/*', 'accept-language': 'en-US,en;q=0.9', 'user-agent': UA, origin: WEB_ORIGIN, referer: `${WEB_ORIGIN}/en/sports` };
}
...
async function getAccessId(fetcher) {
  ...
  try {
    const response = await fetcher(CONFIG_URL, {
      headers: { ...headers(), 'x-bwin-browser-url': `${WEB_ORIGIN}/en/sports`, 'x-from-product': 'host-app' },
  ...
  try {
    ...
    try {
      ...
      const data = await browserJsonFetch(String(url), {
        origin: WEB_ORIGIN,
        referer: `${WEB_ORIGIN}/en/sports`,
        userAgent: UA,
        timeoutMs: 22_000,
        headers: { 'x-bwin-browser-url': `${WEB_ORIGIN}/en/sports`, 'x-from-product': 'host-app' },
      });
...
export async function fetchBetMgmPublic(sport, { fetcher = globalThis.fetch, force = false } = {}) {
  ...
  const task = (async () => {
    ...
    const value = { records, fetchedAt: new Date().toISOString(), endpoint: `sports.${STATE}.betmgm.com`, transport: fetched.transport, eventsChecked: Math.min(fixtures.length, maxEvents), groupsChecked: Number(fetched.groupsChecked || 0) };
