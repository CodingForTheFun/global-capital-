...
const BASE = 'https://api.sportsbook.fanduel.com';
...
export async function fetchFanDuelPublic(sport, { fetcher = globalThis.fetch, force = false } = {}) {
  ...
  const task = (async () => {
    const pageUrl = new URL(`${BASE}/sbapi/content-managed-page`);
    ...
    for (const event of events) {
      const eventUrl = new URL(`${BASE}/sbapi/event-page`);
    ...
    const value = { records, fetchedAt: new Date().toISOString(), endpoint: 'api.sportsbook.fanduel.com', transport: 'fanduel-public', eventsChecked: events.length };
