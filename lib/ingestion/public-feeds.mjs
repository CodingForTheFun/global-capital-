...
export function createPublicFeeds({fetcher=globalThis.fetch,now=Date.now,storeDir=null,mappings=[],feeds=PUBLIC_FEEDS}={}){
 ...
 async function persist(){if(!file)return;const snapshot=JSON.stringify({version:1,feeds:Object.fromEntries(state)});savePromise=savePromise.catch(()=>{}).then(async()=>{await fs.mkdir(storeDir,{recursive:true});await cleanupStaleTemps();const temp=file+'.'+crypto.randomUUID()+'.tmp';try{await fs.writeFile(temp,snapshot);await fs.rename(temp,file);}finally{await fs.rm(temp,{force:true}).catch(()=>{});}});await savePromise;}
 ...
 async function get(feed,url){
  ...
  if(!response.ok){const retry=response.headers.get('retry-after');const seconds=retry&&/^\d+(?:\.\d+)?$/.test(retry)?Number(retry)*1000:Date.parse(retry||'')-now();throw Object.assign(Error('PUBLIC_FEED_HTTP'),{status:response.status,retryMs:Number.isFinite(seconds)&&seconds>0?seconds:null,endpoint:new URL(url).host});}
  ...
  try{for(;;){const {done,value}=await reader.read();if(done)break;total+=value.byteLength;
   if(total>maxFeedBytes())throw Object.assign(Error('PUBLIC_FEED_TOO_LARGE'),{reason:'PUBLIC_FEED_TOO_LARGE',endpoint:new URL(url).host});
   chunks.push(Buffer.from(value));}}finally{await reader.cancel().catch(()=>{});}
 ...
 async function fetchPages(feed){
  ...
  let lastError=null,firstError=null;
  for(const candidate of candidates){
   try{return await fetchFrom(feed,candidate);}catch(error){lastError=error;firstError=firstError||error;}
  }
 ...
 async function refreshOne(feed){
  ...
  const task=(async()=>{
   try{
    const {data,partial,endpoint}=await fetchPages(feed);let parsed=empty();
    ...
    state.set(feed.id,{...parsed,fetchedAt:new Date(now()).toISOString(),nextAt:now()+effectiveTtl(feed,mappings),failures:0,status:parsed.records.length?'available':feed.id==='sleeper'?'roster_only':parsed.contracts.length?'unmapped_contracts':'no_props',partial,httpStatus:200,endpoint,configVersion:FEED_CONFIG_VERSION});
   }catch(error){const failures=(previous?.failures||0)+1;const delay=error.retryMs||Math.min(3600000,60000*2**Math.min(6,failures-1))+Math.floor(Math.random()*10000);state.set(feed.id,{...(previous||empty()),failures,nextAt:now()+delay,status:error.status===429?'cooldown':'unavailable',httpStatus:error.status||null,endpoint:error.endpoint||null,reason:error.reason||error.message||null,configVersion:FEED_CONFIG_VERSION});}
 ...
 async function board(sport,reference){await load();const version=[Math.floor(now()/60000),...state.values()].map(s=>typeof s==='number'?s:s.fetchedAt).join('|'),cached=boards.get(sport);
  ...
  if(prizepicks?.fetchedAt&&Array.isArray(prizepicks.specials))value=attachPrizePicksSpecialRows(value,prizepicks.specials.filter(r=>r.sport===sport&&Date.parse(r.gameStartTime)>now()),prizepicks.fetchedAt);
  boards.set(sport,{source:reference.props,version,value});return value;
 }
 function health(){return feeds.map(f=>{const s=state.get(f.id);return {id:f.id,status:s?.status||'not_checked',httpStatus:s?.httpStatus??null,endpoint:s?.endpoint||null,reason:s?.status==='available'?null:s?.reason||null,failures:s?.failures||0,lineCount:s?.records?.length||0,specialLineCount:s?.specials?.length||0,rosterCount:s?.players?.length||0,contractCount:s?.contracts?.length||0,fetchedAt:s?.fetchedAt||null,nextAt:s?.nextAt||null,partial:!!s?.partial};});}
