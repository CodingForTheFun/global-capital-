...
import { fetchPublicMatchup, resolveNearbyMatchupEvent } from './matchup.mjs';
...
export function createPublicResearch({ fetchImpl = (...args) => fetch(...args), now = () => Date.now() } = {}) {
  ...
  fetchResearch.eventIdentity = async params => {
    const sport=canonicalSport(params?.sport),config=PUBLIC_LEAGUES[sport],start=Date.parse(params?.gameStartTime||'');
    if(!config?.[1]||!params?.homeTeam||!params?.awayTeam||!Number.isFinite(start))
      return {ok:true,available:false,code:'MATCHUP_IDENTITY_UNVERIFIED'};
    const [family,league]=config,date=t=>new Date(t).toISOString().slice(0,10).replaceAll('-','');
    const scoreboard=await request(`/site/v2/sports/${family}/${league}/scoreboard?dates=${date(start-2*86400_000)}-${date(start+2*86400_000)}&limit=1000`,GAME_LOG_TTL);
    if(!scoreboard.data)return {ok:true,available:false,code:'MATCHUP_SOURCE_UNAVAILABLE'};
    const event=resolveNearbyMatchupEvent(scoreboard.data,{...params,sport});
    return event?{ok:true,available:true,source:'ESPN',sourceEventId:event.id,gameId:`${sport.toLowerCase()}:${event.id}`,driftMs:event.driftMs}
