  ...
  const task = (async () => {
    ...
    try {
      const events = fullSlate
        ? await boundedCoverageCall((signal) => fetchEvents(selected, { signal }), { signal: options.signal, timeoutMs: 10000 })
      ...
      const fetchEvent = async (event, signal, strict = false) => {
        ...
        if (strict) {
          ...
          const code = proplineHealth().lastError?.code;
          if (['PROPLINE_DAILY_LIMIT','PROPLINE_QUOTA_RESERVE','PROPLINE_BURST_LIMIT','PROPLINE_RATE_LIMITED','PROPLINE_UNAUTHORIZED'].includes(code)) throw Object.assign(new Error('PropLine coverage paused.'), { code });
          ...
          if (returnedId && returnedId !== eventId && !(payload.merged_from_event_ids || []).map(String).includes(eventId)) throw Object.assign(new Error('Event identity mismatch.'), { code: 'PROPLINE_COVERAGE_EVENT_MISMATCH' });
        }
