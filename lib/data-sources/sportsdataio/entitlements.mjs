  ...
  async function probe() {
    ...
    for (const sport of Object.keys(LEAGUES)) {
      ...
      for (const name of PROBE_FEEDS) {
        ...
        if (name === 'playerProps') {
          // Current aggregated player props are addressed by a real GameID. If
          // there is no game in the current window, entitlement is genuinely
          // unknown — never call a guessed ID and never misclassify a 404.
          const gameId = firstGameId(gamesPayload);
          if (gameId === null) { feeds[name] = GRANT.UNKNOWN; continue; }
