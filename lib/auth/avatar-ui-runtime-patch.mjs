...
export function patchProfileAvatarUi(source) {
  ...
  if (end < 0) throw new Error('Profile avatar patch could not locate the client IIFE boundary.');
  ...
  const patchedEnd = patched.lastIndexOf('})();');
  patched = patched.slice(0, patchedEnd) + runtime + patched.slice(patchedEnd);
  return patched;
}
