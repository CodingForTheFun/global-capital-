...
export function resolveKey(sport) {
  if (process.env.AUTOSCOUT_DISABLE_SPORTSDATAIO === 'true') return '';
...
export function isConfigured() {
  if (process.env.AUTOSCOUT_DISABLE_SPORTSDATAIO === 'true') return false;
...
export function createClient({
  ...
  async function get(url, path, { ttlMs = 0, sport = null, allowStale = true } = {}) {
    // Check before caches too: disabled providers cannot leak old results.
    if (process.env.AUTOSCOUT_DISABLE_SPORTSDATAIO === 'true') return { ok: false, status: 0, reason: 'disabled', data: null, cached: false };
