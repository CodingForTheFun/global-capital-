...
const PROBE_GLOBAL_KEY = Symbol.for('obligeprops.sportradarTrialProbeStarted');
...
export async function probeAllSportradarTrials({ delayMs = 1600 } = {}) {
  ...
  probePromise = (async () => {
    ...
    for (let index = 0; index < ids.length; index += 1) {
      ...
      if (index < ids.length - 1) await sleep(Math.max(1500, Number(delayMs) || 1600));
    }
    return rows;
...
export function sportradarTrialHealth() {
  ...
  return {
    ...
    unavailable: products.filter((row) => row.tested && !row.available && row.code !== 'RATE_LIMITED').map((row) => row.productId),
    rateLimited: products.filter((row) => row.tested && row.code === 'RATE_LIMITED').map((row) => row.productId),
    products,
...
export function startSportradarTrialProbe({ delayMs = 3500 } = {}) {
  ...
  if (globalThis[PROBE_GLOBAL_KEY]) return null;
  globalThis[PROBE_GLOBAL_KEY] = true;
  const timer = setTimeout(() => {
    void probeAllSportradarTrials().then((rows) => {
      console.log('[Sportradar trials]', JSON.stringify({
        ...
        unavailable: rows.filter((row) => !row.available && row.code !== 'RATE_LIMITED').map((row) => ({ id: row.productId, status: row.status, code: row.code })),
        rateLimited: rows.filter((row) => row.code === 'RATE_LIMITED').map((row) => row.productId),
      }));
    }).catch((error) => {
