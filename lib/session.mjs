...
export function createSessionCodec({ secret, ttlMs = DEFAULT_TTL_MS, now = Date.now } = {}) {
  ...
  function makeToken(role = OWNER, subject = OWNER, { ttlMs: requestedTtlMs } = {}) {
    const identity = role === OWNER ? OWNER : `${MEMBER_PREFIX}${encodeSubject(subject)}`;
    const requested = Number(requestedTtlMs);
    const effectiveTtlMs = Number.isFinite(requested) && requested > 0 ? requested : ttlMs;
    const expires = now() + effectiveTtlMs;
