...
export function createSportsDataIoPropBoard({ client = createClient(), now = () => new Date() } = {}) {
  ...
  async function fetchSport(sport) {
    ...
    if (!league) return { sport, offers: [], gamesChecked: 0, unresolvedPlayers: 0, status: 404, errorType: 'UNSUPPORTED_SPORT' };
    if (!resolveKey(sport)) return { sport, offers: [], gamesChecked: 0, unresolvedPlayers: 0, status: 0, errorType: 'NOT_CONFIGURED' };

    const date = formatDate(now());
    let params = { date, season: now().getUTCFullYear(), week: null };
    if (league.by === 'week') {
      const resolved = await timeframe(sport, league);
      if (!resolved) return { sport, offers: [], gamesChecked: 0, unresolvedPlayers: 0, status: 0, errorType: 'TIMEFRAME_UNAVAILABLE' };
    ...
    if (!gamesResult.ok) {
      return { sport, offers: [], gamesChecked: 0, unresolvedPlayers: 0, status: gamesResult.status || 0, errorType: gamesResult.reason || 'GAMES_UNAVAILABLE' };
    }

    const games = uniqueGames(gamesResult.data);
    if (!games.length) return { sport, offers: [], gamesChecked: 0, unresolvedPlayers: 0, status: 200, errorType: null };
    ...
    if (offers.some((offer) => !offer.playerName)) {
  ...
  async function fetchBoard({ sports = DEFAULT_SPORTS, force = false } = {}) {
    ...
    const value = {
      ...
      coverage: rows.map(({ sport, gamesChecked, unresolvedPlayers, status, errorType, offers }) => ({
        sport,
        gamesChecked,
        unresolvedPlayers,
        status,
