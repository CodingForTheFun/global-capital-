...
const RESEND_CODE_TIMER = '<script>(function(){'
  + 'var RESEND_COOLDOWN_SECONDS=60,timers={};'
  + 'function restore(button){button.disabled=false;button.removeAttribute("aria-disabled");button.textContent="Send a new code";button.style.opacity="";button.style.cursor="";}'
  + 'function begin(id){var button=document.getElementById(id);if(!button)return;if(timers[id])clearInterval(timers[id]);var until=Date.now()+RESEND_COOLDOWN_SECONDS*1000;function render(){var left=Math.max(0,Math.ceil((until-Date.now())/1000));if(left>0){button.disabled=true;button.setAttribute("aria-disabled","true");button.textContent="Resend code in "+left+"s";button.style.opacity=".55";button.style.cursor="not-allowed";return;}clearInterval(timers[id]);delete timers[id];restore(button);}render();timers[id]=setInterval(render,250);}'
  + 'function wrap(id,msgId){var button=document.getElementById(id);if(!button||button.dataset.cooldownBound==="1"||typeof button.onclick!=="function")return;var original=button.onclick;button.dataset.cooldownBound="1";button.onclick=async function(event){if(button.disabled)return;button.disabled=true;button.setAttribute("aria-disabled","true");button.textContent="Sending…";button.style.opacity=".55";var value=await original.call(this,event);var msg=document.getElementById(msgId);if(msg&&msg.classList.contains("ok")){begin(id);}else{restore(button);}return value;};}'
  + 'function watch(stepId,id,shouldStart){var step=document.getElementById(stepId);if(!step||typeof MutationObserver!=="function")return;var wasVisible=!step.hidden;new MutationObserver(function(){var visible=!step.hidden;if(visible&&!wasVisible&&(!shouldStart||shouldStart()))begin(id);wasVisible=visible;}).observe(step,{attributes:true,attributeFilter:["hidden"]});}'
  + 'wrap("verifyResend","verifyMsg");wrap("resetResend","resetMsg");'
  + 'watch("verifyStep","verifyResend",function(){var signup=document.getElementById("tabUp");return signup&&signup.getAttribute("aria-selected")==="true";});watch("resetStep","resetResend");'
  + '})();</script>';
...
export function researchLanding(html) {
  ...
  return html.replace('<body>', '<body class="asResearchGate">')
    ...
    .replace('</body>', `${RESEND_CODE_TIMER}</body>`)
