...
export function proplineSportKey(sport) {
  const raw = String(sport ?? '').trim();
  if (!raw) return null;
  const mapped = SPORT_KEYS[raw.toUpperCase()];
  if (mapped) return mapped;
  const providerKey = raw.toLowerCase();
  // Canonical workspace routes already carry PropLine's provider-native key.
  // Accept that key only when its discipline is one this product recognises;
  // this enables on-demand research without widening automatic paid polling.
  return matchProplineSport(providerKey) ? providerKey : null;
}
...
// Only genuine spelling differences are listed; distinct sources such as
// polymarket and polymarket_us must retain separate identities. Everything
// else passes through unchanged so a book PropLine adds later still works.
