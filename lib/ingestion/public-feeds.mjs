...
export const PUBLIC_FEEDS=Object.freeze([
 {id:'prizepicks',url:'https://partner-api.prizepicks.com/projections?per_page=250',ttl:THREE_MINUTES,origin:'https://app.prizepicks.com',referer:'https://app.prizepicks.com/',revision:2},
 {id:'underdog',url:'https://api.underdogfantasy.com/beta/v5/over_under_lines',ttl:THREE_MINUTES,origin:'https://underdogfantasy.com',referer:'https://underdogfantasy.com/',revision:1},
 ...
 async function refreshOne(feed){
  ...
  const task=(async()=>{
   try{
    ...
    state.set(feed.id,{...parsed,fetchedAt:new Date(now()).toISOString(),nextAt:now()+feed.ttl,failures:0,status:parsed.records.length?'available':feed.id==='sleeper'?'roster_only':parsed.contracts.length?'unmapped_contracts':'no_props',partial,httpStatus:200,revision:Number(feed.revision||0)});
   }catch(error){const failures=(previous?.failures||0)+1;const delay=error.retryMs||Math.min(3600000,60000*2**Math.min(6,failures-1))+Math.floor(Math.random()*10000);state.set(feed.id,{...(previous||empty()),failures,nextAt:now()+delay,status:error.status===429?'cooldown':'unavailable',httpStatus:error.status||null,revision:Number(feed.revision||0)});}
