...
async function sports(fetcher) {
  ...
  if (sportsPending) return sportsPending;

  // The sports catalog is shared by every Pinnacle sport. On a cold process or
  // cache expiry the provider fan-out can ask for it concurrently; single-flight
  // that one metadata request so a transient catalog hiccup cannot become a
  // self-inflicted all-sport request burst.
  const staleRows = Array.isArray(sportsCache.rows) && sportsCache.rows.length ? sportsCache.rows : null;
  sportsPending = (async () => {
    try {
      ...
      // Sport IDs are catalog metadata, not quotes. Reusing the last verified
      // catalog after its TTL does not extend or fabricate betting data: every
      // matchup and market below is still fetched fresh and normal quote expiry
  ...
  try { return await sportsPending; }
  finally { sportsPending = null; }
}
