...
export function createClient({
  ...
  // Expired positive rows are intentionally retained until replacement/eviction.
  // They are not returned as fresh, but are available as stale-on-error data.
  ...
  function readCache(path) {
    ...
    if (!row) return null;
    // Do NOT delete an expired row here. A later 429, timeout or provider error
    // may need that exact row for stale-on-error fallback.
