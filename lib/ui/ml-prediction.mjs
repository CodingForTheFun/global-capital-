...
function validationCopy(p){
  ...
  if(v.method==='rolling-player-history') {
    ...
    const strategy=v.selectedStrategy==='recent-mean-fallback'?' · conservative recent-mean fallback active':v.selectedStrategy==='trained-adjustment'?' · trained adjustment active':'';
    return 'Time-ordered rolling check: '+v.observations+' prior games'+comparison+strategy+'. The model is re-fit from that player’s verified history; this is not a league-wide held-out accuracy claim.';
  }
