...
test('tennis uses account-entitled league IDs returned by the SportsGameOdds catalog', async () => {
  const previousKey = process.env.SPORTS_ODDS_API_KEY_HEADER;
  const previousMode = process.env.OBLIGE_PROP_PROVIDER_MODE;
  const previousFetch = globalThis.fetch;
  const urls = [];
  try {
    ...
    globalThis.fetch = async (input) => {
      ...
      if (url.pathname.endsWith('/sports')) return new Response(JSON.stringify({success:true,data:[{sportID:'TENNIS',name:'Tennis'}]}),{status:200,headers:{'content-type':'application/json'}});
      if (url.pathname.endsWith('/leagues')) return new Response(JSON.stringify({success:true,data:[{leagueID:'TENNIS_ATP_ACCOUNT',sportID:'TENNIS',name:'ATP'},{leagueID:'TENNIS_WTA_ACCOUNT',sportID:'TENNIS',name:'WTA'},{leagueID:'NFL',sportID:'FOOTBALL',name:'NFL'}]}),{status:200,headers:{'content-type':'application/json'}});
      if (url.pathname.endsWith('/events')) return new Response(JSON.stringify({success:true,data:[]}),{status:200,headers:{'content-type':'application/json'}});
      throw new Error('Unexpected URL '+url.href);
    };
    ...
    const eventUrl=urls.find((url)=>url.pathname.endsWith('/events'));
    assert.ok(eventUrl);
    assert.equal(eventUrl.searchParams.get('leagueID'),'TENNIS_ATP_ACCOUNT,TENNIS_WTA_ACCOUNT');
