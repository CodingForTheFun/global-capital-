...
export async function researchPlayerProp(params = {}) {
  ...
  const directory = PUBLIC_LEAGUES[sport]?.[1]
    ...
    : Promise.resolve([]);
  const pending = Promise.all([researchUncached(params), directory]).then(([value, leagueTeams]) => {
    const enriched = Array.isArray(leagueTeams) && leagueTeams.length ? { ...value, leagueTeams } : value;
