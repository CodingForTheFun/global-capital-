 ...
 * — /terms, /checkout, /login, legacy diagnostics, and critically every
 ...
 *
 * /apex and /apex/ are legacy customer entry points. When the rebuilt web app
 * is enabled they redirect to /board so old bookmarks cannot silently reopen
 * the retired customer shell. Deeper /apex/* routes remain untouched.
 */
...
export async function serveNewWeb(req, res, { origin = newWebOrigin(), fetchImpl = fetch } = {}) {
  ...
  if (legacyTarget) {
    res.writeHead(307, {
      location: legacyTarget + url.search,
