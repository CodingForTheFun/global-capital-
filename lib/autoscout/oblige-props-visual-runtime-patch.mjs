...
/* The current board uses the compact per-prop sportsbook selector. If an older\n   card path still supplies the legacy rail, hide it instead of duplicating book\n   labels and consuming mobile space. */\n#as5 .asBookRail{display:none!important}\n\n/* Save becomes the small star in the card corner instead of a giant full-width row. */
...
(function(){
  ...
  function ensureHeader(){
    ...
    if(identity){\n      identity.setAttribute('aria-label','Oblige Props research');\n      var propsWord=identity.querySelector('.asPay,.asProps');\n      if(propsWord)propsWord.textContent='props';\n    }\n    var bar=root.querySelector('.asBar'),real=root.querySelector('#asSearch');
