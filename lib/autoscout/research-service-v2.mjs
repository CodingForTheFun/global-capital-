...
import { fetchPublicBasketballResearch } from '../data-sources/espn/research.mjs';
...
async function researchUncached(params = {}) {
  ...
  // Public basketball history supplements the existing context feeds. All
  // threshold calculations still run through the one shared research engine.
  if (['NBA', 'WNBA'].includes(String(params.sport || '').toUpperCase())) {
    const history = await fetchPublicBasketballResearch(params);
    if (history.available) {
      return finalizeResearch({ ...params, opponent: history.opponent, gameLog: history.gameLog, player: history.player,
        context: clearAttempt?.context || logAttempt?.context || null,
        source: history.source, cached: history.cached, season: history.season, coverage: history.coverage });
    }
  }
