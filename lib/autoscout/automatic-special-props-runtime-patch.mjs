...
const SAFE_SPECIAL_DIRECTION_ANCHOR = "  var direction=row.side==='UNDER'?'Less':row.side==='OVER'?'More':'';";
...
function replaceExactCount(source, anchor, replacement, label, expectedCount) {
  const count = source.split(anchor).length - 1;
  if (count !== expectedCount) throw new Error(`Automatic special-prop UI patch expected ${expectedCount} ${label}; found ${count}.`);
...
export function patchAutomaticSpecialPropsUi(source) {
  ...
  patched = replaceExactCount(
    ...
    'clear-filters anchors',
    2,
  );
  // Recent Five owns the special-line renderer. Require its safe direction logic
  // to be present exactly once so build drift still fails closed, but do not
  // rewrite the same renderer a second time.
  patched = replaceOnce(
    patched,
    SAFE_SPECIAL_DIRECTION_ANCHOR,
    SAFE_SPECIAL_DIRECTION_ANCHOR,
    'safe special-line renderer',
  );
