...
export const SPORTSGAMEODDS_LEAGUES = Object.freeze({
  ...
  // SportsGameOdds documents Tennis event queries by league. Keep ATP/WTA/ITF
  // together under the site-wide TENNIS tab rather than using the broader
  // sportID path, which currently returns HTTP 400 for this account.
  TENNIS: Object.freeze(['ATP', 'WTA', 'ITF']),
});
...
export async function fetchSportsGameOddsBoard(sport, options = {}) {
  ...
  // A curated league mapping is more specific than a generic sportID. This is
  // especially important for Tennis, where the provider's documented examples
  // use ATP/WTA/ITF leagueID queries.
  const sportID = leagues.length ? null : (SPORTSGAMEODDS_SPORT_IDS[selected] || null);
  ...
  const task = (async () => {
    ...
    try {
      ...
      // Aggregate sportID tabs need catalog entitlement validation. Tennis also
      // validates its curated league IDs because league availability is tied to
      // the exact SportsGameOdds key and may differ from documented examples.
      ...
      if (selected === 'TENNIS' && catalog.leagues.length) {
        // Use the exact league IDs returned by this account's authenticated
        // catalog. Names such as ATP/WTA/ITF are labels, not guaranteed IDs.
        requestedLeagues = [...new Set(
          catalog.leagues
            .filter((row) => row.sportID === selected)
            .map((row) => row.id)
            .filter(Boolean),
        )];
        if (!requestedLeagues.length) {
          return emptyBoard(selected, {
            ...
            requestedLeagueIDs: [...leagues],
            availableLeagueIDs: catalog.leagues.map((row) => row.id),
          });
        }
      }
      ...
      const now = Date.now();
      // SportsGameOdds' Tennis quick-start examples use a minimal events
      // request (leagueID + oddsAvailable + includeAltLines + limit). The
      // generic team-sport filters below are not required for Tennis and the
      // live account currently rejects that combination with HTTP 400.
      const eventParams = selected === 'TENNIS'
        ? {
            oddsAvailable: 'true',
      ...
      // SportsGameOdds accepts one Tennis leagueID per event request on the
      // current v2 account. A comma-joined ATP,WTA,ITF value returns HTTP 400.
      // Fetch the entitled tours separately, then de-duplicate events by eventID.
      // Billing remains bounded by the same per-tour eventLimit and the existing
      // demand/quota scheduler.
      let payload;
      let rows;
      if (selected === 'TENNIS' && requestedLeagues.length > 1) {
        const payloads = await Promise.all(requestedLeagues.map((leagueID) => (
          sportsGameOddsGet('/events', { ...eventParams, leagueID }, requestOptions)
        )));
        const byEvent = new Map();
        for (const currentPayload of payloads) {
          for (const event of (Array.isArray(currentPayload?.data) ? currentPayload.data : [])) {
            const eventID = text(event?.eventID || event?.id);
            if (eventID && !byEvent.has(eventID)) byEvent.set(eventID, event);
          }
        }
        rows = [...byEvent.values()];
        payload = payloads.find((item) => text(item?.notice)) || payloads[0] || { success: true, data: [] };
      } else {
        payload = await sportsGameOddsGet('/events', {
          ...eventParams,
