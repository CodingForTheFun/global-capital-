...
export async function runTrainingCycle({
  ...
  for (const sport of sports) {
    ...
    // MLB also retrains when new box-score facts arrived, even with no new props.
    let enrich = null, enriched = 0;
    let observations = null;
    if (sport === 'MLB') {
      observations = await readObservations(sport, dir);
    ...
    if (!incoming.length && !enriched && existingArtifact) continue;
    observations ||= await readObservations(sport, dir);
    if (incoming.length) {
      if (sport !== 'MLB') observations = mergeObservations(observations, incoming, { now });
    ...
    trained.push({ sport, promote: result.promote, reasons: result.reasons, diag: cycleDiagnostics(observations, result.metrics, s, enrich) });
    await yieldToEventLoop();
  }
