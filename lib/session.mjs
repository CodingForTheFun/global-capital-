 ...
 * immediately too. The one-second cache avoids a disk read on every API call.
 */
export function ownerPasskeyRequired(env = process.env, now = Date.now()) {
  ...
  if (passkeyStateCache.file === file && now - passkeyStateCache.checkedAt < 1_000) return passkeyStateCache.required;
  let required = false;
  try {
