...
// a complete season, but real current-season rows are still a valid grounded
// season-to-date sample and should not be hidden behind an all-or-nothing flag.
export function analyzeResearch(base = {}, line = base.line, side = base.side, venue = 'all') {
  ...
  const seasonAnalytics = rollingAnalytics(seasonRows, threshold, direction).windows.season;
  const complete = base.coverage?.seasonComplete === true && base.season != null;

  // Always compute SZN from actual current-season games we possess. Coverage
  // reports whether that sample is complete so callers never mistake partial
  // season-to-date data for a complete provider archive.
  if (seasonRows.length) analytics.windows.season = { ...seasonAnalytics, available: true, partial: !complete };
