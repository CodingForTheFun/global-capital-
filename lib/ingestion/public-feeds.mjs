...
import {fetchHttp2Json} from './public-http2.mjs';
const THREE_MINUTES=180000;
const TEN_MINUTES=600000;
const TRANSPORT_VERSION=2;
const DEVICE_ID=crypto.randomUUID();
const PUBLIC_UA=process.env.AUTOSCOUT_PUBLIC_FEED_USER_AGENT||'Mozilla/5.0 (iPhone; CPU iPhone OS 18_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/18.6 Mobile/15E148 Safari/604.1';
export const PUBLIC_FEEDS=Object.freeze([
 {id:'prizepicks',url:'https://api.prizepicks.com/projections?per_page=250',ttl:THREE_MINUTES,origin:'https://app.prizepicks.com',referer:'https://app.prizepicks.com/',http2:true,fallbackOrigins:['https://partner-api.prizepicks.com']},
 {id:'underdog',url:'https://api.underdogfantasy.com/beta/v5/over_under_lines',ttl:THREE_MINUTES,origin:'https://underdogfantasy.com',referer:'https://underdogfantasy.com/',http2:true},
...
export function createPublicFeeds({fetcher=globalThis.fetch,http2Fetcher=fetchHttp2Json,now=Date.now,storeDir=null,mappings=[],feeds=PUBLIC_FEEDS}={}){
 ...
 async function load(){if(!loadPromise)loadPromise=(async()=>{if(!file)return;try{const data=JSON.parse(await fs.readFile(file,'utf8'));if(data.version===1)for(const feed of feeds){const s=data.feeds?.[feed.id];if(s&&Array.isArray(s.records)&&Array.isArray(s.players)&&Array.isArray(s.contracts)){const restored={...s};if(feed.http2&&restored.transportVersion!==TRANSPORT_VERSION&&[403,426].includes(Number(restored.httpStatus)))restored.nextAt=0;state.set(feed.id,restored);}}}catch{}})();await loadPromise;}
 function headers(feed){
  const h={accept:'application/json,text/plain,*/*','user-agent':PUBLIC_UA,'accept-language':'en-US,en;q=0.9','cache-control':'no-cache',pragma:'no-cache'};
  ...
  if(feed.id==='prizepicks'){h['x-device-id']=DEVICE_ID;h['content-type']='application/json';}
  return h;
 }
 async function fetchJson(feed,url){
  const response=await fetcher(url,{headers:headers(feed),signal:AbortSignal.timeout(12000),redirect:'follow'});
 ...
 async function get(feed,url){
  const target=new URL(url);const origins=[target.origin,...(feed.fallbackOrigins||[])];let lastError=null;
  for(const origin of [...new Set(origins)]){
   const candidate=new URL(`${target.pathname}${target.search}`,origin).href;
   try{return feed.http2?await http2Fetcher(candidate,headers(feed),{now}):await fetchJson(feed,candidate);}
   catch(error){lastError=error;if(![403,404,426].includes(Number(error?.status)))break;}
  }
  throw lastError||Error('PUBLIC_FEED_UNAVAILABLE');
 }
 async function fetchPages(feed){
  let url=feed.url,combined=null,partial=false;const seen=new Set();const base=new URL(feed.url);const allowedOrigins=new Set([base.origin,...(feed.fallbackOrigins||[])]);
  for(let page=0;page<10;page++){
   ...
   if(!next)break;
   const u=new URL(next,feed.url);if(!allowedOrigins.has(u.origin)||u.pathname!==base.pathname)throw Error('PUBLIC_FEED_BAD_NEXT');
 ...
 async function refreshOne(feed){
  ...
  const task=(async()=>{
   try{
    ...
    state.set(feed.id,{...parsed,fetchedAt:new Date(now()).toISOString(),nextAt:now()+feed.ttl,failures:0,status:parsed.records.length?'available':feed.id==='sleeper'?'roster_only':parsed.contracts.length?'unmapped_contracts':'no_props',partial,httpStatus:200,transportVersion:TRANSPORT_VERSION});
   }catch(error){const failures=(previous?.failures||0)+1;const delay=error.retryMs||Math.min(3600000,60000*2**Math.min(6,failures-1))+Math.floor(Math.random()*10000);state.set(feed.id,{...(previous||empty()),failures,nextAt:now()+delay,status:error.status===429?'cooldown':'unavailable',httpStatus:error.status||null,transportVersion:TRANSPORT_VERSION});}
 ...
 function refresh(){if(refreshPromise)return refreshPromise;refreshPromise=(async()=>{await load();for(const feed of feeds)await refreshOne(feed);})().finally(()=>{refreshPromise=null;});return refreshPromise;}
 ...
 function health(){return feeds.map(f=>{const s=state.get(f.id);return {id:f.id,status:s?.status||'not_checked',httpStatus:s?.httpStatus??null,failures:s?.failures||0,lineCount:s?.records?.length||0,rosterCount:s?.players?.length||0,contractCount:s?.contracts?.length||0,fetchedAt:s?.fetchedAt||null,nextAt:s?.nextAt||null,partial:!!s?.partial};});}
