...
/**
 * Parse only source-labelled PrizePicks alternate variants whose exact side is
 * explicit in the feed. PrizePicks now allows Goblin/Demon on either More or
 * Less, so a projection-level odds_type by itself is intentionally not enough
 * to decide which side receives the badge.
 */
export function normalizePrizePicksSpecials(payload) {
  ...
  for (const row of payload.data) {
    ...
    // Keep observing projection-level variants for diagnostics, but never turn
    // them into a More/Less badge unless the payload names that side.
    ...
    for (const [direction, type] of explicit) {
      out.push({ ...base, side: direction, specialType: type, isAlternate: true, specialVerified: true,
        specialSourceId: text(row.id), specialTypeSource: 'outcome_metadata' });
    }
  }
  return out;
}
 ...
 * At most one closest line per Goblin/Demon + side is attached to a card so a
 * provider with many alternates cannot flood the customer board.
 */
export function attachPrizePicksSpecialRows(board, specials, ingestedAt = new Date().toISOString()) {
  ...
  for (const special of list(specials)) {
    if (special?.specialVerified !== true || special?.isAlternate !== true || special?.specialTypeSource !== 'outcome_metadata' ||
        !['goblin','demon'].includes(special?.specialType) || !['OVER','UNDER'].includes(special?.side)) continue;
    ...
    const row = { ...base,
      id: `prizepicks-special:${special.specialSourceId}:${special.side}`,
      side: special.side, line: special.line, price: null, sportsbook: 'PrizePicks', sportsbookKey: 'prizepicks', provider: 'prizepicks',
      isAlternate: true, specialType: special.specialType, specialVerified: true, specialSourceId: special.specialSourceId,
      specialTypeSource: special.specialTypeSource, providerUpdatedAt: special.updatedAt || null, updatedAt: special.updatedAt || null,
      ingestedAt,
    };
    const key = [base.eventId,base.playerId,base.marketId,special.specialType,special.side].join('|');
