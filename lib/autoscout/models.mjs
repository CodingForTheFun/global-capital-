...
import { ADDITIONAL_PUBLIC_SPORTS, SCOPED_PUBLIC_SPORTS, SPORTSGAMEODDS_BOARD_SPORTS } from './board-coverage-catalog.mjs';

export const NORMALIZED_SCHEMA_VERSION = 1;
export const AUTOMATIC_SPORTS = Object.freeze(['NFL','NBA','WNBA','MLB','NHL','NCAAF','NCAAB']);
// Soccer and tennis are opt-in page/public-feed requests; do not enlarge automatic paid polling.
// SOCCER is how the public feeds actually tag club football - they have never
// posted MLS, EPL or UCL as a sport - so without it the board route answered
// every soccer request with UNSUPPORTED_SPORT and the props sat unreachable.
export const SUPPORTED_SPORTS = Object.freeze([...AUTOMATIC_SPORTS,'SOCCER','MLS','EPL','UCL','TENNIS']);

// Sports the BOARD may serve, which is deliberately wider than SUPPORTED_SPORTS.
//
// SUPPORTED_SPORTS governs providers: propline.mjs derives PROPLINE_SPORTS from
// it and the-odds-api validates against it, so adding a sport there widens paid
// polling. The DFS feeds meanwhile tag plenty of sports nobody pays per request
// for - esports, golf, MMA - and those props were already sitting in
// active_props while the board route answered UNSUPPORTED_SPORT, the same
// failure the SOCCER comment above describes.
//
// Serving them costs no provider requests. They have no ESPN league, so
// research reports unavailable for them rather than inventing a hit rate.
//
// CS/CS2 and DOTA/DOTA2 are one sport under two source spellings. Both are
// listed rather than collapsed, because normalizing them belongs in the feed
// normalizer, not in an allowlist.
export const PUBLIC_FEED_SPORTS = Object.freeze([
  'CS','CS2','VAL','LOL','DOTA','DOTA2',
  'PGA','EUROGOLF','MMA','CRICKET','DARTS','AFL','LAX','BAD','CFL','FIFA','HANDBALL','F1SZN',
  ...ADDITIONAL_PUBLIC_SPORTS,
  ...Object.keys(SCOPED_PUBLIC_SPORTS),
]);
export const BOARD_SPORTS = Object.freeze([...new Set([...SUPPORTED_SPORTS, ...PUBLIC_FEED_SPORTS, ...SPORTSGAMEODDS_BOARD_SPORTS])]);
