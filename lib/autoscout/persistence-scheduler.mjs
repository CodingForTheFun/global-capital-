...
const LEASE_RETRY_DELAY_MS = 15_000;
...
// Line history needs the same quote observed twice inside one process: the
// first observation seeds the comparison, the second is what can differ. That
// only happens if a sport comes round again within the snapshot window.
//
// Three independently tunable numbers decide it - the cycle interval, how many
// sports each cycle takes, and how often snapshots are computed - and a bad
// combination does not fail. It records nothing, quietly, forever. On
// 2026-09-15 one sport per five-minute cycle with thirty-minute snapshots meant
// each sport came round once an hour against a half-hour window, and history
// stopped for seven hours with every health field reporting green.
//
// So the arithmetic is checked out loud instead.
export function snapshotFeasibility({
  ...
  return {
    ...
    sweepSeconds,
    snapshotSeconds,
    // What the operator would have to change to make history possible.
...
function startPropLineRefresh() {
  // PropLine does not write to the database here, so it does not need to sit
  // behind slower public-feed fetches or snapshot RPCs. Starting it as soon as
  // this replica owns the shared ingest lease warms the customer cache sooner
  // while state.running inside the supplement still prevents overlap.
...
function scheduleLeaseRetry(reason) {
  // The process timer and the database's authoritative next_at can differ by a
  // few seconds after bootstrap. A clean unclaimed lease in that narrow window
  // should not postpone TTL-backed provider ingestion for another full cycle.
  // Retry once after the current persistence pass has fully unwound; the
  // database lease remains authoritative and a tagged retry never recurses.
  if (leaseRetryTimer || String(reason).includes('lease-retry')) return;
  ...
  try {
    ...
    if (publicWorkerConfigured()) {
      try {
        ...
        // A clean `claimed=false` is coordination, not a database outage. Keep
        // all provider fan-out behind the lease, but do not let a stale/other
        // owner suppress idempotent canonical writes from the already-refreshed
        // in-memory board.
        if (!publicClaimed) {
          ...
          // Another scheduled/bootstrap pass may have acquired the lease before
          // the queued retry fired. Cancel that now-unnecessary extra pass.
          clearTimeout(leaseRetryTimer);
          leaseRetryTimer = null;
        }
      } catch (error) {
        // An actual failed lease/health probe means the database is degraded.
        // Do not follow that failure with the heavier canonical board and
        // retention writers; keep live feeds warm and retry on the next cycle.
    ...
    // PropLine is deliberately attached to the same shared scheduler lease as
    // the public ingestion workers. Browser requests only read its warmed cache;
    // this one owner is the only code path allowed to spend its daily allowance.
    // Start it immediately after the lease is acquired so unrelated slow feeds
    // cannot postpone the first PropLine board refresh for several minutes.
    ...
    // Tennis is public/cache only here and never widens automatic primary-provider polling.
    ...
    syncRunning = false;
    if (retryLeaseAfterRun) scheduleLeaseRetry(reason);
  }
}
