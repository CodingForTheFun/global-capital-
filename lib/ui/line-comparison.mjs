...
export function compareResearchQuotes(group, {line, side = 'OVER', now = Date.now()} = {}) {
  ...
  for (const row of group.rows || []) {
    ...
    // Public DFS/sportsbook adapters persist a verified ingestion timestamp even
    // when the upstream offer does not expose its own updated-at field. Treat
    // that observation time as the final freshness fallback instead of marking
    // a currently visible card stale/undated in Best Line Finder.
    const at = stamp(row.providerUpdatedAt || row.updatedAt || row.ingestedAt);
  ...
  for (const direction of ['OVER','UNDER']) {
    ...
    // A best available price is useful even when only one sportsbook currently
    // quotes the exact line. We still never synthesize a price, and consensus
    // remains gated on two distinct fresh books below.
    bestPrices[direction] = sameLine.length ? sameLine.filter(q=>payout(q.price) === best) : [];
  ...
  const consensusQuotes=fresh.filter(q=>q.side === side);
  const consensusBooks=new Set(consensusQuotes.map(q=>q.bookKey));
  return {rows:[...rows.values()],offers,omitted,selectedLine:selected,side,
    bestPrices,bestLines,consensus:consensusBooks.size>=2?median(consensusQuotes.map(q=>q.line)):null};
}
