...
function hasTruncatedTeamToken(player, team) {
  const playerTokens = identity(player).split(' ').filter(Boolean);
  const teamTokens = identity(team).split(' ').filter(Boolean);
  if (playerTokens.length < 2 || playerTokens.length !== teamTokens.length) return false;

  // Fail closed only when every token except the final nickname is identical
  // and the apparent player name is a very short truncation of that nickname.
  // This catches feed/parser damage such as "Arizona Diamondbac" for
  // "Arizona Diamondbacks" without rejecting real players who merely share a
  // city or one team-name token.
  if (!playerTokens.slice(0, -1).every((token, index) => token === teamTokens[index])) return false;
  const playerLast = playerTokens.at(-1);
  const teamLast = teamTokens.at(-1);
  return playerLast.length >= 4 && teamLast.length >= 4
    && teamLast.startsWith(playerLast)
    && teamLast.length - playerLast.length > 0
    && teamLast.length - playerLast.length <= 4;
}
...
export function isVerifiedPlayerPropRow(row) {
  ...
  if (!player || ['team', 'game', 'match', 'home', 'away', 'home team', 'away team'].includes(player)) return false;
  ...
  const eventTeams = [row.homeTeam, row.awayTeam]
    ...
    .filter(Boolean);
  if (eventTeams.includes(player)
    || eventTeams.some((team) => hasSameTeamBoundary(player, team) || hasTruncatedTeamToken(player, team))) return false;
  ...
  if (participantTeamAliases.includes(player)
    || participantTeamAliases.some((team) => hasSameTeamBoundary(player, team) || hasTruncatedTeamToken(player, team))) return false;
