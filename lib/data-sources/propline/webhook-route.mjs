...
function deliveryItems(payload, fallbackType) {
  ...
  if (items.some((entry) => !entry.payload || typeof entry.payload !== 'object' || Array.isArray(entry.payload))) return null;
  return items;
}

function consumerDelivery(payload, items, sequence, gap) {
  const seq = Number(sequence) || null;
  if (!payload?.batch) {
    const item = items[0];
    return { type: item.type, payload: item.payload, sequence: seq, gap, deliveryId: item.deliveryId };
  }

  // The realtime consumer is already batch-aware and persists once after its
  // inner loop. Keep the signed envelope as one consumer call so a 100-event
  // batch does not become 100 synchronous volume writes before the HTTP response
  // can flush. Child event types and delivery IDs are preserved inside the batch,
  // and event_type is copied into each data object so mixed-type batches remain
  // independently typed by sanitizeEvent().
  return {
    type: '',
    payload: {
      ...payload,
      ...
      events: items.map((item) => ({
        ...
        data: { ...item.payload, event_type: item.type },
      })),
    },
    sequence: seq,
    gap,
...
export async function handleProplineWebhook(req, res, { onEvent = null } = {}) {
  ...
  // per signed POST; child delivery_id values remain the idempotency keys inside
  // the batch-aware realtime consumer.
  ...
  if (typeof onEvent === 'function') {
    try {
      await onEvent(consumerDelivery(payload, items, sequence, gap));
    } catch (error) {
      const type = headerType || items[0].type || 'batch';
      console.log(`[PropLine webhook] handler failed type=${type} code=${String(error?.code || 'HANDLER_FAILED').slice(0, 60)}`);
    }
  }
  return true;
}
