...
#as5 .asNav{bottom:calc(10px + env(safe-area-inset-bottom))!important;left:50%!important;right:auto!important;transform:translateX(-50%)!important;width:min(900px,calc(100% - 28px))!important;display:grid!important;grid-template-columns:repeat(3,minmax(74px,1fr)) minmax(150px,1.25fr) repeat(3,minmax(74px,1fr))!important;align-items:center!important;gap:3px!important;padding:5px!important;background:linear-gradient(180deg,rgba(19,29,49,.72),rgba(7,13,26,.78))!important;border:1px solid rgba(124,151,190,.36)!important;border-radius:22px!important;box-shadow:0 16px 46px rgba(0,0,0,.36),inset 0 1px 0 rgba(255,255,255,.055)!important;backdrop-filter:blur(26px) saturate(150%)!important;-webkit-backdrop-filter:blur(26px) saturate(150%)!important}
...
#as5 .asNav button{display:grid!important;grid-template-rows:24px 14px!important;place-items:center!important}
...
@media(max-width:650px){#as5 .asNav{bottom:calc(6px + env(safe-area-inset-bottom))!important;width:calc(100% - 14px)!important;grid-template-columns:repeat(6,minmax(0,1fr))!important;padding:4px!important;border-radius:19px!important;gap:1px!important}#as5 .asNavBrand{display:none!important}#as5 .asNav button,#as5 .asNav a{height:44px!important;padding:2px 2px!important;border-radius:14px!important;font-size:9px!important}#as5 .asNav button{grid-template-rows:22px 13px!important}#as5 .asNavIcon{font-size:18px!important}#as5{padding-bottom:calc(70px + env(safe-area-inset-bottom))!important}}
...
function shell(){
 ...
 <header class="asTop"><div class="asBar"><a class="asIdentity" href="/apex" aria-label="Auto Scout Research"><div class="asLogo">A</div><span class="asBrand">Props</span></a><div class="asSports" id="asSports" aria-label="Sports"></div><span class="asGrow"></span><span class="asStatus" id="asStatus">Current sportsbook lines</span><span id="asRefresh" hidden aria-hidden="true"></span></div></header>
...
function recentFiveStrip(g,r,line,side){
 ...
 if(!usable.length){
  return '<section class="asRecentFive asRecentFiveEmpty" aria-label="Last five recent results unavailable">'
   +'<div class="asRecentFiveHead"><b>Last 5</b><span>'+esc(shortMarket(g))+'</span></div>'
 ...
 return '<section class="asRecentFive" aria-label="Last '+usable.length+' recent '+esc(shortMarket(g))+' results">'
  +'<div class="asRecentFiveHead"><span><b>Last '+usable.length+'</b><em>'+esc(shortMarket(g))+'</em></span>'
 ...
 return '<article class="asRow asCard" data-open="'+esc(g.key)+'" tabindex="0" aria-label="Research '+esc(g.playerName+' '+g.market)+'">'
  +'<div class="asCardHead">'
   ...
   +'<div class="asCardId"><div class="asCardName"><span class="asPlayer">'+esc(r?.entityType==='team'?r.player?.playerName||g.playerName:g.playerName)+'</span>'
    ...
    +(position?'<span class="asPos">'+esc(position)+'</span>':'')+'<span class="asLeagueBadge">'+esc(g.sport)+'</span></div>'
 ...
 try{
  var response=await nativeFetch('/api/apex/research-batch',{method:'POST',signal:controller.signal,headers:{'content-type':'application/json'},
   body:JSON.stringify({props:jobs.map(function(job,index){var {g,line,side}=job;
    ...
    return {key:String(index),sport:g.sport,playerName:g.playerName,market:g.market,
...
function renderListLight(){
 var viewNote=document.getElementById('asViewNote'),notes={players:'Every live player on the selected sport, alphabetized for fast lookup. This view never requires a projection or historical sample to appear.',popular:'Verified L5 trend leaders rank first. When trend history is still loading, props fall back to strongest live book coverage instead of showing an empty tab.',snipes:'Best live line-shopping board. True snipe signals rank first; otherwise props are ordered by the largest current cross-book line gap and quote coverage.',discrepancies:'Compare the same prop across books. Multi-book props rank first by quote coverage and line gap; if only one book is available, the live quote still remains visible instead of a blank page.'};
...
async function loadAccount(){
 ...
 if(button){button.textContent=account.authenticated?'Account':'Sign in';button.classList.toggle('asPrimary',!account.authenticated);}if(window.__syncProfileAvatar)window.__syncProfileAvatar();
}
...
if(propRoute()){sport=propRoute().sport;restoreFilters();}shell();reportSigninRedirect();loadAccount();loadSaved().then(()=>{savedRouteReady=true;if(!loading){renderListLight();syncPropRoute();}});load();setInterval(function(){if(!document.hidden&&!drawerState&&!loading)load();},30000);
(function(){})();
...
(function(){
  ...
  function ensureHeader(){
    ...
    if(identity && !identity.classList.contains('asWordmark')){
      ...
      identity.innerHTML='<span class="asOblige">oblige</span><span class="asPay">pay</span>';
    }
