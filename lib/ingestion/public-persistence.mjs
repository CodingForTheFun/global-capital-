...
async function rpcStore(functionName, action, payload = {}, { allowDirect = true, timeoutMs = storeTimeoutMs() } = {}) {
  ...
  // At production volume that wrapper accumulated very high call and temporary
  // block volume while equivalent direct RPCs avoided that spill. When direct
  // primary is enabled, keep background writes on that token-guarded path only.
  // A failed write retains last-good data and the next scheduled cycle retries;
  // it must not fan back into PostgREST and recreate the pressure storm.
...
export async function releasePublicCycle(owner) {
  if (!owner) return {};
  try {
    return await store('release', { owner });
  } catch (directError) {
    // Release is tiny, idempotent lease bookkeeping. If the direct database
    // connection is the thing that failed during a cycle, use the already-pooled
    // PostgREST path once so the scheduler is not stranded behind a long lease.
    try {
      return await store('release', { owner }, { allowDirect: false, timeoutMs: 2_500 });
    } catch (pooledError) {
      throw Object.assign(pooledError instanceof Error ? pooledError : new Error('PUBLIC_RELEASE_FAILED'), { cause: directError });
    }
  }
}
...
export async function persistPublicSnapshot(source, rows, observedAt = new Date().toISOString()) {
  ...
  let written = 0;
  // Public boards contain thousands of two-sided rows. Moderate batches reduce
  // HTTP/database connection churn by an order of magnitude while staying far
  // below the RPC payload ceiling. Omitted rows are pruned only after all chunks
  // succeed, so a failed refresh never destroys the last-good customer board.
