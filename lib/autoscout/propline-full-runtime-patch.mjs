...
function proplineLineHistoryHtml(data){
 ...
 return proplineFullBlock('Recorded line history','<div class="asPlMetric"><b>'+esc(String(data.points.length))+'</b><span>snapshots · '+esc(first.line==null?'—':dec(first.line))+' → '+esc(last.line==null?'—':dec(last.line))+'</span></div>');
}
function loadProplineFull(g){
 var host=document.getElementById('asProplineFull');if(!host||!g||!g.sport)return;
 var token=++proplineFullToken,sport=g.sport,eventId=g.eventId||'',player=g.playerName||'',market=g.marketId||'',side=defaultSide(g),line=boardLine(g),quote=bestPrice(g,side,line,true),book=quote&&quote.sportsbookKey||'';
 var asks=[
...
function proplineSgpEligible(){
 ...
 if(!slip.every(function(p){return p.eventId===eventId&&p.sport===sport&&p.marketKey;}))return null;
...
export function patchProplineFullUi(source) {
  ...
  patched = replaceOnce(
    ...
    'slip.push({key:key,sport:g.sport,playerName:g.playerName,market:g.market,marketKey:g.marketId||g.market,',
    'slip record',
  );
  patched = replaceOnce(
    patched,
