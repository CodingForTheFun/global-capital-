...
function sourceQualifiedFantasyMarketKey(record,marketKey){
 const key=text(marketKey);
 if(text(record?.book).toLowerCase()!=='prizepicks'||!key)return key;
 const label=`${text(record?.market)} ${key}`;
 if(!/\bfantasy(?:\s+(?:score|points?))?\b/i.test(label))return key;
 const sport=canonicalSport(record?.sport),position=text(record?.position).toUpperCase();
 if(sport==='NBA')return 'prizepicks:player_fantasy_score';
 if(sport==='MLB'){
  if(/^(?:P|SP|RP|LHP|RHP)$/.test(position)||/\bpitcher\b/i.test(label))return 'prizepicks:player_pitcher_fantasy_score';
  if(position||/\b(?:hitter|batter)\b/i.test(label))return 'prizepicks:player_hitter_fantasy_score';
 }
 return key.startsWith('prizepicks:')?key:`prizepicks:${key}`;
}
...
export function normalizedFeedBoard(records,reference={props:[]},at=new Date().toISOString()){
 ...
 for(const r of records){
  ...
  const baseMarketKey=ref?.marketId||inferredKey||r.fallbackMarketKey;
  const marketKey=sourceQualifiedFantasyMarketKey(r,baseMarketKey);
...
export function normalizedDataFromBoardRows(rows=[],at=new Date().toISOString()){
 ...
 for(const row of Array.isArray(rows)?rows:[]){
  try{
   ...
   const player=normalizedPlayer({id:row.playerId,providerPlayerId:row.providerPlayerId,provider:row.provider,sport:row.sport,name:row.playerName,team:row.team,position:row.position,ingestedAt:at});
   ...
   const line=normalizedBookmakerLine({propId:prop.id,provider:row.provider,bookmakerKey:row.sportsbookKey,bookmakerName:row.sportsbook,side:row.side,line:row.line,price:row.price,providerUpdatedAt:row.providerUpdatedAt,ingestedAt:at});
