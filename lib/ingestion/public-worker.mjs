...
export function createPublicIngestionRunner({
  ...
  async function browserDfsSnapshot(source) {
    ...
    const payload = await browserJsonFetch(config.url, { origin: config.origin, referer: config.referer });
  ...
  async function ingestDfs(source) {
    try {
      ...
      if ((!snapshot || !SUCCESS.has(snapshot.status) || snapshot.partial) && [403, 426].includes(Number(snapshot?.httpStatus))) {
        try {
      ...
      if (!snapshot || !SUCCESS.has(snapshot.status) || snapshot.partial) {
        const failure = {
          status: snapshot?.partial ? 'partial' : snapshot?.status || 'unavailable',
          retained: true,
          fetchedAt: snapshot?.fetchedAt || null,
          httpStatus: snapshot?.httpStatus ?? null,
          fallbackCode: fallbackError ? text(fallbackError?.code || 'PUBLIC_BROWSER_FAILED').slice(0, 80) : null,
          fallbackStatus: fallbackError?.status ?? null,
        };
        await status(source, failure);
        return { source, persisted: false, retained: true, httpStatus: failure.httpStatus, fallbackCode: failure.fallbackCode, fallbackStatus: failure.fallbackStatus };
      }
      ...
      await status(source, { status: snapshot.status, rows: rows.length, fetchedAt: observedAt, written: Number(result?.written || 0), transport: snapshot.transport || 'direct-public' });
      return { source, persisted: true, rows: rows.length, written: Number(result?.written || 0), transport: snapshot.transport || 'direct-public' };
    } catch (error) {
      await status(source, { status: 'unavailable', retained: true, code: text(error?.code || 'PUBLIC_INGEST_FAILED').slice(0, 80), httpStatus: error?.status ?? null });
      return { source, persisted: false, retained: true, code: text(error?.code || 'PUBLIC_INGEST_FAILED'), httpStatus: error?.status ?? null };
    }
  }
  ...
  async function ingestDraftKings(sport) {
    ...
    try {
      ...
      await status(source, { status: 'unavailable', retained: true, code: text(error?.code || 'DRAFTKINGS_INGEST_FAILED').slice(0, 80), httpStatus: error?.status ?? null });
      return { source, persisted: false, retained: true, code: text(error?.code || 'DRAFTKINGS_INGEST_FAILED'), httpStatus: error?.status ?? null };
    }
  }
