...
const permanentLineOnlyCodes = new Set(['UNSUPPORTED_MARKET','STAT_NOT_AVAILABLE']);
...
async function researchUncached(params = {}) {
  ...
  if (PUBLIC_LEAGUES[String(params.sport || '').toUpperCase()]) {
    ...
    // Persist only the already-verified ESPN response. This performs no extra ESPN
    // fetch and failures are swallowed by the persistence helper.
    await persistResearchGameLogs(history, params);
    return { ...finalizeResearch({ ...params, opponent: history.opponent, opponentId: history.opponentId, isHome: history.isHome,
