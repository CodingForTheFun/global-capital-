  ...
  try {
    ...
    if (action === 'revoke') {
      ...
      await createAccountNotice({
        userId: target.id,
        type: 'pro-access-removed',
        title: 'Pro access updated',
        message: 'The Oblige Props owner removed complimentary Pro access from your account. Your account remains active with the access currently available to it.',
      });
    ...
    await createAccountNotice({
      userId: target.id,
      type: action === 'extend' ? 'pro-access-extended' : 'pro-access-granted',
      title: action === 'extend' ? 'Pro access extended' : 'Pro access granted',
      message: action === 'extend'
        ? `The Oblige Props owner added ${days} more day${days === 1 ? '' : 's'} of complimentary Pro access to your account. Your current access is set through ${new Date(expiresAt).toLocaleDateString('en-US')}.`
        : `The Oblige Props owner granted your account ${days} day${days === 1 ? '' : 's'} of complimentary Pro access, through ${new Date(expiresAt).toLocaleDateString('en-US')}.`,
    });
    log?.log?.(`[Oblige Props admin] owner ${user.id} ${action}ed ${days} day(s) of complimentary pro access for ${target.id}`);
    json(res, 200, {
      ok: true,
      access: ownerAccessShape(access),
      message: action === 'extend' ? `Complimentary access extended by ${days} day${days === 1 ? '' : 's'}.` : `Complimentary Pro access granted for ${days} day${days === 1 ? '' : 's'}.`,
    });
    return true;
  } catch (error) {
