...
export const PROPLINE_GAME_SPORTS = Object.freeze({
  NFL: 'football_nfl', NCAAF: 'football_ncaaf', NBA: 'basketball_nba',
  WNBA: 'basketball_wnba', NCAAB: 'basketball_ncaab', MLB: 'baseball_mlb',
  NHL: 'hockey_nhl', TENNIS: 'tennis', MLS: 'soccer_mls', EPL: 'soccer_epl',
  UCL: 'soccer_uefa_champions_league', PGA: 'golf', MMA: 'mma_ufc', ESPORTS: 'esports', CS2: 'esports',
});
...
const basketball = schema({
  ...
  threes: ['player_threes', '3-Pointers Made', 'Three Pointers Made'], steals: ['player_steals'],
...
const mma = schema({ significant_strikes: ['player_significant_strikes'], takedowns: ['player_takedowns'], completed_rounds: ['Total Rounds', 'fight_completed_rounds'] });
const esports = schema({ kills_map_1: [], kills_maps_1_2: [], kills_maps_1_2_3: [], headshots_map_1: [], headshots_maps_1_2: [], headshots_maps_1_2_3: [] });
const schemas = { NFL: football, NCAAF: football, NBA: basketball, WNBA: basketball, NCAAB: basketball, MLB: baseball, NHL: hockey, TENNIS: tennis, MLS: soccer, EPL: soccer, UCL: soccer, PGA: golf, MMA: mma, ESPORTS: esports, CS2: esports };
...
export function rawStatFor(params = {}) {
  ...
  // A recognized full-game key must never override a period-qualified label.
...
export function normalizeGameArchive(payload, params = {}, now = Date.now()) {
  ...
  for (const row of payload.games.slice(0, 100)) {
    ...
    const noPlay = row?.played === false || row?.did_not_play === true || row?.retired === true || row?.walkover === true || (['NBA', 'WNBA', 'NCAAB'].includes(sport) && numeric(row?.stats?.minutes) === 0);
  ...
  const gameLog = [...rows.values()].sort((a, b) => Date.parse(b.date) - Date.parse(a.date)).slice(0, Math.min(40, Math.max(5, Number(params.games) || 20)));
...
export async function fetchPropLineGameResearch(params = {}, { get = proplineGet, configured = proplineConfigured, now = Date.now } = {}) {
  ...
  try {
    // All stats in one bounded player request: switching markets/line/side uses
    // the same shared cached archive, with existing reserve and Retry-After gates.
    const payload = await get(`/v1/sports/${PROPLINE_GAME_SPORTS[sport]}/players/${encodeURIComponent(text(params.playerName))}/games`, { limit: 40 }, { ttlSeconds: 900, timeoutMs: 8000 });
