...
function sideOf(value) {
  ...
  if (!side) return null;
  const hasOver = /(^|[^A-Z])OVER([^A-Z]|$)/.test(side) || side.startsWith('OVER') || side.endsWith('OVER');
  const hasUnder = /(^|[^A-Z])UNDER([^A-Z]|$)/.test(side) || side.startsWith('UNDER') || side.endsWith('UNDER');
  if (hasOver && !hasUnder) return 'OVER';
  if (hasUnder && !hasOver) return 'UNDER';
  return null;
}

function mappedName(map, id, fallback = null) {
  if (!map || id === null || id === undefined || id === '') return text(fallback) || null;
  const value = map instanceof Map ? map.get(String(id)) : map[String(id)];
  return text(value) || text(fallback) || null;
}
...
function marketContext(node, parent = {}, metadata = {}) {
  return {
    ...
    marketType: mappedName(metadata.marketTypeById, node?.BettingMarketTypeID, node?.BettingMarketType) || parent.marketType || null,
    betType: mappedName(metadata.betTypeById, node?.BettingBetTypeID, node?.BettingBetType) || parent.betType || null,
    periodType: mappedName(metadata.periodTypeById, node?.BettingPeriodTypeID, node?.BettingPeriodType) || parent.periodType || null,
...
function offerFromOutcome(outcome, context, { sport, book = null, consensus = false, metadata = {} } = {}) {
  if (!coreAvailable(outcome)) return null;
  const outcomeType = mappedName(metadata.outcomeTypeById, outcome?.BettingOutcomeTypeID, outcome?.BettingOutcomeType);
  const side = sideOf(outcomeType);
 ...
 *
 * SportsDataIO can scramble descriptive betting fields in some subscription
 * tiers while leaving their IDs intact. Callers may pass `metadata` maps built
 * from the provider's BettingMetadata endpoint so Outcome/Bet/Market/Period
 * IDs are resolved back to stable names before normalization.
 */
export function normalizePlayerPropOffers(payload, { sport = null, sportsbook = null, metadata = {} } = {}) {
  ...
  function visit(node, parent = {}) {
    ...
    const context = marketContext(node, parent, metadata);
    let bookRowsAdded = 0;
    const outcomes = Array.isArray(node.BettingOutcomes) ? node.BettingOutcomes : [];
    for (const outcome of outcomes) {
      const book = sportsbookFromOutcome(outcome);
      if (add(offerFromOutcome(outcome, context, { sport, book, consensus: false, metadata }))) bookRowsAdded++;
    }
    ...
    if (!wantedBook && bookRowsAdded === 0) {
      ...
      for (const outcome of consensus) add(offerFromOutcome(outcome, context, { sport, consensus: true, metadata }));
    }
...
export function summarizePlayerPropShape(payload, { metadata = {} } = {}) {
  const counts = {
  ...
  const visit = (node) => {
    ...
    for (const outcome of [...betting, ...consensus]) {
      const outcomeType = mappedName(metadata.outcomeTypeById, outcome?.BettingOutcomeTypeID, outcome?.BettingOutcomeType);
      if (sideOf(outcomeType)) counts.overUnderOutcomes++;
...
export function summarizeSportsbookCoverage(payload, target = 'PrizePicks', { sport = null, metadata = {} } = {}) {
  const offers = normalizePlayerPropOffers(payload, { sport, metadata });
