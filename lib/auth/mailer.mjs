...
// Plus MAIL_FROM (e.g. "Oblige Props <noreply@yourdomain.com>").
//
...
export function mailFrom() {
  return process.env.MAIL_FROM || 'Oblige Props <onboarding@resend.dev>';
}
...
/**
 * What /api/health reports about mail. Enough to tell at a glance whether a
 * password reset can actually be delivered - which is the question this whole
 * subsystem was silently answering "no" to for its first 182 sign-ups - and
 * nothing that identifies a key or an inbox. The sending domain is public
 * information: it is on every email the site sends.
 */
export function mailHealth(env = process.env) {
  const provider = env.RESEND_API_KEY ? MAIL_PROVIDER.RESEND
    : env.POSTMARK_API_TOKEN ? MAIL_PROVIDER.POSTMARK
    : env.SENDGRID_API_KEY ? MAIL_PROVIDER.SENDGRID
    : env.MAILGUN_API_KEY && env.MAILGUN_DOMAIN ? MAIL_PROVIDER.MAILGUN
    : MAIL_PROVIDER.NONE;
  const from = String(env.MAIL_FROM || '');
  const domain = from.match(/@([^>\s]+)/)?.[1] || null;
  return { configured: provider !== MAIL_PROVIDER.NONE, provider, fromDomain: domain };
}
...
<div style="font-weight:700;letter-spacing:.14em;font-size:12px;color:#7d93b8;text-transform:uppercase">Oblige Props</div>
...
export function verificationEmail(code) {
  return {
    subject: `${code} is your Oblige Props verification code`,
    text: `Your Oblige Props verification code is ${code}\n\nIt expires in 10 minutes.\n\nIf you did not create an account, you can ignore this email.`,
...
export function resetEmail(code) {
  return {
    subject: `${code} is your Oblige Props password reset code`,
    text: `Your Oblige Props password reset code is ${code}\n\nIt expires in 10 minutes.\n\nIf you did not request a password reset, ignore this email and your password will stay unchanged.`,
...
export function passwordChangedEmail() {
  return {
    subject: 'Your Oblige Props password was changed',
    text: 'Your Oblige Props password was just changed, and all other sessions were signed out.\n\nIf this was not you, reset your password immediately.',
