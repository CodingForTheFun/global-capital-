...
export function normalizeSettledFantasyHistory(payload, archive, params, sportKey, now = Date.now()) {
  ...
  const requestedId = text(params.providerPlayerId).match(/^propline:(.+)$/)?.[1];
  if (requestedId && archive.player_id && requestedId !== text(archive.player_id)) return failed('Historical player identifiers conflict.');
