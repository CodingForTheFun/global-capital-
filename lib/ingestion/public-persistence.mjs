...
export async function releasePublicCycle(owner) {
  ...
  try {
    ...
    // Lease release is tiny and idempotent. If the direct connection is the
    // failure, use the already-pooled PostgREST route once so a transient DB
    // network issue cannot strand the scheduler for the full lease duration.
    try {
