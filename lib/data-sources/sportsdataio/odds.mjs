...
function offerFromOutcome(outcome, context, { sport, book = null, consensus = false } = {}) {
  ...
  // PlayerID is preferred, but some SportsDataIO packages can omit/scramble it
  // while still returning the canonical market-level PlayerName. Never fall
  // back to sportsbook Participant free-text.
  if ((!playerId && !playerName) || !side || line === null || !market) return null;
...
export function normalizePlayerPropOffers(payload, { sport = null, sportsbook = null } = {}) {
  ...
  function add(row) {
    ...
    const playerIdentity = row.playerId ?? normalizePlayerName(row.playerName);
    if (!playerIdentity) return false;
    const key = [row.sportsbookKey, row.sport, playerIdentity, row.market.toLowerCase(), row.side, row.line].join('|');
