...
export const EMPTY_FILTERS = Object.freeze({
  ...
  bookmakers: [],
  researchAvailability: 'ALL',
  researchThresholds: {},
  savedOnly: false,
  minProjectionDelta: null,
  startsBefore: null,
  startsAfter: null,

  // Enrichment-backed filters. Each is inapplicable until a stats provider
  // supplies the field, so they never exclude props on absent data.
  injuryStatuses: [],
  startersOnly: false,
  minExpectedMinutes: null,
  minUsageRate: null,
  liveStatuses: [],
  minProjectionEdge: null,
  lineupStatuses: [],
  startersByDepthChart: false,
...
export function normalizeFilters(input = {}) {
  ...
  return {
    ...filters,
    bookmakers: upperList(filters.bookmakers),
    ...
    injuryStatuses: upperList(filters.injuryStatuses),
    liveStatuses: upperList(filters.liveStatuses),
    lineupStatuses: upperList(filters.lineupStatuses),
    startersByDepthChart: filters.startersByDepthChart === true,
    excludeInjured: filters.excludeInjured === true,
    startersOnly: filters.startersOnly === true,
...
export function isFilterApplicable(filterId, prop) {
  ...
  switch (filterId) {
    ...
    case 'injuryStatuses':
      return Boolean(prop.injuryStatus);
    case 'startersOnly':
      return prop.isStarter !== null && prop.isStarter !== undefined;
    case 'minExpectedMinutes':
      return isSetNumber(prop.expectedMinutes);
    case 'minUsageRate':
      return isSetNumber(prop.usageRate);
    case 'liveStatuses':
      return Boolean(prop.liveStatus);
    case 'minProjectionEdge':
      return isSetNumber(prop.projection) && isSetNumber(prop.line);
    case 'lineupStatuses':
      return Boolean(prop.lineupStatus);
    case 'startersByDepthChart':
      return isSetNumber(prop.depthChartOrder);
    case 'excludeInjured':
      return Boolean(prop.injuryStatus);
    default:
      return true;
...
export function availableFilters(props = []) {
  ...
  const conditional = ['minScore', 'maxScore', 'qualityTiers', 'minConfidence', 'minEdge', 'maxEdge', 'positiveEdgeOnly', 'minHitRate', 'matchup', 'maxDataAgeMinutes', 'teams',
    'injuryStatuses', 'startersOnly', 'minExpectedMinutes', 'minUsageRate', 'liveStatuses', 'minProjectionEdge',
    'lineupStatuses', 'startersByDepthChart', 'excludeInjured'];
...
export function evaluatePropAgainstFilters(prop, rawFilters = {}) {
  ...
  check('bookmakers', filters.bookmakers.length > 0, true,
    () => filters.bookmakers.includes(String(prop.sportsbookKey || '').toUpperCase()));
  check('savedOnly', filters.savedOnly === true, true, () => prop.saved === true);
  check('researchAvailability', filters.researchAvailability !== 'ALL', true,
    () => filters.researchAvailability === 'AVAILABLE' ? prop.researchAvailable === true
      : filters.researchAvailability === 'UNAVAILABLE' ? prop.researchAvailable === false : prop.researchAvailable == null);
  for (const window of ['l5', 'l10', 'l15', 'l20', 'season', 'h2h']) {
    const minimum = toNumberOrNull(filters.researchThresholds?.[window]);
    check(`research-${window}`, minimum !== null, true, () => {
      const rate = hitRateFor(prop, window);
      return rate !== null && rate >= minimum;
    });
  }
  check('minProjectionDelta', isSetNumber(filters.minProjectionDelta), true,
    () => isSetNumber(prop.projection) && isSetNumber(prop.line) &&
      (prop.side === 'UNDER' ? Number(prop.line) - Number(prop.projection) : Number(prop.projection) - Number(prop.line)) >= Number(filters.minProjectionDelta));
  check('startsBefore', Boolean(filters.startsBefore), true,
    () => Date.parse(prop.gameStartTime) <= Date.parse(filters.startsBefore));
  check('startsAfter', Boolean(filters.startsAfter), true,
    () => Date.parse(prop.gameStartTime) >= Date.parse(filters.startsAfter));
  ...
  check('injuryStatuses', filters.injuryStatuses.length > 0, isFilterApplicable('injuryStatuses', prop),
    () => filters.injuryStatuses.includes(String(prop.injuryStatus || '').toUpperCase()));

  check('startersOnly', filters.startersOnly, isFilterApplicable('startersOnly', prop),
    () => prop.isStarter === true);

  check('minExpectedMinutes', isSetNumber(filters.minExpectedMinutes), isFilterApplicable('minExpectedMinutes', prop),
    () => Number(prop.expectedMinutes) >= Number(filters.minExpectedMinutes));

  check('minUsageRate', isSetNumber(filters.minUsageRate), isFilterApplicable('minUsageRate', prop),
    () => Number(prop.usageRate) >= Number(filters.minUsageRate));

  check('liveStatuses', filters.liveStatuses.length > 0, isFilterApplicable('liveStatuses', prop),
    () => filters.liveStatuses.includes(String(prop.liveStatus || '').toUpperCase()));

  check('minProjectionEdge', isSetNumber(filters.minProjectionEdge), isFilterApplicable('minProjectionEdge', prop),
    () => {
      const directional = String(prop.side).toUpperCase() === 'UNDER'
        ? Number(prop.line) - Number(prop.projection)
        : Number(prop.projection) - Number(prop.line);
      return directional >= Number(filters.minProjectionEdge);
    });

  check('lineupStatuses', filters.lineupStatuses.length > 0, isFilterApplicable('lineupStatuses', prop),
    () => filters.lineupStatuses.includes(String(prop.lineupStatus || '').toUpperCase()));

  check('startersByDepthChart', filters.startersByDepthChart, isFilterApplicable('startersByDepthChart', prop),
    () => Number(prop.depthChartOrder) === 1);

  // Excludes players ruled out or doubtful; a questionable player is still shown
  // because the user may want to see the line before a late scratch.
  check('excludeInjured', filters.excludeInjured, isFilterApplicable('excludeInjured', prop),
    () => !['OUT', 'DOUBTFUL'].includes(String(prop.injuryStatus || '').toUpperCase()));
...
export function activeFilterChips(filters = {}) {
  ...
  for (const status of normalized.injuryStatuses) push('injuryStatuses', status);
  if (normalized.startersOnly) push('startersOnly', 'Starters only');
  if (isSetNumber(normalized.minExpectedMinutes)) push('minExpectedMinutes', `${normalized.minExpectedMinutes}+ min`);
  if (isSetNumber(normalized.minUsageRate)) push('minUsageRate', `Usage ${normalized.minUsageRate}%+`);
  for (const status of normalized.liveStatuses) push('liveStatuses', status);
  if (isSetNumber(normalized.minProjectionEdge)) push('minProjectionEdge', `Proj edge ${normalized.minProjectionEdge}+`);
  for (const status of normalized.lineupStatuses) push('lineupStatuses', `${status} lineup`);
  if (normalized.startersByDepthChart) push('startersByDepthChart', 'Depth-chart starters');
  if (normalized.excludeInjured) push('excludeInjured', 'Exclude out/doubtful');
...
export function clearFilter(filters, id, value = null) {
  ...
  const listFields = ['sports', 'markets', 'players', 'teams', 'opponents', 'games', 'qualityTiers', 'matchup', 'injuryStatuses', 'liveStatuses', 'lineupStatuses'];
...
const LIST_KEYS = ['sports', 'markets', 'players', 'teams', 'opponents', 'games', 'qualityTiers', 'matchup', 'injuryStatuses', 'liveStatuses', 'lineupStatuses'];
const NUMBER_KEYS = ['minScore', 'maxScore', 'minConfidence', 'minEdge', 'maxEdge', 'minHitRate', 'minRulesPassed', 'maxDataAgeMinutes', 'minExpectedMinutes', 'minUsageRate', 'minProjectionEdge'];
...
export function filtersToQuery(filters = {}) {
  ...
  if (normalized.startersOnly) params.set('startersOnly', '1');
  if (normalized.startersByDepthChart) params.set('startersByDepthChart', '1');
  if (normalized.excludeInjured) params.set('excludeInjured', '1');
...
export function filtersFromQuery(query = '') {
  ...
  filters.startersOnly = params.get('startersOnly') === '1';
  filters.startersByDepthChart = params.get('startersByDepthChart') === '1';
  filters.excludeInjured = params.get('excludeInjured') === '1';
