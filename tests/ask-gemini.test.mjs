...
test('Gemini Ask sends canonical bounded card, server-only key and system grounding',async()=>{
 ...
 try{let calls=0;const out=await askAboutProp({question:'Count results',prop:{sport:'NBA',playerName:'Test',market:'Points',line:25.5,side:'UNDER',gameLog:[{value:30},{value:20},{value:24},{value:28}],secret:'MUST_NOT_APPEAR'},fetchImpl:async(url,opts)=>{
 calls++;assert.ok(url.endsWith(':generateContent'));assert.ok(!url.includes('test-only'));assert.equal(opts.headers['x-goog-api-key'],'test-only');const body=JSON.parse(opts.body);assert.ok(body.systemInstruction);assert.equal(JSON.parse(body.contents[0].parts[0].text).card.selectedSide,'UNDER');assert.ok(!opts.body.includes('MUST_NOT_APPEAR'));return new Response(JSON.stringify({candidates:[{content:{parts:[{text:'Do not show reasoning',thought:true},{text:'Two results exceeded 25.5.'}]},finishReason:'STOP'}]}),{status:200});}});
