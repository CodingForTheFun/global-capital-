...
async function providerUsage() {
  ...
  try {
    const response = await fetch(USAGE_URL, { headers: { 'x-api-key': key(), accept: 'application/json' }, signal: AbortSignal.timeout(10_000) });
...
async function requestMode(targetUrl, { targetHeaders = {}, browser = false, returnPageSource = false, estimatedCost = 1, transport }) {
  ...
  try {
    ...
    const headers = { 'x-api-key': key(), accept: 'application/json,text/plain,*/*' };
    ...
    await settle(estimatedCost, actualCost).catch(() => {});
  }
}
